...
- For native crashes, the faulting module name and instruction offset, plus the names of the modules
  found on the crashing thread's stack — read locally from the crash dump on next launch to tell
  whether the mod, the base game, or another mod crashed. Reading the stack happens on your machine
  and produces module names only: the crash dump file itself, its memory contents, the stack data,
  and any embedded screenshot are **not** sent; file-system paths and your OS user name are masked
  out of the names.
...
- A one-time list of the **names** of the mods you have enabled, as the base game reports them. Names only — no file paths, no folder layout, nothing about where a mod came from. It is read once per session to tell whether a crash happens only in combination with another mod: without it, a mod that breaks Civic Survival is invisible unless its own code is the one that crashes.
...
- Save names
- Local files
- File paths of your mods (only their names are sent)
- Precise location
...
**https://discord.gg/nhytdnKFeW**. Tell us your player ID and ask for deletion.
