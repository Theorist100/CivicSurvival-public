...
When the air raid siren sounds, the strike targets are already picked — buildings in the
line of fire (and residential blocks around each predicted impact point) go into
**evacuation**: everyone physically inside runs for the nearest working **Emergency
Shelter** (the vanilla Disaster Services building). Casualties are drawn only from people
actually inside a collapsing building — so reaching a shelter, or simply being out of the
building when it is hit, genuinely saves lives. Drone strikes give minutes of warning
(siren plus flight time); ballistic missiles arrive too fast to outrun — against those,
only air defense helps.

What makes shelters work:

- **Placement beats capacity.** People flee on foot and the alert lasts minutes — a
  runner covers a few hundred meters. One shelter per residential district, inside the
  district, saves far more than one big shelter across town. Build them close to housing.
- **Keep them powered.** An unpowered or under-construction shelter never opens its doors:
  pathfinding will not send anyone there.
- **After the all-clear** (a grace period past the last impact), everyone still en route
  turns back home, and the shelter gradually releases the people inside.
- The **wave debriefing** shows the evacuation outcome: how many reached shelter and how
  many didn't make it in time. If the second number dominates, your shelters are too far
...
- **Developer diagnostics (telemetry)** — **off by default, opt-out under Online.** A separate
  ...
  hardware snapshot (CPU cores, RAM, GPU + VRAM, OS platform), a one-time list of the names of
  your enabled mods (names only — used to tell a mod conflict from a Civic Survival bug), and an
  end-of-session balance summary, alongside the random install/session ID and mod/game version. Turning diagnostics
...
**Simplified Chinese** follows the neutral English experience. The language you choose
defines the experience.

If the game runs in Chinese, the mod picks Simplified Chinese automatically — including
on a Traditional Chinese game, which has no separate table of its own.
...
**https://discord.gg/nhytdnKFeW**. Enabling telemetry (and Bug Reporting if
