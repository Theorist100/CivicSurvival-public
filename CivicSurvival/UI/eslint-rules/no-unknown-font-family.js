...
// Themes reference the shared stacks in themes/fonts.ts instead of inlining the
// literal, so the declaration is where the families are checked now.
const FONT_STACK_CONSTANT = /^FONT_STACK/u;
...
module.exports = {
    ...
    create(context) {
        const checkStack = (node) => {
            const families = node.value.split(",").map(stripQuotes).filter(Boolean);
        ...
        return {
            Property(node) {
                ...
                checkStack(node.value);
            },

            VariableDeclarator(node) {
                if (node.id.type !== "Identifier" || !FONT_STACK_CONSTANT.test(node.id.name)) return;
                if (!node.init || node.init.type !== "Literal" || typeof node.init.value !== "string") return;

                checkStack(node.init);
            },
        };
    },
};
