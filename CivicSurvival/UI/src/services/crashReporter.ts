...
import { classifyFrames } from "./faultOrigin";
import { ensureUrlSearchParamsCompat } from "./urlSearchParamsCompat";
import { makeXhrTransport } from "./xhrTransport";
...
export function ensureSentryInitialized(): void {
    ...
    try {
        ...
        Sentry.init({
            ...
            tracesSampleRate: 0,
            // Gameface exposes no Fetch API, and the SDK's built-in transport is fetch-only —
            // with the default, every event is collected and then silently dropped at the send
            // step. XMLHttpRequest works here (vanilla CS2 UI runs on it), see xhrTransport.ts.
            transport: makeXhrTransport,
            // BrowserApiErrors goes out with tracing and replay: it patches
            // EventTarget.prototype.addEventListener process-wide, so every vanilla CS2 handler
            // ends up wrapped by us and any vanilla fault inside one carries CivicSurvival.mjs in
            // its stack. That misattributes vanilla bugs to the mod here and in the C# channel
            // too, where attribution keys on "CivicSurvival" appearing anywhere in the stack (a
            // vanilla wheel-handler crash reached prod that way on 0.3.30). Uncaught errors of
            // our own still arrive through GlobalHandlers — window.onerror and unhandledrejection,
            // both wired in index.tsx — and through ErrorBoundary.
            ...
            beforeSend(event) {
                ...
                // Authorship tags. GlobalHandlers stays in the integration set, so every uncaught
                // fault on the page arrives here — vanilla ones included, since the mod UI shares
                // the page with the game's own. Tagging by throw site keeps those one filter away
                // instead of an evening spent resolving minified offsets; see faultOrigin.ts.
                // exception.values is ordered oldest-cause-first, so the thrown error is last.
                const values = event.exception?.values;
                const thrown = values && values.length > 0 ? values[values.length - 1] : undefined;
                const { origin, modInStack } = classifyFrames(thrown?.stacktrace?.frames);
                event.tags = { ...event.tags, fault_origin: origin, mod_in_stack: modInStack };
