...
import { RankUpCelebration } from "components/arena";
...
import { isCrashReporterInitialized, reportError } from "services/crashReporter";
import { originOfFile } from "services/faultOrigin";
...
const ProfiledWarRoom = withProfiler(() => <ModRoot name="WarRoom"><WarRoomOverlay /></ModRoot>, "WarRoom");
...
const ProfiledRankUp = withProfiler(() => <ModRoot name="RankUpCelebration"><RankUpCelebration /></ModRoot>, "RankUpCelebration");
const ProfiledChirper = withProfiler(() => <ModRoot name="FloatingChirper"><FloatingChirperPanel /></ModRoot>, "FloatingChirper");
const ProfiledSettings = withProfiler(() => <ModRoot name="SettingsWindow"><SettingsWindow /></ModRoot>, "SettingsWindow");
...
//
// The mod UI shares one page with the game's own, so this fires for vanilla faults too. The log
// line therefore carries the throw-site authorship: `ErrorEvent.filename` IS the throw site, so a
// row is readable without resolving minified offsets afterwards.
//
// Reporting is conditional on purpose. Once Sentry is initialized its GlobalHandlers integration
// owns uncaught errors and rejections, and reporting here as well would send every fault twice.
// Before init there is no such owner, so this is the only path that can reach the pre-consent
// queue — which is what makes early-startup faults survivable at all.
window.addEventListener("error", (event) => {
    scLog(`[GLOBAL ERROR] origin=${originOfFile(event.filename)} ${event.message} at ${event.filename}:${event.lineno}:${event.colno}`);
    if (event.error?.stack) {
        scLog(`[GLOBAL ERROR] Stack: ${event.error.stack}`);
    }
    if (!isCrashReporterInitialized()) {
...
const register: ModRegistrar = (moduleRegistry) => {
    ...
    // War Room fullscreen command overlay (opened from the RADAR tab; self-hides when closed)
    moduleRegistry.append("Game", ProfiledWarRoom);
    scLog("WarRoomOverlay registered");
    ...
    // Ladder rank-up celebration banner (auto-dismiss, top-center)
    moduleRegistry.append("Game", ProfiledRankUp);
    scLog("RankUpCelebration registered");
    ...
    // Settings window (opened by the gear in the Dashboard's GlobalStatus bar). Its own root
    // beside the Dashboard, not inside it: that keeps it clear of the GlobalStatus bar's
    // clipping without a portal, and inside the game UI layer so it hides with the city.
    moduleRegistry.append("Game", ProfiledSettings);
    scLog("SettingsWindow registered");
