...
import { IconBird, getIconComponent } from "../../shared/common/Icons";
            ...
            {posts.length === 0 ? (
                ...
                <Column gap={theme.spacing.sm} style={{ padding: theme.spacing.sm }}>
                    {visiblePosts.map((post) => {
                        // A named speaker (the hero on air) or an enemy carrier carries its own
                        // face; the ordinary crowd wears the CHIPPER bird, so a post that MATTERS
                        // is recognisable at a glance in a feed full of neighbours. Mood is carried
                        // by the card's colour (border + avatar tint), never by a glyph in the face.
                        const AvatarIcon = post.avatarId ? getIconComponent(post.avatarId) : null;
                        return (
                            <div key={`${post.author}-${post.timestamp}`} style={moodStyles[post.mood].post}>
                                <Row align="center" style={{ marginBottom: "8rem" }}>
                                    <div style={moodStyles[post.mood].avatar}>
                                        {AvatarIcon !== null ? <AvatarIcon /> : <IconBird />}
                                    </div>
                                    <div>
