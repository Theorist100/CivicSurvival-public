...
const GridOpsSectionContent: React.FC<GridOpsSectionContentProps> = ({ cognitiveOpen }) => {
    ...
    // The selected-district hooks scope their result by the user's CURRENT selection.
    // Switching districts while a request is in flight orphans the optimistic pending
    // latch: the terminal result keeps the old district's discriminator and can never
    // match the new target (same latch class as the procurement modal). Drop the
    // optimistic state whenever the scoping key changes.
    const selectedScopeResetRef = useRef(() => { /* replaced each render */ });
    selectedScopeResetRef.current = () => {
        selectedDistrictAction.reset();
        selectedInternetAction.reset();
    };
    useEffect(() => {
        selectedScopeResetRef.current();
    }, [selectedDistrictKey]);
