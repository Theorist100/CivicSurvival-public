...
export const createStyles = (theme: Theme) => ({
    ...
    balance: {
        ...
        // Label takes the leftover width; the value column is a FIXED width, so every
        // number in the panel shares one right edge. minWidth alone only worked while
        // the font was monospaced (equal digit widths made the ragged edge invisible);
        // with the shipped sans, "14814 MW" and "-7460 MW" are different widths and the
        // column wandered. Sizes are one step down from the panel default — the sans is
        // wider than the pixel mono it replaced, and the long labels (DEMAND THROTTLE,
        // AVAILABLE OUTPUT) no longer fit at the old size.
        //
        // The value column is sized to the widest number the panel can print — nine
        // glyphs ("-14874 MW") in the 16rem mono, ~87rem — and no wider: every rem it
        // takes beyond that is a rem the two-word labels don't get, and at 104rem they
        // wrapped to a second line and stretched the panel past its scroll bound.
        label: {
        ...
        value: (color: string): React.CSSProperties => ({
            ...
            flexShrink: 0,
            whiteSpace: "nowrap" as const,
            // (No tabular-nums: cohtml ignores font-variant-numeric — the fixed column
            // width above is what keeps the right edge steady.)
        }),
