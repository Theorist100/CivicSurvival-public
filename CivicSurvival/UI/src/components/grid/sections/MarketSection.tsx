...
interface MarketSlot {
    text: string;
    color: string;
    /** Status states (error/unavailable/frozen/pending) read bold; the risk/yield readout is regular. */
    emphasis: boolean;
}

interface SlotInputs {
    /** Failed-request message, if any (highest priority). */
    error: string;
    /** Unavailable/freeze text to show when trading is blocked ("" when trading is open). */
    blockedText: string;
    /** In-flight request. */
    pending: boolean;
    pendingText: string;
    /** Default readout (risk % or daily yield) shown when nothing above applies. */
    readoutText: string;
    readoutColor: string;
    crisisColor: string;
    mutedColor: string;
...
// Resolve the single slot line by priority. Kept at module scope so the two
// four-way branches don't inflate the component's cognitive complexity.
const buildSlot = (i: SlotInputs): MarketSlot => {
    if (i.error) return { text: i.error, color: i.crisisColor, emphasis: true };
    if (i.blockedText) return { text: i.blockedText, color: i.crisisColor, emphasis: true };
    if (i.pending) return { text: i.pendingText, color: i.mutedColor, emphasis: true };
    return { text: i.readoutText, color: i.readoutColor, emphasis: false };
};

// Slot sits above the preset row and must not widen the right column past it,
// or a long freeze-reason combo would push the row out of line. Cap it to the
// preset row's width and clip with an ellipsis; the full text stays in `title`.
const slotStyle = (slot: MarketSlot, sizeXS: string): React.CSSProperties => ({
    fontSize: sizeXS,
    color: slot.color,
    fontWeight: slot.emphasis ? 700 : 400,
    marginBottom: "4rem",
    maxWidth: "260rem",
    overflow: "hidden",
...
const MarketSectionContent: React.FC<MarketSectionContentProps> = ({ actions }) => {
    ...
    const importPending = importAction.isPending;
    ...
    const exportPending = exportAction.isPending;
    ...
    // Single status/indicator slot above each preset row. The right column must
    // stay two tiers tall (slot + buttons) in EVERY state, or the footer's fixed
    // 280rem height overflows and the last line clips. So error, unavailable,
    // freeze, pending and the risk/yield readout share one line by priority
    // instead of stacking a separate indicator above and a status below.
    const importSlot = buildSlot({
        error: importError,
        blockedText: !shadowImportAvailable ? importUnavailableText : "",
        pending: importPending,
        pendingText: l.t("UI_PROCESSING"),
        readoutText: `${l.t("UI_MARKET_IMPORT_RISK")} ${discoveryRisk > 0 ? `${Math.round(discoveryRisk * 100)}%` : l.t("UI_MARKET_NA")}`,
        readoutColor: riskColor,
        crisisColor: accents.crisis.accent,
        mutedColor: theme.colors.textMuted,
    });
    const exportSlot = buildSlot({
        error: exportError,
        blockedText: !shadowExportAvailable ? exportUnavailableText : "",
        pending: exportPending,
        pendingText: l.t("UI_PROCESSING"),
        readoutText: `${l.t("UI_MARKET_INCOME")} ${dailyIncome > 0 ? `+${formatMoney(dailyIncome)}${l.t("UI_UNIT_PER_DAY")}` : l.t("UI_MARKET_NA")}`,
        readoutColor: dailyIncome > 0 ? accents.schemes.accent : theme.colors.textMuted,
        crisisColor: accents.crisis.accent,
        mutedColor: theme.colors.textMuted,
    });
        ...
        <div style={s.market.container}>
            ...
            <div style={s.market.row}>
                ...
                <div style={{ display: "flex", flexDirection: "column" as const, minHeight: 0, alignItems: "flex-end" }}>
                    {/* One slot: error / unavailable / pending / risk readout by priority */}
                    <div style={slotStyle(importSlot, theme.typography.sizeXS)} title={importSlot.text}>
                        {importSlot.text}
                    </div>
                    <Row>
            ...
            <div style={s.market.row}>
                ...
                <div style={{ display: "flex", flexDirection: "column" as const, minHeight: 0, alignItems: "flex-end" }}>
                    {/* One slot: error / unavailable / pending / yield readout by priority */}
                    <div style={slotStyle(exportSlot, theme.typography.sizeXS)} title={exportSlot.text}>
                        {exportSlot.text}
                    </div>
                    <Row>
