        ...
        <>
            ...
            <div style={styles.section}>
                ...
                <Row style={{
                    ...
                    <button
                        onClick={() => triggerCivic(B.DebugGrantShadowCash, 1_000_000)}
                        style={actButtonStyles.inactive}
                    >
                        +$1M Shadow Cash
                    </button>
                </Row>
            ...
            {/* Corruption Debug — force the maintenance-contract decision modal.
                Deferred to the next MaintenanceContractSystem tick; while paused the
                offer appears right after unpause. */}
            <div style={styles.sectionNoBorder}>
                <DebugSectionTitle>Corruption (Debug)</DebugSectionTitle>
                <Row style={{
                    flexWrap: "wrap" as const,
                    marginTop: "4rem",
                }}>
                    <button
                        onClick={() => triggerCivic(B.DebugSpawnContractOffer)}
                        style={actButtonStyles.inactive}
                    >
                        Spawn Contract Offer
                    </button>
                </Row>
            </div>

            {/* Countermeasures Debug — force the two forced-choice modals so their
                pause-safe decision routes can be tested on demand. Applies
                immediately (sync mutator), works while paused. */}
            <div style={styles.sectionNoBorder}>
                <DebugSectionTitle>Countermeasures (Debug)</DebugSectionTitle>
                <Row style={{
                    flexWrap: "wrap" as const,
                    marginTop: "4rem",
                }}>
                    <button
                        onClick={() => triggerCivic(B.DebugForceInvestigationChoice)}
                        style={actButtonStyles.inactive}
                    >
                        Force Investigation Choice
                    </button>
                    <button
                        onClick={() => triggerCivic(B.DebugForcePoliceChoice)}
                        style={actButtonStyles.inactive}
                    >
                        Force Police Choice
                    </button>
