...
export const ScenarioTab: React.FC<TabProps> = ({ debug, styles, theme, accents }) => {
    ...
    // Forced target stratum for dev PSYOPS raids: 0 = weighted (in-play), 1/2/3 = Poor/Middle/Wealthy.
    const [psyTargetStratum, setPsyTargetStratum] = useState<0 | 1 | 2 | 3>(0);
    const raw = debug.raw;
        ...
        <>
                ...
                <>
                    ...
                    <div style={styles.sectionNoBorder}>
                        ...
                        <div style={{ display: "flex", flexWrap: "wrap", marginBottom: "6rem" }}>
                            ...
                            <button onClick={() => triggerCivic(B.DebugTestExplosion)} style={{ ...btnWarn, marginRight: "4rem" }}>Test Explosion</button>
                            <button onClick={() => triggerCivic(B.DebugDrainAAAmmo)} style={btnWarn}>Drain AA Ammo</button>
                        </div>
            ...
            {subTab === "intel" && (
                <>
                    ...
                    <div style={styles.section}>
                        <DebugSectionTitle>PSYOPS Raids</DebugSectionTitle>
                        {/* In play a raid only composes when a wave enters its PsyAttack phase. These
                            drive the same composition on demand: -1 = full raid for the current wave
                            (gap-biased dominant + secondaries), 0/1/2 = a single carrier, so one type
                            can be read against one hero posture. The Target row forces the stratum —
                            without it a stratum-gated effect (draft fear, fake sick-out) means
                            re-rolling until the right class comes up. Grep the log for [PsyOpsRaid] /
                            [PsyOpsHarm]. Land Now pulls the in-flight contacts down to this instant —
                            damage only exists after they land, and the flight is hours of game time. */}
                        <div style={{ display: "flex", flexWrap: "wrap", marginBottom: "4rem" }}>
                            {([[0, "Auto"], [1, "Poor"], [2, "Middle"], [3, "Wealthy"]] as const).map(([id, label]) => (
                                <button key={id} onClick={() => setPsyTargetStratum(id)}
                                    style={{ ...(psyTargetStratum === id ? btnSuccess : btnWarn), marginRight: "4rem", marginBottom: "4rem" }}>
                                    {psyTargetStratum === id ? `● ${label}` : label}
                                </button>
                            ))}
                        </div>
                        <div style={{ display: "flex", flexWrap: "wrap", marginBottom: "6rem" }}>
                            <button onClick={() => triggerCivic(B.DebugLaunchPsyRaid, 0, psyTargetStratum)} style={{ ...btnWarn, marginRight: "4rem", marginBottom: "4rem" }}>Propaganda</button>
                            <button onClick={() => triggerCivic(B.DebugLaunchPsyRaid, 1, psyTargetStratum)} style={{ ...btnWarn, marginRight: "4rem", marginBottom: "4rem" }}>Rumor</button>
                            <button onClick={() => triggerCivic(B.DebugLaunchPsyRaid, 2, psyTargetStratum)} style={{ ...btnDanger, marginRight: "4rem", marginBottom: "4rem" }}>Fake Video</button>
                            <button onClick={() => triggerCivic(B.DebugLaunchPsyRaid, -1, psyTargetStratum)} style={{ ...btnDanger, marginRight: "4rem", marginBottom: "4rem" }}>Full Raid</button>
                            <button onClick={() => triggerCivic(B.DebugLandPsyRaid)} style={{ ...btnSuccess, marginBottom: "4rem" }}>Land Now</button>
                        </div>
                    </div>
                    <div style={styles.section}>
                        <DebugSectionTitle>Quests</DebugSectionTitle>
                        {/* Force-arms a quest past its chance/cooldown/once-per-city gate; the
                            lifecycle then runs on real events. Kotleta completes on the next district
                            blackout that ENDS (Force Collapse + recovery covers it), Voice counts
                            blunted psy landings (Launch + right speaker + Land Now). Grep [Quest]. */}
                        <div style={{ display: "flex", flexWrap: "wrap" }}>
                            <button onClick={() => triggerCivic(B.DebugTriggerQuest, 1)} style={{ ...btnWarn, marginRight: "4rem", marginBottom: "4rem" }}>Arm Kotleta Lights</button>
                            <button onClick={() => triggerCivic(B.DebugTriggerQuest, 2)} style={{ ...btnWarn, marginRight: "4rem", marginBottom: "4rem" }}>Arm Right Voice</button>
                        </div>
                    </div>
                    <div style={styles.sectionNoBorder}>
                        ...
                        <div style={{ display: "flex", flexWrap: "wrap" }}>
                            ...
                            <button onClick={() => triggerCivic(B.DebugForceDraftOffer)} style={{ ...btnWarn, marginRight: "4rem", marginBottom: "4rem" }}>Force Draft Offer</button>
                        </div>
                    </div>
                </>
            )}
