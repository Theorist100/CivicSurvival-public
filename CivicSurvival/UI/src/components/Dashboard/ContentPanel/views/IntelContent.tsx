...
// Navy calm-down (approved mock 2026-07-22, follows the COMMAND tab): neutral button
// with the price/action text — colour is reserved for the destructive action (crisis)
// and the active Counter-OSINT state (schemes).
const navyButton: React.CSSProperties = {
    padding: "8rem 14rem",
    background: "transparent",
    border: `1rem solid ${WARROOM_NAVY.borderStrong}`,
    borderRadius: "4rem",
    color: WARROOM_NAVY.label,
    cursor: "pointer",
    fontSize: "11rem",
    fontWeight: 600,
    flex: 1,
...
// Valera's OPSEC note as an amber-edged briefing card instead of a bare muted line.
const valeraNote: React.CSSProperties = {
    background: WARROOM_NAVY.surface,
    border: `1rem solid ${WARROOM_NAVY.border}`,
    borderLeft: `3rem solid ${WARROOM_NAVY.amber}`,
    borderRadius: "4rem",
    padding: "8rem 12rem",
    fontSize: "11rem",
    lineHeight: 1.5,
...
const OpSecPanel = memo(({ actions, spotters }: OpSecPanelProps) => {
    ...
    const compromised = spotters.SpotterCount !== 0;
    const penalty = spotters.SpotterPenaltyPercent ?? 0;
    // Counter-OSINT reads its state off the button it lives on ("Enable" vs "Disable"),
    // and its ground fills while it runs — a status row of its own said the same thing
    // one line further down. What it DOES stays in the tip.
    const cosintTip = spotters.CanToggleCounterOSINT ? l.t("COSINT_NOTE") : counterLockedReason;

    // Column-with-gap here, not on the parent: Flex hands its gap down as a margin on each
    // child element, and a child that is a COMPONENT (this panel) swallows the style prop —
    // so the spacing has to be owned by whoever renders the actual blocks.
    return (
        <Column gap={theme.spacing.sm}>
            <SectionHeader
                title={l.t("INTEL_PANEL_TITLE")}
                titleAs="span"
                titleStyle={s.panelHeader(WARROOM_NAVY.label)}
                extra={compromised && (
            ...
            {/* Spotter count and the intercept penalty it buys the enemy are two numbers,
                not two sections: side by side they read as one verdict on the OPSEC picture. */}
            <Row gap={theme.spacing.xs} align="stretch">
                <StatTile
                    label={l.t("OPSEC_SPOTTERS")}
                    value={spotters.SpotterCount}
                    color={compromised ? accents.crisis.accent : WARROOM_NAVY.text}
                    alertColor={compromised ? accents.crisis.accent : undefined}
                />
                <StatTile
                    label={l.t("OPSEC_PENALTY")}
                    value={`-${penalty}%`}
                    color={penaltyColor}
                    alertColor={penalty > 10 ? accents.crisis.accent : undefined}
                />
            </Row>

            {compromised ? (
                <div style={valeraNote}>{l.t("OPSEC_VALERA_NOTE")}</div>
            ) : (
                <div style={s.clear}>{l.t("OPSEC_CLEAR")}</div>
            )}

            <Row gap={theme.spacing.sm} align="stretch">
                <HoverTip text={spotters.CanSbuVisit ? l.t("OPSEC_SBU_TIP") : sbuLockedReason} style={{ flex: 1, display: "flex" }}>
                    <button
                        style={{ ...navyButton, width: "100%" }}
                ...
                <HoverTip text={spotters.CanEvacuationRun ? l.t("OPSEC_EVAC_TIP") : evacuationLockedReason} style={{ flex: 1, display: "flex" }}>
                    <button
                ...
                <HoverTip text={cosintTip} style={{ flex: 1, display: "flex" }}>
                    <button
                        style={spotters.CounterOSINTActive
                            ? {
                                ...s.button(accents.schemes.accent),
                                width: "100%",
                                background: hexToRgba(accents.schemes.accent, 0.14),
                            }
                            : { ...navyButton, width: "100%" }}
                    ...
                    >
                        {counterOsintAction.isPending
                            ...
                            ? l.t("COSINT_BTN_DISABLE", formatCostArg(spotters.CounterOSINTDailyCost))
                            : l.t("COSINT_BTN_ENABLE", formatCostArg(spotters.CounterOSINTDailyCost))}
                    </button>
                </HoverTip>
            </Row>
            <div style={s.stats}>
    ...
    return (
        <Column style={{
            ...
            {isPreWar ? (
                ...
                <>
                    {/* No card wrappers here: each section already draws its own framed
                        surface (navy-tinted via the sections themselves) — a wrapper made a
                        double frame AND ate 24rem of the 280rem column, wrapping the
                        forecast tiles mid-word (live report 2026-07-22). */}
...
const IntelContentComponent: React.FC = () => {
    ...
    return (
        <div style={{
            ...
            {/* Sized to hold its worst case (police case + choice grid + critical-heat warning
                + an unbought insider) without scrolling. overflowY stays `auto` as a safety net
                for a locale whose strings wrap further than ours. */}
            <Column style={{
                flex: 1,
                padding: theme.spacing.md,
                overflowY: "auto" as const,
