    ...
    return (
        <Column style={{
            ...
            {/* Header: title + status badges share one baseline (panelHeader's built-in
                ...
                The "?" carries the pool explanation — why a town fields so few crews, what
                shrinks the pool and what raises it — mirroring the AA column's help slot. */}
            <SectionHeader
                title={l.t("MANPOWER_TITLE")}
                titleAs="span"
                titleStyle={{ ...s.panelHeader(accents.operations.accent), marginBottom: 0 }}
                style={{ flexWrap: "wrap", marginBottom: "10rem" }}
                extra={<>
                ...
                help={<HelpSection id="manpower" title={l.t("MANPOWER_TITLE")}>{l.t("HELP_MANPOWER")}</HelpSection>}
            />

            {/* A dash, not "0 / 0", while the pool is unmeasured. Zero here is a real state
                (every head committed) and the player acts on it; printing it for a city
                nobody has counted is the display half of the same defect the placement gate
                carries on the decision half. */}
            <StatRow
                label={l.t("MANPOWER_AVAILABLE")}
                value={manpower.IsManpowerMeasured
                    ? `${manpower.ManpowerAvailable} / ${manpower.ManpowerTotal}`
                    : "—"}
                color={manpowerColor}
            />
            <ProgressBar
                value={manpower.IsManpowerMeasured ? manpower.ManpowerPercent : 0}
                color={manpowerColor}
                height="8rem"
            />
            ...
            {manpower.ManpowerPatriotismFactor < 100 && (
                /* The bare percentage was unreadable: players saw "Patriot 50%" with no way to
                   learn what moved it, and asked in Discord rather than in game. Naming the
                   aggregate and its size points straight at the schemes that caused it. */
                <Row style={{ padding: "0 10rem", marginTop: "4rem" }}>
                    <span style={{ fontSize: "11rem", color: theme.colors.textPrimary, opacity: 0.7 }}>
                        {`${l.t("MANPOWER_PATRIOT_CAUSE")}: ${manpower.ManpowerCorruptionScore}/100`}
                    </span>
                </Row>
            )}
