...
export const StrikeBuildSection: React.FC = () => {
    ...
    const droneAction = useRequestAction(() => {
    ...
    const ballisticAction = useRequestAction(() => {
        scLog("[CivicSurvival] Place ballistic launcher (DEFENSE tab, STRIKE mode)");
        triggerCivic(B.PlaceBallisticLauncher);
        return true;
    }, gw.BallisticLauncherPlacementRequest);

    // One tile helper serves both munitions: they differ only in trigger, request lifecycle and
    // readout, so the take-in-hand behaviour stays in one place.
    const unitApi = (unit: StrikeUnit) => unit === "drone"
        ? { action: droneAction, trigger: B.PlaceDroneLauncher, logName: "drone launcher" }
        : { action: ballisticAction, trigger: B.PlaceBallisticLauncher, logName: "ballistic launcher" };
    ...
    const selectedAction = droneSelected ? droneAction : ballisticAction;
        ...
        <>
            ...
            <Flex wrap="wrap" gap="6rem">
                ...
                {tile(
                    ...
                    l.t("UI_STRIKE_LAUNCHES_BALLISTIC"),
                    true,
            ...
            <div style={{
                ...
                {stat(
                    l.t("UI_STRIKE_STAT_LAUNCHES"),
                    droneSelected ? l.t("UI_STRIKE_LAUNCHES_DRONES") : l.t("UI_STRIKE_LAUNCHES_BALLISTIC"),
                )}
                {stat(
                    l.t("UI_DEFENSE_DEPLOYED"),
                    String(droneSelected ? gw.DroneLauncherCount : gw.BallisticLauncherCount),
                )}
                {/* Name the purse (the AA card precedent): this money leaves the shadow
                    wallet, and colour alone is not a label. */}
                {stat(
                    l.t("AA_COST"),
                    l.t("UI_AA_COST_SHADOW", formatCostArg(droneSelected ? gw.DroneLauncherCost : gw.BallisticLauncherCost)),
                    accent,
                )}
                <div style={{
                    marginTop: "8rem",
                    ...
                    {droneSelected ? l.t("UI_STRIKE_LAUNCHER_NOTE") : l.t("UI_STRIKE_BALLISTIC_NOTE")}
                </div>
                {selectedAction.failureReasonId ? (
                    <div style={{
                        marginTop: "8rem",
                        padding: "7rem",
                        border: `1rem solid ${theme.colors.border}`,
                        ...
                        {l.tDynamic(selectedAction.failureReasonId)}
                    </div>
                ) : null}
            </div>
        </>
