...
import { IconRadar } from "../../../../shared/common/Icons";
...
export const WarRoomIntel: React.FC = memo(() => {
    ...
    const s = useMemo(() => ({
        // Intel briefing CARD (approved mock 2026-07-22): a framed navy surface with an
        // iconed header — the flat borderless list read as filler next to the ops rows.
        block: {
            display: "flex",
        ...
        title: {
            ...
            paddingBottom: "6rem",
            borderBottom: `1rem solid ${WARROOM_NAVY.border}`,
        } as React.CSSProperties,
        titleIcon: {
            display: "flex",
            alignItems: "center",
    ...
    // Readout colour language (approved mock): known facts — white, UNKNOWN — point
    // amber, genuinely critical (attack underway) — the one hot crisis accent.
    const waveTypeKnown = Boolean(intel.WaveTypePrediction);
    const etaStatus = intel.TimeEstimate?.Status;
    const etaColor = etaStatus === "in-attack"
        ? accents.crisis.accent
        : etaStatus === "unknown" ? WARROOM_NAVY.amber : WARROOM_NAVY.text;

    return (
        <div style={s.block}>
            <div style={s.title}>
                <span style={s.titleIcon}><IconRadar /></span>
                {l.t("UI_WARROOM_INTEL")}
            </div>

            <div style={s.subTitle}>{l.t("UI_WARROOM_INTEL_FORECAST")}</div>
            <div style={s.row}>
                <span style={s.rowLabel}>{l.t("UI_WARROOM_INTEL_WAVE_TYPE")}</span>
                <span style={s.rowValue(waveTypeKnown ? WARROOM_NAVY.text : WARROOM_NAVY.amber)}>{waveType}</span>
            </div>
            <div style={s.row}>
                <span style={s.rowLabel}>{l.t("UI_WARROOM_INTEL_COMPOSITION")}</span>
                <span style={s.rowValue(swarmKnown ? WARROOM_NAVY.text : WARROOM_NAVY.amber)}>{composition}</span>
            </div>
            <div style={s.row}>
                <span style={s.rowLabel}>{l.t("UI_WARROOM_INTEL_WINDOW")}</span>
                <span style={s.rowValue(etaColor)}>{eta}</span>
            </div>
