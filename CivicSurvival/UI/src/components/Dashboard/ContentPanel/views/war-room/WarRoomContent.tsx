...
const WarRoomReady = memo(({
    ...
    // BUILD LAUNCHER: the overlay covers the map, so the tool cannot be used here. Take the
    // launcher in hand first (sync trigger), then land the Dashboard on the DEFENSE tab in
    // STRIKE build mode and drop the overlay — the placement tool is already live when the
    // map becomes visible. One helper for both munitions; only the trigger differs.
    const buildLauncher = (trigger: typeof B.PlaceDroneLauncher | typeof B.PlaceBallisticLauncher) => {
        triggerCivic(trigger);
        setDefenseBuildMode("strike");
        navigateDashboard("war", "defense");
        onClose();
    };
    const onBuildDroneLauncher = () => buildLauncher(B.PlaceDroneLauncher);
    const onBuildBallisticLauncher = () => buildLauncher(B.PlaceBallisticLauncher);
    ...
    const ballisticLauncherCount = gw.BallisticLauncherCount ?? 0;
            ...
            <div style={s.body}>
                ...
                <div style={s.sidePanel("right")}>
                    ...
                    <div style={s.arsenalBlock}>
                        ...
                        {/* Launcher emplacements — one per munition, each the origin its own
                            outbound strikes fly from. The fullscreen overlay covers the map, so
                            placement cannot happen here: the button closes the overlay, lands the
                            Dashboard on the DEFENSE tab with the build section in STRIKE mode, and
                            takes that launcher in hand (sync and pause-safe) — the tool is already
                            live when the tab appears. Counts are separate because a drone cannot
                            launch from a ballistic pad and vice versa. */}
                        <div style={s.counterRow}>
                        ...
                        <div
                            ...
                            role="button"
                            tabIndex={0}
                            onClick={onBuildDroneLauncher}
                            onKeyDown={(e) => {
                                if (e.key === "Enter" || e.key === " ") {
                                    e.preventDefault();
                                    onBuildDroneLauncher();
                                }
                            }}
                        >
                        ...
                        <div style={s.counterRow}>
                            <span style={s.counterLabel}>{l.t("UI_WARROOM_BALLISTIC_LAUNCHERS")}</span>
                            <span style={s.counterValue(accents.schemes.accent)}>{ballisticLauncherCount}</span>
                        </div>
                        <div
                            style={s.arsenalButton(accents.schemes.accent)}
                            role="button"
                            tabIndex={0}
                            onClick={onBuildBallisticLauncher}
                            onKeyDown={(e) => {
                                if (e.key === "Enter" || e.key === " ") {
                                    e.preventDefault();
                                    onBuildBallisticLauncher();
                                }
                            }}
                        >
                            {l.t("UI_WARROOM_BUILD_BALLISTIC_LAUNCHER")}
                        </div>
                    </div>
