...
import { useUpdateAnnounce } from "../../../hooks/useUpdateAnnounce";
...
const DomainTabsComponent: React.FC<DomainTabsProps> = ({ activeDomain, onDomainChange }) => {
    ...
    const l = useLocale();
    // Update-announce countdown takes over the BETA badge slot while active.
    const announce = useUpdateAnnounce();
            ...
            {announce.active
                ? (
                    <div style={s.updateBadge}>
                        {announce.minutesRemaining > 0
                            ? l.t("UI_UPDATE_COUNTDOWN_BADGE", String(announce.minutesRemaining))
                            : l.t("UI_UPDATE_COUNTDOWN_BADGE_NOW")}
                    </div>
                )
