...
export const createGlobalStatusStyles = (theme: Theme, accents: Accents) => {
    const phaseColors = getPhaseColors(accents, theme.colors.schemePurple);
    ...
    metricsGroupPrimary: {
        display: "flex",
        alignItems: "center",
        // shrink 0: the compact numeric metrics (Hz / MW / % / cut) are always shown
        // in full. At large values (e.g. "390 cut") the overflow used to clip the last
        // badge to "390 cu"; instead the verbose CRISIS badge in the secondary group
        // (which has its own overflow) absorbs the shrink and truncates first.
        flex: "0 0 auto",
        minWidth: 0,
        ...
        // space collapses to zero and "cut" butts against "CRISIS". A margin never
        // collapses. Coherent UI has no flex gap, so it lives here as marginRight.
    ...
    // Threat badge — never shrinks, so when the bar is full only the CRISIS
    // economy detail (to its left) gives way, not the phase pill.
    ...
    // Crisis Economy Badge — compact "CRISIS D1" pill; the full economy readout
    // lives in its hover tip, so the chip itself never needs to shrink.
    crisisEconomyBadge: {
        display: "flex",
        alignItems: "center",
        padding: `${theme.spacing.xs} 6rem`,
    ...
    // Quest chip — situational like the crisis badge; accent passed per phase
    // (operations = active, schemes = done, crisis = failed).
    questBadge: (accent: string, accentDim: string): React.CSSProperties => ({
        display: "flex",
        alignItems: "center",
        padding: `${theme.spacing.xs} 6rem`,
        ...
        // Never shrinks: the bar carries AT MOST ONE quest chip (a lone quest's full chip,
        // or the "QUESTS N" counter), and one compact chip always fits next to the other
        // badges. The shrink-with-ellipsis attempt lost to cohtml (it clipped the chip's
        // CONTENT edge instead — live report 2026-07-22), so the width guarantee is now
        // structural, not elastic.
        flex: "0 0 auto",
        whiteSpace: "nowrap" as const,
        cursor: "pointer",
    }),

    questBadgeLabel: (accent: string): React.CSSProperties => ({
        fontSize: "12rem",
        fontWeight: theme.typography.weightBold,
        color: accent,
        textTransform: "uppercase" as const,
