...
export const CrisisEconomyBadge: React.FC<BadgeProps> = memo(({ styles: s }) => {
    ...
    // Compact chip: "CRISIS D1" only — the bar's center is contested (quest chips live
    // to the left) and the full economy readout used to be its widest tenant, forcing
    // the left-edge clip on its neighbours. The full detail moved into the hover tip.
    const details = [
        crisisDay > 0 ? l.t("UI_STATUS_CRISIS_DAY", crisisDay) : "",
        taxMult < 1 ? l.t("UI_STATUS_TAX_REDUCTION", taxReduction) : "",
        !loansOk ? l.t("UI_STATUS_NO_LOANS") : "",
    ].filter((d) => d.length > 0).join(", ");

    return (
        <>
            <div style={s.crisisEconomyBadge}>
                <HoverTip text={details} style={s.crisisEconomyLabel}>
                    {crisisDay > 0
                        ? `${l.t("UI_STATUS_CRISIS")} ${l.t("UI_STATUS_CRISIS_DAY_SHORT", crisisDay)}`
                        : l.t("UI_STATUS_CRISIS")}
                </HoverTip>
            </div>
            <div style={s.separator} />
        </>
