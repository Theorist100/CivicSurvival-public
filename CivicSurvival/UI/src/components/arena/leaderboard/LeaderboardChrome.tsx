...
export const LeaderboardOptInOverlay: React.FC<LeaderboardOptInOverlayProps> = ({ onlineEnabled, onlineConsentRecorded }) => {
    ...
    // gate (onlineConsentRecorded) as SettingsOnlineContent — not a second consent mechanism.
    // On a first enable the prompt is shown here; once a decision is recorded, the button
    // toggles Online directly, matching SettingsOnlineContent.handleGlobalConnectionToggle.
    const [showConsent, setShowConsent] = useState(false);
    // Local latch mirroring SettingsOnlineContent: closes the sub-frame window between a consent
