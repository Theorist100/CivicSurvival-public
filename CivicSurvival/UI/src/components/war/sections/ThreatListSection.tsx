...
import { radarThemes, threatColors } from "@themes/radar";
...
import { OutboundBlock, ThreatLegendBlock, WaveStatusBlock } from "./threat-list/ThreatListBlocks";
...
export interface ThreatListSectionProps {
    ...
    // Player's outbound counter-strikes, listed apart from the enemy threats above.
    outbound?: Array<{ type: ThreatType; eta: number }>;
    phase: WavePhase;
    phaseColor: string;
    phaseName: string;
    timeInPhase: number;
...
export const ThreatListSection: React.FC<ThreatListSectionProps> = memo((props) => {
    ...
    // Outbound salvo grouped by type — same shape as the enemy legend, allied accent.
    const outboundGroups = useMemo(() => {
        const groups: Record<TranslationKey, { count: number; minEta: number }> = {} as Record<TranslationKey, { count: number; minEta: number }>;
        for (const track of outbound) {
            const typeKey = THREAT_TYPE_MAP[track.type] ?? DEFAULT_THREAT_KEY;
            if (!groups[typeKey]) {
                groups[typeKey] = { count: 0, minEta: Infinity };
            }
            groups[typeKey].count++;
            groups[typeKey].minEta = Math.min(groups[typeKey].minEta, track.eta);
        }
        return (Object.entries(groups) as Array<[TranslationKey, { count: number; minEta: number }]>)
            .sort((a, b) => a[1].minEta - b[1].minEta);
    }, [outbound]);

    const outboundGroupStyles = useMemo(() => {
        const map: Partial<Record<TranslationKey, React.CSSProperties>> = {};
        for (const [typeKey] of outboundGroups) {
            map[typeKey] = {
                padding: "8rem 10rem",
                background: hexToRgba(threatColors.outbound, 0.07),
                borderRadius: "4rem",
                borderLeft: `4rem solid ${threatColors.outbound}`,
            };
        }
        return map;
    }, [outboundGroups]);

    const outboundTitleStyle = useMemo((): React.CSSProperties => ({
        fontSize: "11rem",
        fontWeight: 700,
        textTransform: "uppercase" as const,
        color: threatColors.outbound,
    ...
    const s = useMemo(() => ({
        ...
        phaseIndicator: {
            ...
            boxShadow: isActiveAssaultPhase(phase) ? `0 0 8rem ${phaseColor}` : "none",
            marginRight: "8rem",
        } as React.CSSProperties,
        phaseName: {
            color: phaseColor,
    ...
    return (
        <Column>
            ...
            <OutboundBlock
                ...
                outboundGroupStyles={outboundGroupStyles}
                titleStyle={outboundTitleStyle}
                styles={s}
            />
