...
export const ReliefTab = memo(({ actions }: ReliefTabProps) => {
    ...
    const distributePending = distributeAction.isPending;
    const procurementPending = procurementAction.isPending;
    const distributeErrorKey = distributeAction.failureReasonId;
    const procurementErrorKey = procurementAction.failureReasonId;
    ...
    const s = useMemo(() => ({
        ...
        // The running daily cost is a live spend decision, not a footnote — read it
        // at the secondary-text size, not the tiny 10rem hint size.
        costHint: {
    ...
    // The payer differs by wave: offshore shadow cash when ShadowEconomy is open,
    // the municipal budget otherwise — the hint names the real one.
    const dailyCostKey = buckwheat?.ShadowFunded ? "UI_RELIEF_DAILY_COST" : "UI_RELIEF_DAILY_COST_BUDGET";

    const exportData = optionalBindingData(exportBinding);

    return (
        <Column style={s.container}>
            {/* Reserve Display — vertically CENTERED: cohtml does not support
                align-items:baseline (falls back to flex-start, the unit floats to the
                top), and flex-end drops the small text below the number's optical
                middle. Center keeps the tonnage, unit and status read on one level. */}
            <Row align="center" style={s.reserveRow}>
            ...
            {(procurementLevel > 0 || procurementPending || procurementErrorKey) && (
                <div style={s.costHint}>
                    ...
                    : procurementLevel > 0
                        ? l.t(dailyCostKey, formatMoney(dailyCost))
                        : null}
                </div>
            )}
