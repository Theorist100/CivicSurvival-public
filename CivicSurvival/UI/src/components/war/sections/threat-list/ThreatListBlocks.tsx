...
interface OutboundBlockProps {
    outboundGroups: ThreatGroup[];
    outboundGroupStyles: Partial<Record<TranslationKey, React.CSSProperties>>;
...
// Player's outbound counter-strikes, listed apart from the enemy legend so the two
// factions never share a block. Rendered only when a salvo is airborne; grouped by type
// like the enemy legend but with an allied accent (no ETA line — outbound ETA is time to
// the enemy, not a threat to the city).
export const OutboundBlock: React.FC<OutboundBlockProps> = ({
    outboundGroups,
    outboundGroupStyles,
    titleStyle,
    styles: s,
    ...
    if (outboundGroups.length === 0) return null;

    return (
        <Column gap="4rem" style={s.legendBox}>
            <div style={titleStyle}>
                {l.t("UI_WARROOM_OUTBOUND")}
            </div>

            {outboundGroups.map(([typeKey, data]) => (
                <Column key={typeKey} gap="2rem" style={outboundGroupStyles[typeKey]}>
