...
const IntroModalView: React.FC = () => {
    ...
    // One-shot guard: Coherent/Gameface can dispatch the button's click twice for a
    // single activation (one frame apart), which fired OnAcceptReality a second time
    // in the Silence phase (C# guard logged "wrong state"). Same pattern the other
    // action buttons use (VictoryModal, ModLoadFailure). pendingRef swallows the
    // duplicate; the modal dismisses right after accept, so it never re-enables.
    const { execute: onAccept, isPending } = useAsyncAction(acceptReality);
    ...
    const customStyles = useMemo(() => ({
        ...
        // Intel line: for Town/City this modal is the LAST beat before StartWar (accepting
        // fires IntroCompleteEvent -> StartWar), so the "build air defence" priority in the
        // war-begins modal arrives only after the decision is made. The one actionable
        // warning therefore lives here, set apart from the narrative lines so it reads as
        // a briefing rather than another atmospheric beat.
        intel: {
            color: m.textPrimary,
            fontSize: "14rem",
            lineHeight: 1.7,
            margin: 0,
            padding: "12rem 16rem",
            borderLeft: `3rem solid ${m.accents.crisis}`,
            backgroundColor: m.overlay50,
        } as React.CSSProperties,
        button: {
            padding: "16rem 48rem",
            ...
            <div style={modalStyle}>
                ...
                <div style={bodyStyle}>
                    <div style={customStyles.textBlock}>
                        ...
                        <p style={customStyles.intel}>{l.t("INTRO_TEXT_8")}</p>
                    </div>
