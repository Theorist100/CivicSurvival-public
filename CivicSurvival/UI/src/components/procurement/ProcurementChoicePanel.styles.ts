...
export const createStyles = (t: Theme, accents: Accents) => ({
    overlay: {
        ...
        // The Coherent mod-layer root does not hit-test (pointer-events: none is
        // inherited), so every interactive surface must opt in explicitly — same
        // as modalStyles.ts. Without this every click fell through the modal
        // into the game (vanilla info panels opened under the cursor).
