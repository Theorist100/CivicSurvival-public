...
export const IntelPreview: React.FC<IntelPreviewProps> = memo(({
    ...
    const isPending = upgradeAction.isPending;
    ...
    // What the CURRENT level already grants (max level hits the full-awareness branch below).
    const getCurrentLevelDesc = (): string =>
        intelLevel === 0 ? l.t("UI_GW_INTEL_CURRENT_LV0") : l.t("UI_GW_INTEL_CURRENT_LV1");
        ...
        <div style={s.intelContainer}>
            ...
            <div style={s.intelContent}>
                {isMaxLevel ? (
                    ...
                    <span style={{ color: theme.colors.textMuted }}>{getCurrentLevelDesc()}</span>
                )}
            </div>
