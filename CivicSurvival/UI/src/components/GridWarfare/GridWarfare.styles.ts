...
export const createGridWarfareStyles = (theme: Theme, accents: Accents) => ({
    ...
    // the domain colour lives ONLY in the left edge (navy calm-down, approved mock
    // 2026-07-22) — active state still warms the surface, idle rows are neutral.
    operationRow: (edgeColor: string, isActive: boolean) => ({
        ...
        padding: "8rem 12rem",
        marginBottom: "6rem",
        background: isActive ? hexToRgba(edgeColor, 0.08) : WARROOM_NAVY.surface,
        borderRadius: theme.layout.borderRadius,
        border: `1rem solid ${isActive ? hexToRgba(edgeColor, 0.5) : WARROOM_NAVY.border}`,
