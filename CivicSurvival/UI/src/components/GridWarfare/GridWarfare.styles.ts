...
export const createGridWarfareStyles = (theme: Theme, accents: Accents) => ({
    ...
    // ========== Operation Rows (wide "rows" layout of OperationSlots) ==========
    // One horizontal row per operation for wide surfaces (dashboard COMMAND tab):
    // the domain colour moves to a left edge instead of a full-perimeter border,
    // and the row grows vertically (flex) so three rows fill the stable tab height.
    operationRow: (edgeColor: string, isActive: boolean) => ({
        display: "flex",
        alignItems: "center",
        flex: "1 1 0",
        minHeight: "56rem",
        padding: "8rem 12rem",
        marginBottom: "6rem",
        background: isActive ? hexToRgba(edgeColor, 0.08) : `${theme.colors.paper}`,
        borderRadius: theme.layout.borderRadius,
        border: `1rem solid ${isActive ? hexToRgba(edgeColor, 0.5) : theme.colors.border}`,
        borderLeft: `3rem solid ${edgeColor}`,
        transition: "background-color 0.2s ease, border-color 0.2s ease",
    } as React.CSSProperties),
    ...
    // Fixed-width action zone keeps every row's right edge aligned regardless of state.
    rowActions: {
        display: "flex",
        alignItems: "center",
        width: "230rem",
    ...
    rowButtonPrimary: (color: string, enabled: boolean = true) => ({
        flex: 1,
        minWidth: 0,
        padding: "8rem 6rem",
    ...
    rowButtonGhost: (color: string, enabled: boolean = true) => ({
        width: "86rem",
        flexShrink: 0,
        padding: "8rem 6rem",
        marginLeft: "6rem",
