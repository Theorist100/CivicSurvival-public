        ...
        <div style={rows ? { ...s.slotsContainer, flex: 1, minHeight: 0 } : s.slotsContainer}>
            ...
            <Row justify="space-between" align="center" style={{ marginBottom: rows ? "6rem" : "4rem" }}>
                <span style={{
                    fontSize: "11rem",
                    fontWeight: 700,
                    // Section header, not an alarm — the crisis red competed with every
                    // real alert on the tab (navy calm-down, mock 2026-07-22).
            ...
            {GRID_OPERATION_TYPES.map((type) => {
                ...
                // Every operation flies a physical munition, so EXECUTE (not Prepare/Cancel, not
                // arsenal purchase) is gated on owning a launcher of the matching kind. The munition
                // per operation follows ArsenalKindMap.ForCategory on the C# side: a drone strike
                // rides a drone, blackout and disinfo ride a ballistic round. Without the gate the
                // C# launch simply refuses and rolls the operation back, which reads as a dead
                // button — the reason id says which launcher to build instead.
                const needsBallistic = type !== "drone";
                const launcherCount = needsBallistic
                    ? (state.BallisticLauncherCount ?? 0)
                    : (state.DroneLauncherCount ?? 0);
                const executeLockedReasonId = launcherCount === 0
                    ? (needsBallistic ? "UI_GW_EXECUTE_NEED_BALLISTIC_LAUNCHER" : "UI_GW_EXECUTE_NEED_LAUNCHER")
                    : "";
                const common = {
                    type,
                    slot,
