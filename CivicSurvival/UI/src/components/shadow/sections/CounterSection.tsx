...
import { GlassCase, InlineWarning, ProgressBar, SectionTitle, StatRow, StatTile } from "../../shared/ui";
...
const CounterSectionContent: React.FC<CounterSectionContentProps> = ({ corruptionActions }) => {
    ...
    const requestPending = choiceAction.isPending;
    ...
    // Corruption and Heat are two shares of a whole, not two sections: as a pair of tiles they
    // read side by side with the phase that drives them, and the panel keeps the ~200rem of
    // height an open case needs for its progress bar, choices and warnings.
    return (
        <div style={s.section}>
            {/* Phase sits beside the title, NOT at the right edge: GlassCase parks its WAVE badge
                in that corner (absolute, top-right) and would print straight over it. */}
            <Row align="center" gap={theme.spacing.sm}>
                <SectionTitle style={{ marginBottom: 0 }}>{l.t("UI_COUNTER_TITLE")}</SectionTitle>
                <span style={{
            ...
            <Row gap={theme.spacing.xs} align="stretch" style={{ marginTop: theme.spacing.xs }}>
                <StatTile
                    label={l.t("UI_COUNTER_CORRUPTION")}
                    value={`${cm.CorruptionScore ?? 0}%`}
                    color={riskColor}
                    bar={cm.CorruptionScore ?? 0}
                    barColor={riskColor}
                    alertColor={(cm.CorruptionScore ?? 0) > 50 ? accents.crisis.accent : undefined}
                />
                <StatTile
                    label={l.t("UI_COUNTER_HEAT")}
                    value={`${cm.Heat ?? 0}%`}
                    sub={l.t(heatKey)}
                    color={heatColor}
                    bar={cm.Heat ?? 0}
                    barColor={heatColor}
                    alertColor={heatKey === "UI_COUNTER_HEAT_LEVEL_CRITICAL" || heatKey === "UI_COUNTER_HEAT_LEVEL_DANGER"
            ...
            {showChoiceControls && (
                <div style={s.choicePanel}>
                    ...
                    {/* One row, not a 2x2 grid: the responses are a single choice, and stacking them
                        two-high cost the panel a whole row of height it does not have. */}
                    <Row gap={theme.spacing.xs} align="stretch" style={{ marginTop: theme.spacing.sm }}>
                        {choiceButtons.map((choice) => {
                            ...
                            return (
                                <button
                                    key={choice.value}
                                    type="button"
                                    style={{ ...s.choiceButton(disabled, accents.crisis.accent), flexGrow: 1, flexShrink: 1, flexBasis: 0, minWidth: 0 }}
                                    disabled={disabled}
