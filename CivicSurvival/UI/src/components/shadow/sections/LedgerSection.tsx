...
// Income/expense rows are running totals since the city was founded, not a daily rate
// and not a forecast: CityBudgetHost only ever adds to m_IncomeBySource and persists it
// across saves. The "All Time" section titles carry that — they are load-bearing, not
// decoration. These land in the CITY TREASURY; the offshore balance above is a separate
// wallet, which is why a large income row can sit next to a $0 offshore fund.
const INCOME_L10N: Record<string, TranslationKey> = {
    DonorAid: "UI_LEDGER_INC_DONOR_AID",
    // Crisis injection into the treasury (CrisisEconomicsSystem), NOT the "Emergency
    // Fund Raid" scheme that pays offshore — hence the distinct "Crisis Aid" label.
...
export const LedgerSection = memo(() => {
    ...
    return (
        <GlassCase
            ...
            name="Shadow Ledger"
            description="Total city liquidity: official treasury, offshore funds, and the war expenses, income and debt booked since the city was founded. Unlocks when shadow economy systems come online."
        >
            {/* Total Liquidity */}
            <div style={s.section}>
                <SectionHeader
            ...
            {/* Net Position — the bottom line as a traffic-light card: the title says
                what the number MEANS (cost vs profit) and the frame carries the sign,
                so the verdict reads before the digits do. */}
            {(finance.hasExpenses || finance.hasIncome) && (
                <div style={{
                    ...s.section,
                    border: `2rem solid ${finance.netColor}`,
                    background: hexToRgba(finance.netColor, 0.08),
                }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <div style={{ ...s.sectionTitle, marginBottom: 0, color: finance.netColor }}>
                            {l.t(finance.netPosition >= 0 ? "UI_LEDGER_NET_POSITION_PROFIT" : "UI_LEDGER_NET_POSITION_COST")}
                        </div>
                        {/* One string, not two adjacent JSX expressions — Coherent breaks
                            {sign}{value} onto separate lines; a single text node stays inline. */}
                        <div style={{ ...s.valueLarge(finance.netColor), textAlign: "right", padding: 0 }}>
