...
/**
 * Munition kind ids — must mirror the C# AmmoKind enum (Core/Types/ThreatTypes.cs). What a gun
 * eats is derived there from its weapon class, never listed by hand, so this stays a two-member
 * enum no matter how many AA types exist. A third member is earned by a genuinely different
 * economy (a cheap-rocket MANPADS, say), not by a new gun.
 */
export const AMMO_KIND = {
    Shells: 0,
    Rockets: 1,
} as const;

export type AmmoKind = (typeof AMMO_KIND)[keyof typeof AMMO_KIND];

/**
 * Placement mode ids — must mirror the C# AAPlacementMode enum
 * (Core/Components/Domain/AirDefense/AAInstallationPayload.cs). How a barrel is acquired: bought,
 * granted from the city reserves, or donated.
 */
export const AA_PLACEMENT_MODE = {
    Paid: 0,
    Heritage: 1,
    DonorCredit: 2,
    BlackMarket: 3,
...
export type AaPlacementModeId = (typeof AA_PLACEMENT_MODE)[keyof typeof AA_PLACEMENT_MODE];

/** Mode id back to the enum NAME the placement trigger carries (payload is "prefab|Mode"). */
export function placementModeName(mode: number): "Paid" | "Heritage" | "DonorCredit" | "BlackMarket" {
    if (mode === AA_PLACEMENT_MODE.Heritage) return "Heritage";
    if (mode === AA_PLACEMENT_MODE.DonorCredit) return "DonorCredit";
    if (mode === AA_PLACEMENT_MODE.BlackMarket) return "BlackMarket";
    return "Paid";
}
