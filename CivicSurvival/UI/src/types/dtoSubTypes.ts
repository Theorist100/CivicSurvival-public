...
export type WavePhase = "calm" | "alert" | "attack" | "psyattack" | "recovery";
...
export const isWavePhase = (value: unknown): value is WavePhase =>
    value === "calm" || value === "alert" || value === "attack" || value === "psyattack" || value === "recovery";

/** Phases that read as an active enemy assault (UI mirror of WaveExecutor's IsUnderAttack set). */
export const isActiveAssaultPhase = (phase: WavePhase): boolean =>
    phase === "attack" || phase === "psyattack";
...
// Lives here (not domainDtos.ts) so domainDtos.generated.ts can import it for
// nested RequestResult defaults without a domainDtos.ts <-> generated.ts
// import cycle; re-exported from domainDtos.ts via `export * from './dtoSubTypes'`.
