...
// The single "did the backend actually deliver this yet" gate for every
// bindingDataOrDefault consumer. False for `loading` (the owning C# system
// has not published — e.g. its feature/singleton does not exist yet) and for
// `invalid` (payload rejected by the type guard): in both cases the value in
// hand is a locally constructed DEFAULT_*, not live game state. Any surface
// that derives a verdict, a status colour, or an "all clear / critical" claim
// from binding data MUST gate that claim on this flag and render a neutral
// muted placeholder ("—" / UI_NO_DATA, same geometry) until it flips true.
// Do NOT hand-roll `state.status === "ready"` checks next to
// bindingDataOrDefault — use this helper so readiness gating stays one
// greppable pattern.
export function isBindingLive<T>(state: BindingState<T>): boolean {
