...
// Why the conference button is locked, keyed by DonorStatus (from GetStatusString).
// "available" needs no reason; "sanctions" keeps its own detailed warning on the button.
const CONFERENCE_LOCK_KEYS: Record<string, TranslationKey> = {
    too_early: "AID_LOCKED_TOO_EARLY",
    insufficient_attention: "AID_LOCKED_INSUFFICIENT_ATTENTION",
    no_uses: "AID_LOCKED_NO_USES",
    cooldown: "AID_LOCKED_COOLDOWN",
    trust_locked: "AID_LOCKED_UNAVAILABLE",
    unavailable: "AID_LOCKED_UNAVAILABLE",
};
...
export function useDonorData() {
    ...
    return useMemo(() => {
        ...
        return {
            ...
            conferenceLockedReasonKey: donor.DonorStatus === "available"
                ? null
                : CONFERENCE_LOCK_KEYS[donor.DonorStatus] ?? null,
            shockColor,
            trustColor,
