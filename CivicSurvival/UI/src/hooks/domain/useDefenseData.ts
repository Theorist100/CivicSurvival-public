...
export function useDefenseData() {
    ...
    return useMemo(() => {
        // An unmeasured pool is not an alarming one. The thresholds below read a percentage
        // that, in the unmeasured window, is a frozen last-good value — and the lowest band
        // paints the panel crisis-red, which is the loudest thing this widget can say about a
        // number nobody has taken. Neutral while unknown; the dash next to it carries the
        // actual message.
        const manpowerColor = !manpower.IsManpowerMeasured ? theme.colors.textMuted
            : manpower.ManpowerPercent < 25 ? accents.crisis.accent
