...
/**
 * Whether {@link populationPercent$} came from a reading the city actually answered.
 * False = draw a dash: the percent then holds the last good value and says nothing about
 * now. There is no in-band way to spell that on a float binding, which is why it travels
 * on its own channel instead of as a sentinel percentage.
 */
export const populationMeasured$ = bindCivicValue(B.PopulationMeasured, false);
...
/** Dismiss the info-war (PSYOPS) after-action report modal */
export function dismissPsyDebriefing(): void {
    triggerCivic(B.DismissPsyDebriefing);
}

/** Acknowledge the inbound info-war raid alert */
export function dismissPsyInbound(): void {
    triggerCivic(B.DismissPsyInbound);
}
