...
// Narrative quests
export const questState$ = bindCivicValue(B.QuestState, "{}");

// Grid Warfare
export const gridWarfareState$ = bindCivicValue(B.GridWarfareState, "{}");
// Mirror enemy city STRIKE-view snapshot { header, signals, targets, aa } — already quantized
// C#-side to the player's effective intel level. Published once per meaningful change (not per
// frame), separate from the GridWarfareState payload; parsed once, memoized by the raw string.
export const mirrorCity$ = bindCivicValue(B.MirrorCity, "{}");
