...
// Air-defense funnel: WAR tab opened → RADAR coverage viewed → DEFENSE build panel opened.
// CrisisTutorialSystem records each as a "first open in Crisis" telemetry step.
export function notifyOpenWarTab(): void {
    triggerCivic(B.OnOpenWarTab);
}

export function notifyOpenRadarView(): void {
    triggerCivic(B.OnOpenRadarView);
}

export function notifyOpenDefenseBuild(): void {
    triggerCivic(B.OnOpenDefenseBuild);
}
