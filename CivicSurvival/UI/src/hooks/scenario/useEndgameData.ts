...
export function useEndgameData() {
    ...
    return useMemo(() => {
        ...
        return {
            ...
            // A dash, not a number, when nobody measured the city. The C# side used to
            // re-publish the last good percentage as if it were current and the endgame
            // summary printed it — a display owes "unknown", never a stale figure wearing a
            // fresh figure's clothes.
            populationPercentDisplay: raw.populationMeasured
                ? format.percent(raw.populationPercent, 0, "ratio")
                : "—",
