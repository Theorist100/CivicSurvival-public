...
namespace CivicSurvival.Services.RemoteConfig
{
    ...
    public partial class UpdateAnnounceSystem : ThrottledUISystemBase
    {
        ...
        // Online gate is read from settings EVERY tick, never snapshotted at OnStartRunning:
        // NetworkConnectionEnabled is seeded from ConsentStore during save deserialization
        // (ModSettingsSerializationSystem), which can land AFTER this system starts — a
        // boot-time snapshot froze "off" for the whole session on every client (prod nginx:
        // zero /config/announce polls ever, while news/telemetry from the same sessions
        // flowed). These two fields only track the last logged gate state so the log still
        // records off/on transitions without spamming.
        private bool m_GateLogged;
        private bool m_LastGateOnline;
        ...
        protected override void OnThrottledUpdate()
        {
            ...
            if (m_Config == null || m_Settings == null)
                return;

            // Live read (see the field comment): the seeded/toggled setting is the truth,
            // a snapshot is not. Cheap main-thread property read once per throttled tick.
            bool online = m_Settings.NetworkConnectionEnabled;
            if (!m_GateLogged || online != m_LastGateOnline)
            {
                m_GateLogged = true;
                m_LastGateOnline = online;
                Log.Info(online
                    ? " Online — update announces will be polled"
                    : " Online off — update announces not polled (gate re-checked every tick)");
            }
            if (!online)
                return;
