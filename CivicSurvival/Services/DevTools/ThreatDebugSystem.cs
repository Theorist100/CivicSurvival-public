...
namespace CivicSurvival.Services.DevTools
{
    ...
    public partial class ThreatDebugSystem : TriggeredThrottledUISystemBase
    {
        ...
        private volatile bool m_PendingStrikeSelected;

        // Selected-building strike harness: aims one inbound ballistic at whatever the player has
        // selected in the vanilla info panel, instead of leaving the choice to the wave target
        // selector. Everything after the intent is the production path — perimeter spawn, lofted
        // arc, air-defence interception, arrival damage — so a chosen structure comes under real
        // fire rather than having a hit simulated on it.
        private Game.UI.InGame.SelectedInfoUISystem m_StrikeSelectedInfo = null!;
        private ThreatSpawnSystem? m_StrikeSpawnSystem;
        private ComponentLookup<Game.Buildings.Building> m_StrikeBuildingLookup;
        private bool m_StrikePausedLogged;
        ...
        protected override void ConfigureTriggers(TriggerRegistry triggers)
        {
            ...
            triggers.AddUngated(DebugShowModUpdatedRestart, OnDebugShowModUpdatedRestart);
            ...
            triggers.AddUngated(DebugStrikeSelectedBallistic, OnDebugStrikeSelectedBallistic);
        }
        ...
        protected override void OnCreate()
        {
            ...
            m_StrikeBuildingLookup = GetComponentLookup<Game.Buildings.Building>(true);
            m_StrikeSelectedInfo = World.GetOrCreateSystemManaged<Game.UI.InGame.SelectedInfoUISystem>();
        }
        ...
        protected override void OnStartRunning()
        {
            ...
            // Resolved here, not in OnCreate: the Waves domain registers the spawner during feature
            // load, and a gated-off Waves feature leaves it absent — null is a valid state the
            // strike button reports instead of materialising a system past the gate.
            m_StrikeSpawnSystem = World.GetExistingSystemManaged<ThreatSpawnSystem>();
        }
        ...
        protected override void OnThrottledUpdate()
        {
            ...
            // Kept pending while paused instead of dropped: the round needs a ticking simulation to
            // fly, so the click fires as soon as the player resumes.
            if (m_PendingStrikeSelected && ExecuteStrikeSelectedBallistic())
                m_PendingStrikeSelected = false;
        ...
        // Test hook for the update-restart notice: fires BOTH real channels (vanilla
        // MessageDialog + React modal) exactly like a live ModInstanceWatch detection.
        // Direct call, no pending flag: both channels are managed UI-thread work with no
        // ECS state — same sync-UI route the real detection path uses. The watcher's
        // once-per-process latch is deliberately untouched.
        private void OnDebugShowModUpdatedRestart()
        {
            string vanillaNotice = CivicSurvival.Core.UI.VanillaRestartNotice.Show();
            Log.Info($"[DEBUG] Restart notice test fired: vanillaNotice={vanillaNotice}");
#pragma warning disable CIVIC098 // ModalCoordinator.Instance is static readonly = new(), never null
#pragma warning disable CIVIC239 // Best-effort test surface: a busier slot queues the request
            // inside the coordinator — nothing to retry on false, mirroring the real detection.
...
#pragma warning restore CIVIC098
        ...
        private void OnDebugStrikeSelectedBallistic() => m_PendingStrikeSelected = true;

        /// <summary>
        /// Aim one inbound ballistic at the building selected in the vanilla info panel. Returns
        /// false only while the simulation is paused — the request then stays pending and fires on
        /// resume; every other outcome (nothing selected, selection is not a building, spawner
        /// absent, launch recorded) consumes the click and says so in the log.
        /// </summary>
        private bool ExecuteStrikeSelectedBallistic()
        {
            if (World == null || !World.IsCreated) return true;

            // A recorded intent needs a ticking simulation to become a flying round; on pause the
            // consumer would leave it hanging, so hold the request instead.
            if (m_SimulationSystem.selectedSpeed <= 0f)
            {
                if (!m_StrikePausedLogged)
                {
                    Log.Warn("[DEBUG] Ballistic strike on selected building held until simulation resumes");
                    m_StrikePausedLogged = true;
                }
                return false;
            }
            m_StrikePausedLogged = false;

            if (m_StrikeSpawnSystem == null)
            {
                Log.Warn("[DEBUG] Ballistic strike skipped: ThreatSpawnSystem absent (Waves feature gated off)");
                return true;
            }

            var selected = m_StrikeSelectedInfo.selectedEntity;
            if (selected == Unity.Entities.Entity.Null)
            {
                Log.Warn("[DEBUG] Ballistic strike skipped: no building selected");
                return true;
            }

            m_StrikeBuildingLookup.Update(this);
            m_DemolishDestroyedLookup.Update(this);
            m_DemolishDeletedLookup.Update(this);

            if (!m_StrikeBuildingLookup.HasComponent(selected))
            {
                Log.Warn($"[DEBUG] Ballistic strike skipped: selection {selected.Index}:{selected.Version} is not a building");
                return true;
            }

            if (m_DemolishDestroyedLookup.HasComponent(selected) || m_DemolishDeletedLookup.HasComponent(selected))
            {
                Log.Warn("[DEBUG] Ballistic strike skipped: selected building is already destroyed");
                return true;
            }

            if (!m_StrikeSpawnSystem.DebugStrikeBuilding(selected))
                Log.Warn("[DEBUG] Ballistic strike not recorded — prefabs unresolved or intent host missing");
...
#pragma warning restore CIVIC343
        ...
        private void ExecuteKill500CiviliansNative()
        {
            try
            {
                ...
                bool hasEcb = false;
                int killed = 0;
                // POPULATION-MEASURE-EXEMPT: a #if DEBUG developer command that KILLS citizens, not
                // one that counts them. The query is the enumeration it mutates through, it stops at
                // the first 500 hits (`target`), and `killed` is a progress counter for the break
                // condition and the log line — it can never be the city's size because the loop
                // exits before seeing the rest of it. Absent from Release builds entirely.
