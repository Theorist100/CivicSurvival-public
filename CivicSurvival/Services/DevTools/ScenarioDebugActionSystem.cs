...
namespace CivicSurvival.Services.DevTools
{
    ...
    public partial class ScenarioDebugActionSystem : ThrottledUISystemBase
    {
        ...
        private IDraftExemptionDebugMutator? m_DraftExemptionMutator;
        private IEconomyDebugMutator? m_EconomyMutator;
        private IPsyOpsDebugMutator? m_PsyOpsMutator;
        private IQuestDebugMutator? m_QuestMutator;
        private ITerrainHeightReader m_TerrainHeightReader = null!;
        // The one owner of "how many people live in the city". Resolved in OnStartRunning; the
        // snapshot below is only reachable from a debug trigger, which registers in that same
        // OnStartRunning, so there is no earlier caller to race.
        private ICityPopulationReader m_CityPopulation = null!;
        ...
        // Monotonic per-session counter: unique enqueue keys and unique refund idempotency keys.
        private int m_ShadowGrantCounter;
        ...
        protected override void OnStartRunning()
        {
            ...
            AddBinding(new TriggerBinding(Group, DebugForceDraftOffer, () => EnqueueDebugAction(DebugForceDraftOffer, OnDebugForceDraftOffer)));
            ...
            // Key includes the grant counter so two fast clicks don't coalesce in the pending queue.
            AddBinding(new TriggerBinding<int>(Group, DebugGrantShadowCash, value => EnqueueDebugAction($"{DebugGrantShadowCash}:{++m_ShadowGrantCounter}", () => OnDebugGrantShadowCash(value))));
            // A typed raid and a full raid share one trigger keyed by payload, so a fast click of
            // Rumor→FakeVideo cannot coalesce in the pending queue (the key would dedupe them).
            AddBinding(new TriggerBinding<int, int>(Group, DebugLaunchPsyRaid, (type, stratum) => EnqueueDebugAction($"{DebugLaunchPsyRaid}:{type}:{stratum}", () => OnDebugLaunchPsyRaid(type, stratum))));
            AddBinding(new TriggerBinding(Group, DebugLandPsyRaid, () => EnqueueDebugAction(DebugLandPsyRaid, OnDebugLandPsyRaid)));
            AddBinding(new TriggerBinding<int>(Group, DebugTriggerQuest, value => EnqueueDebugAction($"{DebugTriggerQuest}:{value}", () => OnDebugTriggerQuest(value))));
        }
        ...
        private void OnDebugForceDraftOffer()
        {
            try
            {
                ResolveDraftExemptionMutator()?.DebugForceOffer("scenario_button");
            }
            catch (Exception ex) { Log.Warn($"[DEBUG] OnDebugForceDraftOffer failed: {ex}"); }
        }

        private void OnDebugLaunchPsyRaid(int typeOverride, int stratumOverride)
        {
            try
            {
                ResolvePsyOpsMutator().DebugLaunchRaid(typeOverride, stratumOverride, "scenario_button");
            }
            catch (Exception ex) { Log.Warn($"[DEBUG] OnDebugLaunchPsyRaid failed: {ex}"); }
        }

        private void OnDebugLandPsyRaid()
        {
            try
            {
                ResolvePsyOpsMutator().DebugLandInFlight("scenario_button");
            }
            catch (Exception ex) { Log.Warn($"[DEBUG] OnDebugLandPsyRaid failed: {ex}"); }
        }

        private void OnDebugTriggerQuest(int questId)
        {
            try
            {
                ResolveQuestMutator().DebugForceQuest(questId, "scenario_button");
            }
            catch (Exception ex) { Log.Warn($"[DEBUG] OnDebugTriggerQuest failed: {ex}"); }
        }

        private void OnDebugGrantShadowCash(int amount)
        {
            try
            {
                if (amount <= 0) return;
                var wallet = ServiceRegistryFeatureExtensions.TryGetOrNullObject(NullShadowWalletService.Instance);
                if (ReferenceEquals(wallet, NullShadowWalletService.Instance))
                {
                    Log.Warn("[DEBUG] GrantShadowCash skipped: shadow wallet service unavailable");
                    return;
                }
                // The refund path credits the balance bypassing freeze/act gates — exactly what a
                // debug grant wants. The unique key keeps the idempotency set from swallowing repeats.
                string key = $"debug_grant_{m_ShadowGrantCounter}";
                if (wallet.TryApplyRefund(amount, "debug grant", key))
                    Log.Info($"[DEBUG] Granted ${amount:N0} shadow cash");
                else
                    Log.Warn("[DEBUG] GrantShadowCash failed: wallet singleton unavailable");
            }
            catch (Exception ex) { Log.Warn($"[DEBUG] OnDebugGrantShadowCash failed: {ex}"); }
        }
        ...
        private IDraftExemptionDebugMutator ResolveDraftExemptionMutator()
        {
            if (m_DraftExemptionMutator is null)
                m_DraftExemptionMutator = ServiceRegistryFeatureExtensions.TryGetOrNullObject(NullDraftExemptionDebugMutator.Instance);
            return m_DraftExemptionMutator;
        }

        private IPsyOpsDebugMutator ResolvePsyOpsMutator()
        {
            if (m_PsyOpsMutator is null)
                m_PsyOpsMutator = ServiceRegistryFeatureExtensions.TryGetOrNullObject(NullPsyOpsDebugMutator.Instance);
            return m_PsyOpsMutator;
        }

        private IQuestDebugMutator ResolveQuestMutator()
        {
            if (m_QuestMutator is null)
                m_QuestMutator = ServiceRegistryFeatureExtensions.TryGetOrNullObject(NullQuestDebugMutator.Instance);
            return m_QuestMutator;
        }
        ...
        private ScenarioLogSnapshot CaptureScenarioSnapshot()
        {
            ...
            // -1 for a refusal, so a preset log tells "nobody lives here" (0) apart from "there was
            // no city to ask" (-1). A debug snapshot is the one place where folding the refusal
            // into a sentinel is fine: nothing decides on this number, it is printed.
            int population = m_CityPopulation.TryGetResidentCount(out int residents) ? residents : -1;
