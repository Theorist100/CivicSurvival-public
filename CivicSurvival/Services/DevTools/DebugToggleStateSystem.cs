...
namespace CivicSurvival.Services.DevTools
{
    ...
    public partial class DebugToggleStateSystem : TriggeredThrottledUISystemBase
    {
        ...
        {
            "movement" or "droneSpawn" or "target" or "damage" or "opDamage" or "identify"
                or "mhr" or "cogStats" => true,
            _ => false
        };
        ...
        private void OnDebugToggleSystem(string systemName, bool enabled)
        {
            try
            {
                ...
                if (systemName.StartsWith("ab:"))
                {
                    ...
                    // One-click psy-pipeline attribution (FPS investigation 2026-07-22):
                    // whole Cognitive domain, then the MHR resolve chain alone, then the stats
                    // aggregator alone — three STATE COMPARISON blocks in one combat session.
                    if (systemName == "ab:psy:all")
                    {
                        var psyKeys = new[] { "d:cognitive", "mhr", "cogStats" };
                        var psyToggles = new Action<bool>[]
                        {
                            on => ToggleDomainOrBulk("d:cognitive", on),
                            on => DomainToggleRegistry.ToggleSingleSystem(World, "mhr", on),
                            on => DomainToggleRegistry.ToggleSingleSystem(World, "cogStats", on),
                        };
                        PerformanceProfiler.StartABTestSequence(psyKeys, psyToggles, 3, 6);
                        DebugABTestUiState.SetStatus(systemName);
                        UpdateSnapshot();
                        Log.Info("[DEBUG] A/B sequence: psy pipeline (3 tests x 3 cycles x 30s)");
                        return;
                    }
                    ...
                    // Single-system A/B (e.g. ab:mhr) — isolates one system's cost from its domain.
                    string bareKey = systemName.Substring(3);
                    if (IsSingleSystemKey(bareKey))
                    {
                        PerformanceProfiler.StartABTest(bareKey, 3, 6,
                            on => DomainToggleRegistry.ToggleSingleSystem(World, bareKey, on));
                        DebugABTestUiState.SetStatus(systemName);
                        UpdateSnapshot();
                        Log.Info($"[DEBUG] A/B test started: {bareKey} (3 cycles x 30s)");
                        return;
                    }

                    string domain = "d:" + bareKey;
