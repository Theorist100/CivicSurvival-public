...
namespace CivicSurvival.Services.DevTools
{
    ...
    public static class ErrorReportService
    {
        ...
        // A repeat click on the same send button within this window is a duplicate, not a new
        // report: one player shipped 11 identical reports in 9 seconds (2026-07-14). Short on
        // purpose — it kills rapid-click bursts without punishing a deliberate re-send (which
        // carries a fresh log tail). Per-channel stamps so a legitimate "report + mod list"
        // pair sent back-to-back still goes through.
        private const int SUBMIT_COOLDOWN_SECONDS = 15;
        private static readonly char[] s_PathSeparators = { '/', '\\' };
        private static DateTime s_LastManualReportUtc = DateTime.MinValue;
        private static DateTime s_LastModListUtc = DateTime.MinValue;
        private static DateTime s_LastCrashDumpUtc = DateTime.MinValue;
        private static DateTime s_LastLoadFailureUtc = DateTime.MinValue;

        // Load-failure report budgets. The playset/SDK evidence rides FIRST so the final
        // MAX_REPORT_CHARS cap trims the generic report tail, never the evidence. PdxSdk.log for a
        // large playset runs to hundreds of KB, so it is line-FILTERED (our modId + sync-decision
        // markers), not blindly windowed — a head/tail slice of a busy log misses the removal.
        private const int MODDING_LOG_HEAD_CHARS = 16 * 1024;
        private const int PDXSDK_FILTERED_CHARS = 48 * 1024;
        private const long PDXSDK_SCAN_LIMIT_BYTES = 8L * 1024 * 1024;
        // The SDK flow steps whose log lines reconstruct a playset-sync / removal decision.
        private static readonly string[] s_PdxSdkLineMarkers =
        {
            "QueueRemoval", "DeleteModFromDisk", "ProcessPendingRemovals", "DownstreamSync",
            "ListModsInPlayset", "GetModsInActivePlayset", "QueueDownload", "CleverWithDeletions",
        };
        ...
        private static bool IsOnCooldown(DateTime lastSentUtc)
        {
            return (DateTime.UtcNow - lastSentUtc).TotalSeconds < SUBMIT_COOLDOWN_SECONDS;
        }
...
#pragma warning restore CIVIC169
            ...
            if (IsOnCooldown(s_LastManualReportUtc))
            {
                Log.Info("Manual report suppressed: previous report still within the cooldown window");
                return ManualReportResult.RateLimited;
            }
            ...
            try
            {
                ...
                s_LastManualReportUtc = DateTime.UtcNow;
        ...
        /// <summary>
        /// Submit the load-failure diagnostics report (ModLoadFailure modal). On top of the standard
        /// report it carries the playset evidence a prefab-absent case needs: the Modding.log head
        /// (active playset + mod list) and the PdxSdk.log lines about our mod / the SDK's sync,
        /// download-queue and removal decisions — the only visible record of what the Paradox backend
        /// answered for this player's playset.
        /// </summary>
        public static ManualReportResult SubmitLoadFailureReport(string userComment = "")
        {
            var telemetry = TelemetryService.Instance;
            if (telemetry == null)
            {
                Log.Warn("Cannot submit load-failure report: TelemetryService not available");
                return ManualReportResult.TelemetryUnavailable;
            }
...
#pragma warning restore CIVIC169
            if (settings == null || !settings.TelemetryEnabled)
            {
                Log.Warn("Cannot submit load-failure report: Telemetry is disabled");
                return ManualReportResult.TelemetryDisabled;
            }

            if (IsOnCooldown(s_LastLoadFailureUtc))
            {
                Log.Info("Load-failure report suppressed: previous report still within the cooldown window");
                return ManualReportResult.RateLimited;
            }

            try
            {
                var report = GenerateLoadFailureReport();
                telemetry.RecordManualReport(
                    SanitizeForReport("[LOAD FAILURE] " + userComment, MAX_USER_COMMENT_CHARS),
                    Truncate(report, MAX_REPORT_CHARS));
                s_LastLoadFailureUtc = DateTime.UtcNow;
                return ManualReportResult.Sent;
            }
            catch (Exception ex)
            {
                Log.Exception("Failed to submit load-failure report", ex);
                return ManualReportResult.Failed;
            }
        }

        /// <summary>
        /// Compose the load-failure report: SDK/playset evidence first (so the final size cap trims
        /// the generic report, never the evidence), then the standard diagnostics report.
        /// </summary>
        private static string GenerateLoadFailureReport()
        {
            var sb = new StringBuilder();
            sb.AppendLine("=== Civic Survival Load-Failure Diagnostics ===");
            sb.AppendLine($"Generated: {DateTime.Now:yyyy-MM-dd HH:mm:ss}");
            sb.AppendLine($"Mod Version: {Mod.VERSION}");
            sb.AppendLine();

            sb.AppendLine("--- Modding.log (head: active playset + mod list) ---");
            AppendFileHead(sb,
                Path.Combine(ModPaths.LogsDirectory, ModPaths.ModdingLogFile),
                MODDING_LOG_HEAD_CHARS,
                "Modding.log not found (no active playset log yet).");
            sb.AppendLine();

            sb.AppendLine("--- PdxSdk.log (filtered: our mod id + SDK sync/removal decisions) ---");
            AppendPdxSdkEvidence(sb, Path.Combine(ModPaths.LogsDirectory, ModPaths.PdxSdkLogFile));
            sb.AppendLine();

            sb.AppendLine("--- Standard diagnostics report ---");
        ...
        /// <summary>
        /// Append the PdxSdk.log lines that reconstruct the SDK's playset decisions: every line
        /// mentioning our stable ModId (empty on a dev install — then markers only) plus every line
        /// with a sync/removal flow marker. Bounded by scan bytes and output chars; reports what was
        /// dropped so a capped extract never reads as complete.
        /// </summary>
        private static void AppendPdxSdkEvidence(StringBuilder sb, string path)
        {
            string modId = Core.Systems.Bootstrap.ModInstallResolver.StableModId();
            int written = 0;
            int matched = 0;
            bool capped = false;
            try
            {
                ...
                long scanLength = Math.Min(fs.Length, PDXSDK_SCAN_LIMIT_BYTES);
                bool scanTruncated = fs.Length > PDXSDK_SCAN_LIMIT_BYTES;
                using var reader = new StreamReader(fs);

                string? line;
                while ((line = reader.ReadLine()) != null && fs.Position <= scanLength)
                {
                    if (!IsPdxSdkEvidenceLine(line, modId)) continue;

                    matched++;
                    if (written + line.Length + 1 > PDXSDK_FILTERED_CHARS)
                    {
                        capped = true;
                        continue;
                    }
                ...
                if (matched == 0)
                    sb.AppendLine(modId.Length == 0
                        ? "No sync/removal marker lines found (dev install: no mod-id filter)."
                        : $"No lines matched mod id '{modId}' or sync/removal markers.");
                if (capped)
                    sb.AppendLine($"[PdxSdk.log extract capped at {PDXSDK_FILTERED_CHARS} chars; {matched} lines matched in total]");
                if (scanTruncated)
                    sb.AppendLine($"[PdxSdk.log larger than {PDXSDK_SCAN_LIMIT_BYTES / (1024 * 1024)} MB; only the first part was scanned]");
            }
            catch (FileNotFoundException)
            {
                sb.AppendLine("PdxSdk.log not found.");
            }
            ...
            {
                Log.Exception("Failed to read PdxSdk.log for load-failure report", ex);
                sb.AppendLine($"Failed to read PdxSdk.log: {ex.GetType().Name}: {SanitizeForReport(ex.Message, 512)}");
            }
        }

        private static bool IsPdxSdkEvidenceLine(string line, string modId)
        {
            if (modId.Length != 0 && line.Contains(modId, StringComparison.Ordinal))
                return true;

            foreach (var marker in s_PdxSdkLineMarkers)
            {
        ...
        /// <summary>
        /// Append a size-capped head of a text file (StreamReader over FileShare.ReadWrite — no
        /// File.Exists pre-check, TOCTOU-safe). Shared by the mod-list and load-failure reports.
        /// </summary>
        private static void AppendFileHead(StringBuilder sb, string path, int maxChars, string missingNote)
        {
            try
            {
                using var fs = new FileStream(path, FileMode.Open, FileAccess.Read, FileShare.ReadWrite);
                ...
                if (read == buffer.Length)
                    sb.AppendLine().AppendLine($"[{Path.GetFileName(path)} truncated]");
            }
            catch (FileNotFoundException)
            {
                sb.AppendLine(missingNote);
            }
            catch (DirectoryNotFoundException)
            {
                sb.AppendLine(missingNote);
            ...
            {
                Log.Exception($"Failed to read {Path.GetFileName(path)} for report", ex);
                sb.AppendLine($"Failed to read {Path.GetFileName(path)}: {ex.GetType().Name}: {SanitizeForReport(ex.Message, 512)}");
            }
        }
...
#pragma warning restore CIVIC169
            ...
            if (IsOnCooldown(s_LastModListUtc))
            {
                Log.Info("Mod list submit suppressed: previous mod list still within the cooldown window");
                return ManualReportResult.RateLimited;
            }
            ...
            try
            {
                ...
                s_LastModListUtc = DateTime.UtcNow;
                return ManualReportResult.Sent;
            }
            catch (Exception ex)
            {
...
#pragma warning restore CIVIC169
            ...
            if (IsOnCooldown(s_LastCrashDumpUtc))
            {
                Log.Info("Crash dump submit suppressed: previous dump still within the cooldown window");
                return ManualReportResult.RateLimited;
            }
...
#pragma warning restore CIVIC029
            ...
            s_LastCrashDumpUtc = DateTime.UtcNow;
            return ManualReportResult.Sent;
        }
...
#pragma warning restore CIVIC052
        ...
        private static string GenerateModListReport()
        {
            ...
            // The mod list is at the top of the file, so a size-capped head is the right window.
            AppendFileHead(sb,
                Path.Combine(ModPaths.LogsDirectory, ModPaths.ModdingLogFile),
                MAX_LOG_TAIL_BYTES,
                "Modding.log not found (no active playset log yet).");
