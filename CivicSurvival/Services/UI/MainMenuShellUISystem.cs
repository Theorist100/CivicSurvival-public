...
namespace CivicSurvival.Services.UI
{
    ...
    public partial class MainMenuShellUISystem : CivicUISystemBase
    {
        ...
        {
            // Every JS message is proof the React bundle is executing — feed the uiAlive latch
            // the FinalizeMissing diagnostics read (form α can kill the UI bundle while C# keeps
            // running; see UiBundleSignal).
            Core.Systems.Bootstrap.UiBundleSignal.Record();
            // Route through LogContext (not Mod.Log directly) so a transient logger-stream
            // failure is swallowed by SafeLog instead of propagating out of this cohtml
            // trigger callback and aborting the UI update.
            Log.Info($"[JS] {message}");
        }
