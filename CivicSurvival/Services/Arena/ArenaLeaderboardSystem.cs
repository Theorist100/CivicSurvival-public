...
#pragma warning restore CIVIC150
        ...
        // (same singletons the wave snapshot samples server-side). Population and average
        // happiness both come from the single owner of the vanilla city population record —
        // they live on one component, so one owner answers both and the ladder never holds
        ...
        private void OnWaveStartingProsperity(WaveStartingEvent evt)
        {
            ...
            // Event handler: re-resolve here as well as in OnStartRunning, since a wave can
            // start on a path that has not yet run this system's OnStartRunning.
            m_CityPopulation ??= ServiceRegistry.Instance.Require<ICityPopulationReader>();

            // One measure, one scoring rule, and the server's query owns both. A wave is scored
            // exactly when the owner answered with a count above zero — the same two conditions
            // the ladder query applies to the wave snapshot (a null count folds to 0 there and
            // the query requires Population > 0). A refusal is not a zero-population city: score
            // nothing rather than divide by an unmeasured one.
            // The query carries a third condition this build cannot fail: it scores only
            // snapshots from contract 1.33 and up, where Population became the resident count
            // instead of the raw citizen tally (_MEASURE_CUTOFF_EXPR in the arena repository).
            // Every wave this code accumulates is stamped with the current contract, so the
            // local estimate and the confirmed score stay on one base.
            if (!m_CityPopulation.TryGetResidentCount(out int residents)) return;
            if (residents <= 0) return;

            // Same record, same read, so a refusal here is impossible once the count answered.
            // If it happened anyway, the wave snapshot would send happiness 0 and the server
            // would score the wave at zero — so the client does the same instead of dropping the
            // wave, keeping the two sides on one rule. Health is simply not part of this index.
            int happiness = m_CityPopulation.TryGetCityAverages(out int averageHappiness, out _)
                ? Math.Min(Math.Max(averageHappiness, 0), 100)
                : 0;
            int homeless = 0;
            try
            {
            ...
            long index = (long)Math.Round(
                happiness * (1.0 - (double)Math.Min(homeless, residents) / residents));

            // A measured zero IS a scored wave — a city where everyone is homeless survived the
            // wave at index zero, and the server counts it in `waves` and in `avg_index`. The
            // guard that used to drop it here left the score identical (adding zero) but stated
            // the opposite rule, so any local wave counter added later would have inherited an
            // average that flatters a ruined city. Unmeasured (the two returns above) and
            // measured-zero stay different things.
            lock (m_Lock)
            {
...
#pragma warning restore CIVIC114
            ...
            // Canonical resolution point (CIVIC403). Placed BEFORE the once-only guard below:
            // the guard exists for the settings/auth block, not for service wiring.
            m_CityPopulation ??= ServiceRegistry.Instance.Require<ICityPopulationReader>();
