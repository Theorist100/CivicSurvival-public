...
namespace CivicSurvival.Services.Telemetry
{
    ...
    public sealed class TelemetryPersistence
    {
        ...
        // Manual bug reports ride the same crash-safe segment machinery (atomic write, recovery
        // header, .recovering rename, quarantine, consent-revoke sweep) but on their own prefix:
        // they resend to a dedicated endpoint (/manual-report), not the event batch pipeline, so
        // they recover independently of pending events.
        private const string REPORT_PREFIX = "telemetry_report_";
        ...
        /// <summary>
        /// Persist a manual bug report body (the exact /manual-report POST JSON) to its own
        /// crash-safe segment before the send, so a total send failure survives to the next launch.
        /// The returned path is deleted on a confirmed ACK (<see cref="DeletePendingSegment"/>).
        /// Mirrors <see cref="AppendToDisk"/> but keeps the raw JSON verbatim — the report is not a
        /// telemetry event and rides a different endpoint, so it is stored and resent as-is.
        /// </summary>
        public string? WriteReportToDisk(string json)
        {
            if (string.IsNullOrEmpty(json)) return null;

            try
            {
                var path = CreateSegmentPath(REPORT_PREFIX);
                lock (m_PendingLock)
                {
                    WriteSegment(path, WithRecoveryHeader(json + "\n"));
                }
            ...
            {
                Log.Error($" Report disk write failed: {ex}");
                return null;
            }
        }

        /// <summary>
        /// Recover manual bug reports a previous session persisted but never got a send ACK for.
        /// Returns each surviving report body (the raw /manual-report JSON) and consumes its segment,
        /// exactly like event recovery — the caller re-persists via <see cref="WriteReportToDisk"/>
        /// on the resend, so a report whose resend fails again is not lost. Independent of
        /// <see cref="RecoverPendingEvents"/>: reports resend to their own endpoint, not the batch.
        /// </summary>
        public List<string> RecoverReportSegments()
        {
            ...
            try
            {
                lock (m_PendingLock)
                {
                    foreach (var path in EnumerateReportRecoveryFiles())
                    {
                        RecoverTextSegment(path, result, "pending report");
                    }
                }

                if (result.Count > 0)
                    Log.Info($" Recovered {result.Count} pending report(s) from previous session");
            }
            catch (Exception ex)
            {
                Log.Error($" Report recovery failed: {ex}");
            }
...
#pragma warning restore CIVIC147
        ...
        public void ClearAllQueued()
        {
            ...
            lock (m_PendingLock)
            {
                lock (m_RetryLock)
                {
                    deleted = DeleteFiles(EnumeratePendingRecoveryFiles()
                        .Concat(EnumerateRetryRecoveryFiles())
                        .Concat(EnumerateReportRecoveryFiles()));
                }
            }
            if (deleted > 0)
...
#pragma warning restore CIVIC147
        ...
        private IEnumerable<string> EnumerateReportRecoveryFiles()
        {
            var dir = m_Config.LogsDirectory;
            if (!Directory.Exists(dir)) return Array.Empty<string>();

            var files = new List<string>();
            files.AddRange(Directory.GetFiles(dir, REPORT_PREFIX + "*.jsonl"));
            files.AddRange(Directory.GetFiles(dir, REPORT_PREFIX + "*" + RECOVERING_SUFFIX));
            return files.Distinct(StringComparer.Ordinal).OrderBy(File.GetCreationTimeUtc).ToArray();
        }
