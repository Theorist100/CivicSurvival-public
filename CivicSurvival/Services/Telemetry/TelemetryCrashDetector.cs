...
namespace CivicSurvival.Services.Telemetry
{
    ...
    internal sealed class TelemetryCrashDetector : IDisposable
    {
        ...
        // ---- Minidump ownership thresholds (consumed by the MinidumpInherited verdict) ----
        // Nothing inside a dump ties it to a session, so the freshest dump written before this
        // process started is paired with whatever unclean shutdown we are reporting — even when
        // that shutdown produced no dump of its own (kill / power loss / alt-F4). Prod carries such
        // pairings at 6.7M seconds of "freshness" (77 days); genuine crashes recovered on the next
        // launch land at 136-142 s, the cost of crash -> relaunch -> city load -> detector. Beyond
        // this cutoff the dump is reported as inherited from an earlier incident: ~13x the observed
        // genuine latency (a slow relaunch or a long load still fits), three orders of magnitude
        // under every observed stale pairing, and past the point where a dump and an unclean exit
        // can plausibly belong to the same sitting.
        private const double MINIDUMP_INHERITED_AFTER_SECONDS = 1800.0;

        // Slack on the "dump predates the crashed session" comparison: the dump is only called
        // inherited when it is older than the crashed session's last known-live timestamp by more
        // than this, so a dump written in the same moment as the final heartbeat is never flagged
        // on timestamp granularity alone.
        private const double MINIDUMP_ANCHOR_GRACE_SECONDS = 60.0;

        // A dump this fresh was written in the cycle that just ended, so it describes THIS crash.
        // CRASH_TRIAGE has applied the same threshold by hand in every query since the 0.2.x hunt
        // (genuine crashes recover at 136-142 s, phantoms at hours to months); computing it here
        // means a reader cannot forget it and count an inherited dump as a fresh crash.
        private const double MINIDUMP_FRESH_WITHIN_SECONDS = 180.0;
...
#pragma warning restore CIVIC052
            ...
            var heartbeatUtc = DateTime.MinValue;
            if (!string.IsNullOrEmpty(persistedHeartbeatUtc)
                && DateTime.TryParse(persistedHeartbeatUtc, CultureInfo.InvariantCulture, DateTimeStyles.AssumeUniversal | DateTimeStyles.AdjustToUniversal, out heartbeatUtc))
            {
                phaseAgeSeconds = (float)Math.Max(0.0, (nowUtc - heartbeatUtc).TotalSeconds);
            }

            // The one derived fact the reader must not be left to compute: does this dump belong to
            // the crash being reported? Two independent proofs, either of which is enough, and both
            // asserting the same thing so a single boolean stays unambiguous:
            //  (1) the dump predates the last moment the crashed session is known to have been alive.
            //      A crash dump is written AT the crash, so a dump older than that is from an earlier
            //      incident. This is the only rule that catches a short-interval false pairing (crash,
            //      relaunch, then a dumpless kill minutes later) that no absolute cutoff can see.
            //  (2) no anchor survived, or it did and the dump is still older than the inherited cutoff
            //      — a dump and an unclean exit that far apart cannot be shown to belong together.
            // Raw MinidumpFreshnessSeconds stays in the event untouched, so a stricter freshness rule
            // (CRASH_TRIAGE uses < 180 s for "fresh") still applies downstream; this flag only closes
            // the far end. Null when no dump was found at all — absence is not a verdict either way.
            bool? minidumpInherited = null;
            if (minidumpFreshnessSeconds.HasValue)
            {
                // Last moment the crashed session is known to have been alive: its final heartbeat
                // from the context file, or its marker write time — whichever is later. Both are
                // lower bounds on the crash itself, so the later one is the tightest bound we have.
                // Neither file is written by THIS session before recovery consumes them
                // (CrashContextProvider deliberately does not persist at boot), so both belong to
                // the run that died.
                var lastKnownAliveUtc = recoveredRecord.CapturedUtc > heartbeatUtc ? recoveredRecord.CapturedUtc : heartbeatUtc;
                var predatesCrashedSession = lastKnownAliveUtc != DateTime.MinValue
                    && (lastKnownAliveUtc - minidump.Utc).TotalSeconds > MINIDUMP_ANCHOR_GRACE_SECONDS;

                minidumpInherited = predatesCrashedSession
                    || minidumpFreshnessSeconds.Value > MINIDUMP_INHERITED_AFTER_SECONDS;
            }
            ...
            var breadcrumbEvent = new TelemetryEvent
            {
                ...
                Data = new DiagnosticsNativeCrashBreadcrumbData
                {
                    ...
                    ActiveRegions = string.IsNullOrEmpty(recoveredRecord.ActiveRegions) ? null : recoveredRecord.ActiveRegions,
                    ActiveRegionCount = recoveredRecord.ActiveRegionCount,
                    ...
                    MinidumpFresh = minidumpFreshnessSeconds.HasValue
                        ? minidumpFreshnessSeconds.Value < MINIDUMP_FRESH_WITHIN_SECONDS
                        : (bool?)null,
                    MinidumpInherited = minidumpInherited,
                    ...
                    // PE identity of the faulting build. Makes "offset X is function Y" checkable
                    // against the player's binary instead of resting on the local install matching.
                    FaultingModuleTimeDateStamp = fault.Found ? "0x" + fault.TimeDateStamp.ToString("X8", CultureInfo.InvariantCulture) : null,
                    FaultingModuleCheckSum = fault.Found ? "0x" + fault.CheckSum.ToString("X8", CultureInfo.InvariantCulture) : null,
                    StackModules = fault.Found && !string.IsNullOrEmpty(fault.StackModules)
                        ? PiiRedactor.Redact(fault.StackModules)
                        : null,
                    OurModuleInStack = fault.Found && !string.IsNullOrEmpty(fault.StackModules)
                        ? fault.OurModuleInStack
                        : (bool?)null,
                    OurModuleTimeDateStamp = fault.Found && fault.OurModuleTimeDateStamp != 0
                        ? "0x" + fault.OurModuleTimeDateStamp.ToString("X8", CultureInfo.InvariantCulture)
                        : null,
                    OurModuleCheckSum = fault.Found && fault.OurModuleTimeDateStamp != 0
                        ? "0x" + fault.OurModuleCheckSum.ToString("X8", CultureInfo.InvariantCulture)
                        : null,
...
#pragma warning restore CIVIC052
            ...
            long money)
        {
            ...
            // Population and money ride along in the line so a clean exit can be checked from the
            // log alone (Axiom 1): the whole point of the last-known holder is that this field
            // stopped being zero on every quit, and "not measured" says the holder was never fed
            // rather than the city being empty.
            string popStr = population.HasValue ? $"{population.Value}" : "not measured";
            Log.Info($" Session end: {exitReason}, {totalPlaytimeSeconds:F0}s, day {gameDay}, {act}, pop {popStr}, money {money:N0}");
        }
