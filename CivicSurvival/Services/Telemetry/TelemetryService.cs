...
#pragma warning restore CA1001
    {
        ...
        // m_SubsystemsReady so turning diagnostics off does not silence the fuel.
        ...
        protected override void OnStartRunning()
        {
            ...
            if (!m_Config.Enabled)
            {
                ...
                // events still leave with diagnostics off. The pipeline ALSO brings up the
        ...
        public void SetEnabled(bool optIn)
        {
            ...
            else
            {
                ...
                // the chronicle / stats / leaderboard fuel.
                StopDiagnostics();
        ...
        /// Online, independent of the diagnostics opt-in. Idempotent.
        /// </summary>
        ...
        private void StartDiagnostics()
        {
            ...
            // Diagnostics is now effective (opt-in AND online) — the same gate RecordManualReport
            // requires. Flush any manual report that a previous session persisted but never got a
            // send ACK for (total send failure). Idempotent server-side on ReportId, so one that
            // actually reached the server before is deduped, not duplicated.
            ResendPendingManualReports();
        ...
        // Robustness: write the clean-shutdown flag on Unity's quit signal too. ECS
...
#pragma warning restore CIVIC005
            ...
            // Population, like money: the value the pulse snapshotted while the world was alive.
            // A live read here cannot work on the destroy path — Mod.OnDispose has already cleared
            // the registry, so Require throws and the catch below turned that into a zero, which
            // is why every clean exit reported a population of zero. Empty when the session ended
            // before the first measurement (menu session, or a load that never reached a city);
            // that is the one honest empty value this field has. Read outside the try because,
            // unlike the singleton queries below, it cannot throw.
#pragma warning disable CIVIC005 // Telemetry fallback: session_end must record even if subsystems torn down
            int? population = m_Pulse?.LastKnownPopulation;
#pragma warning restore CIVIC005
...
#pragma warning restore CIVIC005
        ...
        protected override void OnUpdateImpl()
        {
            ...
            // and the HTTP batch upload run with diagnostics off. Per-record diagnostics-
        ...
        private void OnOnlineStateChanged(OnlineConnectionStateChangedEvent evt)
        {
            ...
            if (evt.Enabled)
            {
                ...
                // chronicle / stats / leaderboard fuel starts flowing immediately.
        ...
        /// <summary>
        /// Record a manual bug report submitted by the user. Builds the /manual-report POST body
        /// (with a stable idempotency key) and hands it to <see cref="SendManualReport"/>, which
        /// persists it crash-safely before sending.
        /// </summary>
        public void RecordManualReport(string userComment, string reportContent)
        {
            ...
            // A stable ReportId is the idempotency key: a POST that times out AFTER the server
            // already stored the row is retried with the SAME id (HttpUtils maxRetries), and the
            // server dedups on it — no more duplicate rows. Generated once here, so both the
            // in-session retry and any cross-session resend carry the same id. JsonConvert escapes
            // the multi-line report body. The serialized body is stored and resent verbatim.
            ...
            SendManualReport(body);
            Log.Info(" Manual bug report submitted");
        }

        /// <summary>
        /// Persist a manual report body to a crash-safe segment, then POST it to the dedicated
        /// /manual-report endpoint (NOT the telemetry-event pipeline — the full report text would
        /// exceed the per-event 10KB Data cap). The segment rides the same durable machinery as
        /// telemetry events (recovery header, .recovering rename, quarantine, consent-revoke sweep).
        /// On a confirmed ACK the segment is dropped; on failure it is left for a next-launch resend
        /// (idempotent server-side on the body's ReportId). Shared by the live submit path and the
        /// startup resend sweep.
        /// </summary>
        private void SendManualReport(string body)
        {
            // Persist BEFORE the send so a total send failure survives to the next launch, where
            // ResendPendingManualReports flushes it. Deleted on a confirmed ACK below.
            var segmentPath = m_Persistence.WriteReportToDisk(body);
            ...
            CivicSurvival.Core.Utils.HttpUtils.PostAsync(
                ...
                onComplete: result =>
                {
                    if (result.Success)
                    {
                        m_Persistence.DeletePendingSegment(segmentPath);
        ...
        /// <summary>
        /// Flush manual reports a previous session persisted but never got a send ACK for. Called
        /// when diagnostics becomes effective (the same gate <see cref="RecordManualReport"/> uses).
        /// <see cref="TelemetryPersistence.RecoverReportSegments"/> consumes the stored segments;
        /// <see cref="SendManualReport"/> re-persists each on the resend, so a report whose resend
        /// also fails is not lost. Idempotent server-side on ReportId: a report that actually reached
        /// the server before its local ACK was lost is deduped on resend, not duplicated.
        /// </summary>
        private void ResendPendingManualReports()
        {
            if (m_Persistence == null || m_Config.FileOnlyMode || m_Transport == null || m_Auth == null)
                return;

            var pending = m_Persistence.RecoverReportSegments();
            if (pending.Count == 0) return;

            Log.Info($" Resending {pending.Count} pending manual report(s) from a previous session");
