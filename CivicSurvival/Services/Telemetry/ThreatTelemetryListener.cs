    ...
    internal sealed class ThreatTelemetryListener : IDisposable
    {
        ...
        private void OnWaveEnded(WaveEndedEvent evt)
        {
            ...
            Record(Balance.WaveResult, new BalanceWaveResultData
            {
                ...
                // Duration in SIMULATION seconds plus the speed it was played at. Wall-clock timing
                // is unusable for balance on its own: vanilla serves 60 × selectedSpeed sim ticks per
                // real second, so the same wave reads ~3× shorter at 4× than at 1×. 0 = not measured
                // (wave resumed from a save), which the backend must exclude rather than read as fast.
                DurationSimSeconds = (float)Math.Round(evt.DurationSimSeconds, 1),
                AvgGameSpeed = (float)Math.Round(evt.AvgGameSpeed, 2)
            });
        }

        // Per-impact allocation replaces the premade static payloads: LeakReason varies per
        // impact. Impacts are rare events (tens per wave) — no hot-path pressure.
        private void OnThreatImpact(ThreatImpactEvent evt)
            => Record(Threat.Impact, new ThreatImpactData
            {
                IsBallistic = evt.IsBallistic,
                LeakReason = (int)evt.LeakReason
            });
