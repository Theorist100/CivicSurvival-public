...
namespace CivicSurvival.Services.Telemetry
{
    ...
    public sealed class TelemetryPulse : IDisposable
    {
        ...
        // Cap on the joined mod-name list. Generous for a real playset (a heavy one runs 30-40 mods,
        // ~25 chars each) and small enough that a pathological name cannot bloat every session's
        // payload. Truncation loses tail names, never breaks the event.
        private const int MOD_LIST_MAX_CHARS = 2000;
        private const float MS_PER_SECOND = 1000f;
        // Nominal grid frequency reported when GridStressData is not up yet (pre-war boot window).
        private const float NOMINAL_GRID_FREQUENCY_HZ = 50.0f;
        ...
        private readonly EntityQuery m_GridStressQuery;
        private readonly EntityQuery m_MilestoneQuery;
        private readonly EntityQuery m_EnemyStateQuery;
        ...
        // Vanilla-maintained city statistics (homeless / unemployed) for the prosperity
        // fields of the wave snapshot. Main-thread read API, same one the vanilla infoview
        // UI systems call every frame; we call it once per wave.
        private readonly CityStatisticsSystem m_CityStatisticsSystem;
        // Cached DeathRate statistics entity (stable per loaded city; re-found on miss —
        // covers load boundaries where the world is rebuilt and the entity changes).
        private Entity m_DeathRateStatEntity;

        // Calm-period speed anchor: sim frame + wall clock at the END of the previous wave. The
        // wave snapshot turns the pair into the effective speed the player ran the peacetime
        // stretch at, which is what separates "speeds through the calm, drops to 1× for the
        // strike" from "lives at 4×". The wave's own speed is measured separately by
        // WaveExecutor and rides on balance.wave_result. 0 = no anchor (first wave of the
        // session, or the previous wave ended before this pulse existed) ⇒ reported as null.
        private uint m_CalmAnchorFrame;
        private DateTime m_CalmAnchorUtc;
...
#pragma warning restore CIVIC167
        ...
        // Last resident count the owner actually answered with, for the two consumers that
        // cannot take an empty value: session_end (written from OnDestroy, after the registry
        // is gone — a live read there throws) and the native-crash context (a flat int on
        // disk). Deliberately snapshotted in the FUNCTIONAL sample, not next to
        // m_LastKnownMoney: the balance sampler self-gates on the diagnostics opt-in, so a
        // player with diagnostics off would leave the holder at its initial value forever.
        // Null until the first successful measurement, and reset back to null on a city change
        // (OnGameLoaded) so a crash in the new city cannot report the previous city's size.
        private int? m_LastKnownPopulation;

        /// <summary>Last resident count measured during play; null until one was measured.</summary>
        public int? LastKnownPopulation => m_LastKnownPopulation;
        ...
        // The single owner of the vanilla city population record: the resident count for the
        // samples below AND the happiness/health pair of the wave snapshot. Both come from one
        // contract on purpose — the count used to arrive from a separate citizen tally, so one
        // wave snapshot carried two different measures of the same city.
        // [OwnedByFeatureId(Population)] without a null object ⇒ Require (CIVIC463); registered
        // in the producer's OnCreate, so it is up by the time the pulse runs from
        // TelemetryService.OnUpdate. Resolved once and cached, like m_CapacitySnapshotReader.
        private ICityPopulationReader? m_CityPopulationReader;
        ...
        public TelemetryPulse(
            ...
            EntityQuery gridStressQuery,
            EntityQuery milestoneQuery,
            EntityQuery enemyStateQuery,
            ...
            Func<int> getActiveBlackoutDistricts)
        {
            ...
            m_GridStressQuery = gridStressQuery;
            m_MilestoneQuery = milestoneQuery;
            m_EnemyStateQuery = enemyStateQuery;
            ...
            m_EventBus.Subscribe<WaveEndedEvent>(OnWaveEndedAnchorCalm);
        ...
        public void Dispose()
        {
            ...
            m_EventBus.Unsubscribe<WaveEndedEvent>(OnWaveEndedAnchorCalm);
        ...
        private void RecordSessionStart()
        {
            ...
            // (functional/analytics split). The chronicle reads performance.sample, not
            // this hardware event.
            if (!m_Config.Enabled) return;
        ...
        /// <summary>
        /// Which other mods are loaded, once per session. Recorded next to the hardware snapshot and
        /// behind the same diagnostics gate: it is crash-triage input, not a player-facing feature.
        ///
        /// Why it exists: a foreign mod is only attributable today when its own compiled module is the
        /// faulting one. When it corrupts data and vanilla faults, nothing in the crash record names
        /// it — the class that took the longest to close in this project looked exactly like that,
        /// with our own bug surfacing as a vanilla render fault. Sessions that run our mod alone are
        /// the control group; a crash there cannot be blamed on anyone else.
        ///
        /// Names only. The vanilla list carries no paths and no player identity, and it is redacted
        /// anyway, because a mod is free to name itself anything.
        /// </summary>
        private void RecordModEnvironment()
        {
            ...
            try
            {
                enabled = Game.Modding.ModManager.GetModsEnabled();
            }
            catch (Exception ex)
            {
                // Diagnostics must never take the session down with it.
                Log.Warn($" Mod environment unavailable: {ex}");
                return;
            }
            ...
            Array.Copy(enabled, names, enabled.Length);
            Array.Sort(names, StringComparer.Ordinal);

            var joined = PiiRedactor.Redact(string.Join(";", names));
            if (joined.Length > MOD_LIST_MAX_CHARS)
                joined = joined.Substring(0, MOD_LIST_MAX_CHARS);

            // "Alone" is about CODE mods: the vanilla list also carries UI modules, and ours is in it.
            bool solo = CountForeignCodeMods(names) == 0;

            Record(Diagnostics.ModEnvironment, new DiagnosticsModEnvironmentData
            {
                Mods = joined,
            ...
            Log.Info($" Mod environment: {names.Length} entries, solo={solo}");
        }

        /// <summary>
        /// Entries that are neither ours nor a UI module of ours. Substring match on the mod name is
        /// deliberate: vanilla reports the asset's full name, which carries our name plus a suffix
        /// depending on how the mod was installed (subscribed vs local build).
        /// </summary>
        private static int CountForeignCodeMods(string[] names)
        {
            int foreign = 0;
            for (int i = 0; i < names.Length; i++)
            {
                if (names[i] != null && names[i].IndexOf(Mod.MOD_NAME, StringComparison.OrdinalIgnoreCase) < 0)
                    foreign++;
            }
            return foreign;
        }
...
#pragma warning restore CIVIC100
            ...
            // Population: the vanilla city record through its owner. A refusal (no city entity)
            // leaves the sample EMPTY — an unmeasured city is not an empty one, and the server
            // averages this field. A successful read also refreshes the last-known holder, the
            // only value the two consumers that cannot take an empty number (session_end at
            // teardown, the native-crash context on disk) have to work with.
            if (m_World.IsCreated)
            {
                try
                {
                    m_CityPopulationReader ??= ServiceRegistry.Instance.Require<ICityPopulationReader>();
                    if (m_CityPopulationReader.TryGetResidentCount(out var residents))
                    {
                        population = residents;
                        m_LastKnownPopulation = residents;
                    }
                }
                catch (Exception ex)
                {
                    if (Log.IsDebugEnabled) Log.Debug($" Population read failed: {ex.Message}");
                }
            }
            ...
            // The crash context takes the LAST MEASURED population, not this sample's possibly
            // empty one: it is a flat int on disk, it feeds the "context is empty" test, and the
            // crash panel casts it straight to int. Before the first measurement the holder is
            // null and 0 is written — which is what "empty" means there, and the reset on city
            // change keeps it from being the previous city's number.
            CrashContextProvider.Update(new CrashContextSnapshot(
                gameDay,
                act,
                activeThreats,
                waveState.WaveNumber,
                m_LastKnownPopulation ?? 0,
                memoryMB,
                nativeMemoryMB,
                DateTime.UtcNow));
            ...
            // build-out and wave number. Classified as functional telemetry.
            ...
            // developer diagnostics, so they must NOT leave the machine when Online
        ...
        /// Key metric: blackout_percent — target is ~20%
        /// </summary>
        private void RecordBalanceReport()
        {
            ...
            // not emit it when Online is on but diagnostics is off. The window
        ...
        private void OnGameLoaded(GameLoadedEvent evt)
        {
            ...
            // The last-known-population holder lives HERE, not in the crash context, so
            // CrashContextProvider.Reset() above does not clear it. Without this line a crash
            // or a quit early in the newly loaded city would report the previous city's size —
            // the exact defect the Reset() call above exists to prevent, one field to the side.
            m_LastKnownPopulation = null;
        }

        /// <summary>
        /// Open the calm-period speed window when a wave ends. Paired with the read in
        /// <see cref="OnWaveStarting"/>; the interval between the two is exactly the peacetime
        /// stretch, so no gating on phase is needed.
        /// </summary>
        private void OnWaveEndedAnchorCalm(WaveEndedEvent evt)
        {
            m_CalmAnchorFrame = m_SimulationSystem.frameIndex;
            m_CalmAnchorUtc = DateTime.UtcNow;
        }
        ...
        {
            // One config snapshot for the whole handler (CIVIC347: avoid a torn hot-reload read).
                ...
                try
                {
                    ...
                    foreach (var aa in aaInstallations)
                    {
                        ...
                        switch (aa.Type)
                        {
                            ...
                            case AAType.HawkSAM: aaHawk++; break;
                            default: break;
                        }
                    }
                }
            ...
            int manpowerUsed = 0;
            int operationalAa = 0;
            // Causes, not just the outcome: the pool size alone cannot tell "clean city" from
            // "the corruption aggregate silently stopped being computed", nor a floored pool from
            // an honestly small one. Defaults match a clean, unfloored city.
            float patriotFactor = 1f;
            float visibleCorruptionScore = 0f;
            int exportLoadPercent = 0;
            bool manpowerFloorEngaged = false;
            if (m_MobilizationQuery.TryGetSingleton<MobilizationStateSingleton>(out var mob))
            {
                ...
                patriotFactor = mob.PatriotismFactor;
                visibleCorruptionScore = mob.VisibleCorruptionScore;
                exportLoadPercent = mob.ExportLoadPercent;
                manpowerFloorEngaged = mob.IsDefenceFloorEngaged;
                var fleet = new AirDefenseForecast.FleetComposition(aaHeritage, aaBofors, aaGepard, aaPatriot, aaHawk);
                operationalAa = AirDefenseForecast.OperationalAaFleet(balanceCfg, manpowerTotal, in fleet);
            }
            ...
            // Grid-health context at wave start: frequency/stress from GridStressData, the
            // capacity-loss decomposition from the resolver snapshot (shared ComputeLossBreakdown —
            // same numbers as the FLEET CAPACITY UI block and the [CapacityLoss] debug line).
            // Nominal defaults when the singletons are not up yet (pre-war boot window).
            float gridFrequencyHz = NOMINAL_GRID_FREQUENCY_HZ;
            float gridStressPercent = 0f;
            if (m_GridStressQuery.TryGetSingleton<GridStressData>(out var stressData))
            {
                gridFrequencyHz = stressData.CurrentFrequency;
                gridStressPercent = stressData.StressPercent;
            }
            float fleetSaturationFactor = 1f;
            var losses = default(Core.Logic.PowerLossBreakdown);
            m_CapacitySnapshotReader ??= ServiceRegistry.Instance.Require<IPowerCapacitySnapshotReader>();
            if (m_CapacitySnapshotReader.TryGetSnapshot(out var waveCapSnap))
            {
                fleetSaturationFactor = waveCapSnap.FleetTargetFactor;
                losses = waveCapSnap.ComputeLossBreakdown();
            }
            ...
            // City record: the resident count and the happiness/health averages are answered by
            // one owner, so a single snapshot can no longer mix a citizen tally with the vanilla
            // averages. A refusal (no city entity) leaves the count EMPTY — the prosperity ladder
            // divides homeless by it, so a substituted zero would either divide by zero or score
            // a full city as ruined; an empty count drops the wave out of the ladder instead.
            // The averages stay numeric: zero averages remain distinguishable from vanilla's
            // neutral 50, and nothing divides by them.
            int? population = null;
            int happinessAvg = 0;
            int healthAvg = 0;
            if (m_World.IsCreated)
            {
                try
                {
                    ...
                    if (m_CityPopulationReader.TryGetCityAverages(out var happiness, out var health))
                    {
                        happinessAvg = happiness;
                        healthAvg = health;
                    }
                }
                catch (Exception ex)
                {
                    if (Log.IsDebugEnabled) Log.Debug($" City population read failed: {ex.Message}");
                }
            }
            ...
            int milestone = m_MilestoneQuery.TryGetSingleton<Game.City.MilestoneLevel>(out var ml)
                ...
                : -1;
            // Homeless and unemployed come from the vanilla statistics system (same main-thread
            // API the infoview UI reads every frame) — managed reads, no sync points.
            int homelessCount = 0;
            int unemployedCount = 0;
            long deathsTotal = 0;
            try
            {
                homelessCount = m_CityStatisticsSystem.GetStatisticValue(Game.City.StatisticType.HomelessCount);
                unemployedCount = m_CityStatisticsSystem.GetStatisticValue(Game.City.StatisticType.Unemployed);
                // Lifetime deaths = sum of raw per-sample m_Value over the DeathRate buffer.
                // NOT GetStatisticValueLong / GetStatisticDataArray*: those return the DERIVED
                // m_TotalValue, whose meaning depends on the prefab's StatisticCollectionType
                // (for Daily it is a 32-sample sliding window, not a lifetime total — decompile
                // CityStatisticsSystem.ResetEntityJob). Raw m_Value accumulates +1 per death
                // identically for every collection type, the buffer is never truncated and is
                // fully serialized into saves, so the sum is the true cumulative count since
                // city start — including indirect deaths (service collapse) that the per-wave
                // Casualties counter never sees.
                deathsTotal = ComputeDeathsTotal();
            }
            catch (Exception ex)
            {
                if (Log.IsDebugEnabled) Log.Debug($" City statistics read failed: {ex.Message}");
            }

            // Enemy-pressure context: how much the player's counter-strikes weakened this wave.
            // Same formula as the threat-count scaling (WaveSimulationContext.ComputeEnemyPressure),
            // read at wave start; null when grid warfare is not active (EnemyState singleton absent).
            float? enemyPressure = null;
            int enemyRespiteAxes = 0;
            float? enemyPhysicalAxis = null;
            float? enemyDigitalAxis = null;
            float? enemySocialAxis = null;
            if (m_EnemyStateQuery.TryGetSingleton<EnemyState>(out var enemyState))
            {
                float gameTimeHours = Core.Systems.GameTimeSystem.TryGetGameHours(out var gh) ? gh : 0f;
                enemyPressure = (float)Math.Round(
                    WaveContextGatherer.ComputeEnemyPressure(in enemyState, balanceCfg.GridWarfare, gameTimeHours), 3);
                enemyRespiteAxes = enemyState.RespiteActiveAxisCount(gameTimeHours);
                enemyPhysicalAxis = (float)Math.Round(enemyState.PhysicalAxis, 1);
                enemyDigitalAxis = (float)Math.Round(enemyState.DigitalAxis, 1);
                enemySocialAxis = (float)Math.Round(enemyState.SocialAxis, 1);
            }

            // Speed context. GameSpeed is the setting the strike opens on; CalmAvgGameSpeed is what
            // the peacetime stretch before it actually ran at (sim seconds served per real second,
            // so a pause drags it down — that is the honest reading). Together they show whether the
            // player slows down for the fight or plays the whole session at one speed. The anchor is
            // consumed here so a wave never reports a window that already fed an earlier snapshot.
            float currentGameSpeed = m_SimulationSystem.selectedSpeed;
            float calmSpeedRaw = SimSpeedMath.EffectiveSpeed(
                SimSpeedMath.SimSecondsBetween(m_CalmAnchorFrame, m_SimulationSystem.frameIndex),
                SimSpeedMath.RealSecondsBetween(m_CalmAnchorUtc, DateTime.UtcNow));
            float? calmAvgGameSpeed = calmSpeedRaw > 0f ? (float?)Math.Round(calmSpeedRaw, 2) : null;
            m_CalmAnchorFrame = 0u;
            m_CalmAnchorUtc = default;
            ...
            Record(Balance.WaveSnapshot, new BalanceWaveSnapshotData
            {
                ...
                PatriotFactor = (float)Math.Round(patriotFactor, 3),
                VisibleCorruptionScore = (float)Math.Round(visibleCorruptionScore, 2),
                ExportLoadPercent = exportLoadPercent,
                ManpowerFloorEngaged = manpowerFloorEngaged,
                OperationalAa = operationalAa,
                PatriotInterceptsDrones = patriotInterceptsDrones,
                GridFrequencyHz = (float)Math.Round(gridFrequencyHz, 2),
                GridStressPercent = (float)Math.Round(gridStressPercent, 3),
                FleetSaturationFactor = (float)Math.Round(fleetSaturationFactor, 3),
                LossKnockedOutMw = (int)(losses.KnockedOutKW / 1000),
                LossDamageMw = (int)(losses.DamageLossKW / 1000),
                LossSaturationMw = (int)(losses.SaturationLossKW / 1000),
                LossFuelMw = (int)(losses.FuelLossKW / 1000),
                LossAllowedMw = (int)(losses.AllowedKW / 1000),
                ...
                EnemyRespiteAxes = enemyRespiteAxes,
                EnemyPhysicalAxis = enemyPhysicalAxis,
                EnemyDigitalAxis = enemyDigitalAxis,
                EnemySocialAxis = enemySocialAxis,
                GameSpeed = (float)Math.Round(currentGameSpeed, 1),
                CalmAvgGameSpeed = calmAvgGameSpeed
            });
            ...
            string integrityStr = cogAvg.HasValue
                ...
                : "not measured";
            string pressureStr = enemyPressure.HasValue
                ? $"{enemyPressure.Value:F2} ({enemyRespiteAxes} respite)"
                : "n/a";
            string calmSpeedStr = calmAvgGameSpeed.HasValue ? $"{calmAvgGameSpeed.Value:F2}x" : "n/a";
            // Spelling the empty count as "not measured" rather than letting int? render as an
            // empty string: the log is how the wave that dropped out of the ladder is told apart
            // from one that scored zero (Axiom 1).
            string popStr = population.HasValue ? $"{population.Value}" : "not measured";
            Log.Info($" Wave #{evt.WaveNumber} snapshot ({evt.ThreatsExpected} threats): ${money:N0}, {productionMW}MW/{demandMW}MW (bal:{balanceMW}), {aaCount} AA ({aaAmmoTotal}/{aaAmmoMax} rounds), integrity {integrityStr}, pop {popStr}, {blackoutDistricts} blackout, happiness {happinessAvg}/health {healthAvg}, homeless {homelessCount}, unemployed {unemployedCount}, deaths {deathsTotal}, milestone {milestone}, enemy pressure {pressureStr}, speed {currentGameSpeed:F1}x (calm {calmSpeedStr})");
        }

        /// <summary>
        /// Lifetime citizen deaths for the loaded city: direct sum of raw per-sample
        /// <c>m_Value</c> over the vanilla DeathRate <c>CityStatistic</c> buffer. Correct
        /// regardless of the prefab's StatisticCollectionType (see call-site comment).
        /// Main thread, once per wave snapshot; the statistics entity is cached and re-found
        /// only when it goes stale (load boundary).
        /// </summary>
        private long ComputeDeathsTotal()
        {
            var em = m_World.EntityManager;
            if (m_DeathRateStatEntity == Entity.Null
                || !em.Exists(m_DeathRateStatEntity)
                || !em.HasBuffer<Game.City.CityStatistic>(m_DeathRateStatEntity))
            {
                m_DeathRateStatEntity = FindDeathRateStatEntity(em);
                if (m_DeathRateStatEntity == Entity.Null)
                    return 0;
            }

            if (!em.HasBuffer<Game.City.CityStatistic>(m_DeathRateStatEntity))
                return 0;
            var buffer = em.GetBuffer<Game.City.CityStatistic>(m_DeathRateStatEntity, isReadOnly: true);
            double total = 0.0;
            for (int i = 0; i < buffer.Length; i++)
                total += buffer[i].m_Value;
            return (long)Math.Round(total, MidpointRounding.AwayFromZero);
        }

        /// <summary>
        /// Locate the live DeathRate statistics entity: the one carrying a CityStatistic
        /// buffer whose prefab's StatisticsData is StatisticType.DeathRate.
        /// CityStatisticsSystem keeps the (type → entity) map private, so this walks the
        /// handful of statistics entities once per cache fill.
        /// </summary>
        private static Entity FindDeathRateStatEntity(EntityManager em)
        {
            using var query = em.CreateEntityQuery(
                ComponentType.ReadOnly<Game.City.CityStatistic>(),
                ComponentType.ReadOnly<Game.Prefabs.PrefabRef>());
            using var entities = query.ToEntityArray(Allocator.Temp);
            for (int i = 0; i < entities.Length; i++)
            {
                var prefab = em.GetComponentData<Game.Prefabs.PrefabRef>(entities[i]).m_Prefab;
                if (prefab == Entity.Null || !em.HasComponent<Game.Prefabs.StatisticsData>(prefab))
                    continue;
                if (em.GetComponentData<Game.Prefabs.StatisticsData>(prefab).m_StatisticType
                    == Game.City.StatisticType.DeathRate)
                    return entities[i];
            }
            return Entity.Null;
        }
