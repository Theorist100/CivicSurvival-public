...
namespace CivicSurvival.Services.Telemetry
{
    ...
    public sealed class TelemetryPulse : IDisposable
    {
        ...
        private readonly EntityQuery m_CityPopulationQuery;
...
#pragma warning restore CIVIC167
        ...
        public TelemetryPulse(
            ...
            EntityQuery cityPopulationQuery,
            ...
            Func<int> getActiveBlackoutDistricts)
        {
            ...
            m_CityPopulationQuery = cityPopulationQuery;
...
#pragma warning restore CIVIC100
        ...
        private void OnWaveStarting(WaveStartingEvent evt)
        {
            ...
            // Prosperity context: vanilla city-level quality-of-life indicators. Population
            // singleton carries the averages CountHouseholdDataSystem maintains; homeless and
            // unemployed come from the vanilla statistics system (same main-thread API the
            // infoview UI reads every frame). All are singleton/managed reads — no sync points.
            int happinessAvg = 0;
            int healthAvg = 0;
            if (m_CityPopulationQuery.TryGetSingleton<Game.City.Population>(out var cityPop))
            {
                happinessAvg = cityPop.m_AverageHappiness;
                healthAvg = cityPop.m_AverageHealth;
            }
            ...
            string pressureStr = enemyPressure.HasValue
                ...
                : "n/a";
            Log.Info($" Wave #{evt.WaveNumber} snapshot ({evt.ThreatsExpected} threats): ${money:N0}, {productionMW}MW/{demandMW}MW (bal:{balanceMW}), {aaCount} AA ({aaAmmoTotal}/{aaAmmoMax} rounds), integrity {integrityStr}, pop {population}, {blackoutDistricts} blackout, happiness {happinessAvg}/health {healthAvg}, homeless {homelessCount}, unemployed {unemployedCount}, deaths {deathsTotal}, milestone {milestone}, enemy pressure {pressureStr}");
        }
