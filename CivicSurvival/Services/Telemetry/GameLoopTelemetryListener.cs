...
namespace CivicSurvival.Services.Telemetry
{
    ...
    internal sealed class GameLoopTelemetryListener : IDisposable
    {
        ...
        // Grid-event enrichment source (capacity-loss breakdown). Engineering-owned but
        // AlwaysOpen, so it is registered by the time the first grid event fires; resolved
        // lazily and cached — same pattern as TelemetryPulse.m_CapacitySnapshotReader.
        private IPowerCapacitySnapshotReader? m_CapacitySnapshotReader;

        // People living in the city, from the single owner of the vanilla city record, so a
        // recorded transition can be checked against the size of the city it happened in.
        // Registered in the producer's OnCreate; resolved lazily and cached — same pattern as
        // m_CapacitySnapshotReader above.
        private ICityPopulationReader? m_CityPopulationReader;

        // Last act actually recorded, so the post-load re-publish of the act event is not
        // mistaken for a transition. Null until the first real change this session.
        private Act? m_LastAct;
        ...
        public GameLoopTelemetryListener(IEventBus eventBus, TelemetryRecorder recorder, string sessionId)
        {
            ...
            m_EventBus.Subscribe<AirDefensePlacedEvent>(OnAirDefensePlaced);
            ...
            m_EventBus.Subscribe<ModLoadModalActionEvent>(OnModLoadModalAction);
            m_EventBus.Subscribe<ModUpdateModalEvent>(OnModUpdateModal);
            m_EventBus.Subscribe<ModInstallRemovedEvent>(OnModInstallRemoved);
        ...
        public void Dispose()
        {
            ...
            m_EventBus.Unsubscribe<AirDefensePlacedEvent>(OnAirDefensePlaced);
            ...
            m_EventBus.Unsubscribe<ModLoadModalActionEvent>(OnModLoadModalAction);
            m_EventBus.Unsubscribe<ModUpdateModalEvent>(OnModUpdateModal);
            m_EventBus.Unsubscribe<ModInstallRemovedEvent>(OnModInstallRemoved);
        ...
        {
            // Grid stress transitions carry the capacity-loss decomposition (same
            // ComputeLossBreakdown as the FLEET CAPACITY UI block), so the dashboard can see
            // WHY the grid was in deficit at the moment of the warning/collapse/recovery.
            // Null for every other infra subtype — the fields stay event-specific.
            bool isGridEvent = evt.Type == InfraEventType.GridStressWarning
                || evt.Type == InfraEventType.GridCollapse
                || evt.Type == InfraEventType.GridRecovery;
            float? fleetSaturationFactor = null;
            Core.Logic.PowerLossBreakdown? losses = null;
            if (isGridEvent)
            {
                ...
                {
                    fleetSaturationFactor = (float)Math.Round(capSnap.FleetTargetFactor, 3);
                    losses = capSnap.ComputeLossBreakdown();
                }
            }
            ...
            Record(Infra.Status, new InfraStatusData
            {
                ...
                // Shady-contract attribution rides only the disaster subtype — the producer
                // (PowerPlantDisasterSystem) is the only InfraEvent publisher that sets it.
                IsShady = evt.Type == InfraEventType.PowerPlantDisaster ? evt.IsShady : (bool?)null,
                FleetSaturationFactor = fleetSaturationFactor,
                LossKnockedOutMw = losses.HasValue ? (int?)(losses.Value.KnockedOutKW / 1000) : null,
                LossDamageMw = losses.HasValue ? (int?)(losses.Value.DamageLossKW / 1000) : null,
                LossSaturationMw = losses.HasValue ? (int?)(losses.Value.SaturationLossKW / 1000) : null,
                LossFuelMw = losses.HasValue ? (int?)(losses.Value.FuelLossKW / 1000) : null,
                LossAllowedMw = losses.HasValue ? (int?)(losses.Value.AllowedKW / 1000) : null
            });
        }

        private void OnActChanged(ActChangedEvent evt)
        {
            // The act event is re-published after every load so subscribers can re-initialise.
            // Recording that as a transition would invent one: it inflates the act-change stream
            // and, worse, stamps it with the current war day, which is exactly the field this
            // event now carries to tell an earned transition from a premature one. Only a real
            // change of act is recorded.
            if (m_LastAct == evt.NewAct)
                return;

            m_LastAct = evt.NewAct;

            // The pair of acts alone cannot separate an earned transition from a premature one.
            // War day and the population make that a single query: an Exodus on war day 2 with a
            // full city is a bug, on day 40 with a tenth of it is the mechanic working. A refusal
            // (no city entity) keeps the field empty rather than reporting an unmeasured city as
            // an empty one — the field is nullable for exactly that distinction.
            m_CityPopulationReader ??= ServiceRegistry.Instance.Require<ICityPopulationReader>();
            int? population = m_CityPopulationReader.TryGetResidentCount(out var residents)
                ? residents
                : (int?)null;

            var timeProvider = GameTimeSystem.Instance;
            int? warDay = timeProvider != null ? timeProvider.Current.WarDay : null;
        ...
        {
            Record(AirDefense.Placement, new AirDefensePlacementData
            {
                Result = evt.Success ? "success" : "failed",
                Reason = evt.Reason,
                AaType = evt.AaType.ToSnakeCase(),
                Cost = evt.Cost
            });
        }
        ...
        private void OnModLoadModalAction(ModLoadModalActionEvent evt)
            => Record(Diagnostics.ModLoadModal, new DiagnosticsModLoadModalData
            {
                Action = evt.Action,
                Cause = evt.Cause
            });

        private void OnModUpdateModal(ModUpdateModalEvent evt)
            => Record(Diagnostics.UpdateModal, new DiagnosticsUpdateModalData
            {
                Modal = evt.Modal,
                Action = evt.Action
            });

        private void OnModInstallRemoved(ModInstallRemovedEvent evt)
            => Record(Diagnostics.ModInstallRemoved, new DiagnosticsModInstallRemovedData
            {
                Phase = evt.Phase,
                Cause = evt.Cause,
                ListingSource = evt.ListingSource,
                ElapsedSinceBootSeconds = evt.ElapsedSinceBootSeconds,
                InCity = evt.InCity,
                NewerSibling = evt.NewerSibling,
                UnsubscribeSeen = evt.UnsubscribeSeen,
                ModIdsBefore = evt.ModIdsBefore,
                ModIdsAfter = evt.ModIdsAfter
            });
