...
namespace CivicSurvival.Core.Services
{
    ...
    public static class SatireRegistry
    {
        ...
        /// <summary>
        /// Try to get a random message by key prefix, succeeding only when the key is registered
        /// AND the chosen locale variant is actually present in the loaded locale.
        ///
        /// This is the content contract for callers that publish to a player-visible feed and must
        /// stay silent rather than show a raw placeholder. It closes both gaps of <see cref="GetMessage(string)"/>:
        /// an unregistered key (which GetMessage returns verbatim) and a registered key whose
        /// localization is missing (which GetMessage resolves to LocalizationManager's "[key]"
        /// sentinel). On either miss it returns false; only a real, localized line yields true.
        /// Thread-safe via lock.
        /// </summary>
        public static bool TryGetMessage(string keyPrefix, out string message)
        {
            SatireConfig config;
            bool found;
            lock (s_Lock)
            ...
            int index = NextVariantIndex(keyPrefix, config.VariantCount);
            string localeKey = $"{config.BaseLocalizationKey}_{index}";
            ...
            message = LocalizationManager.Get(localeKey);
            return true;
        }
