...
## v0.3.30 — Settings stay where you put them

- The settings window no longer stays on screen over the main menu after you leave a city, and it no longer closes on its own while you are using it. It closes when you close it: the X, the gear, or Escape.
- Bug reports are easier to read: shadow export income no longer buries the log under hundreds of tiny entries, so a report you send shows what actually happened.

---

## v0.3.29 — The war arrives when the town does

- A settlement that grows into a town now enters the war the way a city does: the opening scene, the air raid, the first strike and the volunteers' call. Below that size nothing comes for it — and the city now says so instead of leaving you wondering.
- Attacks come at a steadier pace, and a small city no longer waits hours between waves.
- The volunteers' air-defense request counts the guns you already have standing, instead of failing a city that came prepared.
- A request left over from another city no longer appears after you load a save.
- The city no longer falls into exodus in the opening seconds of the war, and a city that recovers can leave the exodus behind instead of being stuck in it for good.
- No more defeat screen for a settlement that had simply not grown yet.
- Refugees arrive once the settlement is big enough to take them in.
- Warnings that the city is emptying no longer greet you right after loading a healthy save.

---

## v0.3.28 — Crews come back to their guns

- Crews come back to their guns. A gun left short-handed when the city ran out of people now fills up again as soon as there are people to spare, instead of staying half-manned for good.
- Patriotism answers for every scheme the city can see, not just power sold abroad. Siphoned fuel, rigged contracts and spared districts all cost you volunteers now; money sitting in an offshore account does not, because nobody on the street can see it.
- The city tells you when it has run out of people to crew the guns, and the manpower panel names what is holding your volunteers back.
- You can always crew the guns the city started you with, however badly things are going.

---

## v0.3.27 — A chance to arm before the first strike

- The volunteers call before the first strike: you get the controls early and can put air defense up while there is still time. Answer their request and the city holds steadier through the opening raid.
- Finished and failed requests now announce their outcome instead of passing unnoticed.
- Notices arrive one at a time instead of all at once.
- The intro plays at its intended pace.
- Air defense carries more ammunition, and the city can mobilise more crews.
- Fixed: the mod could report its own files as missing after an update, and online status could stay stale after loading a save.

---

## v0.3.26 — Shelter evacuation, siren timing, update warnings

- Drone strikes now cause casualties based on who is actually at the impact site; citizens panic and run for shelter.
- Sirens fixed: proper Alert phase on the first wave, no more cutting out after ~40 seconds.
- Fake videos in one raid no longer stack media-trust damage (fixes media trust stuck at zero).
- Update warnings fixed: the pre-update countdown now reaches players, and the restart notice shows even if the mod's UI cannot load.

---

## v0.3.25 — Narrative quests, resupply pricing, combat performance

- Narrative quests: characters call with offers — answer, decline, or let it ring to defer. Active quests show as status-bar chips.
- Air defense: automatic resupply now charges the same tariff as the emergency refill button.
- Combat performance: removed psychology-simulation stutter spikes and cut its steady combat cost.
...
## v0.3.23 — Diagnostics and telemetry fixes

- Diagnostics and telemetry fixes.
