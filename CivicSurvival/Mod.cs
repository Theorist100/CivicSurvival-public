...
#pragma warning restore S1144, CA1823
        ...
        public static readonly ILog Log = LogManager
            ...
            // Detach our logger from Colossal's Backtrace crash channel. On every Error+ line
            // Colossal.Logging.CustomLogHandler calls BacktraceHelper.SendReport(ex, ..., log.logPath),
            // and Backtrace then does File.ReadAllBytes(CivicSurvival.log) asynchronously in its Send
            // coroutine. That read opens the file with FileShare.Read, which is incompatible with the
            // write handle we hold open all session (keepStreamOpen below), so it throws a
            // "Sharing violation" IOException OUT of the coroutine — past SendReport's try/catch —
            // onto the on-screen error overlay. We cannot change Backtrace's share mode and must keep
            // the stream open (it removes the per-line reopen NRE race, see the static ctor). Our own
            // errors already ship through our telemetry (error.report / bug_reports / Sentry), so the
            // Colossal Backtrace channel carries nothing we read; silencing it here is loss-free.
            .SetBacktraceEnabled(false)
        ...
        static Mod()
        {
            // Hold the log file handle open for the whole session. With the default
            // (keepStreamOpen == false) the vanilla UnityLogger opens+closes the file on
            // EVERY line; when a reopen momentarily loses the race to an AV/OneDrive scan of
            // LocalLow, Open() swallows the IO error and Internal_WriteStream then dereferences
            // a null m_StreamWriter — a NullReferenceException thrown out of the logging call
            // into whatever gameplay/UI system logged. Keeping the stream open removes the
            // per-line reopen (and its failure window) and drops the open/close syscall churn.
            Log.keepStreamOpen = true;
...
#pragma warning restore CIVIC052
        ...
        public void OnLoad(UpdateSystem updateSystem)
        {
            ...
            // subscribers load from the subscription cache, where the audio assets actually
            // live. asset.path is the loaded DLL in both cases, so deriving the directory
            // from it is the only path that works for shipped subscribers (audio; icons are
            // inlined in the UI bundle). An unresolved asset is fatal — see ResolveModInstallDirectory.
            ...
            // Test builds only: open the PDX.SDK's own flow log all the way up. The SDK hands the
            // game every internal step (playset sync, QueueRemoval, ProcessPendingRemovals,
            // DeleteModFromDisk) at L0/L1, but the vanilla "PdxSdk" Colossal logger defaults to
            // Info and drops them — PdxSdk.log ends up nearly empty. Raising effectiveness to
            // Verbose makes PdxSdk.log record the full mod-folder lifecycle, which is the direct
            // evidence channel for the prefab-absent instance-deletion investigation.
            Colossal.Logging.LogManager.GetLogger("PdxSdk")
                .SetEffectiveness(Colossal.Logging.Level.Verbose);
            Log.Info("[TEST BUILD] PdxSdk logger effectiveness raised to Verbose for SDK flow tracing");
#endif

            // Buffer our own boot-window Error lines until the diagnostics log-hook attaches —
            // bootstrap errors (icon check below, ParadoxNativeLoader abort) otherwise race the
            // hook attach and are lost to prod entirely. In-memory only; nothing leaves the
            // machine until diagnostics actually starts (see TelemetryCrashDetector).
            Services.Telemetry.TelemetryCrashDetector.BeginEarlyCapture();

            // Sample the SDK's active playset as early as possible (pre-first-sync) — the α
            // investigation needs "was the activation row alive at boot" as a per-report fact
            // (MOD_COK_LOADING §6, activeSetBoot). Paradox installs only: a dev install has no
            // playset row to sample. The probe is a MainThreadDispatcher updater — GameManager
            // pumps it every frame from boot, menu included — and unregisters after one answer.
            if (Core.Systems.Bootstrap.ModInstallResolver.StableModId().Length != 0)
                Core.Systems.Bootstrap.ModInstallResolver.StartBootActiveSetProbe();
            ...
            // active gates (closed-feature semantics).
        ...
        private void RegisterFeatures()
        {
            ...
            // Cross-domain coordinator features
        ...
        public void OnDispose()
        {
            ...
            Core.Services.BuildingPlacementHandlerRegistry.Clear();
            ...
            // boundary (same AppDomain hosts the same Game.dll).
