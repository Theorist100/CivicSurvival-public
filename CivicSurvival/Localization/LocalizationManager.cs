...
#pragma warning restore CIVIC080
        ...
        // ------------------------------------------------------------------
        // Vanilla dictionary overrides
        // ------------------------------------------------------------------

        /// <summary>
        /// Vanilla localization key of the efficiency-tooltip row for Efficiency slot 26
        /// (<c>EfficiencyFactor.CityModifierHospitalEfficiency</c>).
        /// <c>PowerCapacityResolverSystem</c> folds the mod's grid-producer output factor
        /// (damage × construction × saturation × fuel) into that slot, and vanilla's
        /// <c>EfficiencySection</c> labels the tooltip row by slot name — without an
        /// override players read "Hospital Efficiency" on every power plant and go build
        /// hospitals. Vanilla never writes slot 26 on any building (decompile-verified:
        /// the enum member has zero writers in Game.dll), so the row can only ever carry
        /// the mod's factor and overriding its label is safe. If a future game patch
        /// starts writing this slot, hospitals would show this label too — re-check the
        /// decompile on game updates.
        /// </summary>
        private const string EfficiencyFactorLabelKey =
            "SelectedInfoPanel.EFFICIENCY_FACTORS[CityModifierHospitalEfficiency]";

        private static readonly List<(string LocaleId, Colossal.Localization.MemorySource Source)> s_VanillaOverrides = new();

        private const string DefaultEfficiencyFactorLabel = "Grid output (damage, fuel & demand)";

        // Keys are VANILLA locale ids (the game ships zh-HANS/zh-HANT, never zh-CN), so the
        // Chinese entries must use those ids to reach the vanilla dictionary. uk-UA only shows
        // up when a Ukrainian localization mod is installed alongside.
        private static readonly Dictionary<string, string> s_EfficiencyFactorLabels = new()
        {
            ...
            ["zh-HANT"] = "電網出力（損壞、燃料與需求）",
        };

        private static string GetEfficiencyFactorLabel(string localeId)
            => s_EfficiencyFactorLabels.TryGetValue(localeId, out var label)
                ? label
                : DefaultEfficiencyFactorLabel;

        /// <summary>
        /// Overrides the slot-26 efficiency-factor label in the VANILLA dictionary for
        /// every locale the game has loaded. Registered once per mod load; sources are
        /// removed in <see cref="Cleanup"/> so a hot-reload does not stack duplicates.
        /// </summary>
        private static void RegisterVanillaOverrides()
        {
            try
            {
                ...
                if (locManager == null)
                {
                    Log.Warn("Vanilla localization manager unavailable — efficiency-factor label override skipped");
                    return;
                }

                foreach (var localeId in locManager.GetSupportedLocales())
                {
                    var source = new Colossal.Localization.MemorySource(new Dictionary<string, string>
                    {
                        [EfficiencyFactorLabelKey] = GetEfficiencyFactorLabel(localeId),
                    });
                    locManager.AddSource(localeId, source);
                    s_VanillaOverrides.Add((localeId, source));
                }

                Log.Info($"Registered efficiency-factor label override for {s_VanillaOverrides.Count} locales");
            }
            catch (Exception ex)
            {
                Log.WarnException("Failed to register vanilla localization overrides", ex);
            }
        }

        private static void UnregisterVanillaOverrides()
        {
            try
            {
                var locManager = Game.SceneFlow.GameManager.instance?.localizationManager;
                if (locManager != null)
                {
                    foreach (var (localeId, source) in s_VanillaOverrides)
                        locManager.RemoveSource(localeId, source);
                }
            }
            catch (Exception ex)
            {
                Log.WarnException("Failed to unregister vanilla localization overrides", ex);
            }
            finally
            {
                s_VanillaOverrides.Clear();
            }
        }
            ...
            try
            {
                ...
                if (locManager != null)
                {
                    ...
                    if (!string.IsNullOrEmpty(activeLocale))
                    {
                        ...
                        // The game ships zh-HANS and zh-HANT; both resolve to our Simplified
                        // table — written Chinese stays mutually readable, which beats English.
                        // Not clamped: Chinese carries the neutral EN text, so it needs no build gate.
                        if (activeLocale.StartsWith("zh", System.StringComparison.OrdinalIgnoreCase))
                            return "zh-CN";
                    }
                }
            }
        ...
        {
            // Chinese carries the neutral English text, so it ships on every build.
