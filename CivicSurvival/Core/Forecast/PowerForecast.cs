...
namespace CivicSurvival.Core.Forecast
{
    ...
    public static class PowerForecast
    {
        ...
        // FORECAST-APPROX: the runtime saturation target is computed from the damage-RESIDUAL
        // nameplate (ComputeFleetTargetFactor), while this projection passes the full nameplate —
        // with damage pinned = 1 the two coincide on a healthy fleet, and on a damaged fleet the
        // forecast over-degrades slightly (consistent with pinning damage = 1 above; feeding a
        // fleet damage factor here would require per-plant damage the forecast does not model).
