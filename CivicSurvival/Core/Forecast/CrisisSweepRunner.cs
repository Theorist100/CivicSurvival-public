    ...
    public sealed class CrisisSweepRunner
    {
        ...
        private static void RunInvariant(in CrisisSweepRequest req, in LiveInputs live, int archetype, int populationPeak, ref CrisisSweepResultSingleton result)
        {
            ...
            if (live.TryGetPower(out float liveProductionMW, out _, out float liveLargestPlantMW, out _, out _))
            {
                ...
                {
                    // Pre-war: no mobilization snapshot, so the pool is recruited from the city as it
                    // is NOW, not as large as it ever was. Falls back to the peak only when the owner
                    // refuses (no city entity), which in this branch cannot happen — power is live.
                    int basePopulation = live.TryGetPopulation(out int livePopulation)
                        ? livePopulation
                        : populationPeak;
                    pool = ManpowerForecast.Pool(basePopulation, req.Patriotism, req.Morale, warDay, isConscription: false);
                }
            }
            else
            {
            ...
            else
            {
                ...
                // DEFENSE-DRIVEN loss: the surviving generation is the share the air
                ...
                // wave leak regulator. An undefended zone (fCov → 0 ⇒ leak → 1) is physically
        ...
        private static void RunPacing(in CrisisSweepRequest req, in LiveInputs live, int archetype, int populationPeak, ref CrisisSweepResultSingleton result)
        {
            ...
            // Psy waves insert the PsyAttack beat between Attack and Recovery. Gated through the same
            // arming predicate the wave clock uses (PsyOpsArming.WillFire) so the forecast cycle cannot
            // drift from the runtime cycle; the pacing worst case is Crisis-act by construction (this
            // runner models the Crisis sweep), and wave 20 >= MIN_RAID_WAVE, so the beat is always
            // armed here — the predicate call keeps that coupled to the rule, not assumed.
            float psy = PsyOpsArming.WillFire(Act.Crisis, waveNum)
                ? WaveForecast.PhaseDuration(GamePhase.PsyAttack, waveNum, frequencyMod, 0.5f)
                : 0f;
            ...
            float cycle = calmSeconds + alert + attack + psy + recovery;
...
#pragma warning restore CIVIC021
            ...
            // Base the timeline's recomputed pool on the city as it stands, not on the largest it ever
            // was. The pool has to be recomputed per wave because war fatigue decays it with the day
            // count — but recomputing it from the peak makes the forecast promise crews that left with
            // the population, and the verdict it feeds is "is this city still recoverable". The peak
            // remains the fallback for the archetype path, where no city exists to ask.
            int timelineBasePopulation = live.TryGetPopulation(out int liveTimelinePopulation)
                ? liveTimelinePopulation
                : populationPeak;
            ...
            for (int run = 0; run < runs; run++)
            {
                ...
                while (t < horizon)
                {
                    ...
                    if (t >= nextWave)
                    {
                        ...
                        int pool = ManpowerForecast.Pool(timelineBasePopulation, req.Patriotism, req.Morale, curWarDay, req.IsConscription);
                        pool = math.max(0, pool - liveCommitted);

                        // DEFENSE-DRIVEN per-wave loss: leak is the unprotected fraction of the
                        ...
                        // formula the invariant uses, plus the wave leak regulator (which pulls the
                        // kill share toward MaxWaveInterceptFraction without exempting any target).
                        // When coverage → 0 (no crewed AA in range) leak → 1 and
