    ...
    public static class ManpowerForecast
    {
        ...
        public static int Pool(int populationPeak, float patriotism, float morale, int warDay, bool isConscription)
            => ManpowerLogic.CalculateTotalManpower(
                ...
                // Forecast assumption: no draft evasion. The dodger level is a live psy-war
                // transient (enemy Propaganda pressure × player counter), not a policy input the
                // sweep models — the baseline pool deliberately reads it at 1.0.
                // Same category: the Draft-Exemption scheme deductions (sold deferrals /
                // disabilities) are live corruption state, excluded like casualties are.
                dodgerFactor: 1f,
                isConscription);
    }
}
