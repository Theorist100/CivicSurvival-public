...
namespace CivicSurvival.Core.Forecast
{
    ...
    /// leak fraction the crisis sweep uses to drive generation loss. Composes the
    /// <see cref="AALogic"/> intercept leaf (per-shot accuracy with rising evasion) and the wave
    /// leak regulator — NOT new balance arithmetic. Lifted verbatim out of CrisisSweepSystem so the
    ...
    public static class AirDefenseForecast
    {
        ...
        /// <summary>Leak fraction: covered drones leak per the regulated per-shot loop (see
        /// <see cref="RegulatedCoveredLeak"/>), uncovered drones always leak. Heritage base chance
        /// (archetype path).</summary>
        public static float Leak(RemoteBalanceConfig cfg, float fCov, float baseChance, int shotsPerDrone)
        {
            float coveredLeak = RegulatedCoveredLeak(
                cfg, baseChance, cfg.AAUnits.HeritageMaxAmmo, cfg.AAUnits.HeritageMaxAmmo, shotsPerDrone);
            return fCov * coveredLeak + (1f - fCov) * 1f;
        }

        /// <summary>
        /// Covered-drone leak fraction under the wave leak regulator
        /// (<see cref="LeakRegulatorLogic"/>): the AA's own per-shot lethality, pulled toward
        /// <c>Waves.MaxWaveInterceptFraction</c> but never past what suppression can actually reach.
        ///
        /// The regulator is a set point, not a hard floor — that is the whole difference from the
        /// per-target leak floor this replaced, and it is why the forecast no longer needs a
        /// FORECAST-APPROX note here: both sides now express the same continuous quantity (a wave
        /// kill share), instead of the forecast approximating a per-building binary verdict.
        ///
        /// Weak AA never reaches the set point, so it stays at its own lethality; strong AA is
        /// pulled down to the set point, but only as far as the minimum multiplier permits.
        /// </summary>
        private static float RegulatedCoveredLeak(
            RemoteBalanceConfig cfg, float baseChance, int currentAmmo, int maxAmmo, int shotsPerDrone)
        {
            var waves = cfg.Waves;
            float target = math.clamp(waves.MaxWaveInterceptFraction, 0f, 1f);
            float kill = 1f - PerDroneSurvive(baseChance, currentAmmo, maxAmmo, shotsPerDrone);

            // Regulator off (strength 0) or no ceiling to hold (target >= 1): raw lethality.
            if (waves.LeakRegulatorStrength <= 0f || target >= 1f)
                return 1f - kill;

            // Hardest the regulator can pull: every shot's chance scaled by the minimum multiplier.
            float minMult = math.clamp(waves.LeakRegulatorMinMultiplier, 0f, 1f);
            float killFullySuppressed = 1f - PerDroneSurvive(baseChance * minMult, currentAmmo, maxAmmo, shotsPerDrone);

            float lowerBound = math.min(killFullySuppressed, kill);
            return 1f - math.clamp(target, lowerBound, kill);
        }
        ...
        public readonly struct FleetComposition
        {
            ...
            public readonly int Hawk;

            public FleetComposition(int heritage, int bofors, int gepard, int patriot, int hawk)
            {
                Heritage = heritage;
                Bofors = bofors;
                Gepard = gepard;
            ...
            public int Total => Heritage + Bofors + Gepard + Patriot + Hawk;
        }
        ...
        // with launcher attrition + CrewMath.CommitFleet in the "AA late-war fidelity" increment.
        private static float OperationalFraction(RemoteBalanceConfig cfg, int manpowerPool, in FleetComposition fleet)
        {
            float crewDemand =
                ...
                fleet.Hawk * ParamsFor(cfg, AAType.HawkSAM).Crew;
            if (crewDemand <= 0f)
                return 0f;
            return math.clamp(math.max(0, manpowerPool) / crewDemand, 0f, 1f);
        }

        /// <summary>Live mixed-fleet leak fraction. Generalises <see cref="Leak"/> across the
        /// type mix: each type contributes a coverage share (op_count·disc(range)/area) and leaks at its
        /// own regulated per-shot survive within that share. The uncovered remainder (1 − min(1, Σ coverage)) always leaks. With one Heritage type
        ...
        public static float FleetLeak(RemoteBalanceConfig cfg, int manpowerPool, in FleetComposition fleet, float areaKm2, int shotsPerDrone, bool patriotInterceptsDrones)
        {
            ...
            AccumulateType(cfg, AAType.HeritageBofors, fleet.Heritage, frac, areaKm2, shotsPerDrone, ref totalCovered, ref weightedCoveredLeak);
            AccumulateType(cfg, AAType.Bofors40mm, fleet.Bofors, frac, areaKm2, shotsPerDrone, ref totalCovered, ref weightedCoveredLeak);
            AccumulateType(cfg, AAType.Gepard, fleet.Gepard, frac, areaKm2, shotsPerDrone, ref totalCovered, ref weightedCoveredLeak);
            // Hawk is a dedicated anti-drone missile launcher — it always enters the drone-leak math
            // (unlike the Patriot, whose drone role is opt-in), and never touches the ballistic layer.
            AccumulateType(cfg, AAType.HawkSAM, fleet.Hawk, frac, areaKm2, shotsPerDrone, ref totalCovered, ref weightedCoveredLeak);
            ...
            if (patriotInterceptsDrones)
                AccumulateType(cfg, AAType.PatriotSAM, fleet.Patriot, frac, areaKm2, shotsPerDrone, ref totalCovered, ref weightedCoveredLeak);
        ...
        public static float FleetCoverage(RemoteBalanceConfig cfg, int manpowerPool, in FleetComposition fleet, float areaKm2, bool patriotInterceptsDrones)
        {
            ...
            float totalCovered =
                ...
                CoverShare(cfg, AAType.HawkSAM, fleet.Hawk, frac, areaKm2) +
        ...
        /// op_count·disc(range)/area; coveredLeak per <see cref="RegulatedCoveredLeak"/>.</summary>
            ...
            ref float totalCovered, ref float weightedCoveredLeak)
        {
            ...
            float coveredLeak = RegulatedCoveredLeak(cfg, p.BaseChance, p.MaxAmmo, p.MaxAmmo, shotsPerDrone);
            ...
            RemoteBalanceConfig cfg, int manpowerPool, in FleetComposition fleet, float areaKm2, int cityMW, int ballisticTargets, bool patriotInterceptsDrones)
        {
            ...
            // (AABuildingPlacementHandler), so the forecast MUST scale by the SAME formula or it
