...
namespace CivicSurvival.Core.Utils
{
    ...
    internal static class JsonStream
    {
        ...
        /// <summary>
        /// Find a top-level long-integer field by key. Returns <paramref name="fallback"/> if the
        /// field is absent, non-numeric, or malformed. Use for unix timestamps — int truncates
        /// them after 2038.
        /// </summary>
        public static long TryReadLongField(string json, string key, long fallback = 0)
        {
            var token = TryReadScalarField(json, key);
            return token != null
                && long.TryParse(token, System.Globalization.NumberStyles.Integer, System.Globalization.CultureInfo.InvariantCulture, out long value)
                ? value
                : fallback;
        }
