...
namespace CivicSurvival.Core.Utils
{
    ...
    public static class PerformanceProfiler
    {
        ...
        /// Default rides the CIVIC_PERF compile switch (csproj EnableCivicPerf):
        /// true on local dev builds, false on any publish build — players never pay
        /// probe overhead. Before this gate, Enabled defaulted to true unconditionally
        /// and nothing ever set it false, so shipped builds profiled every player.
        /// </summary>
#if CIVIC_PERF
        ...
        /// No-op without CIVIC_PERF: a published build neither profiles nor creates
        /// PERF.log at all (the FPS/memory OnFrame path early-returns on s_Initialized).
        /// </summary>
...
#pragma warning restore CIVIC052
        ...
        public static MeasureScope Measure(string name)
        {
            ...
            // → no-op default scope, zero cost: the shipping default via CIVIC_PERF
            // (csproj EnableCivicPerf — off on publish builds, on locally).
