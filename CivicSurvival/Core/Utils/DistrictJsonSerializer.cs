...
namespace CivicSurvival.Core.Utils
{
    ...
    public struct RadarThreatDto
    {
        ...
        public bool IsOutbound;    // true for the player's outbound counter-strike (allied track)
    }
    ...
    /// <summary>
    /// One telecom broadcast coverage circle for the radar "mental view" overlay: world
    /// ground position and signal range (meters), read from the vanilla telecom towers.
    /// Geometric only — "where the city is on the air". Serialized via JsonBuilder by
    /// ThreatUISystem. Distinct from RadarDefenseDto (air-defense) so the two overlays can
    /// carry different colours/semantics.
    /// </summary>
    public struct RadarBroadcastDto
    {
        public float X, Z;
        public float Range;
    }
