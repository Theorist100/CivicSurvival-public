    ...
    public readonly struct LogContext
    {
        ...
        private static bool IsLogUnavailable(System.Exception ex)
        {
            return ex is System.TypeInitializationException
                ...
                || IsLoggerInfrastructureFailure(ex)
                || ex.InnerException != null && IsLogUnavailable(ex.InnerException);
        }

        // A contended/dead UnityLogger file stream makes the vanilla writer throw:
        // Internal_WriteStream dereferences a null m_StreamWriter after a failed reopen
        // (NullReferenceException), and a closed/reopening stream surfaces as
        // ObjectDisposed/InvalidOperation. These are logging-infrastructure faults, not
        // caller errors, and must not propagate into the gameplay/UI system that happened
        // to log. Swallow ONLY when the throw originates inside Colossal.Logging, so a
        // NullReferenceException from our own message interpolation is still surfaced.
        private static bool IsLoggerInfrastructureFailure(System.Exception ex)
        {
            ...
            return trace != null && trace.Contains("Colossal.Logging", System.StringComparison.Ordinal);
        }
