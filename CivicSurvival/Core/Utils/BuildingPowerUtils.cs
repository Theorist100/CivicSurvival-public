...
namespace CivicSurvival.Core.Utils
{
    ...
    public static class BuildingPowerUtils
    {
        ...
        /// <summary>
        /// Get the live district reference (index + version) for a building entity, so callers that
        /// persist the key can ride the engine entity remap via DistrictKeyCodec. Returns
        /// <see cref="DistrictRef.Null"/> (unzoned) if the building has no district.
        /// </summary>
        public static DistrictRef GetBuildingDistrictRef(Entity building, ComponentLookup<CurrentDistrict> districtLookup)
        {
            if (!districtLookup.HasComponent(building))
                return DistrictRef.Null;

            return DistrictRef.FromEntity(districtLookup[building].m_District);
        }
