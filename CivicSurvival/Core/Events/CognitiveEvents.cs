...
namespace CivicSurvival.Core.Events
{
    ...
    /// <summary>
    /// Command: add tons to the buckwheat reserve from an external (non-procurement) source —
    /// a quest reward. Mirrors the CorruptionGainEvent cross-domain pattern: the publisher
    /// (Narrative QuestSystem) never touches Cognitive state; the reserve owner applies the write.
    ///
    /// Published by: QuestSystem (Narrative)
    /// Consumed by: BuckwheatSystem (Cognitive, singleton owner)
    /// </summary>
    public record GrantBuckwheatCommand(float Tons, string Reason) : IGameEvent;

    /// <summary>
    /// Command: adjust media trust by a delta (positive for a quest reward). Same cross-domain
    /// shape as <see cref="GrantBuckwheatCommand"/>; the trust owner (TelemarathonSystem) applies
    /// the write through its existing single-writer entry point (ApplyMediaTrustHit, CIVIC175).
    ///
    /// Published by: QuestSystem (Narrative)
    /// Consumed by: TelemarathonSystem (Cognitive, singleton owner)
    /// </summary>
    public record AdjustMediaTrustCommand(float Delta, string Reason) : IGameEvent;
}
