...
{
    /// <summary>
    /// A post for the social feed. <paramref name="BypassAuthorCooldown"/> exempts the post
    /// from the feed's per-author anti-spam window (NOT from the duplicate filter): meant for
    /// rare lifecycle beats — a quest's start and its resolution can land within the 60s
    /// real-time window at high game speed, and silently losing the resolution reads as a
    /// broken story. Ambient chatter must keep the default (false).
    /// </summary>
    public record SocialPostEvent(string Author, string Message, SocialMood Mood, bool BypassAuthorCooldown = false) : IGameEvent;
    ...
    /// <summary>
    /// Diagnostic: lifecycle of the player-facing ModLoadFailure modal (core threat .cok
    /// absent or unregistered). Action is "shown" (the UI actually rendered the notice),
    /// "continue" (player dismissed) or "send" (player clicked the report button); Cause is
    /// the disk-check verdict the modal displayed. Answers whether the broken-install advice
    /// was seen at all and what the player did with it. Published by: ScenarioUISystem
    /// (from the UI trigger fired by the modal component).
    /// </summary>
    public record ModLoadModalActionEvent(string Action, string Cause) : IGameEvent;

    /// <summary>
    /// Diagnostic: lifecycle of the update-safety modals. Modal is "pending" (ModUpdatePending —
    /// pre-publish announce countdown) or "restart" (ModUpdatedRestart — install folder deleted
    /// under the live process); Action is "shown" (the UI actually rendered the notice — the only
    /// proof it reached the player) or "dismissed". Zero shown-events next to a C#-side show
    /// request expose a dead React layer, the same blind spot ModLoadModalActionEvent closes for
    /// load failures. Published by: ScenarioUISystem (from the UI trigger fired by the modal
    /// components).
    /// </summary>
    public record ModUpdateModalEvent(string Modal, string Action) : IGameEvent;

    /// <summary>
    /// Diagnostic: the install folder was deleted while this process kept running. Carries the
    /// deletion's facts as values, so prod can count deletions and split them by cause without
    /// parsing the watchdog's prose. Published twice per deletion: Phase "detected" at the moment
    /// the folder is found gone — where Cause is provisional, since the backend listing has not
    /// answered yet — and Phase "settled" from the follow-up 5-30s later, whose cause is the one
    /// to trust. Both are published because short sessions end before the follow-up runs at all.
    /// Published by: ModInstanceWatchSystem.
    /// </summary>
    public record ModInstallRemovedEvent(
        string Phase,
        string Cause,
        string ListingSource,
        int ElapsedSinceBootSeconds,
        bool InCity,
        bool NewerSibling,
        bool UnsubscribeSeen,
        int ModIdsBefore,
        int ModIdsAfter) : IGameEvent;
    ...
    /// <summary>
    /// Published when the vanilla clock crosses an hour boundary (24 per game day).
    /// Exists so income that logically accrues per day can be paid out in 24 hourly
    /// slices: a single midnight payout left the player staring at an unchanged
    /// wallet for a whole game day (~15 real minutes at 1x) after switching a scheme
    /// on, with no feedback that anything worked.
    ///
    /// Slice-payers must divide their daily amount by <see cref="GameRate.HOURS_PER_DAY"/>
    /// so the per-day total is unchanged — this event is about cadence, not amounts.
    /// NOT a replacement for <see cref="DayChangedEvent"/>: per-day probabilities,
    /// billing cycles and once-a-day decisions stay on the day tick, where their
    /// balance is calibrated.
    ///
    /// Published by: GameTimeSystem (vanilla hour boundary; skipped while paused —
    /// the sim does not tick, so no hours pass).
    /// </summary>
    /// <param name="DayNumber">Total days since game start</param>
    /// <param name="Hour">Hour that was just entered (0-23)</param>
    public record HourChangedEvent(int DayNumber, int Hour) : ISequencedEvent
    {
        // Global hour index — strictly increasing across day boundaries, so the
        // EventBus watermark drops re-delivery of an hour already handled.
        public long Sequence => (long)DayNumber * (long)GameRate.HOURS_PER_DAY + Hour;
    }
    ...
    /// <summary>
    /// Published when the player commits — or is denied — an air-defense installation placement.
    /// Feeds the air-defense funnel telemetry: attempt vs success vs the resource that blocked it
    /// (the tail of "opened war tab → saw coverage → built"). A player command, not a derived-state
    /// transition — a reloaded save does not re-emit it.
    ///
    /// Published by: AABuildingPlacementHandler (Commit = success; manpower/budget aborts = failure)
    /// Consumed by: GameLoopTelemetryListener
    /// </summary>
    /// <param name="Success">True if the installation was placed; false if the attempt was refused.</param>
    /// <param name="Reason">Failure cause snake_case (none / insufficient_manpower / insufficient_budget); "none" on success.</param>
    /// <param name="AaType">Resolved AA type snake_case (e.g. heritage_bofors, gepard).</param>
    /// <param name="Cost">Budget charged (success) or the price that could not be paid (budget failure); 0 otherwise.</param>
    /// <param name="Crew">Crew the installation takes. Carried because it is per-TYPE, not a
    /// constant: consumers that measure defense effort in PEOPLE (the air-defense quest) would
    /// otherwise have to assume a light gun and undercount every heavy battery.</param>
    public record AirDefensePlacedEvent(bool Success, string Reason, string AaType, int Cost, int Crew) : IGameEvent;

    /// <summary>
    /// Published when the World Shock aid tier changes (DeepConcern/Headlines/GlobalShock).
    /// Carries the shock level and cumulative counters at the transition for telemetry.
    ///
    /// Published by: WorldShockSystem.OnTierChanged (single publisher)
    /// Consumed by: AttentionTelemetryListener
    /// </summary>
    public record ShockTierChangedEvent(
        AidTier FromTier,
        AidTier ToTier,
        float ShockLevel,
        long TotalCasualties,
        long TotalBuildingsDestroyed) : IGameEvent;
    ...
    /// Consumed by: ScenarioStateMachine (triggers StartWar — the only entry into the war,
    /// for a city that was born big and for a village that grew into one alike)
    ...
    /// <summary>
    /// Published when ScenarioStateMachine classifies the settlement by population, and again
    /// when a village outgrows village size — that promotion is what ends the pre-war phase and
    /// sends the settlement into the war through the cold open, so subscribers must survive the
    /// type changing under them, not only a repeat of the same value.
    ///
    /// Published by: ScenarioStateMachine.DetectScenarioType() / TryPromoteScenarioType()
    /// Consumed by: OminousSignsSystem (opens PreWar for Village, closes it on promotion),
    ///              IntroScenarioSystem (postpones the cold open on Village, plays it on
    ///              promotion), RefugeeInfluxCoordinator, OnlineConsentGateSystem,
    ///              ScenarioMilestonesSystem, ScenarioUISystem, GameLoopTelemetryListener
    /// </summary>
    ...
    /// <summary>
    /// The activists' warning window opened during the cold open: the HUD is handed over early
    /// so the player can put air defense up before the first strike. Carries the manpower the
    /// city can still commit, which is what the air-defense quest keys its target off — the
    /// intro owns the window, Narrative owns the quest, and neither imports the other (Axiom 5).
    ///
    /// Published by: IntroScenarioSystem (single publisher)
    /// Consumed by: QuestSystem (arms QuestId.AirDefenseReady)
    /// </summary>
    /// <param name="AvailableManpower">Manpower free at the moment the window opens. EMPTY when
    /// the mobilization pool could not be measured — the publisher does not substitute a zero,
    /// because a zero is a real answer here ("the city has nobody free") and the quest already
    /// treats it as a reason to stay silent. Both cases end in no quest, but for different
    /// reasons, and the consumer says which in its log rather than guessing later.</param>
    public record ActivistWindowOpenedEvent(int? AvailableManpower) : IGameEvent;

    /// <summary>
    /// The city met the activists' call and had guns up before the first strike — grant the
    /// city-wide morale bonus. Temporary by construction: the consumer drops it when the second
    /// wave starts, so the reward covers the strike it was earned for and no longer.
    ///
    /// A command event across the domain boundary (the CorruptionGainEvent pattern): Narrative
    /// grants, Wellbeing owns the happiness ledger, neither imports the other (Axiom 5).
    ///
    /// Published by: QuestSystem (QuestId.AirDefenseReady completion)
    /// Consumed by: DistrictPenaltySystem (global happiness bonus)
    /// </summary>
    public record AirDefenseDrillRewardEvent : IGameEvent;

    /// <summary>
    /// A narrative quest reached a terminal phase. Carries the counter as it stood at
    /// resolution because the slot zeroes it a few game hours later, when the chip clears —
    /// the outcome modal must still be able to say "3 of 4".
    ///
    /// QuestSystem owns quest STATE and stays out of the modal stack; QuestUISystem owns the
    /// screen and decides what is shown. That split is also what keeps the quest system free
    /// of coordinator state it would have to reset on load (CIVIC317).
    ///
    /// Published by: QuestSystem (single publisher)
    /// Consumed by: QuestUISystem (shows QuestCompleted / QuestFailed)
    /// </summary>
    /// <param name="QuestId">C# QuestId value of the resolved quest.</param>
    /// <param name="Success">True = completed, false = failed.</param>
    /// <param name="Progress">Counter reached (0 for quests that are not counted).</param>
    /// <param name="Target">Counter asked for (0 for quests that are not counted).</param>
    public record QuestResolvedEvent(int QuestId, bool Success, int Progress, int Target) : IGameEvent;
    ...
    /// <summary>
    /// WaveScheduler передаёт Executor прогноз СЛЕДУЮЩЕЙ волны, решённый на входе
    /// в Recovery/Calm (тип фиксируется один раз за цикл; оценка размера —
    /// детерминированная, без noise-броска, обновляется на каждом входе в
    /// Recovery/Calm). Executor — единственный писатель полей
    /// WaveStateSingleton.NextWave*; сбрасывает NextWaveForecastReady на Alert,
    /// когда прогноз становится текущей волной.
    /// Published by: WaveScheduler (Scenario domain). Consumed by: WaveExecutor.
    /// </summary>
    /// <param name="HasForecast">True если прогноз решён; false очищает поля прогноза.</param>
    /// <param name="NextWaveType">Тип следующей волны (валиден при HasForecast).</param>
    /// <param name="NextWaveNumber">Номер волны, которую запустит ближайший ScheduleAlertPhase.</param>
    /// <param name="ThreatsForecast">Детерминированная (noiseMult=1) оценка числа угроз.</param>
    public record WaveForecastEvent(
        bool HasForecast,
        WaveType NextWaveType,
        int NextWaveNumber,
        int ThreatsForecast) : IGameEvent;
