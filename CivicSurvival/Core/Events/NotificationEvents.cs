...
namespace CivicSurvival.Core.Events
{
    ...
    public record ThreatImpactEvent(float3 Position, bool IsBallistic, ThreatLeakReason LeakReason = ThreatLeakReason.Unknown) : IGameEvent;
    ...
    public record WaveEndedEvent(
        ...
        // How long the kinetic strike actually lasted, in SIMULATION seconds, plus the effective
        // speed the player ran it at. Wall-clock duration cannot be derived from the event stream:
        // vanilla fits 60 × selectedSpeed sim ticks into a real second, so the same wave reads
        // three times shorter at 4× than at 1×. 0 = not measured (wave resumed from a save, so the
        // attack-start anchor is gone) — NOT a zero-length wave.
        float DurationSimSeconds = 0f,
        float AvgGameSpeed = 0f
    ) : IGameEvent
    {
...
#pragma warning restore S101
    ...
    /// </summary>
    /// <remarks>
    /// <c>AARotation</c> — the AA prop's world rotation (from the same cached Transform as
    /// AAPosition). Lets InterceptorSpawnSystem rotate the asset-local launcher offset into world
    /// space, so the missile appears over the LAUNCHER, not the asset pivot (the Hawk pivot sits on
    /// its truck). <c>default</c> (an invalid all-zero quaternion) means "unknown" — consumers fall
    /// back to no offset (dev-tools path).
    /// </remarks>
    public record AAFireEvent(float3 AAPosition, float3 ThreatPosition, AAType AAType, int AAEntityIndex, int ThreatIndex = 0, int ThreatVersion = 0, bool IsBallistic = false, quaternion AARotation = default) : IGameEvent;
    ...
    public enum ShadowNarrativeEventType
    {
        ...
        WalletConfiscated,
        ProcurementFailed,
        // Contract-offer funnel (offer_generated → offer_declined / offer_expired /
        // ContractSigned) — telemetry needs the non-terminal ends to measure offer
        // frequency and conversion, not just the signed tail.
        OfferGenerated,
        OfferDeclined,
        OfferExpired
    }
