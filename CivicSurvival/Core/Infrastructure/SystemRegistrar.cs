...
using CivicSurvival.Core.Systems.Placement;
...
namespace CivicSurvival.Services.Bootstrap
{
    ...
    public static class SystemRegistrar
    {
        ...
        private static void RegisterCoreSystems(UpdateSystem updateSystem)
        {
            ...
            // Install-folder deletion watch (Paradox SDK swaps the instance folder on a version
            // update with no regard for the running game). ModificationEnd for the same reason as
            // CivicPrefabInitSystem above: the SDK deletes folders regardless of simulation state,
            // so the watch must keep ticking through pause (Axiom 14). One Directory.Exists per
            // 30s — no per-frame cost.
            updateSystem.RegisterAt<ModInstanceWatchSystem>(SystemUpdatePhase.ModificationEnd);
            ...
            // NOTE: DistrictPenaltySystem moved to WellbeingFeature.
            //       WellbeingResolverSystem moved to WellbeingFeature.
            //       GeneratorEfficiency*Systems moved to EfficiencyFeature.
            ...
            // Root of the corruption→spotter→psy→cognitive marker chain. Registered as a plain
            // anchor-less system (same as PowerDataReadyMarker / WorldShockReadyMarker) so it always
            // lives in the group regardless of the Corruption feature's gate — every RegisterAfter that
            // anchors on it (CorruptionStateReadyMarker and, transitively, the whole cognitive/psy
            // cluster) only schedules if this marker is itself registered; without it the anchor is
            // silently ignored and the dependent systems drop out of the update loop entirely. The
            // MaintenanceContractSystem→marker order is expressed from CorruptionDomain (Domain→Core),
            // so no cross-domain reference is needed here.
            updateSystem.RegisterAt<MaintenanceContractReadyMarker>(SystemUpdatePhase.GameSimulation);
            ...
            // Save crash-heartbeat: flips the lifecycle phase to Saving at the top of the
            // Serialize phase (before vanilla starts the heavy scans), so a watchdog ANR
            // during a long synchronous save is classified as a save freeze. Formerly a
            // side duty of the retired VanillaVfxSerializationBoundarySystem.
            updateSystem.RegisterBefore<SaveHeartbeatSystem,
            ...
            // Runs in PostSimulation so its managed orphan sweeps no longer extend
            ...
            updateSystem.RegisterBefore<ModEntityCleanupSystem, global::CivicSurvival.Core.Systems.PsyOrphanScanSystem>(SystemUpdatePhase.PostSimulation);

            // Psy orphan scan — split out of ModEntityCleanupSystem so its same-frame job
            // fence waits only on PsyHouseholdLink/Deleted/Destroyed writers, not the combat
            // sidecar chains (SP:MECS.PsyComplete, PERF.log 2026-07-21). Chained INSIDE the
            // same safe ECB window: transitively after AllowBarrier<ModCleanupBarrier> (via
            // the request chain -> MECS), before FrameMutationDedupClearSystem ->
            // ModCleanupBarrier playback — CreateCommandBuffer throws outside that window.
            updateSystem.RegisterBefore<global::CivicSurvival.Core.Systems.PsyOrphanScanSystem, global::CivicSurvival.Core.Systems.Scheduling.FrameMutationDedupClearSystem>(SystemUpdatePhase.PostSimulation);
            ...
            // Pre-publish update announce poll (server /config/announce → countdown badge +
            // ModUpdatePending modal). UIUpdate: pause-safe (Axiom 14) and UI-facing.
            updateSystem.RegisterAt<Services.RemoteConfig.UpdateAnnounceSystem>(SystemUpdatePhase.UIUpdate);
        ...
        private static void RegisterSharedInfrastructureSystems(UpdateSystem updateSystem)
        {
            ...
            // Generic building-placement pipeline (pause-safe, Axiom 14). Player places a mod
            // building through IBuildingPlacementService; each kind (AA today; Propaganda Center,
            // gas station later) registers an IBuildingPlacementHandler that supplies the
            // building-specific work. Runs in ModificationEnd (ticks in pause) so placement works
            // while GameSimulation is paused. Ordering mirrors the former AA pipeline:
            // command -> detector -> payment -> commit -> lifecycle.
            updateSystem.RegisterBefore<BuildingPlacementCommandSystem, BuildingPlacementDetectorSystem>(SystemUpdatePhase.ModificationEnd);
            updateSystem.RegisterAt<BuildingPlacementDetectorSystem>(SystemUpdatePhase.ModificationEnd);
            updateSystem.RegisterAfter<BuildingPlacementPaymentSystem, BuildingPlacementDetectorSystem>(SystemUpdatePhase.ModificationEnd);
            updateSystem.RegisterAfter<BuildingPlacementCommitSystem, BuildingPlacementPaymentSystem>(SystemUpdatePhase.ModificationEnd);
            updateSystem.RegisterAfter<BuildingPlacementLifecycleSystem, BuildingPlacementCommitSystem>(SystemUpdatePhase.ModificationEnd);
