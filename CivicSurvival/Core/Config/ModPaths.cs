...
namespace CivicSurvival.Core.Config
{
    ...
    public static class ModPaths
    {
        ...
        // Paradox SDK log (LogManager "PdxSdk", L0_Info): the only on-disk record of the SDK's
        // playset sync / download-queue / removal decisions. Read-only — attached to the
        // load-failure report so a prefab-absent player ships the backend's playset answer.
        public const string PdxSdkLogFile = "PdxSdk.log";
        ...
        /// cache where that path doesn't exist and asset-backed I/O (audio; icons are
        /// inlined in the UI bundle) would resolve to nothing. Throws if either argument is empty.
        /// </summary>
