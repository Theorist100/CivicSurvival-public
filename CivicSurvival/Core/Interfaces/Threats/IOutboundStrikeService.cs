...
namespace CivicSurvival.Core.Interfaces.Threats
{
    ...
    public interface IOutboundStrikeService
    {
        ...
        /// (<paramref name="axis"/>, <paramref name="damage"/>, <paramref name="seed"/>,
        /// <paramref name="targetId"/>) resolved at arrival. <paramref name="seed"/> is the
        /// launch-frozen intercept-roll seed, drawn deterministically by the caller (not a runtime
        /// <c>Random</c>); it is recorded on the projectile's payload so the arrival verdict is
        /// reproducible after load and on a server. <paramref name="targetId"/> is the mirror-city
        /// target the strike aims at (<c>EnemyTarget.NoTargetId</c> = resolve automatically at
        /// arrival); it rides the payload so the player's War Room pick actually lands.
        ...
        /// </summary>
        [NullReturn(false)]
        bool Launch(ArsenalKind kind, AttackCategory axis, float damage, uint seed, ushort targetId);
    }
}
