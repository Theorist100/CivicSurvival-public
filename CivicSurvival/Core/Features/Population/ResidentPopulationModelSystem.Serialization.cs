...
namespace CivicSurvival.Core.Features.Population
{
    ...
    ///
    /// Persists exactly one thing: the pending-day cursor of the exodus catch-up, through
    /// the canonical SerializationGuard block. It is the only piece of this system's state
    /// that cannot be recomputed — everything else it publishes (the eligible-household
    /// selection, the per-household live counts and their sum) is derived from the live
    /// entity world and is rebuilt after load. The population counts this block used to
    /// carry were the mod's second measure of city size; they are gone, and old saves that
    /// still contain those fields are read past by the keyed codec's unknown-field skip.
    ///
    /// CS2 reuses system instances across in-game load, so every [NonSerialized] transient
    /// latch is reset unconditionally on the load path — otherwise a stale
    /// m_HasScheduledResult / dependency handle from the previous city would drive
    /// PublishCompletedScheduledResult into Complete() on a dead handle.
    /// </summary>
    public sealed partial class ResidentPopulationModelSystem : IDefaultSerializable, IResettable, IBootDefaultsReset
    {
        ...
        private void ResetTransientLoadState()
        {
            ...
            for (int i = 0; i < m_SelectionSlots.Length; i++)
            {
                ...
                // The stamped alive-citizen sum belongs to the pair being cleared below;
                // dropping it here keeps the previous city's total from surviving into a
                // slot the next city re-borrows.
                m_SelectionSlots[i].AliveCitizens = 0;
            ...
            // city's state; the post-load rebuild republishes with a version bump.
#pragma warning disable CIVIC458 // Boot/empty reset of derived snapshots; republished by the post-load rebuild, not here.
            m_HouseholdSnapshot = default;
            m_PreviousHouseholdSnapshot = default;
#pragma warning restore CIVIC458

            // Producer version cursor returns to the boot baseline so the first publish
            // after load bumps it and observers see a change.
        ...
        public void Serialize<TWriter>(TWriter writer) where TWriter : IWriter
        {
            ...
            try
            {
                // Always write the block regardless of readiness — there is no
                // empty-Serialize (the known save-crash class). A not-ready model simply
                // serializes its current (possibly zero) pending-day cursor.
                var state = new ResidentPopulationPersistState(m_PendingDayChanges);
        ...
        public void Deserialize<TReader>(TReader reader) where TReader : IReader
        {
            ...
            try
            {
                ...
                // Readiness stays NotReady: nothing restorable is published here any more.
                // The selection — the only thing this system publishes — is rebuilt by
                // ValidateAfterLoad and latches SelectionReady when its job completes.
                // Consumers gate on that and retry, so the gap is a skipped tick, not a
                // decision on cold data.
                Log.Info($"Deserialized v{version}: pendingDays={m_PendingDayChanges}");
            }
            catch (Exception ex)
            {
