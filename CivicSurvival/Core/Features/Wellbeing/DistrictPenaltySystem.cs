...
#pragma warning restore CIVIC148
        {
            ...
            { PenaltySource.VIPVisible,       (0.15f, 0.00f) },  // -15% happiness
    ...
    public partial class DistrictPenaltySystem : ThrottledSystemBase
    {
        ...
        public float MaxHappinessPenalty
        {
            get
            {
                ...
                if (IsAirDefenseDrilled) globalPenalty += AIR_DEFENSE_DRILL_BONUS;
        ...
        public float GlobalHappinessPenaltyTotal
        {
            get
            {
                ...
                // Negative on purpose (the drill bonus) — clamped at the top only, so the bonus
                // survives into WellbeingResolverSystem instead of being floored away.
                if (IsAirDefenseDrilled) total += AIR_DEFENSE_DRILL_BONUS;
        ...
        // Air-defense drill (the activists' call was answered before the first strike — the one
        // global entry that is a BONUS, i.e. a negative penalty, the FoodAidProvided convention).
        // Ends at the second wave rather than on a clock: it is the reward for THAT strike.
        // Serialized, so a save between the intro and wave two keeps the bonus it earned.
        public bool IsAirDefenseDrilled { get; private set; }
        private const float AIR_DEFENSE_DRILL_BONUS = -0.10f;       // +10% happiness
        private const int AIR_DEFENSE_DRILL_EXPIRY_WAVE = 2;
        ...
        protected override void OnCreate()
        {
            ...
            SubscribeRequired<AirDefenseDrillRewardEvent>(OnAirDefenseDrillReward);
        ...
        protected override void OnDestroy()
        {
            if (ServiceRegistry.IsInitialized)
            {
                ...
                UnsubscribeSafe<AirDefenseDrillRewardEvent>(OnAirDefenseDrillReward);
        ...
        /// <summary>
        /// The activists' call was answered — the city went into the first strike with guns up.
        /// Grants the city-wide morale bonus until the second wave (see OnWaveStarting).
        /// </summary>
        private void OnAirDefenseDrillReward(AirDefenseDrillRewardEvent evt)
        {
            if (IsAirDefenseDrilled) return;
            IsAirDefenseDrilled = true;
            Log.Info($"Air-defense drill bonus granted: happiness +{-AIR_DEFENSE_DRILL_BONUS:P0} until wave {AIR_DEFENSE_DRILL_EXPIRY_WAVE}");
        }

        /// <summary>
        /// Expiry leg of the drill bonus: it covers the opening strike it was earned for, and
        /// the second wave ends it. Keyed off the wave NUMBER, not a clock, so a paused or
        /// slow-played city gets exactly the same reward window as a fast one.
        /// </summary>
        private void OnWaveStarting(WaveStartingEvent evt)
        {
            if (!IsAirDefenseDrilled || evt.WaveNumber < AIR_DEFENSE_DRILL_EXPIRY_WAVE) return;
            IsAirDefenseDrilled = false;
            Log.Info($"Air-defense drill bonus ended (wave #{evt.WaveNumber})");
        }
