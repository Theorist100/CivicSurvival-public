...
namespace CivicSurvival.Core.Systems
{
    ...
    public partial class CrisisSweepSystem : CivicSystemBase, IPostLoadValidation
    {
        ...
        // The single owner of "how many people live here". The sweep already receives the PEAK from
        // the scenario; this supplies the CURRENT size, which is what the severity timeline must
        // recompute its pool from — a timeline running off the peak keeps crewing guns from people
        ...
        protected override void OnUpdateImpl()
        {
                    ...
                    .WithEntityAccess())
            {
                ...
                {
                    // Diagnostic input, so the value is taken for what it is: the sweep reports
                    // what the city recorded, and refusing a peak from the superseded counter
                    // would only replace a real number with the 1000 placeholder above.
                    populationPeak = math.max(scenario.PopulationPeak.Value, 1);
...
#pragma warning restore CIVIC017
        ...
        /// start / pre-war) OR when its numbers are frozen behind the population gate — either
        /// way the manpower inputs stay on their archetype-derived pool.
        ///
        /// This is a SWITCH wearing the shape of a read, not a display: presence of the singleton
        /// was never the right condition for it, because the singleton exists from the first
        /// frame. In the unmeasured window the old test returned success with a frozen (at boot,
        /// all-zero) pool, and the forecast quietly ran off zeros instead of the archetype preset
        /// it was written to fall back to.</summary>
        ...
        /// the severity verdict systematically pessimistic on any city with a strike, a build or an import.
        /// NOTE: this mirrors the PUBLISHED (full-nameplate) aggregate only. The resolver-internal
        /// saturation-target aggregate uses the damage-RESIDUAL nameplate by design — do not "align" this
        /// filter to it (see the NameplateKW docstring on the intentional full/residual split).</summary>
        private static bool CountsTowardNameplate(in PowerCapacityPlantSnapshot p)
        {
            ...
            // (snapshot-side IsKnockedOut mirrors PowerCapacityMath.IsKnockedOut).
            return p.Channel == CapacityChannel.GridProducer
        ...
        private bool TryGetLiveFleet(out AirDefenseForecast.FleetComposition fleet)
        {
            ...
            fleet = new AirDefenseForecast.FleetComposition(
                view.HeritageCount, view.BoforsCount, view.GepardCount, view.PatriotCount, view.HawkCount);
            return true;
        }
...
#pragma warning restore CIVIC218
        ...
        private LiveInputs GatherLiveInputs()
        {
            ...
            // POPULATION (one-shot read of the owner). A refusal means there is no city entity, and
            // the runner then falls back to the peak exactly as it did before this axis existed.
            bool populationLive = m_CityPopulationReader.TryGetResidentCount(out int livePopulation);
            ...
            return new LiveInputs(
                ...
                populationLive, populationLive ? livePopulation : 0);
        }
    }
}
