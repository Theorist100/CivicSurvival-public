...
namespace CivicSurvival.Core.Systems
{
    ...
    public partial class DiagnosticReportSystem : CivicSystemBase
    {
        ...
        // Burning-vehicle probe. A vehicle carrying OnFire is normal ONLY while it is stopped
        // at an accident site (AccidentVehicleSystem.IgniteFire); a DRIVING vehicle with OnFire
        // is a broken lifecycle (accident state lost while the fire survived — e.g. across a
        // load) that nothing in vanilla can ever stop, extinguish or damage. The probe logs
        // every burning vehicle with its position and accident/moving flags at Info, so a
        // player log proves or rules out the state without anyone hunting the vehicle by eye
        // (consecutive positions in the series also prove it is actually driving).
        private EntityQuery m_BurningVehicleQuery;
        private ComponentLookup<InvolvedInAccident> m_InvolvedInAccidentLookup;
        private ComponentLookup<Game.Objects.Moving> m_MovingLookup;
        ...
        protected override void OnCreate()
        {
            ...
            m_BurningVehicleQuery = GetEntityQuery(
                ComponentType.ReadOnly<OnFire>(),
                ComponentType.ReadOnly<Game.Vehicles.Vehicle>(),
                ComponentType.Exclude<Game.Common.Deleted>());
            m_InvolvedInAccidentLookup = GetComponentLookup<InvolvedInAccident>(true);
            m_MovingLookup = GetComponentLookup<Game.Objects.Moving>(true);
        ...
        protected override void OnUpdateImpl()
        {
            ...
            // Info-level (not Debug): the state it hunts is a player-visible anomaly and must be
            // diagnosable from a production log. No-ops (one IsEmptyIgnoreFilter check) while the
            // city has no burning vehicle, so it adds no steady-state log noise.
            LogBurningVehicleProbe();
        ...
        /// <summary>
        /// Logs every vehicle currently carrying <c>OnFire</c> — entity, world position and the
        /// accident/moving flags that separate the legal state (stopped at an accident site,
        /// waiting for a fire engine) from the broken one (driving with a fire nothing will ever
        /// extinguish). Emitted on the system's own ~17s cadence while any burning vehicle
        /// exists, so the position series doubles as a track of the vehicle's route.
        /// </summary>
        private void LogBurningVehicleProbe()
        {
            // Cheap metadata gate: the query is empty in every healthy city, so the
            // Transform-reading SystemAPI.Query below (which completes the Transform write
            // chain) only ever runs while a burning vehicle actually exists.
            if (m_BurningVehicleQuery.IsEmptyIgnoreFilter)
                return;

            m_InvolvedInAccidentLookup.Update(this);
            m_MovingLookup.Update(this);

            // CIVIC050/CIVIC051: rare-event diagnostic — gated above on a non-empty query,
            // so the string build and per-entity lookups never run on the steady-state path.
#pragma warning disable CIVIC050, CIVIC051
            var sb = new System.Text.StringBuilder();
            int count = 0;
            foreach (var (fire, transform, entity) in
                SystemAPI.Query<RefRO<OnFire>, RefRO<Game.Objects.Transform>>()
                    .WithAll<Game.Vehicles.Vehicle>()
                    .WithNone<Game.Common.Deleted>()
                    .WithEntityAccess())
            {
                if (count > 0) sb.Append("; ");
                sb.Append(entity.Index).Append(':').Append(entity.Version);
                sb.Append($" pos=({transform.ValueRO.m_Position.x:F0},{transform.ValueRO.m_Position.z:F0})");
                sb.Append(" acc=").Append(m_InvolvedInAccidentLookup.HasComponent(entity) ? 1 : 0);
                sb.Append(" moving=").Append(m_MovingLookup.HasComponent(entity) ? 1 : 0);
                sb.Append($" intensity={fire.ValueRO.m_Intensity:F1}");
                count++;
            }
            Log.Info($"[BURNING-VEHICLE] count={count}: {sb}");
#pragma warning restore CIVIC050, CIVIC051
        }
