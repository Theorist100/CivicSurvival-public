...
namespace CivicSurvival.Services.UI
{
    ...
    public partial class SettingsUISystem : CivicUIPanelSystem
    {
        ...
        // Whether the UI currently shows the crash-dump list. Set by the UI through
        // SetCrashDumpScan and read for one purpose only: skipping the dump-directory stat
        // on the 500ms tick while nobody is looking at it.
        //
        // Its predecessor, IsSettingsExpanded, meant "the settings window is open" and was
        // introduced when another system read it to decide how much district detail to put
        // in its DTO. That reader is long gone; what remained was the mod's only piece of UI
        // open/closed state living outside React, and it is why the settings window could
        // come back on its own over the main menu. Settings are dashboard views now — this
        // field no longer decides whether anything is visible.
        private bool m_ScanCrashDumps;
        ...
        {
            Triggers.Add<bool>(SetCrashDumpScan, FeatureIds.Settings, OnSetCrashDumpScan);
            ...
            Triggers.Add(SendLoadFailureReport, FeatureIds.Settings, OnSendLoadFailureReport);
...
#pragma warning disable S1075 // Hardcoded URI by design — the whitelist must live in code, not config
            ...
            {
                LINK_DISCORD => "https://discord.gg/nhytdnKFeW",
...
#pragma warning restore S1075
        ...
        private void EmitSettingsState()
        {
            ...
            var dto = new SettingsDto
            {
                ...
                // Crash-dump list is a directory stat; only scan while the REPORT view is on
                // screen so no other tab pays for it on the 500ms tick.
                CrashDumpsJson = m_ScanCrashDumps ? ErrorReportService.GetCrashDumpListJson() : "[]",
        ...
        private void OnSetCrashDumpScan(bool scanning)
        {
            if (m_ScanCrashDumps == scanning) return;
            m_ScanCrashDumps = scanning;
            Log.Info($"Crash-dump scan: {scanning}");
        }
        ...
        private void OnSendReport()
        {
            ...
            SetReportStatus(MapReportResultToStatusKey(ErrorReportService.SubmitManualReport(), "manual report"));
        }

        // Load-failure variant (ModLoadFailure modal): the standard report plus the playset/SDK
        // evidence (Modding.log head, PdxSdk.log sync/removal lines) a prefab-absent case needs.
        private void OnSendLoadFailureReport()
        {
            if (!EnsureSettings()) return;
            if (!m_Settings.TelemetryEnabled)
            {
                SetReportStatus(ReasonIds.SettingsStatusTelemetryDisabled);
                Log.Warn("Cannot send load-failure report: telemetry is disabled");
                return;
            }

            SetReportStatus(ReasonIds.SettingsStatusSending);
            SetReportStatus(MapReportResultToStatusKey(ErrorReportService.SubmitLoadFailureReport(), "load-failure report"));
        }
        ...
        private void OnSendModLog()
        {
            ...
            SetReportStatus(ReasonIds.SettingsStatusSending);
            SetReportStatus(MapReportResultToStatusKey(ErrorReportService.SubmitModList(), "mod list"));
        }
        ...
        private void OnSendCrashDumps(string selectedCsv)
        {
            ...
            SetReportStatus(MapReportResultToStatusKey(ErrorReportService.SubmitCrashDumps(names), "crash dump"));
        }

        /// <summary>
        /// One shared submit-result → status-key mapping for all four report channels (manual /
        /// load-failure / mod list / crash dump). NoDump only ever comes back from the dump channel.
        /// </summary>
        private string MapReportResultToStatusKey(ErrorReportService.ManualReportResult result, string context)
        {
            switch (result)
            {
                case ErrorReportService.ManualReportResult.Sent:
                    return ReasonIds.SettingsStatusReportSent;
                case ErrorReportService.ManualReportResult.NoDump:
                    return ReasonIds.SettingsStatusDumpNone;
                case ErrorReportService.ManualReportResult.TelemetryDisabled:
                    return ReasonIds.SettingsStatusTelemetryDisabled;
                case ErrorReportService.ManualReportResult.TelemetryUnavailable:
                    return ReasonIds.SettingsStatusReportUnavailable;
                case ErrorReportService.ManualReportResult.RateLimited:
                    return ReasonIds.SettingsStatusReportRateLimited;
                case ErrorReportService.ManualReportResult.Failed:
                case ErrorReportService.ManualReportResult.Unknown:
                    return ReasonIds.SettingsStatusReportFailed;
                default:
                    Log.Warn($"Unknown {context} result: {result}");
                    return ReasonIds.SettingsStatusReportFailed;
            }
        }
