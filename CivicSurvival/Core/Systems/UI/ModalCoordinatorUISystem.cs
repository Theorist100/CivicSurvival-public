...
#pragma warning disable CIVIC098 // ModalCoordinator.Instance is static readonly = new(), never null
        ...
        protected override void OnPanelUpdate()
        {
            ...
            // Drives the gap between modals (see ModalCoordinator.SHOW_GAP_SECONDS): this is
            // the UI phase, so it ticks on real time and keeps running while the game is
            // paused — a queued modal must not wait for the simulation to resume.
            coordinator.Tick(UnityEngine.Time.unscaledDeltaTime);
