...
namespace CivicSurvival.Core.Systems.Effects
{
    ...
    ///
    /// Both entities are runtime-only and carry vanilla <c>Game.Effects.EffectInstance</c> —
    /// the engine's own "runtime effect entity, never serialized" designation: both
    /// serialization scanners exclude it (<c>SerializerSystem.m_Query</c> None-set and
    /// <c>PrimaryPrefabReferencesSystem.m_PrefabRefQuery</c>), so the anchor pair stays out
    /// of every save snapshot WITHOUT being torn down on the save path. Tearing it down
    /// per save (the old Deleted-tag boundary) poisoned the VFX instance↔index mapping:
    /// its Remove messages were consumed one frame later against an already-compacted
    /// EnabledEffectData list (VFXSystem.cs:215 resolves the graph by the record CURRENTLY
    /// at the index), leaving orphaned fire instances that then rode whatever record
    /// reused the index — the "burning driving van" artifact. No vanilla system sweeps
    /// EffectInstance entities mid-session (audio/culling consumers look up their own
    /// ambience entities only); ClearSystem destroys them at the load boundary, where the
    /// anchor is rebuilt anyway.
    ///
    /// They must not carry Game.Tools.Temp: ToolClearSystem routinely deletes Temp
    /// entities while the default tool clears ordinary cursor state.
    /// </summary>
...
#pragma warning disable CIVIC051, CIVIC134
            ...
            em.AddComponent<EffectInstance>(m_Dummy);
            ...
            em.AddComponent<EffectInstance>(m_Anchor);
