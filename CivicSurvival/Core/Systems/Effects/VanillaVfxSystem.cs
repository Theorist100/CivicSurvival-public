    ...
    public partial class VanillaVfxSystem : CivicSystemBase
    {
        ...
        public int AttachEffectBatch(NativeList<VfxAttachRequest> requests)
        {
            ...
            for (int r = 0; r < requests.Length; r++)
            {
                ...
                {
                    WarnDroppedRequest(owner, effectPrefab, effectIndex, "request carries a Null effect prefab");
                    continue;
                }
...
#pragma warning disable CIVIC051
                ...
                {
                    WarnDroppedRequest(owner, effectPrefab, effectIndex, "owner has no PrefabRef/EnabledEffect buffer");
                    continue;
                }
                ...
                {
                    WarnDroppedRequest(owner, effectPrefab, effectIndex,
                        $"owner prefab {ownerPrefab.Index} has no Effect buffer (exhaust never bound?)");
                    continue;
                }
                ...
                {
                    WarnDroppedRequest(owner, effectPrefab, effectIndex,
                        $"effect index out of range (prefab {ownerPrefab.Index} has {prefabEffects.Length} element(s))");
                    continue;
                }
                if (prefabEffects[effectIndex].m_Effect != effectPrefab)
                {
                    WarnDroppedRequest(owner, effectPrefab, effectIndex,
                        $"element mismatch: prefab {ownerPrefab.Index} element [{effectIndex}]={prefabEffects[effectIndex].m_Effect.Index}, requested {effectPrefab.Index} — the record would be removed as WrongPrefab");
                    continue;
                }
...
#pragma warning restore CIVIC051
        ...
        // One Warn per unique (owner prefab is folded into the reason string) drop cause — a dropped
        // attach request is a wiring bug (element/record mismatch, missing binding), and dropping it
        // SILENTLY is how the Hawk flew without an exhaust for weeks: the effect simply never appeared
        // and nothing pointed at the cause. Deduplicated so a per-cycle re-attach controller (every 3
        // render frames per missile) cannot flood the log; per-owner detail stays in the first hit.
        private readonly System.Collections.Generic.HashSet<string> m_DropWarned = new();

        private void WarnDroppedRequest(Entity owner, Entity effectPrefab, int effectIndex, string reason)
        {
            string key = $"{effectPrefab.Index}:{effectIndex}:{reason}";
            if (!m_DropWarned.Add(key))
                return;
            Log.Warn($"[VFX:ATTACH] request dropped (first occurrence, further identical drops muted): " +
                     $"owner={owner.Index}:{owner.Version} effectPrefab={effectPrefab.Index} effectIndex={effectIndex} — {reason}");
        }
        ...
        protected override void OnGamePreload(Purpose purpose, GameMode mode)
        {
            ...
            // Load boundary only. Saves need no anchor work at all: the anchor and dummy
            // carry vanilla EffectInstance from creation, which both serialization scanners
            // exclude (SerializerSystem None-set, PrimaryPrefabReferencesSystem PrefabRef
            // query) — the pair never enters a save snapshot and never gets torn down.
            // The old save-side Deleted-tag boundary is gone: its Remove messages were
            // consumed against an already-compacted EnabledEffectData list and orphaned
            // VFX instances (see VanillaEffectAnchor class doc).
...
#pragma warning restore CIVIC051
        ...
        ///
        /// Defensive path only: saves no longer touch the anchor (EffectInstance keeps
        /// it out of serialization — see VanillaEffectAnchor), so this fires only if
        /// something external destroys the runtime anchor mid-session.
        ///
        /// m_NextEffectId is NOT reset here. Effect ids identify entries in
        /// EnabledEffectData; reusing id=0 after an anchor rebuild collides with any
        /// still-referenced entries from the previous anchor era. The id sequence is
        ...
        private void ResetTransientState(string logMessage, bool invalidateEffectCache, bool clearPendingRequests)
        {
            ...
            m_Diag.Reset();
            // Re-arm the drop diagnostics for the next city: entity indices recycle across loads, so
            // the muted keys would both leak memory and mute a NEW city's first genuine drop.
            m_DropWarned.Clear();
            m_NextEffectId = 0;
            m_InitFailureFrames = 0;
