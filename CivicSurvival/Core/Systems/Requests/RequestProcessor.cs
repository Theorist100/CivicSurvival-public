...
namespace CivicSurvival.Core.Systems.Requests
{
    ...
    public abstract partial class EmittingRequestProcessor<TRequest> : CivicSystemBase
        where TRequest : unmanaged, IComponentData
    {
        private EntityCommandBufferSystem m_Barrier = null!;
        ...
        /// <summary>
        /// Pause-safe processors registered in <c>SystemUpdatePhase.ModificationEnd</c>
        /// (AXIOM 14: it ticks while paused, GameSimulation does not) override this to
        /// true so result/destroy commands play back on the vanilla
        /// <c>ModificationEndBarrier</c> — the <c>GameSimulationEndBarrier</c> window is
        /// closed outside GameSimulation and would throw on CreateCommandBuffer.
        /// Keep the override and the registration phase in lockstep.
        /// </summary>
        protected virtual bool RunsInModificationEnd => false;

        protected sealed override void OnCreate()
        {
            base.OnCreate();

            m_Barrier = RunsInModificationEnd
                ? World.GetOrCreateSystemManaged<ModificationEndBarrier>()
                : World.GetOrCreateSystemManaged<GameSimulationEndBarrier>();
        ...
        [CompletesDependency("RequestProcessor.OnUpdateImpl: request entities are transient and processed in small batches; ToEntityArray materialises the batch for sequential validation with sub-tick-safe ECB playback via the phase-matched barrier (GameSimulationEndBarrier, or ModificationEndBarrier for pause-safe processors)")]
