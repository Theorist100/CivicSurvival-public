...
#pragma warning disable CIVIC150 // Transient by design: reset + re-derived in OnInitialize on every load; persisting it would wrongly skip the bind on a fresh prefab entity
        ...
        // One-shot (per load) Error latch for the revoked-entity verdict (LogRevokedCoreEntities):
        // core refs alive but their PrefabSystem entities gone — the mid-process playset
        // deactivation bucket that FinalizeMissing structurally cannot see (m_ResolvePending
        // clears on the surviving refs alone, so no absent verdict ever fires on later loads).
        private bool m_RevokedVerdictLogged;
#pragma warning restore CIVIC150

        // Follow-up watch on a revoked verdict: does it undo itself, and how fast. The verdict is
        // taken in the first second after a city load (m_InitClock restarts there), which is exactly
        // when the active mod set can still be empty — prod has both a case that recovered inside 37s
        // and cases that held for 24-34 minutes, indistinguishable at verdict time. This measures
        // which one happened instead of guessing, and it runs on the dispatcher rather than on this
        // system because the system disables itself the moment the verdict is written.
        private const double REVOKED_RECHECK_INTERVAL_SECONDS = 5.0;
        // Long enough to clear the observed recovery (37s) with room to spare, short enough that the
        // answer still ships for a player who quits soon after. Past it the verdict is confirmed as
        // standing — the cases that held did so for tens of minutes, so nothing is gained by waiting.
        private const double REVOKED_OBSERVE_WINDOW_SECONDS = 120.0;
        private readonly Stopwatch m_RevokedWatchClock = new();
        private Guid m_RevokedWatchUpdater;
        private double m_RevokedNextCheckSeconds;
...
#pragma warning restore CIVIC150
        ...
        protected override void OnDestroy()
        {
            ...
            // Same argument for the revoked follow-up: the dispatcher outlives this system, and an
            // updater still holding a destroyed system would survive an assembly unload.
            if (m_RevokedWatchUpdater != Guid.Empty)
            {
                MainThreadDispatcher.UnregisterUpdater(m_RevokedWatchUpdater);
                m_RevokedWatchUpdater = Guid.Empty;
            }
            base.OnDestroy();
        }
...
#pragma warning restore CIVIC006
        ...
        // Same as SetupDroneLauncher, for the ballistic emplacement (HIMARS.cok): mark the prefab
        // with BallisticLauncherPrefabData so the generic placement pipeline recognises it
        // (BallisticLauncherPlacementHandler). Field emplacement — marker only, no road snap.
#pragma warning disable CIVIC006 // One-time derived prefab marker during bootstrap (single shared prefab entity, HasComponent-guarded idempotent)
        private void SetupBallisticLauncher(PrefabBase prefab)
        {
            if (!m_PrefabSystem.TryGetEntity(prefab, out var entity))
            {
                Log.Warn($"PrefabSystem.TryGetEntity failed for {prefab.name}");
            ...
            if (EntityManager.HasComponent<BallisticLauncherPrefabData>(entity))
                return; // idempotent: marker already present

            EntityManager.AddComponentData(entity, new BallisticLauncherPrefabData());
            Log.Info($"Marked prefab: {prefab.name} as Ballistic Launcher (field emplacement)");
        }
#pragma warning restore CIVIC006
...
#pragma warning restore CIVIC006
        ...
        // Verdict for the bucket FinalizeMissing structurally cannot see: a core prefab whose
        // managed ref survived (OnInitialize keeps refs across loads by design, and
        // m_ResolvePending clears on the refs alone) while its ENTITY is gone from PrefabSystem —
        // the mid-process playset deactivation footprint. Live case 2026-07-21 (session 2c09db94,
        // 0.3.22): instance folder deleted 1h into the session, ThreatSpawn held the next wave,
        // and three later city loads produced ZERO absent verdicts — the surviving refs masked
        // every one. Emits one Error per load (reset in OnInitialize) on the settled point, riding
        // the same probe suite as FinalizeMissing so prod can correlate it with the α timeline.
        private void LogRevokedCoreEntities()
        {
            if (m_RevokedVerdictLogged)
                return;

            // Getter access runs the lazy self-heal first (ResolveCachedEntity re-fetches from the
            // surviving ref), so a merely-stale entity recovers and never reaches this verdict —
            // only a genuinely revoked one (TryGetEntity has no entry anymore) flags true.
            bool droneRevoked = m_AttackDronePrefab != null && AttackDroneEntity == Entity.Null;
            bool rocketRevoked = m_RocketPrefab != null && RocketEntity == Entity.Null;
            bool interceptorRevoked = m_InterceptorPrefab != null && InterceptorEntity == Entity.Null;
            if (!droneRevoked && !rocketRevoked && !interceptorRevoked)
                return;

            m_RevokedVerdictLogged = true;
            // Arm the backend playset-listing probe here too, not only in FinalizeMissing: this bucket
            // never reaches that method (the surviving refs clear m_ResolvePending, so no absent verdict
            // ever fires), and it is precisely the mid-process deactivation the server listing is meant
            // to explain. One-shot — whichever verdict comes first issues the single request.
            ModInstallResolver.StartPlaysetListingProbe();
            Log.Error($"Mod install is inconsistent — state={ModStateClassifier.StateRevokedAfterResolve}. " +
                      "Core prefab entity revoked after resolve: managed refs survive but PrefabSystem " +
                      "no longer holds the entity (mid-process playset deactivation); threat spawning is " +
                      $"held while this state lasts. revoked[drone={droneRevoked}, rocket={rocketRevoked}, interceptor={interceptorRevoked}], " +
                      $"activeSet=[{ModInstallResolver.DescribeActiveSetSafe()}], activeSetBoot=[{ModInstallResolver.BootActiveSetSafe()}], " +
                      $"sdkRow=[{ModInstallResolver.DescribeSdkRowSafe()}], playsetListing=[{ModInstallResolver.PlaysetListingSafe()}], " +
                      $"installedSet=[{ModInstallResolver.InstalledSetSafe()}], syncOngoing={m_ParadoxNativeLoader?.ModSyncOngoing == true}, " +
                      $"installDir={SanitizedInstallDirTail()}, liveDir={ModPaths.SanitizePathTail(ModInstallResolver.LiveInstallDirectoryOrBoot())}, " +
                      $"uiAlive=[{UiBundleSignal.Describe()}], uiModule=[{ModInstallResolver.DescribeUiModuleSafe()}], elapsedMs={m_InitClock.ElapsedMilliseconds}. " +
                      "A follow-up line reports whether this undid itself, and how fast.");

            ArmRevokedRecheck();
        }

        /// <summary>
        /// Start watching a written revoked verdict to see whether it stands. Runs on the main-thread
        /// dispatcher because this system disables its own tick the moment the verdict is written, and
        /// the answer takes up to two minutes to establish. Self-guarded against a double arm; the
        /// updater removes itself as soon as it has an answer, and <c>OnDestroy</c> removes it if the
        /// world goes away first.
        /// </summary>
        private void ArmRevokedRecheck()
        {
            if (m_RevokedWatchUpdater != Guid.Empty)
                return;
            m_RevokedWatchClock.Restart();
            m_RevokedNextCheckSeconds = REVOKED_RECHECK_INTERVAL_SECONDS;
            m_RevokedWatchUpdater = MainThreadDispatcher.RegisterUpdater(PumpRevokedRecheck);
        }

        /// <summary>
        /// Re-reads the three core entities on a slow cadence and writes exactly ONE line: either the
        /// verdict undid itself (with the time it took — the number this investigation has never had)
        /// or it still stands at the end of the observation window. Returns true to unregister itself
        /// in both cases.
        ///
        /// Error level for both outcomes, including the benign one: prod telemetry forwards Error
        /// only, and a recovery that never reaches the server cannot be counted — which is the whole
        /// point of measuring it. The state prefix on the recovery line is "recovered:", not
        /// "inconsistent:", so it is excluded from failure counts by the same query that finds it.
        ///
        /// Reading the entity properties runs their lazy self-heal, which is the intended behaviour
        /// here: if the entity is merely stale it is re-fetched and the verdict correctly reads as
        /// recovered. Every 5s on the main thread — the same cadence class as the folder watchdog.
        /// </summary>
        private bool PumpRevokedRecheck()
        {
            double now = m_RevokedWatchClock.Elapsed.TotalSeconds;
            if (now < m_RevokedNextCheckSeconds)
                return false;
            m_RevokedNextCheckSeconds = now + REVOKED_RECHECK_INTERVAL_SECONDS;

            bool stillRevoked = (m_AttackDronePrefab != null && AttackDroneEntity == Entity.Null)
                                || (m_RocketPrefab != null && RocketEntity == Entity.Null)
                                || (m_InterceptorPrefab != null && InterceptorEntity == Entity.Null);
            bool windowOver = now >= REVOKED_OBSERVE_WINDOW_SECONDS;
            if (stillRevoked && !windowOver)
                return false;

            m_RevokedWatchUpdater = Guid.Empty;
            string state = stillRevoked
                ? ModStateClassifier.StateRevokedAfterResolve
                : ModStateClassifier.StateRecoveredAfterRevoke;
            Log.Error($"Revoked-verdict follow-up — state={state}, afterSeconds={(long)now}, " +
                      $"activeSet=[{ModInstallResolver.DescribeActiveSetSafe()}], " +
                      $"sdkRow=[{ModInstallResolver.DescribeSdkRowSafe()}], " +
                      $"playsetListing=[{ModInstallResolver.PlaysetListingSafe()}], " +
                      $"syncOngoing={ModInstallResolver.DescribeSyncOngoingSafe()}. " +
                      (stillRevoked
                          ? "The verdict stands: the entities were still gone at the end of the window, "
                            + "so threat spawning is held for this session."
                          : "The verdict undid ITSELF — the entities were back, nothing was repaired and "
                            + "nobody acted. Read the original verdict as a reading taken before the "
                            + "active mod set had settled, not as a failure."));
            return true;
        }
...
#pragma warning restore CIVIC050
        ...
        private void FinalizeMissing()
        {
            if (m_ResolvePending)
            {
                ...
                //   sdkRow      — the SDK's OWN activation row for our id, read synchronously right here
                //     (IsIncludedInActivePlayset → two dictionary reads on the local playset store, no
                //     I/O). included=True next to activeSet's idSeen=False means vanilla's runtime set
                //     lost us while the SDK still holds the row (files-present α); included=False means
                //     the row itself is gone — the state the sync path decides removals from. Unlike
                //     installedSet's activeRow it cannot arrive late: it is read in this frame.
                string sdkRow = ModInstallResolver.DescribeSdkRowSafe();
                //   playsetListing — does the BACKEND still list us in the active playset? That listing
                //     is the input the removal decision is actually made from (remoteMods in
                //     PlaysetAndModsSyncService), and nothing we log has ever measured it. Armed
                //     one-shot HERE, at the failure verdict, so a healthy session pays nothing; the
                //     request plus its PathExists sweep run off-frame and are never awaited, so the
                //     FIRST verdict of a process prints "pending" and the later ones (three-four per
                //     failing session, one per city load) carry the answer. Read total= before trusting
                //     listed=: total=0 means the backend contributed nothing and the listing is the
                //     local cache (the full rule is on the probe in ModInstallResolver).
                ModInstallResolver.StartPlaysetListingProbe();
                string playsetListing = ModInstallResolver.PlaysetListingSafe();
                ...
                //   activeSetBoot — the active set as sampled at BOOT (first non-empty answer after
                //     Mod.OnLoad, before the first in-session sync could rewrite it). Next to the
                //     at-finalize activeSet it timestamps WHEN the activation row died: idSeen=True
                //     at boot + idSeen=False here = the row was lost at runtime (the 2026-07-21 α
                //     sessions), not stale since boot. Replaces the dllDir probe — Assembly.Location
                //     is always empty for byte-loaded CS2 mod assemblies, that field could never
                //     expose the skew it was built for (3/3 first live reports: dllDir=empty).
                string activeSetBoot = ModInstallResolver.BootActiveSetSafe();
                //   state — WHICH inconsistency this is, decided by the disk probe rather than by the
                //     symptom that brought us here. The old wording ("core mod prefab(s) absent") named
                //     the symptom in every case and was actively misleading in the common one: when the
                //     install folder has been deleted mid-session the models are not "absent", their
                //     directory is, and reading those reports as a loader failure cost a month of
                //     investigation. files-present-not-registered is the ONLY class where the prefabs
                //     genuinely failed to register. See ModStateClassifier.
                string state = ModStateClassifier.ClassifyPrefabVerdict(filesMissing);
                Log.Error($"Mod install is inconsistent — state={state}. AttackDrone/Tracer/Rocket unavailable this session. " +
                          ...
                          $"nativePhase={nativePhase}, activeSet=[{activeSet}], activeSetBoot=[{activeSetBoot}], sdkRow=[{sdkRow}], playsetListing=[{playsetListing}], installedSet=[{installedSet}], nameVariant={nameVariant}, modelState=[{modelState}], syncOngoing={syncOngoing}, patchState={patchState}, skyve={skyve}, dirListing=[{dirListing}], installDir={installDirTail}, liveDir={liveDirTail}, uiAlive=[{uiAlive}], uiModule=[{uiModule}], elapsedMs={m_InitClock.ElapsedMilliseconds}.");
