...
namespace CivicSurvival.Core.Systems.Bootstrap
{
    ...
    internal sealed class ParadoxNativeLoader
    {
        ...
        // Mod-sync gate. True while the Paradox platform is still synchronising mods (downloading /
        // xdelta-patching / integrity-verifying). A fresh subscriber's core .cok can still be
        // materialising on disk during this window: the install is atomic (VANILLA_MOD_LOADING §9),
        // but a large mod's install can COMPLETE after our 45s finalize cap, and vanilla's own
        // playset re-scan is deferred until the whole sync ends (PdxSdkPlatform:1470 → :1460). So
        // while this is set the owner (CivicPrefabInitSystem) must NOT hard-cap the core models
        // "absent" — that fired a false ModLoadFailure mid-download (the awaiting-disk prod bucket).
        // Seeded synchronously from PdxSdkPlatform.isSyncOnGoing because a sync already running
        // before we subscribed would otherwise be missed (IModSupport exposes only the event, no
        // synchronous status). volatile: the platform raises onModSyncStatusChanged off the main
        // thread; the owner reads this on the main thread. The handler only flips the flag.
        private volatile bool m_ModSyncOngoing;

        /// <summary>
        /// True while Paradox is still synchronising mods — the owner holds the "core prefab absent"
        /// hard-cap verdict while this is set (bounded by its own ceiling: a wedged sync must not
        /// suppress the verdict forever). On a healthy platform the sync ends via
        /// onModSyncStatusChanged onGoing=false and the readiness-gate disk poll picks the .cok up
        /// once the install lands.
        /// </summary>
        public bool ModSyncOngoing => m_ModSyncOngoing;
        ...
        // Readability-probe throttle (≈0.5s at 60 FPS). Opening a file pair is heavier than the
        // File.Exists poll, and an externally locked .cok (long antivirus scan) would otherwise be
        // probed every frame for the whole readiness window. The countdown only ever runs while the
        // files already exist but failed the open probe — the healthy path probes once and proceeds.
        private const int ReadProbeIntervalFrames = 30;
        private int m_ReadProbeCountdown;

        // Event-driven watcher over vanilla's active-set recalculation (two snapshots, "before" from the
        // platform edge and "after" from the asset database's own completion event). Owned here because
        // this is where the platform subscription convention already lives; armed in Activate, detached in
        // Deactivate. Diagnostics only — see ActiveSetTransitionProbe.
        private readonly ActiveSetTransitionProbe m_TransitionProbe = new();
        ...
        {
            SubscribeModSync();
            MainThreadDispatcher.RegisterUpdater(WhenReadyPostLoad);
        }

        // Track Paradox mod-sync so the owner can hold the "absent" verdict while a download/patch is
        // still in flight. Subscribe first, then seed synchronously from the concrete platform (a sync
        // may already be running when we subscribe).
        private void SubscribeModSync()
        {
            PlatformManager pm = PlatformManager.instance;
            // Subscribe BEFORE seeding: with the seed first there is a gap where the final
            // onGoing=false event fires between the property read and the subscription — the flag
            // would latch true with no further event to clear it, and the owner's hard-cap would be
            // held for the rest of the session.
            pm.onModSyncStatusChanged += OnModSyncStatusChanged;
            // Seed from the live sync status. A null PdxSdk platform means a non-Paradox launch (local
            // dev drop) or the PSI is not registered yet; a registered-but-uninitialized one means SDK
            // context creation failed (offline / backend down, PdxSdkPlatform keeps m_SDKContext null)
            // and its isSyncOnGoing getter would dereference that null context. In all these cases
            // there is no Paradox sync to wait on, so the gate stays open (false) and the hard-cap
            // behaves exactly as before. Log it rather than swallow it silently: on a healthy Paradox
            // launch neither case should occur here.
            // Subscription events come off PlatformManager itself, not off the PdxSdk platform, so this
            // arms regardless of the platform checks below: it is the one proof that a folder deletion
            // was the player's own doing (ModSubscriptionWatch), and a missed arm turns that proof into
            // a permanent "unknown".
            ModSubscriptionWatch.Arm();

            var pdx = pm.GetPSI<Colossal.PSI.PdxSdk.PdxSdkPlatform>("PdxSdk");
            if (pdx == null)
                Log.Warn("[ParadoxNative] PdxSdk platform unavailable at subscribe — mod-sync gate seeded open (non-Paradox launch or PSI not yet registered).");
            else if (!pdx.isInitialized)
                Log.Warn("[ParadoxNative] PdxSdk platform not initialized at subscribe (SDK context creation failed) — mod-sync gate seeded open.");
            else
            {
                m_ModSyncOngoing = pdx.isSyncOnGoing;
                // Healthy platform: arm the one-shot installed-set probe (SDK's own "is our mod
                // installed" answer, independent of playset activation) so FinalizeMissing can
                // ride it in the Error line. Read-only and async — see ModInstallResolver.
                ModInstallResolver.StartInstalledSetProbe(pdx);
                // Same gate for the active-set transition probe: it subscribes to this platform's
                // onActivePlaysetChanged / onModStatusChanged, so an absent or uninitialised PdxSdk has
                // no edges to give. Latched to one Error line per process; silent in a healthy session.
                m_TransitionProbe.Arm(pdx);
            }
        }

        // Raised by the platform OFF the main thread — only flip the volatile flag here (no Unity/ECS
        // access). The owner reads ModSyncOngoing on the main thread from OnUpdateImpl.
        private void OnModSyncStatusChanged(IModSupport psi, bool onGoing) => m_ModSyncOngoing = onGoing;

        // Detach the platform mod-sync handler — the owner calls this from OnDestroy. PlatformManager.instance
        // outlives our system, so leaving the handler attached would leak this loader past teardown. Paired
        // with Activate (not named Dispose: this is a plain teardown hook, the type is not IDisposable).
        // The transition probe is torn down the same way and for the same reason, plus its dispatcher
        // updater: an updater that never returns true would survive the assembly unload.
        public void Deactivate()
        {
            PlatformManager.instance.onModSyncStatusChanged -= OnModSyncStatusChanged;
            m_TransitionProbe.Disarm();
            ModSubscriptionWatch.Disarm();
        }
        ...
        private bool WhenReadyPostLoad()
        {
            ...
            // Existence is not arrival: a .cok appearing via a non-atomic writer (antivirus
            // un-quarantine, cloud-sync restore, manual copy) is visible to File.Exists while still
            // half-written. Firing the scan at that moment made vanilla PopulateFromDirectory throw
            // a sharing-violation IOException and skip the file with NO retry (live repro
            // 2026-07-15) — the session ended up worse than waiting. Hold readiness until every
            // core .cok actually opens for read; the probe is throttled and share-permissive
            // (see AllCoreCokReadable), so it neither blocks the writer nor spams the disk.
            if (m_ReadProbeCountdown > 0)
            {
                m_ReadProbeCountdown--;
                return false;
            }
            if (!CivicCokSelfLoader.AllCoreCokReadable())
            {
                if (NativePhase != "awaiting-readable")
                    Log.Info("[ParadoxNative] core .cok on disk but not yet readable (writer still holds it) — holding the native load until it opens.");
                NativePhase = "awaiting-readable"; // on disk but still being written / externally locked
                m_ReadProbeCountdown = ReadProbeIntervalFrames;
                return false;
            }
        ...
        private bool ResolveAndScan(ParadoxModsDataSource ds)
        {
            ...
            // Re-populate already ran and the runtime active set STILL doesn't hold us — the transient
            // GetModsInActivePlayset LocalData-filter has stranded a healthy install for the session
            // (files-present α: folder present, SDK row enabled, yet dropped from GetActiveMods, and
            // re-asking it via Populate cannot help). Before giving up, resolve the install folder from
            // the SDK's authoritative per-mod local record (Context.Mods.GetLocalModData — the source of
            // truth the Skyve mod manager reads), which bypasses that filter. Read-only, no SDK write.
            if (IsParadoxDelivered() && TryScanFromSdkLocalPath(ds))
                return true;

            // The SDK truth also had no usable path (genuinely not installed / files gone) → real miss.
            return AbortNoModPath(ds);
        }

        // Files-present α recovery: resolve our install folder from the SDK's authoritative per-mod
        // local record instead of the stale runtime active set, then run the normal folder scan + native
        // load from it. Returns true when a scan is now in flight (the SDK path resolved AND its core
        // .cok are present on disk); false when the SDK has no usable path or the files are genuinely
        // gone (→ caller aborts as before). The native AddPrefab this feeds is NOT gated by m_ActiveMods
        // (GameManager.OnEntryIsInActivePlaysetChanged decompile-verified: no ModRequirement /
        // ContainsActiveMod check), so active-set membership is not required to register the prefabs.
        private bool TryScanFromSdkLocalPath(ParadoxModsDataSource ds)
        {
            string? sdkPath = ModInstallResolver.MatchSdkLocalPath();
            if (sdkPath == null || !CivicCokSelfLoader.CoreCokPresentIn(sdkPath))
                return false;

            NativePhase = "sdk-localpath-resolve";
            Log.Info($"[ParadoxNative] runtime active set stranded a healthy install (files-present) — resolved our folder to '{ModPaths.SanitizePathTail(sdkPath)}' from the SDK local record (Skyve-style) with core .cok present; scanning + native-loading from it, bypassing the stale GetActiveMods cache.");
            return StartDirectoryScan(ds, sdkPath);
        }
        ...
        // Returns false.
        private bool AbortNoModPath(ParadoxModsDataSource ds)
        {
            NativePhase = "abort-no-modpath";
            if (IsParadoxDelivered())
                // The active-set probe rides the Error channel on purpose: it classifies WHY the
                // path never resolved (empty set = platform returned nothing / offline vs id absent
                // or path-less = SDK LocalData desync) — the per-mod Info lines never reach prod.
                Log.Error("[ParadoxNative] Paradox-delivered mod but active mod path unresolved even after playset re-populate — native .cok re-load skipped; core prefabs will hard-cap as absent. " +
                          $"Active-set probe: {ModInstallResolver.DescribeActiveSet(ds)}.");
            else
...
#pragma warning restore S3011
        ...
        // Find our active mod's CURRENT path in the live playset by stable ModId (not by the boot
        // install folder — its "_<instance>" suffix changes on every Paradox version/re-sync, so a
        // folder-name match strands us the moment Paradox re-materialises the mod into a new instance).
        // Mod equality is by id (decompile: Colossal.PSI.Common.Mod.Equals), so an id match returns the
        // live instance path regardless of the folder counter. Null when we're not in the active set
        // (→ ResolveAndScan drives ds.Populate() once, then retries).
        private static string? ResolveModPath(ParadoxModsDataSource ds)
            => ModInstallResolver.MatchLivePath(ds, log: true);
