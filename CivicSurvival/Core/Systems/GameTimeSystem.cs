...
#pragma warning restore CIVIC324
        ...
        // Last whole hour an HourChangedEvent was published for. Not serialized:
        // hourly income is cadence, not state — a load re-seeds from the current
        // clock and the next boundary pays normally. -1 = unseeded (post-load or
        // first frame), which suppresses a spurious hour event on the seeding tick.
        [NonSerialized] private int m_LastPublishedHour = -1;
        ...
        protected override void OnCreate()
        {
            ...
            // OnUpdateImpl as idempotent retry fallbacks.
...
#pragma warning restore CIVIC226
            ...
            PublishHourBoundary(currentHour, midnightCrossed);
        }

        /// <summary>
        /// Publish <see cref="HourChangedEvent"/> once per whole vanilla hour, so
        /// per-day income can be paid in 24 slices instead of a single midnight
        /// lump. Runs after the day event: a midnight tick delivers the day first,
        /// then hour 0, keeping "new day, then its first hour" ordering.
        ///
        /// Skips ahead rather than replaying: if the sim jumped several hours in
        /// one tick (speed 3x, a long frame), one event fires for the hour landed
        /// on. Slices are cadence, not an accrual ledger — the day tick remains the
        /// authority on per-day totals.
        /// </summary>
        private void PublishHourBoundary(float currentHour, bool midnightCrossed)
        {
            int wholeHour = (int)currentHour;
            if (wholeHour < 0) wholeHour = 0;
            if (wholeHour >= (int)GameRate.HOURS_PER_DAY) wholeHour = (int)GameRate.HOURS_PER_DAY - 1;

            // Post-load / first frame: seed silently, no payout for an hour that
            // may already have been paid before the save.
            if (m_LastPublishedHour < 0)
            {
                m_LastPublishedHour = wholeHour;
                return;
            }

            // Midnight resets the comparison basis — hour 0 of a new day is a
            // boundary even though 0 < previous hour (e.g. 23).
            bool hourCrossed = midnightCrossed ? wholeHour != m_LastPublishedHour : wholeHour > m_LastPublishedHour;
            if (!hourCrossed) return;

            m_LastPublishedHour = wholeHour;
            EventBus?.SafePublish(new HourChangedEvent(m_CurrentDay, wholeHour), "GameTimeSystem");
        }
        ...
        private void UpdateStateSnapshot()
        {
            ...
            // null-deref in Publish would crash deserialize
        ...
        /// Called when WarStartedEvent is received from ScenarioStateMachine.StartWar.
        /// </summary>
