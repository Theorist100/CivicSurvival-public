...
namespace CivicSurvival.Core.Serialization
{
    ...
    public readonly struct CrisisMonitorPersistState
    {
        public CrisisMonitorPersistState(
            float crisisLevel,
            RecordedPopulation totalPopulation,
            RecordedPopulation affectedPopulation,
            int lastUpdateDay)
        {
        ...
        /// <summary>
        /// Denominator of the persisted crisis percentage: the citizens of the published
        /// household selection on the day it was taken. Both this and
        /// <see cref="AffectedPopulation"/> are measurements kept from an earlier moment, so
        /// both carry the counter that took them — and both must carry the SAME one, which is
        /// the defect this pair used to have (a citywide count over a sampled denominator).
        /// </summary>
        public RecordedPopulation TotalPopulation { get; }

        /// <summary>
        /// Numerator of the persisted crisis percentage: the citizens of blacked-out households
        /// on the same day, out of the same selection. A recorded zero is truthful here (nobody
        /// was in a blackout), which is exactly why the stamp and not the value carries absence.
        /// </summary>
        public RecordedPopulation AffectedPopulation { get; }
    ...
    public static class CrisisMonitorCodec
    {
        ...
        public static void Write<TWriter>(in CrisisMonitorPersistState state, TWriter writer)
            where TWriter : IWriter
        {
            // 4 fields before the pair gained its measure stamp. One stamp covers both numbers
            // on purpose: they are the two sides of one percentage taken in one pass out of one
            // selection, and a save in which they disagreed would be meaningless.
            KeyedSerializer.WriteBlockHeader(writer, 5);
            KeyedSerializer.WriteField(writer, "crisisLevel", state.CrisisLevel);
            KeyedSerializer.WriteField(writer, "totalPop", state.TotalPopulation.Value);
            KeyedSerializer.WriteField(writer, "affectedPop", state.AffectedPopulation.Value);
            KeyedSerializer.WriteEnumByteField(writer, "popMeasure", (byte)state.TotalPopulation.Measure);
            ...
            where TReader : IReader
        {
            ...
            var populationMeasure = PopulationMeasure.None;
            int lastUpdateDay = 0;
            ...
            for (int i = 0; i < fieldCount; i++)
            {
                ...
                switch (key)
                {
                    ...
                    case "affectedPop":
                        ...
                        break;
                    case "popMeasure":
                        populationMeasure = KeyedSerializer.ReadEnumByte<TReader, PopulationMeasure>(reader, tag, "popMeasure", PopulationMeasure.None);
                        break;
                    case "lastUpdateDay":
            ...
            // An unstamped save took both numbers with the sampled resident model — the total
            // through the citywide scalar and the affected tally out of the selection, i.e. the
            // very mismatch that made the persisted level lean high. Restored on that measure
            // and left alone: the next day change re-takes the pair on the current one.
            state = new CrisisMonitorPersistState(
                crisisLevel,
                RecordedPopulation.Restore(totalPopulation, populationMeasure, PopulationMeasure.ResidentSelection),
                RecordedPopulation.Restore(affectedPopulation, populationMeasure, PopulationMeasure.ResidentSelection),
                lastUpdateDay);
        }
    }
