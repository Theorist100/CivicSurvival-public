...
namespace CivicSurvival.Core.Serialization
{
    ...
    public static class SpotterAggregateCodec
    {
        ...
        public static void Write<TWriter>(in SpotterAggregatePersistState state, TWriter writer, in LiveDistrictMap map)
            where TWriter : IWriter
        {
            ...
            WriteInternetDisabled(writer, state.InternetDisabledDistricts, map);
        ...
        private static void WriteInternetDisabled<TWriter>(TWriter writer, IReadOnlyList<int> districts, in LiveDistrictMap map)
            where TWriter : IWriter
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                DistrictKeyCodec.Write(writer, "de", districts[i], map, ref stale);
            }
            DistrictKeyCodec.LogWrite("SpotterAggregateCodec.InternetDisabled", count, stale);
        }
            ...
            where TReader : IReader
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                districts[written++] = result == DistrictKeyReadResult.Unzoned ? DistrictUtils.UNZONED_AREA_INDEX : live.Index;
            }

            DistrictKeyCodec.LogRead("SpotterAggregateCodec.InternetDisabled", written, stale);

            if (written == districts.Length)
                return districts;

            var compact = new int[written];
            System.Array.Copy(districts, compact, written);
            return compact;
        }
