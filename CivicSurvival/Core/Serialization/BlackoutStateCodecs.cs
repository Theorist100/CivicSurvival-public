...
namespace CivicSurvival.Core.Serialization
{
    ...
    public static class BlackoutStateCodec
    {
        ...
        public static void Read<TReader>(TReader reader, out BlackoutPersistState state)
            ...
            for (int i = 0; i < fieldCount; i++)
            {
                ...
                switch (key)
                {
                    ...
                    case "districtOverrides":
                        districtOverrides = ReadDistrictOverrides(reader, tag);
                        break;
                    case "blackouts":
                        blackouts = ReadBlackouts(reader, tag);
                        break;
                    case "vips":
                        vips = ReadDistrictRefSet(reader, tag, "vips");
                        break;
                    case "vipBypass":
                        vipBypass = ReadDistrictRefSet(reader, tag, "vipBypass");
                        break;
                    case "penalties":
                        penalties = ReadPenalties(reader, tag);
                        break;
                    case "preShed":
                        preShedStates = ReadPreShedStates(reader, tag);
                        break;
                    case "priorities":
                        priorities = ReadPriorities(reader, tag);
                        break;
                    default:
            ...
            where TWriter : IWriter
        {
            ...
            for (int i = 0; i < entries.Count; i++)
            {
                ...
                DistrictKeyCodec.Write(writer, "de", entries[i].District);
        ...
        private static DistrictScheduleEntry[] ReadDistrictOverrides<TReader>(TReader reader, TypeTag tag)
            where TReader : IReader
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (TryResolveDistrictRef(districtKind, districtEntity, ref stale, out var districtRef) && hasSchedule)
                    entries[written++] = new DistrictScheduleEntry(districtRef, schedule);
            }
            DistrictKeyCodec.LogRead("BlackoutStateCodec.districtOverrides", written, stale);
            return Compact(entries, written);
        }

        private static DistrictCategoriesEntry[] ReadBlackouts<TReader>(TReader reader, TypeTag tag)
            where TReader : IReader
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (TryResolveDistrictRef(districtKind, districtEntity, ref stale, out var districtRef))
                    entries[written++] = new DistrictCategoriesEntry(districtRef, categories);
            }
            DistrictKeyCodec.LogRead("BlackoutStateCodec.blackouts", written, stale);
            return Compact(entries, written);
        }

        private static DistrictRefEntry[] ReadDistrictRefSet<TReader>(TReader reader, TypeTag tag, string name)
            where TReader : IReader
        {
            ...
            DistrictKeyCodec.LogRead($"BlackoutStateCodec.{name}", written, stale);
            return Compact(entries, written);
        }

        private static DistrictPenaltyPersistEntry[] ReadPenalties<TReader>(TReader reader, TypeTag tag)
            where TReader : IReader
        {
            ...
            DistrictKeyCodec.LogRead("BlackoutStateCodec.penalties", written, stale);
            return Compact(entries, written);
        }

        private static PreShedPersistEntry[] ReadPreShedStates<TReader>(TReader reader, TypeTag tag)
            where TReader : IReader
        {
            ...
            DistrictKeyCodec.LogRead("BlackoutStateCodec.preShed", written, stale);
            return Compact(entries, written);
        }

        private static DistrictPriorityEntry[] ReadPriorities<TReader>(TReader reader, TypeTag tag)
            where TReader : IReader
        {
            ...
            DistrictKeyCodec.LogRead("BlackoutStateCodec.priorities", written, stale);
            return Compact(entries, written);
        }
        ...
        /// <summary>
        /// Resolve a district key read via DistrictKeyCodec to a runtime DistrictRef.
        /// Live → live ref, Unzoned → DistrictRef.Null, None/stale → drop (increment
        /// <paramref name="stale"/>, return false).
        /// </summary>
        private static bool TryResolveDistrictRef(byte kind, Entity entity, ref int stale, out DistrictRef district)
        {
            ...
            district = result == DistrictKeyReadResult.Unzoned ? DistrictRef.Null : DistrictRef.FromEntity(live);
            return true;
        }
