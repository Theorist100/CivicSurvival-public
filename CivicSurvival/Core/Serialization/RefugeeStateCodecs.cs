...
namespace CivicSurvival.Core.Serialization
{
    ...
    public readonly struct RefugeeSpawnPersistState
    {
        public RefugeeSpawnPersistState(
            ...
            int spawnCounter,
            RecordedPopulation originalPopulation,
        ...
        /// <summary>
        /// The city size the influx was declared against, with the counter that measured it.
        /// Stored as value + stamp so a save written before stamps existed answers "not on your
        /// measure" instead of handing over a number the live reading cannot be divided by.
        /// </summary>
        public RecordedPopulation OriginalPopulation { get; }
    ...
    public static class RefugeeSpawnCodec
    {
        public static void Write<TWriter>(in RefugeeSpawnPersistState state, TWriter writer)
            where TWriter : IWriter
        {
            // 12 fields before the influx baseline gained its measure stamp.
            ...
            KeyedSerializer.WriteField(writer, "m_OriginalPopulation", state.OriginalPopulation.Value);
            KeyedSerializer.WriteEnumByteField(writer, "m_OriginalPopulationMeasure", (byte)state.OriginalPopulation.Measure);
            ...
            where TReader : IReader
        {
            ...
            for (int i = 0; i < fieldCount; i++)
            {
                ...
                switch (key)
                {
                    ...
                    case "m_OriginalPopulation":
                        ...
                        break;
                    case "m_OriginalPopulationMeasure":
                        originalPopulationMeasure = KeyedSerializer.ReadEnumByte<TReader, PopulationMeasure>(reader, tag, "m_OriginalPopulationMeasure", PopulationMeasure.None);
                        break;
            ...
            state = new RefugeeSpawnPersistState(
                ...
                totalRefugeesAdded,
                spawnCounter,
                // An unstamped save took this with the sampled resident model, and its zero was
                // the "never captured" sentinel — restored on that measure, unscaled, so the
                // collapse window withholds itself rather than dividing across counters.
                RecordedPopulation.Restore(originalPopulation, originalPopulationMeasure, PopulationMeasure.ResidentSelection),
                refugeesAtBorder,
