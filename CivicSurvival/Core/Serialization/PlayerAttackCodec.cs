    ...
    public static class PlayerAttackCodec
    {
            ...
            where TWriter : IWriter
        {
            ...
            for (int i = 0; i < state.Slots.Count; i++)
            {
                ...
                KeyedSerializer.WriteField(writer, "targetId", (int)slot.TargetId);
            }
        }
            ...
            where TReader : IReader
        {
            ...
            for (int i = 0; i < slotCount; i++)
            {
                ...
                bool executionWasBlocked = false;
                bool executionWasVulnerable = false;
                // Absent in pre-phase-C saves (keyed reader defaults it) → unselected / auto.
                ushort targetId = EnemyTarget.NoTargetId;
                ...
                for (int f = 0; f < fieldCount; f++)
                {
                    ...
                    switch (fieldKey)
                    {
                        ...
                        case "executionWasVulnerable":
                            ...
                            break;
                        case "targetId":
                            targetId = (ushort)KeyedSerializer.ReadBoundedInt(reader, fieldTag, "targetId", 0, ushort.MaxValue, EnemyTarget.NoTargetId);
                            break;
                        default:
