...
namespace CivicSurvival.Core.Serialization
{
    ...
    public readonly struct WaveExecutorPersistState
    {
        public WaveExecutorPersistState(
            ...
            int nextWaveNumber,
            int nextWaveThreatsForecast,
            bool nextWaveForecastReady)
        {
            ...
            NextWaveType = SanitizeWaveType(nextWaveType);
            NextWaveNumber = nextWaveNumber < 0 ? 0 : nextWaveNumber;
            NextWaveThreatsForecast = nextWaveThreatsForecast < 0 ? 0 : nextWaveThreatsForecast;
        ...
        public WaveType NextWaveType { get; }
        public int NextWaveNumber { get; }
        public int NextWaveThreatsForecast { get; }
        public bool NextWaveForecastReady { get; }
    ...
    public static class WaveExecutorCodec
    {
        ...
        public static void Write<TWriter>(in WaveExecutorPersistState state, TWriter writer)
            where TWriter : IWriter
        {
            KeyedSerializer.WriteBlockHeader(writer, 13);
            ...
            KeyedSerializer.WriteEnumByteField(writer, "nextWaveType", (byte)state.NextWaveType);
            KeyedSerializer.WriteField(writer, "nextWaveNumber", state.NextWaveNumber);
            KeyedSerializer.WriteField(writer, "nextThreatsForecast", state.NextWaveThreatsForecast);
            KeyedSerializer.WriteField(writer, "nextForecastReady", state.NextWaveForecastReady);
        }
            ...
            where TReader : IReader
        {
            ...
            WaveType nextWaveType = WaveType.Harassment;
            int nextWaveNumber = 0;
            int nextWaveThreatsForecast = 0;
            bool nextWaveForecastReady = false;
            ...
            for (int i = 0; i < fieldCount; i++)
            {
                ...
                switch (key)
                {
                    ...
                    case "scenarioStarted":
                        ...
                        break;
                    case "nextWaveType":
                        nextWaveType = KeyedSerializer.ReadEnumByte<TReader, WaveType>(reader, tag, "nextWaveType", WaveType.Harassment);
                        break;
                    case "nextWaveNumber":
                        nextWaveNumber = KeyedSerializer.ReadClampedInt(reader, tag, "nextWaveNumber", 0, MaxWaveNumber);
                        break;
                    case "nextThreatsForecast":
                        nextWaveThreatsForecast = KeyedSerializer.ReadClampedInt(reader, tag, "nextThreatsForecast", 0, MaxThreatCount);
                        break;
                    case "nextForecastReady":
                        nextWaveForecastReady = KeyedSerializer.ReadBool(reader, tag, "nextForecastReady");
                        break;
                    default:
                        KeyedSerializer.Skip(reader, tag);
                        break;
            ...
            state = new WaveExecutorPersistState(
                ...
                nextWaveType,
                nextWaveNumber,
                nextWaveThreatsForecast,
                nextWaveForecastReady);
        }
    }
}
