...
namespace CivicSurvival.Core.Serialization
{
    ...
    public static class NarrativeNotificationCodec
    {
        ...
        public static void Write<TWriter>(in NarrativeNotificationPersistState state, TWriter writer, in LiveDistrictMap map)
            where TWriter : IWriter
        {
            ...
            if (state.HasResolverState)
                WriteResolverState(writer, state.ResolverState, map);
        }
        ...
        private static void WriteResolverState<TWriter>(TWriter writer, in NarrativeResolverPersistState state, in LiveDistrictMap map)
            where TWriter : IWriter
        {
            ...
            for (int i = 0; i < cognitiveCount; i++)
            {
                ...
                DistrictKeyCodec.Write(writer, "de", state.CognitiveCooldowns[i].DistrictIndex, map, ref stale);
                KeyedSerializer.WriteField(writer, "last", state.CognitiveCooldowns[i].LastTransitionHour);
            }
            DistrictKeyCodec.LogWrite("NarrativeNotificationCodec.Cognitive", cognitiveCount, stale);
        }
            ...
            where TReader : IReader
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                for (int f = 0; f < fieldCount; f++)
                {
                    ...
                    switch (key)
                    {
                        case "deK": districtKind = DistrictKeyCodec.ReadKind(reader, fieldTag, "de"); break;
                        case "de": districtEntity = DistrictKeyCodec.ReadEntity(reader, fieldTag, "de"); break;
            ...
            DistrictKeyCodec.LogRead("NarrativeNotificationCodec.Cognitive", written, stale);
