...
namespace CivicSurvival.Core.Services
{
    ...
    public readonly struct WaveSimulationContext
    {
        ...
        /// crew-aware, see <see cref="Logic.ManpowerLogic.DefensePotentialForDensity"/>). 0 = no signal
    ...
    public static class WaveContextGatherer
    {
        ...
        /// <summary>
        /// The single enemy-pressure formula: aggregate of the enemy's three axes (mean / cap),
        /// weakened by one <c>RespiteWaveWeakenMultiplier</c> per axis inside its post-floor
        /// respite window, clamped to [<see cref="MIN_ENEMY_PRESSURE"/>, 1]. This is the only
        /// seam where the enemy's axis health and the suppression loop feed back into wave
        /// strength — applied here (not on the axis itself) so the durable axis value and its
        /// regen are untouched and only the live wave force reflects the temporary lull.
        /// Shared by <see cref="Gather"/> and wave telemetry so both report the same multiplier.
        /// </summary>
        public static float ComputeEnemyPressure(in EnemyState enemyState, GridWarfareConfig gw, float gameTimeHours)
        {
            float enemyPressure = enemyState.AggregatePressure01(gw.PressureCap);

            int respiteAxes = enemyState.RespiteActiveAxisCount(gameTimeHours);
            if (respiteAxes > 0)
            {
                float weaken = math.clamp(gw.RespiteWaveWeakenMultiplier, 0f, 1f);
                enemyPressure *= math.pow(weaken, (float)respiteAxes);
            }

            return math.clamp(enemyPressure, MIN_ENEMY_PRESSURE, 1f);
        }
        ...
        /// defence-potential density signal via <see cref="ManpowerLogic.DefensePotentialForDensity"/>. 0 →
            ...
            int population = 0)
        {
            ...
            if (enemyStateReady)
                enemyPressure = ComputeEnemyPressure(in enemyState, balanceConfig.GridWarfare, gameTimeHours);
            else
            ...
            if (population > 0)
            {
                // PERF-LOCK: DefensePotentialForDensity scales FULLY with population but reads the
                // WAVE's own curve (Waves.DensityCoeff / Waves.DensityExponent), so buffing the
                // player's crew pool can no longer inflate the drone count (linear render cost) as a
                // side effect. Replaces the half-coupling to ManpowerCoeff at identical output —
                // the threat ceiling this protects is unchanged. See
                // ManpowerLogic.DefensePotentialForDensity.
                float defencePotential = ManpowerLogic.DefensePotentialForDensity(
