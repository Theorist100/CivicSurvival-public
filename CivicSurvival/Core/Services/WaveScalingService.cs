...
namespace CivicSurvival.Core.Services
{
    ...
    public static class WaveScalingService
    {
        ...
        /// <see cref="Logic.ManpowerLogic.DefensePotentialForDensity"/>). Mirror of the surplus-power surcharge:
        ...
        /// Result (undamaged, neutral season, average roll): up to 100MW≈60min, 200MW≈38min,
        /// 500MW≈23min, 2000MW≈14min — the 40-min MinCalmSeconds floor binds from ~200MW up, so
        /// it, not the divisor, is what actually paces a large city.
        /// </summary>
        public static float CalculateCalmSeconds(int citySizeMW, float seasonMod, float frequencyMod, float randomVariance, float surplusRatio = 1f, int intermittentTypes = 0, float baseMinutesOverride = -1f)
        {
            ...
            //
            // Never below 1: the divisor is what makes a big city a busier target, and it must
            // not run backwards. Below the divisor the log drops under 1 and the division turns
            // into a multiplication — a 50MW settlement was waiting 188 minutes against the
            // configured 110, half again as long as a city twice its size, which is the exact
            // opposite of the rule this line implements. The clamp at 1 makes the configured
            // base the LONGEST any city waits, and the divisor only ever shortens it.
            float logFactor = math.log2(citySizeMW / frequencyDivisor + 1f);
            logFactor = math.max(logFactor, 1f);
        ...
        public static float GetPhaseDuration(GamePhase phase, int waveNumber, float frequencyMod, float randomValue)
        {
            ...
            switch (phase)
            {
                ...
                case GamePhase.PsyAttack:
                    return cfg.PsyAttackMin + math.max(0f, cfg.PsyAttackMax - cfg.PsyAttackMin) * randomValue;
