...
namespace CivicSurvival.Core.Services.Countermeasures
{
    ...
    public sealed class CmWalletOps
    {
        ...
        public CmWalletOps(IShadowWalletService walletService, GameSimulationEndBarrier barrier)
        {
        ...
        /// <summary>
        /// Synchronous investigation-bribe deduct (pause-safe, AXIOM 14). Replaces the
        /// former retained ShadowOps queue + CountermeasureBribeIntent drain: affordability
        /// (freeze + markup + pending) then a direct wallet deduct on the calling thread,
        /// so the choice modal resolves in the same tick even at selectedSpeed==0.
        /// </summary>
        public bool TryDeductInvestigationBribe(int amount, out long effectiveCost)
        {
            if (!CanDeduct(amount, out effectiveCost))
                return false;

#pragma warning disable CIVIC237 // TOCTOU safe: main-thread single writer, no yield between check and deduct
            return m_Wallet.TryDeduct(effectiveCost, "CountermeasuresBribe:Investigation");
#pragma warning restore CIVIC237
        }

        /// <summary>
        /// Defensive reversal for the sync bribe flow: idempotent refund of an
        /// already-deducted investigation bribe (bypasses act/freeze gates by design).
        /// </summary>
        public void RefundInvestigationBribe(long effectiveCost)
        {
            if (effectiveCost <= 0)
                return;

            if (!m_Wallet.TryApplyRefund(effectiveCost, "CountermeasuresBribeRefund", $"CountermeasuresBribeRefund:sync:{effectiveCost}"))
                Log.Warn("[Countermeasures] Sync bribe refund could not be applied");
        }
