...
namespace CivicSurvival.Core.Services.Countermeasures
{
    ...
    public static class CorruptionCalculator
    {
        ...
        public static float Calculate(
            ...
            int draftDeferralPercent = 0)
        {
            float score = 0f;
            var corruption = BalanceConfig.Current.Corruption;

            // Export share (0-100) scaled by ExportWeight — at the shipped 0.6 that is up to 60
            // points, the single heaviest indicator. (The old "0-25 points" note here predated the
            // weight moving into config and had drifted; read the contract, not this line.)
            ...
            // Draft-Exemption scheme: stationary contribution per % of the held deferral rate
            score += draftDeferralPercent * balance.DraftExemption.RateCorruptionPerPercent;
        ...
        /// <summary>
        /// Corruption as the CITY can see it — the score that drives willingness to serve
        /// (<c>ManpowerLogic.CalculatePatriotismFactor</c>). Same weights, same clamp, one formula:
        /// this is <see cref="Calculate"/> with the two indicators the citizenry cannot hold
        /// against the administration zeroed out.
        ///
        /// Offshore balance is excluded because it is a STOCK, not a deed: people see power leave
        /// during a blackout, they do not see a bank balance. Scoring the balance would also make
        /// the penalty inescapable — the war chest that funds a counterattack would keep cutting
        /// crews until it was spent, so arming yourself and defending yourself would compete.
        /// Investigations still read the full <see cref="Calculate"/>: an investigator cares about
        /// exactly the money the street cannot see.
        ///
        /// Draft deferrals are excluded because they ALREADY have a manpower price — the sold
        /// exemptions are subtracted head-for-head in <c>ManpowerLogic.CalculateDeferralHeads</c>.
        /// Letting them also depress the multiplier would charge one scheme twice.
        ///
        /// <paramref name="exportLoadPercent"/> is the REALIZED export share
        /// (<see cref="Components.CrossDomain.ShadowExportState.ExportLoadPercent"/>), not the
        /// slider: the city resents power that actually left it.
        /// </summary>
        public static float CalculateVisibleToCity(
