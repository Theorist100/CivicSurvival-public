...
namespace CivicSurvival.Core.Services.Countermeasures
{
    ...
    public sealed class CmChoiceProcessor
    {
            ...
            out string reasonId)
        {
            ...
            switch (choice)
            {
                case InvestigationChoice.Bribe:
                    // Bribe publishes its own choice event (success inside
                    // ResolvePaidInvestigationBribe, failure below) — skip the shared
                    // publish at the bottom to avoid a duplicate.
                    return HandleInvestigationBribe(ref core, ref inv, out reasonId);
            ...
            out string reasonId)
        {
            ...
            // Pause-safe sync payment (AXIOM 14): deduct on the calling thread, then
            // resolve the journalist roll immediately — no retained deduct, no
            // pending window, no refund path (all preflight ran before the money moved).
            if (!m_Wallet.TryDeductInvestigationBribe(baseCost, out long effectiveCost))
            {
            ...
            // Money is spent either way: the journalist may take it and betray —
            // ResolvePaidInvestigationBribe rolls and publishes the outcome event.
            bool committed = ResolvePaidInvestigationBribe(ref core, ref inv, effectiveCost, out reasonId);
            if (!committed)
            {
                // Defensive: unreachable in the sync flow (phase was just validated),
                // but if resolve ever rejects, return the money instead of eating it.
                m_Wallet.RefundInvestigationBribe(effectiveCost);
            }

            return committed ? RequestStatus.Success : RequestStatus.Failed;
        }

        private bool ResolvePaidInvestigationBribe(
            ref CountermeasuresCoreFsm core,
            ref CmInvestigationState inv,
            long effectiveCost,
            out string reasonId)
