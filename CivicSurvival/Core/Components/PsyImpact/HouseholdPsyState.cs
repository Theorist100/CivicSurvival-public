...
#pragma warning restore CIVIC262
        ...
        /// Cached, NOT persisted — re-resolved by MentalHealthResolverSystem on the 30s
        /// slow-profile refresh (<see cref="SocialStratum.Unknown"/> forces an immediate refresh,
        /// so post-load and brand-new sidecars classify on their first slot fire).
        ...
        /// Cached, NOT persisted — re-derived by <see cref="MentalHealthResolverSystem"/> on the 30s
        /// slow-profile refresh (mirror <see cref="CachedStratum"/>; Unknown stratum forces an immediate
        /// refresh); each entity writes its own field, so the parallel resolver is race-free, and it
        /// self-heals on load (stays false until the household's slot fires).
