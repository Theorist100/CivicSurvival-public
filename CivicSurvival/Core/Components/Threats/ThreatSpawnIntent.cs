...
namespace CivicSurvival.Core.Components.Threats
{
    ...
    public struct ThreatSpawnIntent : IBufferElementData
    {
        ...
        /// <summary>
        /// Outbound only: the mirror-city target the strike aims at (the player's War Room pick or
        /// the Execute-time auto-selection; <c>EnemyTarget.NoTargetId</c> = resolve automatically).
        /// Copied by the consumer onto <see cref="OutboundStrikePayload.TargetId"/> so the arrival
        /// resolution honours the pick. Ignored for inbound waves.
        /// </summary>
        public ushort OutboundTargetId;
