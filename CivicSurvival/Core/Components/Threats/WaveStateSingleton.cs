...
namespace CivicSurvival.Core.Components.Threats
{
    ...
    public struct WaveStateSingleton : IComponentData, IEmptySerializable
    {
        ...
        // ===== Next-wave forecast (valid only while NextWaveForecastReady) =====
        // During Calm/Recovery CurrentWaveType/ThreatsExpected still describe the FINISHED wave —
        // the next wave's identity is decided by WaveScheduler at Recovery/Calm entry and mirrored
        // here (single writer: WaveExecutor, fed by WaveForecastEvent). Intel consumers must read
        // these fields — not CurrentWaveType — when previewing the upcoming wave outside Alert/Attack.

        /// <summary>Decided type of the NEXT wave (the one the current Calm leads into).</summary>
        public WaveType NextWaveType;

        /// <summary>Wave number the forecast describes (what ScheduleAlertPhase will launch).</summary>
        public int NextWaveNumber;

        /// <summary>
        /// Deterministic pre-noise threat estimate for the next wave (noiseMult=1). The launch
        /// re-rolls ±20% noise off the live context, so this is an estimate, not the exact count.
        /// </summary>
        public int NextWaveThreatsForecast;

        /// <summary>True once WaveScheduler has decided the next wave; cleared on Alert (forecast becomes current).</summary>
        public bool NextWaveForecastReady;
        ...
        public static WaveStateSingleton Default => new()
        {
            ...
            NextWaveType = WaveType.Harassment,
            NextWaveNumber = 0,
            NextWaveThreatsForecast = 0,
            NextWaveForecastReady = false
        };
