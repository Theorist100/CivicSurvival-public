...
namespace CivicSurvival.Core.Components.Threats
{
    ...
    /// <see cref="OutboundArrivalSignal"/> the GridWarfare effect owner resolves against a
    /// mirror-city target (positional AA intercept roll included).
    ///
    ...
    public struct OutboundStrikePayload : IComponentData, ISerializable
    {
        ...
        /// <c>Random</c>). It rides the projectile to arrival, where the seeded target-strike
        /// resolution turns it into the enemy's intercept verdict. Serialized so a counter-strike
        /// caught in flight by a save replays the SAME verdict on load (the session RNG it replaced
        /// did not), and so a future server fed the same launch seed recomputes the identical
        /// outcome for an offline defender (Wave3Arena Phase-40). Inbound waves carry a 0 seed and
        /// never read it (the faction gate routes them to city terminalization, not the axis channel).
        /// </summary>
        public uint Seed;

        /// <summary>
        /// Mirror-city target this strike aims at (the player's War Room pick or the Execute-time
        /// auto-selection; <c>EnemyTarget.NoTargetId</c> = resolve automatically at arrival).
        /// Serialized with the payload so a mid-flight save keeps the aim; the arrival owner
        /// forwards it into the shared target-selection rule, which still falls back automatically
        /// when the picked target died in flight. Inbound waves never read it.
        /// </summary>
        ...
        public void Serialize<TWriter>(TWriter writer) where TWriter : IWriter
        {
            ...
            try
            {
                ...
                KeyedSerializer.WriteField(writer, "tgt", (int)TargetId);
            }
            finally
            {
        ...
        public void Deserialize<TReader>(TReader reader) where TReader : IReader
        {
            ...
            try
            {
                if (version >= 1)
                {
                    ...
                    if (fc > 0)
                    {
                        // An outbound block without the "tgt" key (older save) means "no explicit
                        // aim" — the sentinel, not target 0.
                        TargetId = Components.Domain.GridWarfare.EnemyTarget.NoTargetId;
                    }
                    for (int i = 0; i < fc; i++)
                    {
                        ...
                        switch (key)
                        {
                            ...
                            case "tgt": TargetId = (ushort)KeyedSerializer.ReadBoundedInt(reader, tag, "tgt", 0, ushort.MaxValue, Components.Domain.GridWarfare.EnemyTarget.NoTargetId); break;
