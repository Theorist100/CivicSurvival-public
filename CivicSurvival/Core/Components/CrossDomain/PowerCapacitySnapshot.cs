...
namespace CivicSurvival.Core.Components.CrossDomain
{
    public readonly struct PowerCapacityPlantSnapshot : IEquatable<PowerCapacityPlantSnapshot>
    {
        ...
        /// <summary>
        /// Damage multiplier <c>max(0, 1 − operational − disaster − explosion)</c> — snapshot-side
        /// mirror of <c>PowerCapacityMath.GetDamageMultiplier</c> for readers outside the
        /// Engineering domain (Axiom 5: they cannot import the pipeline math directly).
        /// </summary>
        public float DamageMultiplier
            => Math.Max(0f, 1f - OperationalDamagePercent - DisasterDamagePercent - ExplosionDamagePercent);

        /// <summary>
        /// Standing ruin: collapse, an active repair window, or accumulated damage that zeroes
        /// the multiplier — snapshot-side mirror of <c>PowerCapacityMath.IsKnockedOut</c> (same
        /// Axiom-5 rationale as <see cref="DamageMultiplier"/>).
        /// </summary>
        public bool IsKnockedOut
            => IsCollapsed || IsUnderRepair || DamageMultiplier <= 0f;
    ...
    public readonly struct PowerCapacitySnapshot : IEquatable<PowerCapacitySnapshot>
    {
        ...
        /// <summary>
        /// Capacity-loss decomposition over the published per-plant snapshots — the same
        /// sequential chain as the resolver's <c>[CapacityLoss]</c> debug aggregate (shared
        /// <see cref="Core.Logic.PowerLossBreakdown"/>), over the same plant set as
        /// <see cref="NameplateKW"/> plus knocked-out ruins (ruins are the first bucket).
        /// Consumers: PowerGrid UI "FLEET CAPACITY" block, telemetry (wave snapshot / grid
        /// stress events).
        /// </summary>
        public Core.Logic.PowerLossBreakdown ComputeLossBreakdown()
        {
            var losses = new Core.Logic.PowerLossBreakdown();
            for (int i = 0; i < Plants.Count; i++)
            {
                var plant = Plants[i];
                if (plant.Channel != CapacityChannel.GridProducer || plant.IsUnderConstruction)
                    continue;
                losses.Add(plant.OriginalCapacityKW, plant.IsKnockedOut,
                    plant.DamageMultiplier, plant.SaturationFactor, plant.FuelFactor);
            }
            return losses;
        }
        ...
        /// surcharge). Degradation and partial damage do NOT touch this, so a spam city cannot
        /// hide over-build behind its own degraded output. Read by Фаза 1 (ratio) and Фаза 7
        ...
        /// saturation toggle).
        ///
        /// NOTE: the saturation-target aggregate (resolver-internal ComputeFleetTargetFactor) is
        /// INTENTIONALLY different — it uses the damage-RESIDUAL nameplate so a strike-damaged
        /// fleet's degradation target eases toward what it can physically serve (otherwise the
        /// city sits in a static deficit until fully repaired). Do NOT "unify" the two: full here
        /// (enemy recon counts built plants), residual there (target must be servable).
        /// </summary>
        ...
        /// strike axis (WaveScheduler). The degradation axis (resolver-internal
        /// ComputeFleetTargetFactor) uses its own damage-RESIDUAL largest-plant value instead —
        /// see the NameplateKW note on the intentional full/residual split.
        /// </summary>
