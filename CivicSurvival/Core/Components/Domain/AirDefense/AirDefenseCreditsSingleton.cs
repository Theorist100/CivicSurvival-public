...
namespace CivicSurvival.Core.Components.Domain.AirDefense
{
    ...
    public struct AirDefenseCreditsSingleton : IComponentData
    {
        ...
        public float LastResupplyHourHawk;
        ...
        /// <summary>
        /// <see cref="CivicSurvival.Core.Components.Threats.WaveStateSingleton.WaveNumber"/> of the last Hawk
        /// emergency resupply. Like the Patriot, the Hawk is missile-armed and wave-gated
        /// (Economy.HawkResupplyCooldownWaves) so a mid-wave top-up cannot trivialize a drone wave.
        /// Persisted so a saved mid-wave gate survives load.
        /// </summary>
        public int LastResupplyWaveHawk;
        ...
        public readonly float GetLastResupplyHour(AAType type) => type switch
        {
            ...
            AAType.HawkSAM => LastResupplyHourHawk,
            _ => NoResupplyHour
        };
        ...
        public void SetLastResupplyHour(AAType type, float gameHour)
        {
            switch (type)
            {
                ...
                case AAType.HawkSAM: LastResupplyHourHawk = gameHour; break;
                default: throw new System.ArgumentOutOfRangeException(nameof(type), type, "Unhandled AAType");
            }
        }

        /// <summary>Wave of the last emergency resupply for a wave-gated missile type
        /// (Patriot, Hawk); <see cref="NoResupplyWave"/> for the freely-refilled gun types.</summary>
        public readonly int GetLastResupplyWave(AAType type) => type switch
        {
            AAType.PatriotSAM => LastResupplyWavePatriot,
            AAType.HawkSAM => LastResupplyWaveHawk,
            // Gun types carry no wave gate — they refill freely (ResupplyCooldownWaves 0).
            AAType.HeritageBofors or AAType.Bofors40mm or AAType.Gepard => NoResupplyWave,
            _ => throw new System.ArgumentOutOfRangeException(nameof(type), type, "Unhandled AAType")
        };

        public void SetLastResupplyWave(AAType type, int waveNumber)
        {
            switch (type)
            {
                case AAType.PatriotSAM: LastResupplyWavePatriot = waveNumber; break;
                case AAType.HawkSAM: LastResupplyWaveHawk = waveNumber; break;
                // Gun types carry no wave gate (their ResupplyCooldownWaves is 0) — no-op.
                default: break;
            }
        }
        ...
        public static AirDefenseCreditsSingleton Default => new()
        {
            ...
            LastResupplyHourHawk = NoResupplyHour,
            LastResupplyWavePatriot = NoResupplyWave,
            LastResupplyWaveHawk = NoResupplyWave
        };
