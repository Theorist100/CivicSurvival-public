...
namespace CivicSurvival.Core.Components.Domain.Cognitive
{
    ...
    public struct BuckwheatSingleton : IComponentData
    {
        ...
        private static void EnsureShape(EntityManager em, Entity entity)
        {
            ...
            // Derived projection of the active food-aid districts (rebuilt each tick by
            // BuckwheatSystem) — read by the cognitive resolver for Buckwheat's per-stratum
            // infection effect. Not serialized (rebuilt from m_DistrictAidExpiry on load).
            if (!em.HasBuffer<BuckwheatAidedDistrictBuffer>(entity))
                em.AddBuffer<BuckwheatAidedDistrictBuffer>(entity);
        }
