...
{
    internal static class BallisticLauncherInstallationLog
    {
        public static readonly LogContext Log = new("BallisticLauncherInstall");
    }

    /// <summary>
    /// A placed ballistic launcher — a field emplacement that outbound counter-strike ballistic
    /// rounds launch from instead of the map centre. Created on a separate mod entity (mirrors
    /// <see cref="DroneLauncherInstallation"/>) by <c>BallisticLauncherPlacementHandler.Commit</c>
    /// via the generic building-placement commit system; <see cref="Building"/> links the placed
    /// object so a demolished launcher drops out of the live set. Multi-instance: any number can
    /// be built, no single-HQ guard.
    ///
    ...
    /// </summary>
    public struct BallisticLauncherInstallation : IComponentData, ISerializable, IBuildingLinked
    {
        ...
        {
            if (!SerializationGuard.TryBeginBlock(reader, SAVE_VERSION, out var version, out var block, nameof(BallisticLauncherInstallation)))
            {
                BallisticLauncherInstallationLog.Log.Warn($"Deserialize: TryBeginBlock FAILED (expected v{SAVE_VERSION}) -> SetDefaults");
                SetDefaults();
                return;
            }
            try
            ...
            {
                BallisticLauncherInstallationLog.Log.Error($"Deserialize {nameof(BallisticLauncherInstallation)} failed: {ex}");
                SetDefaults();
            }
