...
namespace CivicSurvival.Core.Components.Domain.GridWarfare
{
    ...
    /// Each axis is the enemy's "health" along one dimension (floor..cap %), a DERIVED cache
    /// recomputed by <c>MirrorCitySystem</c> from the authoritative <see cref="EnemyTarget"/>
    /// buffer (a successful counter-strike damages a concrete target of the axis its attack
    /// category maps to: Kinetic→Physical, Cyber→Digital, Psyops→Social). Lower axes weaken
    /// waves. The enemy's defence is positional — the mirror city's AA sites — rolled per
    /// arriving outbound strike in <c>MirrorCitySystem.ApplyStrikeToTarget</c>.
    /// </summary>
    public struct EnemyState : IComponentData
    {
        ...
        ///
        /// This is initialization only. The returned axes are then the stored battle state,
        /// recomputed from the mirror city's targets once it generates (strike damage lowers
        /// them, target repair/rebuild raises them). The snapshot is a seed and is not
        /// consulted again.
        ///
        ...
        // ----------------------------------------------------------------------------
        // Respite + act-objective (the suppression → regroup → loot loop)
        // ----------------------------------------------------------------------------
