...
namespace CivicSurvival.Core.Components.AirDefense
{
    ...
    public struct Interceptor : IComponentData
    {
        ...
        /// <summary>Which AA fired it — <see cref="AAType.PatriotSAM"/> or <see cref="AAType.HawkSAM"/>.
        /// Read by InterceptorExhaustSystem to pick the exhaust plume (Hawk = lighter small fire).</summary>
        ...
        /// <summary>Set true at spawn; consumed (cleared) by <c>InterceptorRenderWriteJob</c> on the
        /// first movement tick, which then writes the FULL TransformFrame ring with a synthesized
        /// moving "emergence" history instead of one slot. Vanilla <c>UpdateGroupSystem</c> overwrites
        /// all 4 slots with static zero-velocity copies of the spawn transform on the Created frame
        /// (decompile UpdateGroupSystem.cs:471-483) — left as-is, the interpolation replays that
        /// static history for ~0.5-0.8&#160;s and the missile visibly hangs at the launcher. The one-shot
        /// re-init mirrors the same vanilla full-ring fill one tick later, with moving content.</summary>
        public bool NeedsRingInit;
    }
