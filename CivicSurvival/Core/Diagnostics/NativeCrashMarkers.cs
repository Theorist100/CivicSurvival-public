namespace CivicSurvival.Core.Diagnostics
{
    ...
    public static class NativeCrashMarkers
    {
        ...
        /// <summary>
        /// Coarse pre-reinit bracket for war-save load crashes. Set at the post-deserialize
        /// OnGameLoaded callback (ThreatLoadRenderReinitSystem) when restored threats are present,
        /// and handed off to <see cref="LoadRenderReinit"/> once the first ModificationEnd reinit
        /// begins. Prod shows war load-crashes fire in vanilla lib_burst BEFORE that reinit
        /// (18/18 carry no LoadRenderReinit), so this splits "before the load callback" (marker
        /// absent — early deserialize / prefab load / spatial hash) from "load callback → first
        /// ModificationEnd" (marker present — vanilla render of the restored threat). One disk
        /// write on entry, dropped at the reinit; only on loads that carry restored threats.
        /// </summary>
        public const string LoadThreatRestore = "Runtime.LoadThreatRestore";
