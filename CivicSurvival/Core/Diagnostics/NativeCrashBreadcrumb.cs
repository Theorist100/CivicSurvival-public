...
#pragma warning restore CIVIC003
        ...
        /// <summary>
        /// The in-flight risk regions, ';'-joined and sorted so the same membership always renders
        /// the same string (the set's own order is unspecified, and an unstable rendering would make
        /// identical states look different when grouped in telemetry).
        /// </summary>
        [CallerHoldsLock(nameof(s_Lock))]
        private static string BuildActiveRegionsLocked()
        {
            if (s_Active.Count == 0) return string.Empty;

            var ordered = new string[s_Active.Count];
            s_Active.CopyTo(ordered);
...
#pragma warning restore CIVIC003
            ...
            var builder = JsonBuilder.Object()
                ...
                // Full in-flight membership, not just the most-recent entrant: a resident marker
                // pins Marker for the whole session, so it cannot answer "was our code actually
                // computing when the process died". The set can.
                .Add("activeRegions", BuildActiveRegionsLocked())
                .Add("activeRegionCount", s_Active.Count.ToString(CultureInfo.InvariantCulture))
        ...
        private static bool TryParseRecord(string value, out NativeCrashBreadcrumbRecord record)
        {
            ...
            try
            {
                ...
                record = new NativeCrashBreadcrumbRecord(
                    ...
                    ReadString(values, "activeRegions"),
                    ReadInt(values, "activeRegionCount"));
                return true;
            }
    ...
    public readonly struct NativeCrashBreadcrumbRecord
    {
        ...
        // STICKY: set on every dispatch, never cleared, so it can name a job that finished waves ago.
        // Kept as a hint, not an attribution signal — ActiveRegions answers "was our code running".
        ...
        // Risk regions in flight at write time (';'-joined, sorted) and how many. The membership the
        // single Marker cannot show: a resident marker pins Marker for the whole session, so only this
        // separates "our jobs were mid-flight" from "we were idle and something else faulted".
        // Empty/0 for legacy files written before these fields existed.
        public readonly string ActiveRegions;
        public readonly int ActiveRegionCount;
            ...
            int activeRegionCount = 0)
        {
            ...
            ActiveRegions = activeRegions ?? string.Empty;
