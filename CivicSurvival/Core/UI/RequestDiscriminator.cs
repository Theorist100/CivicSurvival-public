...
namespace CivicSurvival.Core.UI
{
    ...
    public readonly struct RequestDiscriminator : IEquatable<RequestDiscriminator>
    {
        ...
        /// <summary>
        /// Session-local district index used only to deduplicate in-flight retained requests.
        /// It is a raw index (not remapped via DistrictKeyCodec) because it is a dedup key, not a
        /// persisted identity: after save/load the index shifts, so a persisted retained request's
        /// discriminator no longer matches a freshly issued one for the same district. Worst case is
        /// a single missed dedup (one extra request processed); retained requests are TTL-bounded and
        /// terminal, so correctness is unaffected. Rebuilding it from the live payload after load is
        /// deliberately not done — it would couple the dedup optimization to the entity remap.
        /// </summary>
