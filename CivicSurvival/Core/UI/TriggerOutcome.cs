...
namespace CivicSurvival.Core.UI
{
    ...
    public readonly struct TriggerOutcome
    {
        ...
        /// <summary>
        /// Reject the trigger. Pass the SAME discriminator the success path would carry
        /// whenever the consumer scopes its pending state by discriminator
        /// (<c>requestResultForTarget</c>): a rejection published without it never matches
        /// the scoped watcher, the action stays isPending forever and its single-flight
        /// guard silently swallows every further click on that key (GridWarfare
        /// operations incident, 2026-07-20).
        /// </summary>
        public static TriggerOutcome Reject(
            ReasonId reasonId,
        ...
        public static TriggerOutcome RejectRuntime(
            string reasonId,
            string discriminatorKind = "none",
            string discriminatorValue = "") =>
            Reject(ReasonId.FromRuntime(reasonId), discriminatorKind, discriminatorValue);
        ...
        public static void Invoke(string key, string triggerName, Func<TriggerOutcome> handler)
        {
            ...
            switch (outcome.Tag)
            {
                ...
                case TriggerOutcome.OutcomeKind.Reject:
                    RequestResultBridge.RejectNew(
                        key,
                        outcome.ReasonIdFs.ToString(),
