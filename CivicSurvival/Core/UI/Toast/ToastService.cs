...
namespace CivicSurvival.Core.UI.Toast
{
    ...
    public sealed class ToastService : IDisposable
    {
        ...
        public struct ToastData
        {
            ...
            public int DedupKey;
            // Offer/Notice discriminator (rides into ToastDataDto.HasActions): offers render
            // Accept/Decline and justify the Trust header; notices are neutral notifications.
...
#pragma warning restore CA1003
        ...
        /// <summary>
        /// Drop everything that belongs to the city being left: queued and visible toasts, plus
        /// the offer/popup cooldowns (they are measured in GAME days, so another city's day
        /// count reads as a wrong pause here).
        ///
        /// The queue is city state living in a process-lifetime service. Until this method
        /// existed it was emptied only by <see cref="Dispose"/> — that is, by the owning
        /// system's OnDestroy, which CS2 never runs between cities because it reuses system
        /// instances. A quest call from the previous city therefore surfaced in the next one,
        /// and its button did nothing: QuestSystem drops its pending-offer ids on load, so
        /// there was no longer anything to answer the click.
        /// </summary>
        public void ClearSessionState()
        {
            ExpireAllToasts();
            m_LastOfferGameDay = float.MinValue;
            m_LastPopupGameDay = float.MinValue;
            m_NeedsCooldownInit = true;
        }
            ...
            int dedupKey = 0)
        {
            ...
            int toastId = EnqueueCore(type, title, message, acceptLabel, rejectLabel,
                priority, contextData, dedupKey, hasActions: true);
            if (toastId > 0)
            {
        ...
        /// <summary>
        /// Queue a non-corruption offer: Accept/Decline buttons like a corruption toast, but
        /// OUTSIDE the corruption pacing — it neither obeys MinDaysBetweenAnyPopup/Offers nor
        /// pushes their windows (a quest call must not be silently swallowed by bribe pacing,
        /// nor delay the next bribe). Expiry semantics are the caller's: subscribe to
        /// OnToastExpired/OnToastInteracted and match the returned id.
        /// </summary>
        public int QueueOffer(
            ToastType type,
            string title,
            string message,
            string acceptLabel,
            ...
            int dedupKey = 0)
        {
            return EnqueueCore(type, title, message, acceptLabel, rejectLabel,
                priority, contextData, dedupKey, hasActions: true);
        }

        /// <summary>
        /// Queue a neutral notice: title + message + dismiss, no Accept/Decline actions and
        /// no Trust header on the UI side. Shares the dedup, priority and bounded-queue
        /// machinery with offers but deliberately skips the corruption popup/offer cooldowns
        /// AND does not push m_LastPopupGameDay — a quest announcement must neither be
        /// suppressed by bribe-pacing config nor delay the next corruption offer.
        /// </summary>
        public int QueueNotice(
            ToastType type,
            string title,
            string message,
            ToastPriority priority = ToastPriority.Normal,
            ...
            int dedupKey = 0)
        {
            return EnqueueCore(type, title, message, acceptLabel: string.Empty,
                rejectLabel: string.Empty, priority, contextData, dedupKey, hasActions: false);
        }

        // Shared tail of both entry points: dedup → bounded queue → active/pending insert.
        private int EnqueueCore(
            ToastType type,
            string title,
            string message,
            string acceptLabel,
