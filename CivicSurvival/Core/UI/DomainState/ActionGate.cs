...
namespace CivicSurvival.Core.UI.DomainState
{
    public enum ActionKey
    {
        ...
        PoliceBribe = 13,
        DraftDeferralPreset = 14,
        ConstructionKickbackPreset = 15
    }
    ...
    public static class ActionGate
    {
        public static ActionAvailabilityField Resolve(ActionKey key, in ActionContext ctx)
        {
            return key switch
            {
                ...
                // Shadow economy and corruption run from PreWar: the offshore cash you
                // accumulate before the war is exactly what you spend once it starts, and
                // corruption is not a wartime-only mechanic. Money-side checks (wallet
                // present / not frozen / affordable) stay; only the Act.Crisis lock is gone.
                ActionKey.BackupModernization => ActionAvailabilityField.Allow(),
                ActionKey.ShadowTrade => ActionAvailabilityField.Allow(),
                ActionKey.BuckwheatProcurement => ActionAvailabilityField.Allow(),
                ActionKey.CivilianRepair => RequirePhaseSafe(ctx),
                ActionKey.PlantRepair => RequirePhaseSafe(ctx),
                ActionKey.ShadyContractAccept => RequireWalletAvailable(ctx)
                    .And(RequireWalletNotFrozen(ctx))
                    .And(RequireAffordable(ctx)),
                ActionKey.EmergencyFundPreset => RequireWalletAvailable(ctx)
                    .And(RequireWalletNotFrozen(ctx)),
                ActionKey.FuelSiphonPreset => RequireWalletAvailable(ctx)
                    .And(RequireWalletNotFrozen(ctx)),
                ActionKey.DraftDeferralPreset => RequireWalletAvailable(ctx)
                    .And(RequireWalletNotFrozen(ctx)),
                ActionKey.ConstructionKickbackPreset => RequireWalletAvailable(ctx)
                    .And(RequireWalletNotFrozen(ctx)),
                ActionKey.ShadowImport => RequireWalletAvailable(ctx)
                    .And(RequireShadowImportSpendAllowed(ctx)),
                ActionKey.ShadowExport => RequireWalletAvailable(ctx)
                    .And(RequireWalletNotFrozen(ctx)),
                ActionKey.InvestigationBribe => RequireWalletAvailable(ctx)
                    .And(RequireWalletNotFrozen(ctx))
                    .And(RequireAffordable(ctx)),
                ActionKey.PoliceBribe => RequireWalletAvailable(ctx)
