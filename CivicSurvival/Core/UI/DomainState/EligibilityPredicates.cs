...
namespace CivicSurvival.Core.UI.DomainState
{
    public static class WaveStateEligibility
    {
        ...
        {
            if (waveState.CurrentPhase == GamePhase.Attack || waveState.CurrentPhase == GamePhase.PsyAttack || waveState.CurrentPhase == GamePhase.Alert)
            {
    ...
    public static class CityScheduleEligibility
    {
        ...
        {
            if (currentPhase == GamePhase.Attack || currentPhase == GamePhase.PsyAttack || currentPhase == GamePhase.Alert)
            {
                reasonId = ReasonIds.RepairBlockedDuringWave;
                return false;
            }
    ...
    public static class AirDefenseEligibility
    {
        ...
        // Placement gates. The roster feeds every option through these directly: an option is
        // credit-funded (a reserve grant / donor credit) or bought. There used to be a named
        // wrapper per card here — CanPlaceHeritageBofors, CanPlacePaidGepard and so on — solely
        // because DtoEligibility binds a DTO field to a predicate by nameof, so each card needed
        // its own entry point into identical logic. The roster carries its can/reason pair per
        // row now, so the wrappers had no callers left.

        // Every crew gate below takes the free-manpower figure together with the flag that says
        // whether anyone measured it. The flag sits immediately before the number on purpose: a
        // caller that swaps the two positionally gets a compile error instead of a silent
        // "unmeasured pool == zero people", which is precisely the substitution this phase exists
        // to remove.

        public static bool CanPlacePaidAA(int price, bool manpowerMeasured, int availableManpower, int crewRequired, World world, out string reasonId)
        {
            if (price <= 0)
            {
                reasonId = ReasonIds.AaConfigError;
            ...
            if (!CanMeetAACrew(manpowerMeasured, availableManpower, crewRequired, out reasonId))
                return false;
        ...
        /// <summary>
        /// Placement gate for a black-market AA: same crew rule, but the shadow wallet pays. A
        /// frozen wallet reads as its own reason rather than "no funds" — the money is there, the
        /// investigation is what blocks it, and telling the player otherwise sends them hunting for
        /// cash they already have.
        /// </summary>
        public static bool CanPlaceShadowAA(int price, bool manpowerMeasured, int availableManpower, int crewRequired,
            IShadowWalletService wallet, out string reasonId)
        {
            if (price <= 0)
            {
                reasonId = ReasonIds.AaConfigError;
            ...
            if (wallet.IsFrozen)
            {
                reasonId = ReasonIds.AaShadowWalletFrozen;
                return false;
            }

            if (!wallet.IsOperational || wallet.Balance < price)
            {
                reasonId = ReasonIds.AaInsufficientShadowFunds;
                return false;
            }
        ...
        /// <summary>Shadow-funded counterpart of <see cref="AffordablePaidAACount"/>.</summary>
        public static int AffordableShadowAACount(int price, bool manpowerMeasured, int availableManpower, int crewRequired,
            IShadowWalletService wallet)
        {
            // Unmeasured pool: no count can be stated. Zero is the honest answer for a COUNT
            // (the panel shows nothing available), and it is paired with the CanPlace* gate
            // above returning the unavailable reason rather than "not enough people".
            if (!manpowerMeasured)
                return 0;

            if (price <= 0 || crewRequired <= 0 || !wallet.IsOperational || wallet.IsFrozen)
                return 0;

            int manpowerLimit = availableManpower / crewRequired;
            long walletLimit = wallet.Balance > 0 ? wallet.Balance / price : 0;
            long affordable = System.Math.Max(0L, System.Math.Min(manpowerLimit, walletLimit));
        ...
        /// </summary>
        public static int AffordablePaidAACount(int price, bool manpowerMeasured, int availableManpower, int crewRequired, World world)
        {
            // See AffordableShadowAACount: an unmeasured pool states no count.
            if (!manpowerMeasured)
                return 0;
        ...
        public static bool CanPlaceCreditAA(int credits, bool manpowerMeasured, int availableManpower, int crewRequired, out string reasonId)
        {
            if (credits <= 0)
            {
            ...
            return CanMeetAACrew(manpowerMeasured, availableManpower, crewRequired, out reasonId);
        }

        /// <summary>
        /// The crew half of every placement gate. An unmeasured pool is refused with its OWN
        /// reason: the roster is a display and a gate at once, and before this it announced
        /// "insufficient manpower" on a city nobody had counted — the same defect the placement
        /// handler carries, only rendered on the shop front instead of at the click.
        /// </summary>
        private static bool CanMeetAACrew(bool manpowerMeasured, int availableManpower, int crewRequired, out string reasonId)
        {
            if (!manpowerMeasured)
            {
                reasonId = ReasonIds.MobStateUnavailable;
                return false;
            }
    ...
    public static class BuckwheatEligibility
    {
        ...
        // Buckwheat is a shadow-economy purchase — the offshore wallet always funds it
        // (the cynical loop: steal offshore -> buy "humanitarian aid"). Kept as the UI
        // "shadow funded" indicator; the earlier municipal-budget fallback was a
        // temporary stand-in and no longer routes any procurement.
        public static bool ShadowFundingLive
            => FeatureRegistry.IsInitialized
               && FeatureRegistry.Instance.IsAvailable(FeatureIds.ShadowEconomyName);

        // Buckwheat procurement always draws the offshore shadow wallet.
        public static string ProcurementBudgetCategory => BudgetCategory.ShadowOps;

        // Affordability against the offshore shadow wallet.
        private static bool CanAffordProcurementFunds(int baseCost)
            => ServiceRegistryFeatureExtensions.TryGetOrNullObject(NullShadowWalletService.Instance).CanAffordWithPending(baseCost).Affordable;

        public static bool CanAffordProcurement(int procurementLevel, out string reasonId)
            => CanAffordProcurement(procurementLevel, intervalsDue: BuckwheatProcurementAffordabilityIntervals, out reasonId, out _);
            ...
            out int baseCost)
        {
            ...
            if (!CanAffordProcurementFunds(baseCost))
            {
        ...
        public static bool CanSetProcurement25(int currentLevel, out string reasonId)
            => CanSelectProcurementLevel(ProcurementLevel25, currentLevel, out reasonId);

        public static bool CanSetProcurement50(int currentLevel, out string reasonId)
            => CanSelectProcurementLevel(ProcurementLevel50, currentLevel, out reasonId);

        public static bool CanSetProcurement75(int currentLevel, out string reasonId)
            => CanSelectProcurementLevel(ProcurementLevel75, currentLevel, out reasonId);

        public static bool CanSetProcurement100(int currentLevel, out string reasonId)
            => CanSelectProcurementLevel(ProcurementLevel100, currentLevel, out reasonId);

        public static bool CanSelectProcurementLevel(int targetLevel, int currentLevel, out string reasonId)
        {
            ...
            return CanAffordProcurement(targetLevel, out reasonId);
        }
    ...
    public static class PlantRepairEligibility
    {
        public static EligibilityFlag ForPlantRepair(
            ...
            World world)
        {
            if (currentPhase == GamePhase.Attack || currentPhase == GamePhase.PsyAttack || currentPhase == GamePhase.Alert || currentPhase == GamePhase.Recovery)
            {
    ...
    public static class NewsEligibility
    {
        /// <summary>
        /// Declarative nickname-editing gate. The nickname only exists server-side (it is
        /// registered against the auth token), so Online off means the value is unreachable —
        /// mirrors the refusals in GlobalNewsSystem.OnSetNickname and
        /// TelemetryAuth.RegisterNicknameAsync, which both gate on Online alone.
        ///
        /// Owns its own reason rather than borrowing the diagnostics gate: the nickname is
        /// functional server identity, not telemetry, and must not go dark the day the
        /// diagnostics toggle locks for a reason of its own.
        /// </summary>
        public static bool CanEditNickname(bool online, out string reasonId)
        {
            reasonId = online ? "" : ReasonIds.SettingsNicknameNeedsOnline;
            return online;
        }
    }
