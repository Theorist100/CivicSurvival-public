...
namespace CivicSurvival.Domains.AirDefense.Placement
{
    ...
    public sealed class AABuildingPlacementHandler : IBuildingPlacementHandler
    {
        ...
        public bool TryPrepareActivation(World world, PrefabBase prefab, Entity prefabEntity, byte mode, out ReasonId reasonId, out string message)
        {
            ...
            var crewCheck = manpower.CanRecruit(crewRequired);
            if (crewCheck == ManpowerCheck.PoolUnmeasured)
            {
                // An unmeasured pool is not a shortage of people, and the two must not leave here
                // as one answer. The placement is still refused — authorising a commitment
                // against a number nobody has read is the irreversible half of this decision —
                // but the reason says the mobilization state is unavailable, and NO funnel event
                // is published: the funnel counts attempts the city could not afford in people,
                // and folding non-answers into that number would inflate the very metric the
                // manpower lever is tuned against.
                reasonId = ReasonIds.MobStateUnavailable;
                message = $"Manpower pool unmeasured: cannot authorise {crewRequired} crew";
                return false;
            }

            if (crewCheck != ManpowerCheck.Ok)
            {
                int available = manpower.TryGetAvailableManpower(out int free) ? free : 0;
                reasonId = ReasonIds.AaInsufficientManpower;
                message = $"Insufficient manpower: {crewRequired} required, {available} available";
                // Funnel telemetry: attempt blocked by manpower — the resource lever this ties into.
                PublishPlacementResult(false, PLACE_REASON_MANPOWER, prefabData.Type, 0, crewRequired);
                return false;
            }
            ...
            out BuildingPlacementAbort abort)
        {
            ...
            if (requiresBudget)
            {
                if (boughtOnMarket)
                {
                    ...
                    // Crew was already settled by TryPrepareActivation (which refuses an
                    // unmeasured pool outright), so this call is asking only about the wallet:
                    // manpowerMeasured: true with an unbounded pool keeps the crew clause a
                    // no-op instead of re-deciding it against a second reading.
                    if (!AirDefenseEligibility.CanPlaceShadowAA(
                            sourceP.Price, manpowerMeasured: true, int.MaxValue, sourceP.CrewRequired, marketWallet, out var marketReason))
                    {
                        abort = new BuildingPlacementAbort(ReasonIds.AaBudgetFailed,
                            ...
                            deleteGhost: true);
                        // Funnel telemetry: attempt blocked by funds.
                        PublishPlacementResult(false, PLACE_REASON_BUDGET, prefabData.Type, sourceP.Price, sourceP.CrewRequired);
                        return false;
                    }
                }
        ...
        public void Commit(World world, EntityCommandBuffer ecb, Entity intentEntity, in BuildingPlacementIntent intent)
        {
            ...
            // Funnel telemetry: installation placed.
            PublishPlacementResult(true, PLACE_REASON_NONE, payload.ResolvedType, paidBudget, payload.CrewRequired);
        }
        ...
        // ===== Air-defense funnel telemetry =====
        // Reason codes (snake_case) for AirDefensePlacedEvent — the listener records them verbatim.
        private const string PLACE_REASON_NONE = "none";
        private const string PLACE_REASON_MANPOWER = "insufficient_manpower";
        private const string PLACE_REASON_BUDGET = "insufficient_budget";

        // Emit the funnel's action step: attempt (every call) vs success vs the resource that blocked
        // it. SafePublish is null-tolerant (bus may not be registered yet). AaType goes as the raw enum
        // name — the telemetry listener snake-cases it, keeping Services.Telemetry out of this domain
        // handler (Axiom 5).
        private static void PublishPlacementResult(bool success, string reason, AAType aaType, int cost, int crew)
        {
            ServiceRegistry.TryGet<IEventBus>().SafePublish(
                new AirDefensePlacedEvent(success, reason, aaType.ToString(), cost, crew),
                nameof(AABuildingPlacementHandler));
        }
    }
}
