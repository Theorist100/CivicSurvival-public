...
using CivicSurvival.Domains.AirDefense.Placement;
...
namespace CivicSurvival.Domains.AirDefense
{
    ...
    {
        // Mobilization is a hard dependency (locked). Without crew assignment
        // AA cannot fire — base loop fails.
        ...
        public void RegisterSystems(UpdateSystem updateSystem)
        {
            ...
            // AA placement now runs through the generic Core building-placement pipeline
            // (BuildingPlacement* command/detector/payment/commit/lifecycle systems, registered
            // by SystemRegistrar in ModificationEnd — pause-safe, Axiom 14). This domain only
            // registers the AA handler that supplies the building-specific work: prefab marker +
            // crew pre-check, credit/budget resolution, city-scaled magazine, AirDefenseInstallation
            // commit, and credit/budget refunds.
            BuildingPlacementHandlerRegistry.Register(new AABuildingPlacementHandler());
            ...
            // request infrastructure, owned by AirDefense lifecycle.
            // Emergency resupply is a player command — pause-safe ModificationEnd route
            // (Axiom 14): synchronous payment + same-tick ammo apply. The automatic
            // trickle refill keeps its retained batch pipeline in GameSimulation.
            updateSystem.RegisterAt<AirDefenseActionRequestSystem>(SystemUpdatePhase.ModificationEnd);
