...
namespace CivicSurvival.Domains.AirDefense.UI
{
    ...
    public partial class AirDefenseUISystem : CivicUIPanelSystem
    {
        ...
        private void FillAAPlacementRoster(ref AirDefenseDto dto)
        {
            ...
            bool manpowerMeasured;
            using (PerformanceProfiler.Measure("SP:ADUI.Mobilization"))
            {
                // This roster is a display AND a gate, so it reads both halves of the mirror.
                // Default carries IsPoolMeasured=false, which is what makes an absent singleton
                // read as "unknown" instead of as a city with nobody left — the roster used to
                // announce "insufficient manpower" on that fallback.
                var mobState = m_MobilizationQuery.TryGetSingleton<MobilizationStateSingleton>(out var mob)
                    ? mob
                    : MobilizationStateSingleton.Default;
                availableManpower = mobState.AvailableManpower;
                manpowerMeasured = mobState.IsPoolMeasured;
            }
            ...
            foreach (var option in AAPlacementCatalog.All)
            {
                ...
                if (creditsLeft >= 0)
                {
                    canPlace = AirDefenseEligibility.CanPlaceCreditAA(
                        creditsLeft, manpowerMeasured, availableManpower, p.CrewRequired, out reasonId);
                    // Credits are a grant counter, not a manpower figure — it stays truthful
                    // while the pool is unknown, and the row is already gated by canPlace.
                    affordable = creditsLeft;
                }
                else if (option.Mode == AAPlacementMode.BlackMarket)
                {
                    canPlace = AirDefenseEligibility.CanPlaceShadowAA(
                        p.Price, manpowerMeasured, availableManpower, p.CrewRequired, wallet, out reasonId);
                    affordable = AirDefenseEligibility.AffordableShadowAACount(
                        p.Price, manpowerMeasured, availableManpower, p.CrewRequired, wallet);
                }
                else
                {
                    canPlace = AirDefenseEligibility.CanPlacePaidAA(
                        p.Price, manpowerMeasured, availableManpower, p.CrewRequired, World, out reasonId);
                    affordable = AirDefenseEligibility.AffordablePaidAACount(
                        p.Price, manpowerMeasured, availableManpower, p.CrewRequired, World);
                }
