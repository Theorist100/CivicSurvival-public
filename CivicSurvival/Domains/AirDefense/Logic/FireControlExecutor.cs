...
namespace CivicSurvival.Domains.AirDefense.Logic
{
    ...
    internal struct FireControlExecutor
    {
        ...
        /// <summary>
        /// Upper bound on barrels assigned to one threat in a single pass. The actual limit scales
        /// down with threat density (ready barrels / threats), so a busy sky still spreads fire;
        /// this ceiling only stops a whole battery from dumping into one drone when the sky is empty.
        /// </summary>
        private const int MAX_BARRELS_PER_TARGET = 3;
        ...
        public FireControlResult Execute(in FireControlContext ctx)
        {
            ...
            // Barrels already assigned to each threat this pass, indexed by ThreatIndex directly
            // (bounds-checked below before use). Replaces the old one-barrel-per-target set: with
            // fewer threats than ready barrels that rule left most of the battery standing mute,
            // which reads as a broken mod — a lone drone crossing five loaded guns drew exactly one
            // of them. The cap still prevents the whole battery dumping into a single target when
            // other threats need covering.
            var targetAssignments = new NativeArray<int>(ctx.ThreatData.Length, Allocator.Temp);
            int barrelsPerTarget = math.clamp(
                ctx.AAData.Length / math.max(1, ctx.ThreatData.Length),
                1,
                MAX_BARRELS_PER_TARGET);
            ...
            try
            {
                for (int i = 0; i < sortedCandidates.Length; i++)
                {
                    ...
                    // Bounds check for N-1 data, hoisted above the per-target assignment lookup
                    // because that lookup indexes by ThreatIndex. Both sides: the match job only
                    // ever emits non-negative indices, so a negative here means a torn/corrupt
                    // candidate (same memory-corruption class as the >= overrun) — indexing with
                    // it is a native AV, not an exception.
                    if (candidate.AAIndex < 0 || candidate.AAIndex >= ctx.AAData.Length ||
                        candidate.ThreatIndex < 0 || candidate.ThreatIndex >= ctx.ThreatData.Length)
                        continue;

                    if (targetAssignments[candidate.ThreatIndex] >= barrelsPerTarget) continue;
...
#pragma warning restore CIVIC035
                            ...
                            m_EventBus?.SafePublish(new AAFireEvent(aaData.Position, threatData.Position, aaData.Type, fpAaEntity.Index, AARotation: aaData.Rotation), "FireControl");
                    ...
                    int shot = TryIntercept(aaData, threatData, liveAA, ctx.SpotterPenalty, ctx.DetectionBonus, ctx.IdentifiedTargetBonus, ctx.LeakSuppression);
                    shotsFired += shot;
                    if (shot > 0)
                    {
                        shotsByType[(int)aaData.Type] += shot;
                        firedAAIndices.Add(candidate.AAIndex);
                        targetAssignments[candidate.ThreatIndex]++;
                    }
                }
            }
            finally
            {
                ...
                if (targetAssignments.IsCreated) targetAssignments.Dispose();
            }
            ...
            return new FireControlResult
            {
                ...
                ShotsByType = new AirDefenseShotsByType(
                    ...
                    shotsByType[(int)AAType.HawkSAM]),
        ...
        private bool TryGetLiveThreat(Entity threatEntity, out Shahed shahed, out ShahedCombatState combatState)
        {
            ...
            if (m_DeletedLookup.HasComponent(threatEntity)
                ...
                return false;
            if (combatState.IsIntercepted || shahed.IsArrived)
                return false;

            return true;
        }

        private int TryIntercept(AAData aaData, ThreatData threatData, AirDefenseInstallation aa, float spotterPenalty, float detectionBonus, float identifiedTargetBonus, float leakSuppression)
        {
...
#pragma warning restore CIVIC035
            ...
            m_EventBus?.SafePublish(new AAFireEvent(aaData.Position, threatData.Position, aaData.Type, aaEntity.Index, threatEntity.Index, threatEntity.Version, AARotation: aaData.Rotation), "FireControl");
            ...
            // Wave leak regulator, applied LAST so no later bonus can push the wave back over its
            // target kill share. The multiplier is bounded below by Waves.LeakRegulatorMinMultiplier
            // — it never reaches 0, so this target stays killable and the battery is never firing a
            // shot that cannot possibly land. Computed once per pass in FireControlCoordinator.
            chance *= leakSuppression;
            ...
            if (Log.IsDebugEnabled)
                Log.Debug($"[AA:SHOT] {aaData.Type} aa={aaEntity.Index} threat={threatEntity.Index} rounds={rounds} chance={chance:P0} sup={leakSuppression:F2} roll={roll:F2} -> {(hit ? "HIT" : "MISS")} ammo={aa.CurrentAmmo}");
            ...
            else
            {
                ...
                liveCombatState.MissedShotsCount++;
                // Leak-attribution funnel: a resolved shot (even a miss) means this drone was engaged.
                liveCombatState.WasEngaged = true;
...
#pragma warning restore CIVIC035
            ...
            // InterceptProcessingSystem publishes ThreatInterceptEvent for this request. The kill
            // is already decided here — the wave's kill share was applied to the roll above, so
            // the consumer never revokes it.
        }
