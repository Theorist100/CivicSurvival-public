        ...
        public FireControlResult Execute(in FireControlContext ctx)
        {
            ...
            try
            {
                for (int i = 0; i < sortedCandidates.Length; i++)
                {
                    ...
                    // Bounds check for N-1 data. Both sides: the match job only ever emits
                    // non-negative indices, so a negative here means a torn/corrupt candidate
                    // (same memory-corruption class as the >= overrun) — indexing with it is a
                    // native AV, not an exception.
