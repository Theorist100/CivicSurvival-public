...
#pragma warning restore CA1001
    {
        ...
        public JobHandle ScheduleJobChain(JobHandle dependency, in CollectThreatJobHandles collectHandles)
        {
            ...
            // Leak-attribution funnel note: WasCandidate is deliberately NOT stamped by a job in
            // this chain. A job-writer of ShahedCombatState would break the documented invariant
            // that no Burst job writes that component (PERF-LOCK in InterceptProcessingSystem —
            // main-thread RO readers would start force-completing this chain). The orchestrator
            // stamps the bit on the main thread while consuming the N-1 snapshot instead.
