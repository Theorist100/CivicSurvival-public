...
#pragma warning restore CIVIC145
            ...
            ApplyForceCrewReleaseRequests(ecb, releaseEntries);
        ...
        private void ProcessResupplyRequests(EntityCommandBuffer ecb)
        {
                ...
                .WithEntityAccess())
            {
                ...
                // Terminal guard (BuildingPlacementIntent doctrine): destroy below is a
...
#pragma warning restore CIVIC035
        ...
        private void ApplyForceCrewReleaseRequests(EntityCommandBuffer ecb, NativeList<ForceReleaseEntry> releaseEntries)
        {
                ...
                .WithEntityAccess())
            {
                ...
                if (TryTakeReleaseEntry(releaseEntries, aaEntity, out int newCrew))
                {
                    ...
                    // The trim leaves the gun SHORT, and nothing else re-queues a gun that still
                    // holds part of its crew — without re-tagging here the shortfall waits for a
                    // save/load, so every dip of the manpower pool ratchets the fleet down for
                    // good. Tagging at the exact transition costs nothing per tick and lets
                    // AACrewAssignmentSystem recruit the difference back the moment the pool
                    // recovers. Persisted CrewRequired is authoritative; the config lookup is the
                    // legacy-save fallback, same rule as the post-load retag.
                    int crewRequired = aa.ValueRO.CrewRequired > 0
                        ? aa.ValueRO.CrewRequired
                        : AAParams.ForType(BalanceConfig.Current, aa.ValueRO.Type).CrewRequired;
                    if (newCrew < crewRequired)
                    {
                        ecb.AddComponent(aaEntity, new RequestCrewTag { CrewRequired = crewRequired });
                        IncrementEcbCount();
                    }
                }
            }
