        ...
        private static TracerConfig GetTracerConfig(AAType aaType)
        {
            ...
            switch (aaType)
            {
                ...
                case AAType.HeritageBofors:
                    break; // heritage config below
                default:
                    // Missile launchers (Patriot, Hawk) never reach here — OnAAFire returns early for
                    // FiresInterceptorMissile types (they spawn interceptors, not tracers). An arrival
                    // means that gate was bypassed: log it and fall back to the heritage tracer rather
                    // than crash the sim (CIVIC172) or draw a silent wrong tracer.
                    Log.Error($"GetTracerConfig reached for {aaType} (should be gun-only) — using heritage fallback");
                    break;
            }
            return new TracerConfig
            {
