...
namespace CivicSurvival.Domains.AirDefense.Systems
{
    ...
    public partial class InterceptorMovementSystem : CivicSystemBase
    {
        ...
        // Heading treated as "unset" below this squared length → fallback: aim straight at the target.
        // Normally never hit — spawn seeds the heading with the pitched-up launch direction.
        private const float MIN_DIR_SQ = 1e-6f;

        // Boost ramp: the missile leaves the cell slow and reaches cruise speed over the ramp. Purely
        // visual pacing — at full 450 m/s the whole pitched-up climb fits inside the OIS render lag
        // (the 4 pre-filled TransformFrame slots hold the spawn pos for ~2-3 ticks), so the launch
        // read as "hangs, then teleports off on the chase course". A slow boost keeps the climb on
        // screen after the lag window and reads as a real vertical launch + tip-over. Tune by eye.
        private const float BOOST_INITIAL_SPEED = 90f;
        private const float BOOST_DURATION_SECONDS = 1f;

        // Proximity fuse radius, in per-tick steps. The capped turn (35°/tick) gives the missile
        // a discrete turning circle of radius ≈ step / (2·sin(17.5°)) ≈ 1.66×step; a target point
        // that ends up INSIDE that circle (typical when the chased threat dies to another shot and
        // the missile falls back to the fixed last-known position while passing it laterally) can
        // never satisfy the dist <= step arrival check — the missile orbits the point with its
        // burning exhaust until the lifetime cap, reading as a fire circling low over the city.
        // Real missiles detonate at closest approach instead: once the target is behind the
        // current heading within this radius, the approach has been passed — count it as arrival.
        // 2× step covers the 1.66×step orbit radius with margin.
        private const float PROXIMITY_FUSE_STEPS = 2f;
        ...
        protected override void OnUpdateImpl()
        {
                         ...
                         .WithNone<Deleted>())
            {
                ...
                // Boost ramp (see BOOST_INITIAL_SPEED). Age from LaunchFrame in sim seconds —
                // FIXED_TIME_STEP / UPDATE_INTERVAL is the per-sim-frame second, no magic 1/60.
                float ageSeconds = (frameIndex - interceptor.LaunchFrame) * (FIXED_TIME_STEP / UPDATE_INTERVAL);
                float speed = math.lerp(BOOST_INITIAL_SPEED, interceptor.Speed,
                    math.saturate(ageSeconds / BOOST_DURATION_SECONDS));
                float step = speed * dt;

                // Smooth turn (Shahed-style): rotate the persisted heading toward the target by a capped
                // angle per tick — an arc, not an instant snap. The heading is seeded at spawn with the
                // pitched-up launch direction (boost climb); zero-heading is a safety fallback only.
                ...
                // Arrival = direct hit distance OR proximity fuse (closest approach passed:
                // target behind the heading within the fuse radius — see PROXIMITY_FUSE_STEPS).
                bool reached = dist <= step
                    || (dist <= step * PROXIMITY_FUSE_STEPS && math.dot(dir, toTarget) <= 0f);
                float3 newPos = reached ? target : current + dir * step;     // snap on arrival
                float3 velocity = reached ? float3.zero : dir * speed;
