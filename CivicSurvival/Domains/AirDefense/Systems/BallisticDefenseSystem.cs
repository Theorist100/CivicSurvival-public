...
namespace CivicSurvival.Domains.AirDefense.Systems
{
    ...
    public partial class BallisticDefenseSystem : CivicSystemBase, IPostLoadValidation
    {
        ...
        // Wave leak regulator inputs — kills and impacts so far this wave.
        ...
        private void ProcessBuildingPatriotEngagements()
        {
            ...
            // Wave leak regulator — same set point and same counters the drone side reads
            // (FireControlCoordinator.ReadModifiers), so a wave's kill share is held across both
            // weapon classes instead of each holding its own ceiling. Read once per pass.
            var wavesCfg = BalanceConfig.Current.Waves;
            int wavesIntercepted = m_InterceptStatsQuery.TryGetSingleton<InterceptStatsSingleton>(out var iStats)
                ? iStats.InterceptedCount : 0;
            // CoveredHitsCount — same denominator the drone side uses; see the note in
            // FireControlCoordinator.ReadModifiers on why raw hits would invert the incentives.
            int wavesImpacted = m_ThreatOutcomeStatsQuery.TryGetSingleton<ThreatOutcomeStatsSingleton>(out var oStats)
                ? oStats.CoveredHitsCount : 0;
            float leakSuppression = LeakRegulatorLogic.SuppressionMultiplier(
                wavesIntercepted,
                wavesImpacted,
                wavesCfg.MaxWaveInterceptFraction,
                wavesCfg.LeakRegulatorStrength,
                wavesCfg.LeakRegulatorMinMultiplier,
                wavesCfg.LeakRegulatorSmoothing);
            ...
            for (int a = 0; a < liveAA.Length; a++)
            {
                ...
                // cached at rebuild; capability never mutates live).
                ...
                for (int i = 0; i < ballisticCount; i++)
                {
                    ...
                    if (distSq <= rangeSq)
                    {
                        // Leak-attribution funnel: a ready Patriot has this missile in range —
                        // if it still lands unengaged, that is a throughput loss, not a coverage gap.
                        if (!liveInterceptState.WasCandidate)
                        {
                            liveInterceptState.WasCandidate = true;
                            m_InterceptStateLookup[ballisticEntity] = liveInterceptState;
                        }
                ...
                if (closestBallistic != Entity.Null)
                {
                    ...
                    TryInterceptBallistic(now, ref aa, ref cd, aaPos, aaData.Rotation, aaEntity.Index, closestBallistic, closestPos, spotterPenalty, leakSuppression);
        ...
        /// </summary>
        private void TryInterceptBallistic(double now, ref AirDefenseInstallation aa, ref AirDefenseCooldown cd, float3 aaPos, quaternion aaRotation, int aaEntityIndex, Entity ballisticEntity, float3 ballisticPos, float spotterPenalty, float leakSuppression)
        {
            if (!m_InterceptStateLookup.TryGetComponent(ballisticEntity, out var currentState)
                || currentState.IsIntercepted)
            {
                if (Log.IsDebugEnabled) Log.Debug($"Ballistic intercept skipped: missing/already intercepted state on {ballisticEntity.Index}");
                return;
            }

            // Leak-attribution funnel: a Patriot shot is being resolved against this missile
            // (hit or miss decided below) — engaged either way.
            currentState.WasEngaged = true;
            m_InterceptStateLookup[ballisticEntity] = currentState;
            ...
            EventBus?.SafePublish(new AAFireEvent(aaPos, ballisticPos, aa.Type, aaEntityIndex, ballisticEntity.Index, ballisticEntity.Version, IsBallistic: true, AARotation: aaRotation), "BallisticDefenseSystem");
            ...
            // Wave leak regulator, applied last (mirrors FireControlExecutor.TryIntercept). Bounded
            // below by Waves.LeakRegulatorMinMultiplier, so a Patriot never fires an unwinnable shot.
...
#pragma warning restore CIVIC035
                ...
                // InterceptProcessingSystem publishes ThreatInterceptEvent for this request. The
                // kill is already decided here — the wave's kill share was applied to the roll
                // above, so the consumer never revokes it.
