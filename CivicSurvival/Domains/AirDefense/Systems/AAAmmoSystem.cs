...
namespace CivicSurvival.Domains.AirDefense.Systems
{
    ...
    public partial class AAAmmoSystem : EventDrivenSystemBase, IDefaultSerializable, IResettable, IBootDefaultsReset
    {
        ...
        // Cycle totals for the terminal Full event + log line: what this refill cycle actually
        // requested (batch intents priced at the unified tariff). Prod telemetry reads them to
        // verify the tariff on live sessions — before these, a successful cycle left no cost
        // trace anywhere. Intent-level sums: a budget-dropped batch is still counted (its
        // Warn/refund lines are the signal for that rarity). Transient, reset with the cycle.
        [System.NonSerialized] private int m_TrickleCycleRounds;
        [System.NonSerialized] private long m_TrickleCycleCost;
        ...
        public void ResetState()
        {
            ...
            m_TrickleCycleRounds = 0;
            m_TrickleCycleCost = 0;
        ...
        private void TrickleRefillTick()
        {
            ...
            {
                // Missile launchers (Patriot, Hawk) are never auto-refilled: their dear interceptor
                // missiles are restocked only through the deliberate per-type emergency button (flat
                // cost + cooldown), so the calm-phase trickle would otherwise underprice them at the
                // global per-round rate. Keyed on the weapon class, not the type, so a new missile AA
                // falls out automatically.
                if (aa.type.FiresInterceptorMissile()) continue;
            ...
            if (totalDeficit == 0)
            {
                ...
                // carrying the cycle's actual totals so the tariff is verifiable in prod
                // telemetry (a bare Full used to read as "0 rounds, $0" — indistinguishable
                // from free, see 2026-07-22 tariff-unification session).
                if (m_TrickleCycleActive)
                {
                    m_TrickleCycleActive = false;
                    EventBus?.SafePublish(new AAResupplyEvent(
                        AAResupplyResult.Full,
                        Rounds: m_TrickleCycleRounds,
                        Cost: m_TrickleCycleCost), "AAAmmoSystem");
                    Log.Info($"Trickle cycle complete: {m_TrickleCycleRounds} rounds for ${m_TrickleCycleCost:N0}");
                    m_TrickleCycleRounds = 0;
                    m_TrickleCycleCost = 0;
                }
            ...
            // Unified tariff: the trickle prices every round from the same per-type table the
            // emergency button charges (flat ResupplyCost prorated across the type's city-wide
            // capacity) — auto-refill no longer overcharges relative to the manual button.
            int[] perTypeCost = ComputePerTypeRoundCost(aaWithEntities);

            // Delivery plan first (this tick's trickle portion per gun), then the budget clamp
            // over the priced plan.
            var perGun = new int[aaWithEntities.Count];
            var perGunCost = new int[aaWithEntities.Count];
            long plannedCost = 0;
            for (int i = 0; i < aaWithEntities.Count; i++)
            {
                ...
                int deficit = Math.Max(0, aa.max - aa.current);
                if (deficit == 0) continue;
                perGun[i] = TricklePortion(aa.max, deficit);
                perGunCost[i] = perTypeCost[(int)aa.type];
                plannedCost += (long)perGun[i] * perGunCost[i];
            }
            ...
            // Instead deliver only what the city can pay this tick. cityBalance - PendingDeductions
            // is the conservative figure the pipeline's deduct will see, so the clamped batch is
            // payable, not dropped.
            long actualCost = plannedCost;
            if (plannedCost > 0)
            {
                ...
                actualCost = AAAmmoLogic.ClampPlanToBudget(perGun, perGunCost, availableMoney);
                if (actualCost < plannedCost)
                {
                    int clampedRounds = 0;
                    for (int i = 0; i < perGun.Length; i++) clampedRounds += perGun[i];
                    ...
                    // telemetry; leaving them default-0 prints "Only 0/0".
                    if (!m_TrickleUnderfundedNotified)
                    {
                        m_TrickleUnderfundedNotified = true;
                        EventBus?.SafePublish(new AAResupplyEvent(
                            clampedRounds > 0 ? AAResupplyResult.Partial : AAResupplyResult.Failed,
                            Rounds: clampedRounds,
                            Needed: totalDeficit,
                            Cost: plannedCost - actualCost
                        ), "AAAmmoSystem");
                    }
                }
                else
            ...
            for (int i = 0; i < aaWithEntities.Count; i++)
            {
                ...
                CreateResupplyLineIntent(
                    ...
                    aa.current + give, give, perGunCost[i]);
            }

            m_TrickleCycleActive = true;
            m_TrickleCycleRounds += actualRounds;
            m_TrickleCycleCost += actualCost;
        }
...
#pragma warning restore CIVIC243
        ...
        private ResupplyStartResult StartResupplyWithCost()
        {
            ...
            // Unified tariff: full-deficit plan priced from the same per-type table as the
            // emergency button and the trickle, then clamped to the available money in line
            // order (first-AA-first — the old DistributeRounds priority).
            int[] perTypeCost = ComputePerTypeRoundCost(aaWithEntities);
            var perGun = new int[aaWithEntities.Count];
            var perGunCost = new int[aaWithEntities.Count];
            int totalRoundsNeeded = 0;
            long plannedCost = 0;
            for (int i = 0; i < aaWithEntities.Count; i++)
            {
                var aa = aaWithEntities[i];
                int deficit = Math.Max(0, aa.max - aa.current); // H5: corrupt save (current > max) must not go negative
                if (deficit == 0) continue;
                perGun[i] = deficit;
                perGunCost[i] = perTypeCost[(int)aa.type];
                totalRoundsNeeded += deficit;
                plannedCost += (long)deficit * perGunCost[i];
            }
            ...
            long actualCost = AAAmmoLogic.ClampPlanToBudget(perGun, perGunCost, availableMoney);
            int actualRounds = 0;
            for (int i = 0; i < perGun.Length; i++) actualRounds += perGun[i];

            if (actualRounds <= 0)
            {
                Log.Error($"NO RESUPPLY - city is broke! Need ${plannedCost:N0}");
                EventBus?.SafePublish(new AAResupplyEvent(
            ...
            bool isFull = actualRounds == totalRoundsNeeded;
            ...
            CreateResupplyBatch(
                ...
                isFullResupply: isFull,
                requestedRounds: actualRounds,
                neededRounds: totalRoundsNeeded);

            for (int i = 0; i < aaWithEntities.Count; i++)
            {
                if (perGun[i] <= 0) continue;
                var aa = aaWithEntities[i];
                CreateResupplyLineIntent(ecb, batchId, aa.entityIndex, aa.entityVersion,
                    aa.current + perGun[i], perGun[i], perGunCost[i]);
            }

            if (isFull)
                Log.Info($"Resupply requested: {actualRounds} rounds for ${actualCost:N0} (pending budget result)");
            else
                Log.Warn($"PARTIAL resupply: {actualRounds}/{totalRoundsNeeded} rounds (pending budget result)");

            return new ResupplyStartResult(actualRounds, actualCost, isFull);
        }
        ...
        ///
        /// SystemAPI.Query triggers CompleteDependencyBeforeRO sync point.
        ...
        /// </summary>
        /// <summary>
        /// Unified per-round tariff, indexed by (int)AAType: each type's flat emergency
        /// ResupplyCost (the button price, AAResupplyGroups) prorated across the type's
        /// city-wide live magazine capacity. Both the trickle and the cost-bearing full
        /// resupply charge from THIS table, so refilling a type from empty costs the same
        /// as its emergency button — one tariff, two delivery speeds.
        /// </summary>
        private static int[] ComputePerTypeRoundCost(
            List<(int entityIndex, int entityVersion, int current, int max, AAType type)> aaWithEntities)
        {
            var cfg = BalanceConfig.Current;
            int typeCount = AATypes.All.Count;
            var capacity = new int[typeCount];
            foreach (var aa in aaWithEntities)
                capacity[(int)aa.type] += Math.Max(0, aa.max);

            var cost = new int[typeCount];
            for (int i = 0; i < typeCount; i++)
            {
                if (capacity[i] <= 0) continue;
                cost[i] = AAAmmoLogic.PerRoundCostFromFlat(
                    AAParams.ForType(cfg, (AAType)i).ResupplyCost, capacity[i]);
            }
            return cost;
        }
