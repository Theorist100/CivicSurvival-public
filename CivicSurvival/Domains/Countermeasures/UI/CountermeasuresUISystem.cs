...
#pragma warning disable CIVIC098 // ModalCoordinator.Instance is static readonly = new(), never null
    public partial class CountermeasuresUISystem : CivicUIPanelSystem
    {
...
#if DEBUG
            Triggers.Add(DebugForceInvestigationChoice, FeatureIds.Countermeasures, OnDebugForceInvestigationChoice);
            Triggers.Add(DebugForcePoliceChoice, FeatureIds.Countermeasures, OnDebugForcePoliceChoice);
#endif
        }

#if DEBUG
        [System.NonSerialized]
        private CivicSurvival.Core.Interfaces.Domain.Debug.ICountermeasuresDebugMutator? m_DebugMutator;

        private CivicSurvival.Core.Interfaces.Domain.Debug.ICountermeasuresDebugMutator ResolveDebugMutator()
            => m_DebugMutator ??= CivicSurvival.Core.Infrastructure.ServiceRegistryFeatureExtensions
                .TryGetOrNullObject(CivicSurvival.Core.Interfaces.Domain.Debug.NullCountermeasuresDebugMutator.Instance);

        private void OnDebugForceInvestigationChoice()
            => ResolveDebugMutator().DebugForceInvestigationChoice("BalanceDebugPanel");

        private void OnDebugForcePoliceChoice()
            => ResolveDebugMutator().DebugForcePoliceChoice("BalanceDebugPanel");
#endif
        ...
        private TriggerOutcome OnMakeInvestigationChoice(int choice)
        {
            ...
            // No pause reject here, by design — the consumer (CountermeasureChoiceIntakeSystem)
            // runs in ModificationEnd, which ticks while paused (Axiom 14): the forced-choice
            // modal must be resolvable at selectedSpeed==0.
        ...
        private TriggerOutcome OnMakePoliceChoice(int choice)
        {
            ...
            // No pause reject here, by design — see the investigation handler above.
