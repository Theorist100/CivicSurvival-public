...
#pragma warning restore CA1001
    {
        ...
        // The city's size when the influx was declared — a measurement taken then and kept, so
        // it carries the counter that took it. Reading it back is a decision (the collapse
        // window divides the refugee total by it), so it is asked for by measure, and a value
        // from the superseded counter withholds the window instead of being divided into.
        private RecordedPopulation m_OriginalPopulation;

        /// <summary>
        /// Refugee arrival floor as a fraction of the war-start floor (Scenario.VillageMaxPop).
        /// Half keeps the ordering the milestone pair used to express — refugees before the war —
        /// without introducing a second balance key for one derived number.
        /// </summary>
        private const float REFUGEE_FLOOR_FRACTION_OF_WAR_FLOOR = 0.5f;

        // The single owner of "how many people live in the city" — the vanilla city population
        // record. The producer registers it in its OnCreate, ahead of any consumer update, so a
        // refusal here means the city entity itself is absent, not that the answer is late.
        [NonSerialized] private ICityPopulationReader m_CityPopulationReader = null!;
        ...
        // Last reading of the city population record, refreshed at most once per
        // RATE_CACHE_HOURS game hours. It feeds only the rate tier and the growth cap, and both
        // move slowly. This is a staleness tolerance, NOT a sync-point guard: the entity-count
        // query that used to stand here completed no foreign jobs at all (see
        // GetRefugeeRatePerHour), so the previous claim on this line was false.
        // The cache has no reason left to exist beyond inertia; removing it also removes the
        // resets in RefugeeSpawnSystem.Serialization.cs, which is not this change's file.
        ...
        // Edge latch for the "scenario has not published yet" line in CanSpawnRefugees. Ordinary
        // for the first ticks of a session and a real defect if it persists, so it is worth one
        // line per episode. Session-local: the next run states its own answer.
        [System.NonSerialized] private bool m_LoggedSettlementUnknown;
        ...
        public void StartRefugeeInflux()
        {
            ...
            m_HoursElapsed = 0;
            m_TotalRefugeesAdded = 0;
            // People living in the city, from the same owner the arrival floor asks — one
            // measure for both, so the collapse readout below cannot be a ratio between two
            // different counts. Two things to know about this line, neither introduced here:
            // a refusal (no city entity) is no longer folded into a zero — it records nothing,
            // which the collapse window reads as "no baseline" instead of as "a city of zero
            // people". And this is a REMEMBERED measurement: it is stamped with the counter that
            // took it, so a save written before the measure changed refuses the window rather
            // than dividing today's refugee total by a number from another counter.
            // The window still stays shut for the rest of that influx when nothing was recorded
            // — the capture has no retry, which is a separate defect this phase does not fix.
            m_OriginalPopulation = TryGetResidentPopulation(out int startingPopulation)
                ? RecordedPopulation.On(CityMeasure, startingPopulation)
                : RecordedPopulation.NotRecorded;
            m_TimeProvider ??= GameTimeSystem.Instance;
            ...
            Log.Info($"=== REFUGEE INFLUX STARTED === Original pop: {m_OriginalPopulation.Value} on {m_OriginalPopulation.Measure}");
        ...
        protected override void OnUpdateImpl()
        {
            ...
            {
                if (!TryGetResidentPopulation(out int population))
                {
                    // No city entity — there is nothing to take a percentage of. The anchor is
                    // NOT set: m_ParkAnchored stays false and the next update retries. Anchoring
                    // on a refusal would freeze a target of zero for the rest of the run.
                    Log.Warn("=== REFUGEE TARGET ANCHOR DEFERRED === the city population record has no answer (no city entity); retrying next update");
                }
                else
                {
                    m_ParkAnchored = true;
                    ...
                    if (m_TargetRefugees <= 0)
                    {
                        // A zero target satisfies IsTargetReached on this very tick, so leaving
                        // the anchor set would end the influx below without a single refugee and
                        // for the rest of the run. Roll it back and retry, same as a refusal: an
                        // empty target is arithmetic, not a verdict about the city, so retrying
                        // costs nothing and can only start delivering once there is someone to
                        // grow from. This is not the settled-city predicate a later phase owes —
                        // that one decides whether to arm the influx at all; this only declines
                        // to freeze a target that can never be met.
                        m_ParkAnchored = false;
                        m_TargetRefugees = 0;
                        m_AnchorGameHour = 0.0;
                        Log.Warn($"=== REFUGEE TARGET ANCHORED AT ZERO === pop={population}, percent={scenarioCfg.RefugeeTargetPercent}, target=0 — anchor rolled back, retrying next update");
                    }
                }
            }
        ...
        /// <summary>
        /// Whether the settlement is big enough to receive refugees.
        ///
        /// Measured in people, not in the vanilla milestone. The milestone counts unlocks:
        /// UnlockAllMilestones sets it to maximum with zero citizens, and it never decreases
        /// after a city empties out — prod shows 2457 of 3419 war starts already sitting at the
        /// top milestone, 1412 of them below a thousand residents. Gating an inflow of people on
        /// that number pours refugees into an empty map, and they land in PeakPopulation, which
        /// is the denominator of both the victory retention check and the ghost-town readout.
        ///
        /// The floor is half the war-start floor, keeping the original ordering (refugees arrive
        /// before the war, as milestone 3 did against milestone 4) without opening the inflow on
        /// nothing. A city population record that cannot answer (no city entity) refuses here —
        /// it never passes an unanswered question off as "empty".
        /// </summary>
        private bool CanSpawnRefugees()
        {
            ...
            // Declared gate on "anyone has ever lived here", standing in FRONT of the floor rather
            // than instead of it. The floor is a balance constant: whoever lowers it while tuning
            // village sizes silently removes the protection it happens to provide, because nothing
            // in the code says it is one. The flag says it, and it is the only thing that can tell
            // an emptied city from a plot nobody has moved into — the headcount reads zero for both.
            if (!SystemAPI.TryGetSingleton<ScenarioSingleton>(out var scenario))
            {
                // Not an answer, so not treated as one. Refugees are withheld until the scenario
                // publishes, and the reason is stated once per episode: an inflow that never starts
                // must not look like an inflow that was correctly declined (Axiom 1).
                if (!m_LoggedSettlementUnknown)
                {
                    m_LoggedSettlementUnknown = true;
                    Log.Warn("[Refugees] no ScenarioSingleton published yet — arrival withheld until it can be told whether anyone has ever lived in this city");
                }
                return false;
            }

            m_LoggedSettlementUnknown = false;
            if (!scenario.CityEverSettled)
                return false;

            return population >= RefugeeArrivalFloor();
        }

        /// <summary>Population floor for refugee arrival: half of the war-start floor.</summary>
        private static int RefugeeArrivalFloor() =>
            (int)Math.Round(BalanceConfig.Current.Scenario.VillageMaxPop * REFUGEE_FLOOR_FRACTION_OF_WAR_FLOOR);

        /// <summary>
        /// People living in the city right now, from the single owner of that measure — the
        /// vanilla city population record. Tourists, commuters and the dead are excluded there,
        /// and the value is restored from the save before this system's first tick, so there is
        /// no "households are still streaming in" window and no readiness level to wait on.
        ///
        /// <c>false</c> means the city entity does not exist and the question has no answer;
        /// zero WITH a city is a truthful zero, and every caller decides for itself what an
        /// empty city means for it (this system's two decisions are the arrival floor and the
        /// influx baseline — both treat zero as "too small", which is right).
        ///
        /// The helper stays local only to keep the resolve in one place. Verbatim copies of this
        /// shape live in five other systems; folding them into Core is separate work.
        /// </summary>
        /// <summary>
        /// The counter <see cref="TryGetResidentPopulation"/> answers on, stamped into anything
        /// this system remembers. Changing the source means changing this with it.
        /// </summary>
        ...
        {
            // Repeated ??= rather than relying on OnStartRunning alone: post-load validation
            // runs before OnStartRunning, and the influx entry points are reachable from it.
            m_CityPopulationReader ??= ServiceRegistry.Instance.Require<ICityPopulationReader>();
        ...
        private int GetRefugeeRatePerHour(double currentGameHours)
        {
            ...
            // Re-read the city population record at most once per RATE_CACHE_HOURS game hours.
            // The reason is staleness tolerance, not a sync point: the entity-count query that
            // used to stand here completed no foreign jobs (its component types are neither
            // enableable nor change-filtered, so the count synchronised nothing), and the claim
            // that it did was wrong on both this line and the field declaration. The owner's
            // read does wait on vanilla's Population writer — one job per 16 simulation ticks,
            // long finished by the time an hourly tick asks. Tier and growth cap move slowly, so
            // stale-by-24h stays negligible either way.
            ...
            {
                if (TryGetResidentPopulation(out int cityResidents))
                {
                    m_CachedCitizenCount = cityResidents;
                    m_LastRateRecomputeGameHours = currentGameHours;
                }
                // Refusal (no city entity): keep the previous reading and leave the timestamp
                // alone, so the next hourly tick retries instead of pinning a made-up zero for
                // a whole game day.
            }
...
#pragma warning restore CIVIC226
            ...
            if (!m_ShownCollapseModal
                && m_OriginalPopulation.TryGetOn(CityMeasure, out int originalPopulation)
                && originalPopulation > 0)
            {
                float ratio = (float)m_TotalRefugeesAdded / originalPopulation;
...
#pragma warning restore CIVIC098
        ...
        {
            // Reporting the collapse that was already decided above, so the recorded number is
            // taken for its value; the event field is a plain int and stays one.
            int originalPopulation = m_OriginalPopulation.Value;
            float ratio = originalPopulation > 0 ? (float)m_TotalRefugeesAdded / originalPopulation : 0f;
