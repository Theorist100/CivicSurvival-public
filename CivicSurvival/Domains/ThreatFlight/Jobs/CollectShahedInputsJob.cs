...
namespace CivicSurvival.Domains.ThreatFlight.Jobs
{
    ...
    public struct ShahedCollectedInput
    {
        ...
        /// <summary>Snapshot of the PlayerOutboundThreat enableable bit — true for the player's
        /// outbound counter-strike, false for an inbound wave drone. Rides the record so radar
        /// styling can tell the two factions apart without a second chunk pass.</summary>
    ...
    internal struct CollectShahedInputsJob : IJobChunk
    {
        ...
        [ReadOnly] public ComponentTypeHandle<PlayerOutboundThreat> OutboundHandle;
        ...
        public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
        {
            ...
            // Faction bit from the same chunk pass (Axiom 15 — no extra ComponentLookup, no new
            // sync point). PlayerOutboundThreat is present on every threat archetype (spawn +
            // load-reinit both add it), so the mask is always valid for these chunks.
            var outboundMask = chunk.GetEnabledMask(ref OutboundHandle);
            ...
            for (int i = 0; i < chunk.Count; i++)
            {
                ...
                Output.AddNoResize(new ShahedCollectedInput
                {
                    ...
                    IsOutbound = outboundMask.GetBit(i),
