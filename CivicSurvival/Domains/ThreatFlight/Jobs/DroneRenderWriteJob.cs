...
namespace CivicSurvival.Domains.ThreatFlight.Jobs
{
    ...
    public partial struct DroneRenderWriteJob : IJobEntity
    {
        ...
        {
            // Last-hop NaN/Inf backstop before the vanilla render pipeline. A non-finite pose
            // flows unchecked through ObjectInterpolateSystem → InterpolatedTransform → CullingInfo
            // bounds (AV in culling/BRG) — and rotation is no safer than position: vanilla
            // normalizes the quaternion during interpolation, and a NaN component survives the
            // normalize into the culling bounds. The known Inf source is already retired upstream
            // (BallisticMovementJobEntity guards NaN/Inf Speed/TargetPosition); this is a cheap
            // final guard over ALL THREE written fields — skip the write and leave the last-good
            // pose for this frame (crash-hunt 2026-07-21, lens 3: rotation/velocity were the last
            // unguarded NaN path into vanilla culling).
            if (math.any(math.isnan(tp.Position)) || math.any(math.isinf(tp.Position))
                || math.any(math.isnan(tp.Rotation.value)) || math.any(math.isinf(tp.Rotation.value))
                || math.any(math.isnan(tp.Velocity)) || math.any(math.isinf(tp.Velocity)))
                return;
