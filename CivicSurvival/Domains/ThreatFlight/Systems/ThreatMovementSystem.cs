...
#pragma warning restore CIVIC168
        ...
        // Faction bit for the collect job — read RO so radar can style outbound counter-strikes
        // apart from inbound wave threats.
        private ComponentTypeHandle<PlayerOutboundThreat> m_OutboundTypeHandle;
        ...
        protected override void OnCreate()
        {
            ...
            m_OutboundTypeHandle = GetComponentTypeHandle<PlayerOutboundThreat>(true);
                    ...
                    using (PerformanceProfiler.Measure("TMS.RadarAudioPass"))
                    {
                        for (int i = 0; i < m_CollectedPub.Length; i++)
                        {
                            ...
                            radarBuffer.Add(new RadarThreatBuffer
                            {
                                ...
                                IsOutbound = collected.IsOutbound
                            });
...
#pragma warning restore CIVIC078
                ...
                if (updateRadar)
                {
                    ...
                    // WithPresent<PlayerOutboundThreat> keeps inbound missiles (bit present but
                    // disabled) in the query; EnabledRefRO reads the faction bit so an outbound
                    // counter-strike missile is styled allied on radar instead of enemy-red.
                    foreach (var (ballistic, bisState, threatPos, outbound, entity) in
                        SystemAPI.Query<RefRO<Ballistic>, RefRO<BallisticInterceptState>, RefRO<ThreatPosition>, EnabledRefRO<PlayerOutboundThreat>>()
                        .WithPresent<PlayerOutboundThreat>()
                ...
                if (m_CollectedPub.Length > 0 && !SkipMovement)
                {
                    ...
                    using (PerformanceProfiler.Measure("ThreatMovement.MovementSchedule"))
                    {
                        ...
                        // SAFETY-LOCK: scheduled WITHOUT dependsOn and NOT folded into Dependency — legal
                        // ONLY because every captured container is either mod-owned and self-serialized by
                        // the system's own step order (m_CollectedPub/writeOutputs are the pending
                        // double-buffer halves — main thread touches them only after STEP 1's Complete()
                        // next cycle; m_CachedBuildings/m_BuildingGrid swap exclusively behind the same
                        // Complete()) or fenced by its owner's own mechanism (vanilla TerrainData via
                        // AddCPUHeightReader below). The job reads NO ECS chunks, so the ECS dependency
                        // manager has nothing to fence. If you add ANY ECS-chunk/lookup input or an
                        // unfenced vanilla container here, this schedule MUST move onto Dependency
                        // (+ query.AddDependency for chunk readers) — a bare `Schedule` would be the
                        // NeighborEnvy UAF class (crash-hunt 2026-07-21, lenses 1+5 independently
                        // flagged this invariant as unmarked).
...
#pragma warning restore CIVIC343
        ...
        private void ScheduleCollectInputsJob()
        {
            ...
            m_OutboundTypeHandle.Update(this);
            ...
            var collectJob = new CollectShahedInputsJob
            {
                ...
                OutboundHandle = m_OutboundTypeHandle,
