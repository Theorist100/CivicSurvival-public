...
namespace CivicSurvival.Domains.ThreatDamage.Helpers
{
    ...
    /// </summary>
    /// <summary>
    /// Physical-presence test for casualty selection.
    ///
    /// The buffers a building hit is resolved from (<c>Renter</c> → <c>HouseholdCitizen</c>,
    /// <c>Employee</c>, <c>Student</c>, <c>Patient</c>) describe ASSIGNMENT, not location: a
    /// resident who is at work — or sheltering across town — is still listed by their apartment.
    /// A residential building keeps no "who is inside right now" buffer (vanilla only fills
    /// <c>Occupant</c> for shelters/prisons/police), so the only path from a building to its people
    /// is the assignment list; presence is recovered by asking each candidate where they actually
    /// are. <c>CurrentBuilding</c> is that field — the same one vanilla's <c>CitizenEvacuateJob</c>
    /// reads to decide who runs for a shelter, and the one a shelter arrival rewrites. Filtering on
    /// it is what makes evacuation save lives instead of merely animating them, and what stops a
    /// citizen at work from dying with their home.
    ///
    /// A citizen with no <c>CurrentBuilding</c> is not inside any building (in transit, in a
    /// vehicle) and cannot die to a building collapse, so they are filtered out as well.
    /// </summary>
    public readonly struct CasualtyPresenceFilter
    {
        private readonly Entity m_StruckBuilding;
        private readonly ComponentLookup<CurrentBuilding> m_CurrentBuildingLookup;

        public CasualtyPresenceFilter(Entity struckBuilding, ComponentLookup<CurrentBuilding> currentBuildingLookup)
        {
            m_StruckBuilding = struckBuilding;
            m_CurrentBuildingLookup = currentBuildingLookup;
        }

        public bool Accepts(Entity citizen)
        {
            return m_CurrentBuildingLookup.TryGetComponent(citizen, out CurrentBuilding current)
                   && current.m_CurrentBuilding == m_StruckBuilding;
        }
    }
    ...
    public static class BuildingCasualtyHelper
    {
        ...
        /// <param name="presence">Physical-presence filter (see <see cref="CasualtyPresenceFilter"/>)</param>
        ...
        public static void GetVictims(
            ...
            in CasualtyPresenceFilter presence,
                ...
                switch (type)
                {
                    case BuildingType.Residential:
                        AddHouseholdCitizens(building, maxVictims,
                            ref renterLookup, ref householdLookup, ref propertyRenterLookup, ref deletedLookup, in processedVictims, in presence, selectedVictims, victims);
                        break;

                    case BuildingType.School:
                        // Students + Teachers
                        AddStudents(building, maxVictims, ref studentLookup, ref deletedLookup, in processedVictims, in presence, selectedVictims, victims);
                        int remaining = maxVictims - victims.Length;
                        if (remaining > 0)
                            AddEmployees(building, remaining, ref employeeLookup, ref deletedLookup, in processedVictims, in presence, selectedVictims, victims);
                        break;

                    case BuildingType.Hospital:
                        // Patients + Staff
                        AddPatients(building, maxVictims, ref patientLookup, ref deletedLookup, in processedVictims, in presence, selectedVictims, victims);
                    ...
                    case BuildingType.Industrial:
                        ...
                        AddCompanyEmployees(building, maxVictims,
                            ref renterLookup, ref employeeLookup, ref companyLookup, ref propertyRenterLookup, ref deletedLookup, in processedVictims, in presence, selectedVictims, victims);
                        break;

                    default:
                        // Service buildings (power, water) в†’ Employee directly
                        AddEmployees(building, maxVictims, ref employeeLookup, ref deletedLookup, in processedVictims, in presence, selectedVictims, victims);
                        break;
                }
            }
            finally
            ...
            NativeList<Entity> victims)
        {
            ...
            for (int ro = 0; ro < renters.Length; ro++)
            {
                ...
                for (int co = 0; co < citizens.Length && victims.Length < maxVictims; co++)
                {
                    ...
                    _ = TryAddFreshVictim(victim, ref deletedLookup, in processedVictims, in presence, selectedVictims, victims);
                }
            }
        }
