...
namespace CivicSurvival.Domains.ThreatDamage.Systems
{
    ...
    public partial class ThreatTerminalizationSystem : CivicSystemBase
    {
        ...
        // WHY this threat got through (coverage gap / throughput / lethality / leak floor).
            ...
            for (int i = 0; i < m_Outcomes.Count; i++)
            {
                ...
                if (outcome.EmitsImmediateImpact)
                {
                    ...
                    if (outcome.IsBallistic)
                        ...
                        droneHits++;
                    EventBus?.SafePublish(new ThreatImpactEvent(outcome.EventPosition, outcome.IsBallistic, ClassifyLeak(in outcome)), nameof(ThreatTerminalizationSystem));
                }
        ...
        /// Order matters: the leak floor is a deliberate design pass-through and excludes the
        /// threat from targeting, so it is claimed first — otherwise every floored threat
        /// would masquerade as a coverage or throughput failure.
        /// </summary>
        private ThreatLeakReason ClassifyLeak(in ThreatTerminalOutcome outcome)
        {
            ...
            else
            {
                ...
                leakFloor = combat.IsLeaked;
            }

            if (leakFloor) return ThreatLeakReason.LeakFloor;
