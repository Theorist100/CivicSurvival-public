...
namespace CivicSurvival.Domains.ThreatDamage.Systems
{
    ...
    public partial class ThreatTerminalizationSystem : CivicSystemBase
    {
        ...
        // Leak-attribution funnel: read the coverage/engagement bits at impact time to tell
        // WHY this threat got through (coverage gap / throughput / lethality).
        private ComponentLookup<ShahedCombatState> m_ShahedCombatLookup;
        private ComponentLookup<BallisticInterceptState> m_BallisticStateLookup;
        ...
        protected override void OnCreate()
        {
            ...
            m_ShahedCombatLookup = GetComponentLookup<ShahedCombatState>(true);
            m_BallisticStateLookup = GetComponentLookup<BallisticInterceptState>(true);
        ...
        protected override void OnUpdateImpl()
        {
            ...
            m_ShahedCombatLookup.Update(this);
            m_BallisticStateLookup.Update(this);
            ...
            for (int i = 0; i < m_Outcomes.Count; i++)
            {
                ...
                if (outcome.EmitsImmediateImpact)
                {
                    ...
                    if (outcome.IsBallistic)
                        ...
                        droneHits++;
                    var leakReason = ClassifyLeak(in outcome);
                    // Covered = air defense was responsible for this one (it entered some ready
                    // AA's candidate set). NeverCovered/Unknown are excluded so an undefended
                    // corner cannot depress the regulator's measured kill share — see
                    // ThreatOutcomeStatsSingleton.CoveredHitsCount. Derived from the reason we
                    // just classified, so no extra component read.
                    if (leakReason is ThreatLeakReason.CoveredNotEngaged or ThreatLeakReason.EngagedSurvived)
                        coveredHits++;
                    // Info, not Debug: one line per impact on the city — units per wave, tens in a
                    // saturating one — and the only place the leak cause is readable without a
                    // telemetry query. Separates "never entered the candidate set" (range / line of
                    // sight) from "was a candidate, no shot" (crew / cooldown); the fire-control
                    // [AA:SHOT] lines cannot answer this because they only exist once a shot happened.
                    Log.Info($"[LEAK] {(outcome.IsBallistic ? "ballistic" : "drone")} impact at " +
                             $"({outcome.EventPosition.x:F0}, {outcome.EventPosition.z:F0}) reason={leakReason}");
                    EventBus?.SafePublish(new ThreatImpactEvent(outcome.EventPosition, outcome.IsBallistic, leakReason), nameof(ThreatTerminalizationSystem));
                }
            ...
            AddOutcomeStats(hits, droneHits, ballisticHits, coveredHits, crashed);
        ...
        /// <summary>
        /// Leak-attribution funnel: why did this impacting threat get through air defense?
        /// Reads the WasCandidate/WasEngaged bits the AA pipeline stamped during flight.
        ///
        /// There is no longer a "let through by design" bucket: the wave leak regulator
        /// (LeakRegulatorLogic) scales intercept chance instead of exempting targets, so a threat
        /// the regulator held back was shot at and survived — EngagedSurvived, same as any other
        /// miss. How hard the regulator was pulling shows up in the wave's kill share, not per
        /// threat.
        /// </summary>
        private ThreatLeakReason ClassifyLeak(in ThreatTerminalOutcome outcome)
        {
            bool wasCandidate;
            bool wasEngaged;
            ...
            if (outcome.IsBallistic)
            {
                if (!m_BallisticStateLookup.TryGetComponent(outcome.Entity, out var ballistic))
                    return ThreatLeakReason.Unknown;
                wasCandidate = ballistic.WasCandidate;
                wasEngaged = ballistic.WasEngaged;
            }
            else
            {
                if (!m_ShahedCombatLookup.TryGetComponent(outcome.Entity, out var combat))
                    return ThreatLeakReason.Unknown;
                wasCandidate = combat.WasCandidate;
                wasEngaged = combat.WasEngaged;
            }

            if (wasEngaged) return ThreatLeakReason.EngagedSurvived;
            if (wasCandidate) return ThreatLeakReason.CoveredNotEngaged;
            return ThreatLeakReason.NeverCovered;
        }
        ...
        private void AddOutcomeStats(int hits, int droneHits, int ballisticHits, int coveredHits, int crashed)
        {
            if (hits == 0 && crashed == 0)
                return;
...
#pragma warning disable CIVIC069
            ...
            stats.ValueRW.CoveredHitsCount += coveredHits;
            stats.ValueRW.CrashedCount += crashed;
#pragma warning restore CIVIC069
            Log.Info($" Wave #{stats.ValueRO.WaveNumber}: +{hits} hits (drone {droneHits}/ballistic {ballisticHits}, covered {coveredHits}), +{crashed} crashed (total: {stats.ValueRO.HitsCount}/{stats.ValueRO.CrashedCount})");
        }
