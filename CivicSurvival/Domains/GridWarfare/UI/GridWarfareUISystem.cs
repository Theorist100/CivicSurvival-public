...
namespace CivicSurvival.Domains.GridWarfare.UI
{
    ...
    public partial class GridWarfareUISystem : CivicUIPanelSystem
    {
        ...
        protected override void ConfigureTriggers()
        {
            ...
            // Launcher placement — activates the vanilla placement tool synchronously (pause-safe,
            // like the Propaganda Center). Prefab is fixed per kind (the DroneLauncher / HIMARS
            // .cok); mode 0 (shadow-only).
            Triggers.Add(PlaceDroneLauncher, FeatureIds.GridWarfare, RequestResultBridge.DroneLauncherPlacement, OnPlaceDroneLauncher);
            Triggers.Add(PlaceBallisticLauncher, FeatureIds.GridWarfare, RequestResultBridge.BallisticLauncherPlacement, OnPlaceBallisticLauncher);

            // Dev-only: drive a REAL operation of the given attack type — the same Prepare/Execute
            // path the War Room buttons use, with the same cost, munition, damage, target pick and
            // pending-launch ledger. One button per axis (drone rides a drone; cyber and psyops ride
            // a ballistic round). Sync on the UI thread — pause-safe (Axiom 14 route 3).
            Triggers.Add(DebugStrikeDrone, FeatureIds.GridWarfare, () => OnDebugStrike("drone"));
            Triggers.Add(DebugStrikeCyber, FeatureIds.GridWarfare, () => OnDebugStrike("blackout"));
            Triggers.Add(DebugStrikePsyops, FeatureIds.GridWarfare, () => OnDebugStrike("disinfo"));
        ...
        protected override void OnPanelUpdate()
        {
            var dto = new GridWarfareDto
            {
                ...
                BallisticLauncherPlacementRequestJson = RequestResultBridge.Get(RequestResultBridge.BallisticLauncherPlacement).ToJson(),
        ...
        private void FillLauncherCounts(ref GridWarfareDto dto)
        {
            // Cheap structural count of live launcher installs — gates each EXECUTE button
            // (0 = that munition has no launch origin). CalculateEntityCount is a metadata read,
            // no chunk scan / Transform touch.
            dto.DroneLauncherCount = m_DroneLauncherQuery.CalculateEntityCount();
            dto.BallisticLauncherCount = m_BallisticLauncherQuery.CalculateEntityCount();
            // Price each placement handler will charge (pre-markup shadow) — the STRIKE build cards
            // name the purse instead of leaving the cost to be discovered at placement time.
            var gw = BalanceConfig.Current.GridWarfare;
            dto.DroneLauncherCost = gw.DroneLauncherCost;
            dto.BallisticLauncherCost = gw.BallisticLauncherCost;
        }
...
#pragma warning restore S1244, CIVIC072
        ...
        private TriggerOutcome OnPlaceBallisticLauncher()
        {
            Log.Info("OnPlaceBallisticLauncher called");
            return TriggerOutcome.Supersede(ReasonIds.BallisticLauncherCancelled, token =>
                ActivateBallisticLauncherPlacementImmediately(token));
        }

        private void ActivateBallisticLauncherPlacementImmediately(RequestToken token)
        {
            var result = m_PlacementCommand.TryActivatePlacement(
                BuildingPlacementKind.BallisticLauncher,
                "HIMARS",
                (byte)0,
                token);
        ...
        /// <summary>
        /// Dev-only outbound launch used by the debug panel: fires one projectile of
        /// <paramref name="kind"/> from a built launcher of that kind, bypassing the operation flow
        /// (no slot, no arsenal spend, no cost) so the launch geometry and flight can be checked on
        /// their own. Sync on the UI thread — pause-safe (Axiom 14 route 3). Kinetic axis with zero
        /// damage: this is a flight probe, not a strike, so nothing is resolved at arrival.
        /// </summary>
        private void OnDebugStrike(string attackType)
        {
            if (m_PlayerSystem == null || !m_PlayerSystem.Enabled)
            {
                Log.Warn($"Debug strike ({attackType}) skipped: PlayerAttackSystem unavailable");
                return;
            }

            // Execute first: a slot already Ready is the common case when the button is pressed a
            // second time, and Execute is the step that actually flies the munition.
            if (m_PlayerSystem.ExecuteOperation(attackType, out var executeReason))
            {
                Log.Info($"Debug strike ({attackType}): operation executed — real damage, cost and munition, arrival resolves against the mirror city");
                return;
            }

            // Not ready yet — start the same preparation the War Room starts. The second press,
            // after the prepare timer, executes. Both refusals are reported: an empty wallet, a
            // missing launcher or an empty arsenal all surface here as the reason id.
            if (m_PlayerSystem.PrepareOperation(attackType, out var prepareReason))
            {
                Log.Info($"Debug strike ({attackType}): execute refused ({executeReason}), preparation started — press again once the slot is Ready");
                return;
            }

            Log.Warn($"Debug strike ({attackType}) refused — execute: {executeReason}, prepare: {prepareReason}");
        }
