...
namespace CivicSurvival.Domains.GridWarfare
{
    ...
    public class GridWarfareDomain : IFeatureModule, IUiFeatureModule, IDependentFeatureModule
    {
        ...
        public void RegisterSystems(UpdateSystem updateSystem)
        {
            ...
            // Launcher emplacements — the buildable outbound-strike origins, one per munition
            // kind. Placed through the generic Core building-placement pipeline; this domain
            // registers both handlers (prefab marker check, shadow-only payment, multi-instance,
            // installation commit — shared shape in ShadowFieldLauncherPlacementHandler). The
            // launch origin (ThreatSpawnSystem) reads the committed installation sets directly.
            BuildingPlacementHandlerRegistry.Register(new DroneLauncherPlacementHandler());
            BuildingPlacementHandlerRegistry.Register(new BallisticLauncherPlacementHandler());
