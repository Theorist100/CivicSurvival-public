...
{
    /// <summary>
    /// Owner of the <see cref="EnemyState"/> singleton (the mirror enemy's three derived axes).
    /// The axes are written through <see cref="ApplyTargetDerivedAxes"/> by MirrorCitySystem
    /// (targets authoritative, axes a derived cache; recovery = target repair/rebuild).
    /// This system's own tick only maintains the act-objective latch. Player attack damage is
    /// applied by EnemyOperationEffectSystem in ModificationEnd.
    /// </summary>
    [ActIndependent]
...
#endif
    {
        ...
        protected override void OnUpdateImpl()
        {
            ...
            // Freeze until the war starts: nothing to maintain before the counterattack unlocks.
...
#pragma warning restore CIVIC070
            ...
            // Re-arm the act-objective loot once target repair/rebuild has lifted the enemy back
            // out of the all-axes-suppressed state: as soon as any axis climbs above the objective
            // threshold, the previous collapse is over and the next genuine collapse may pay again.
            // The suppression-side latch lives in EnemyOperationEffectSystem; this is its only reset.
            ref var state = ref enemyState.ValueRW;
...
#pragma warning restore S3400
        ...
        /// <summary>
        /// Ownership seam for the target-derived axis recompute (<c>MirrorCitySystem</c>).
        /// <see cref="EnemyState"/> is single-writer-owned by this system, so the mirror-city
        /// model hands its computed axis values here instead of writing the singleton directly.
        /// Values arrive already floor/cap-resolved by <c>MirrorCityMath</c> (floor = the reserve
        /// target's contrib, cap reduced by key-target cuts); the owner re-clamps against the
        /// balance bounds as a defensive invariant and commits the one write.
        /// </summary>
        public void ApplyTargetDerivedAxes(float physicalAxis, float digitalAxis, float socialAxis)
        {
            ...
            state.PhysicalAxis = math.clamp(physicalAxis, AxisFloor, AxisCap);
            state.DigitalAxis = math.clamp(digitalAxis, AxisFloor, AxisCap);
            state.SocialAxis = math.clamp(socialAxis, AxisFloor, AxisCap);
