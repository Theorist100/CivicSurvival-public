...
namespace CivicSurvival.Domains.GridWarfare.Systems
{
    ...
    public partial class PlayerAttackSystem : IDefaultSerializable, IResettable, IBootDefaultsReset
    {
        ...
        /// <summary>
        /// Reset the per-city transient state that must NOT survive into another loaded city.
        /// CS2 reuses the system instance between loads, so this is called from BOTH city-entry
        /// paths — ResetState (new game / boot reset) and Deserialize (LoadGame) — a
        /// <c>[NonSerialized]</c> attribute alone resets nothing:
        ///   - the War Room per-axis strike preference (a pick against city A must not aim strikes
        ///     in city B, whose target ids are small buffer indices that collide across cities);
        ///   - the stability cost discount (derived from CityStabilitySystem's push; city B's push
        ///     re-seeds it, but a Prepare click in the window before that push must not price with
        ///     city A's discount).
        /// </summary>
        private void ResetTransientLoadState()
        {
            for (int i = 0; i < m_PreferredTargetByAxis.Length; i++)
                m_PreferredTargetByAxis[i] = EnemyTarget.NoTargetId;
            ...
            try
            {
                ...
                lock (m_SlotsLock)
                {
                    ...
                    for (int i = 0; i < MAX_SLOTS; i++)
                    {
                        ...
                        // INVARIANT: an Executing slot loaded with ExecutionClaimed=false is
                        // deliberately re-driven (ClaimPendingOperationEffects re-emits the event).
                        // That re-drive is duplicate-safe end-to-end: the launch-frozen seed is a
                        // pure function of (OperationId, ExecutionId, launch hour), the pending-
                        // launch ledger reconcile skips seeds already in flight, and arrival
                        // retires ledger entries by that same seed — so a save taken between launch
                        // and wallet-confirm cannot double-deliver the strike.
                    }
                }
