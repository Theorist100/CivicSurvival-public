...
namespace CivicSurvival.Domains.ShadowEconomy.Systems
{
    ...
    {
        // Same managed-state reset as the fresh leg (no singleton write — the boot leg
        // must not do structural work mid-load-pass); single source so the legs cannot drift.
    ...
    #pragma warning restore CIVIC223
        private void ResetManagedState()
        {
            ...
            // Sanctions change-detection latch: without sanctions there is nothing
            // active, so the toggle re-fires fresh when sanctions next engage.
