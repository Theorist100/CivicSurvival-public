...
namespace CivicSurvival.Domains.ShadowEconomy.UI
{
    ...
    /// Reads from ECS singletons; the import slider applies synchronously via
    /// ShadowTradeDailySystem (pause-safe, AXIOM 14 route 3).
    ///
    ...
    public partial class ShadowImportUISystem : CivicUIPanelSystem
    {
        ...
        {
            // No pause reject here, by design — the import MW is applied synchronously
            ...
            string failReason = "";
            if (m_TradeSystem == null || !m_TradeSystem.TryApplyImportMWImmediate(
                    requestedMW,
                    isPresetPayload ? presetPercent : Systems.ShadowTradeDailySystem.NoPresetPercent,
                    maxMW,
                    expectedDailyCost,
                    out failReason))
            {
            ...
            if (Log.IsDebugEnabled)
                Log.Debug(isPresetPayload
                    ? $"SetImportMW({presetPercent}% => {requestedMW} MW) applied synchronously"
                    : $"SetImportMW({requestedMW}) applied synchronously");
        ...
        private void OnPurchaseArsenal(int payloadKind, int count)
        {
            ...
            // Offensive arsenal follows the launch gate (war start), not the shadow
            ...
            {
                if (Log.IsDebugEnabled) Log.Debug("PurchaseArsenal rejected: pre-war act");
                return;
            }
        ...
        private bool TryRequireShadowTradeConsumerReady(out ReasonId lockedReasonId)
        {
            ...
            // No act gate: shadow import runs from PreWar (electricity import). Offensive
            // arsenal procurement keeps its own Adaptation gate in OnPurchaseArsenal.
