    ...
    public class AttentionSatireProvider : ISatireProvider
    {
        ...
        public IReadOnlyDictionary<string, SatireConfig> GetConfigs()
        {
            return new Dictionary<string, SatireConfig>
            {
                ...
                // Phase 16 "Batalion Monako" - swayed wealthy families emigrating with their capital
                // ({0} = families fled, {1} = capital fled)
                ["SATIRE_EXODUS_MONACO"] = new("SATIRE_EXODUS_MONACO", 3, "LOCAL_RESIDENT", SocialMood.Warning)
            };
        }
    }
}
