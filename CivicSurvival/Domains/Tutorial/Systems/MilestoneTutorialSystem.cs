...
#pragma warning restore CIVIC148
        ...
        // ===== City population (Core interface, Axiom 5 clean) =====
        // The single owner of the vanilla city population record — see
        // TryGetSettledCitizenCount for why GhostTown does not use a plain citizen query.
        ...
        /// <summary>
        /// People living in the city right now, or false when there is no city entity to ask.
        ///
        /// "Settled" is still literal, but it is now a property of the source rather than a gate
        /// this method has to wait for: the count comes from the vanilla city population record
        /// (<see cref="ICityPopulationReader"/>), which the save restores whole and before the
        /// mod's first tick — exactly like <see cref="ScenarioSingleton.PopulationPeak"/>, the
        /// other side of the GhostTown ratio. There is therefore no window in which the count
        /// reads low because citizens have not streamed back in yet; the window that made 66% of
        /// this modal's production showings land in the first minute of a session is closed by
        /// construction rather than by a readiness level.
        ///
        /// Not the plain citizen query: that one counts tourists, commuters and
        /// dead-but-not-deleted citizens, and with it an empty map reached a peak of six
        /// passers-by, so a drop to one fired "the city is dying" on a city that was never built.
        ///
        /// Both sides of the ratio are this same measure from here on: the peak is a high-water
        /// mark of the same owner's count (ScenarioStateMachine). A peak carried in from an older
        /// save was recorded on the resident scalar, which was never smaller, so until the live
        /// number climbs past it the ratio reads a little lower than it used to — a known,
        /// bounded window that the saved-measurement migration closes, not a defect here.
        /// </summary>
        private bool TryGetSettledCitizenCount(out int population)
        {
            // OnStartRunning is canonical; this re-resolves for the load path, where
            // ValidateAfterLoad runs before OnStartRunning in PLVS sequencing.
            m_CityPopulation ??= ServiceRegistry.Instance.Require<ICityPopulationReader>();

            return m_CityPopulation.TryGetResidentCount(out population);
        }
...
#pragma warning restore CIVIC211
            ...
            // Not ready → no verdict, not a negative one: the one-second poll re-checks
            // GhostTown every tick, so withholding it here costs nothing.
            EnqueueRecoverableModal(
                state,
                ModalFlags.GhostTown,
                state.PopulationPeak.TryGetOn(PopulationMeasure.VanillaCityRecord, out int peakPopulation)
                    && peakPopulation > 0
                    && TryGetSettledCitizenCount(out int currentPopulation)
                    && currentPopulation > 0
                    && currentPopulation < peakPopulation * GHOST_TOWN_POPULATION_RATIO);
...
#pragma warning restore CIVIC211
            ...
            // Poll: GhostTown — population < 30% of peak. A verdict, so the peak has to be a
            // measurement on the counter this live reading comes from: a peak inherited from the
            // superseded one is not divided, and the window simply stays shut until live growth
            // re-establishes a peak here. The window keeps its own magnitude conditions (a peak
            // above zero, a city that still has someone in it) — the settled-city flag is
            // strictly weaker and does not replace them.
            if (state.PopulationPeak.TryGetOn(PopulationMeasure.VanillaCityRecord, out int peakPopulation)
                && peakPopulation > 0
                && TryGetSettledCitizenCount(out int currentPop))
            {
                if (currentPop > 0 && currentPop < peakPopulation * GHOST_TOWN_POPULATION_RATIO)
                {
                    // The two numbers behind the verdict. Without them a "the city is dying"
                    // on a city that plainly is not cannot be told apart from a stale peak
                    // carried over from a previous city in the same session.
                    if (TryShow(ModalFlags.GhostTown))
                        Log.Info($"[MilestoneTutorial] GhostTown shown: population {currentPop} of peak {peakPopulation} ({(float)currentPop / peakPopulation:P1}, threshold {GHOST_TOWN_POPULATION_RATIO:P0})");
                }
            }
        }
