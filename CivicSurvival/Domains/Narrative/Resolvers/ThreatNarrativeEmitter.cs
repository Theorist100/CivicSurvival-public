...
namespace CivicSurvival.Domains.Narrative.Resolvers
{
    ...
    internal static class ThreatNarrativeEmitter
    {
        ...
        {
            if (phase != GamePhase.Calm && phase != GamePhase.Alert && phase != GamePhase.Attack
                && phase != GamePhase.PsyAttack && phase != GamePhase.Recovery)
            {
            ...
            string phaseName = phase switch
            {
                ...
                GamePhase.PsyAttack => LocalizationManager.Get("THREAT_PHASE_PSYATTACK"),
                GamePhase.Recovery => LocalizationManager.Get("THREAT_PHASE_RECOVERY"),
                _ => phase.ToString()
            };

            // PsyAttack pushes its own toast — this is how the player learns an info-war
            // raid has opened without watching the PSYOPS board (distinct id, own name).
            if (phase == GamePhase.Alert || phase == GamePhase.Attack || phase == GamePhase.PsyAttack)
            {
                sink.Push(new NarrativeToastDto(
                    NotificationType.SystemAlert,
                    $"wave_{waveNumber}_{phase}",
                    LocalizationManager.Get("THREAT_ALERT_TITLE"),
                    LocalizationManager.Get("THREAT_PHASE_TOAST_BODY", phaseName, waveNumber),
        ...
        public static void EmitHeritageGranted(NotificationState sink, IEventBus? bus, HeritageGrantedEvent evt)
        {
            ...
            // NEWS: official TRO command channel (@TerritorialDefense).
            NarrativeEmitter.EmitNews(
                bus,
                "TRO_COMMANDER",
        ...
        public static void EmitDebris(NotificationState sink, IEventBus? bus, IReadOnlyList<float3> debris)
        {
            ...
            if (debrisCount == 1)
            {
                ...
                // NEWS: official State Emergency Service channel (@RescueService_Official).
                NarrativeEmitter.EmitNews(
                    bus,
                    "DSNS",
