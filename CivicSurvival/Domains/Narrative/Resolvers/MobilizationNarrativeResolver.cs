...
namespace CivicSurvival.Domains.Narrative.Resolvers
{
    ...
    ///
    /// ManpowerCritical arrives once per critical EPISODE, not on a timer: the publisher fires on
    /// the transition into critical and re-arms only after the pool clears the recovery band, and
    /// it seeds its latch on load so a save of an already-critical city does not re-toast. Nothing
    /// to rate-limit here — do not add a cooldown, it would swallow genuine repeat collapses.
