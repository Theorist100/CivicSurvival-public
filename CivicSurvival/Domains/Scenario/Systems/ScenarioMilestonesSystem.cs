...
namespace CivicSurvival.Domains.Scenario.Systems
{
    ...
    public partial class ScenarioMilestonesSystem : CivicSystemBase, IDefaultSerializable, IResettable, IPostLoadValidation
    {
        ...
        /// (<see cref="ICityPopulationReader"/>). The victory check divides it by the recorded
        /// peak, so it has to be the same measure the peak is raised from — which it is, both
        /// sides come from this owner (<c>ScenarioStateMachine.OnThrottledUpdate</c>).
        ///
        /// <c>false</c> means "cannot answer" — no city entity — and for a victory check that
        /// is simply "not yet", never "the city is empty"; an empty city answers <c>true</c>
        /// with zero and fails the retention test on its merits.
        /// </summary>
        ...
        private bool CheckVictoryConditions()
        {
            ...
            // One measure on both sides, enforced rather than assumed: the retention check
            // divides a live reading by a recorded one, so the recorded one must carry this
            // measure's stamp. A peak from the superseded counter yields no verdict — victory is
            // withheld, never granted — which is the only safe direction here. Re-taking the
            // peak from the live count instead would satisfy `current >= peak × 0.5` the instant
            // it was written, i.e. award victory for the measure change itself.
            if (!TryGetResidentPopulation(out int currentPop))
                return false;

            if (!m_StateMachine.PeakPopulation.TryGetOn(PopulationMeasure.VanillaCityRecord, out int peakPop)
                || peakPop <= 0)
            {
                return false;
            }
...
#pragma warning restore CIVIC226
        ...
        private void OnPostLoad(ScenarioTypeDetectedEvent evt)
        {
            ...
            // FIX W7-M5: Skip victory check on load — a city mid-restore can still read empty.
