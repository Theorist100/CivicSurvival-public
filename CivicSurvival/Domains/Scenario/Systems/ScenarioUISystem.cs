...
namespace CivicSurvival.Domains.Scenario.UI
{
    ...
    public partial class ScenarioUISystem : ThrottledUISystemBase
    {
        ...
        // Second channel, not a sentinel in the first. The percent is a float on the wire and
        // every value in [0,1] is a legal reading, so "unknown" has nowhere to live inside it —
        // exactly the shape problem the DTO contract has with numbers, answered the same way.
        private ProfiledBinding<bool> m_PopulationMeasuredBinding = null!;
        ...
        // The one owner of "how many people live here" — this panel shows it, it does not
        // measure it. Pause-safe: the owner answers from a live ECS query, so a readout that
        // ticks in UIUpdate gets the same number whether the simulation is running or paused
        // (Axiom 14).
        private ICityPopulationReader m_CityPopulationReader = null!;
        ...
        protected override void OnCreate()
        {
            ...
            m_PopulationMeasuredBinding = new ProfiledBinding<bool>(Group, PopulationMeasured, false);
            ...
            AddBinding(m_PopulationMeasuredBinding.Binding);
            ...
            AddBinding(new TriggerBinding<string, string>(Group, ModLoadFailureAction,
                CivicGameLifecycle.GameplayOnly<string, string>(OnModLoadFailureAction)));
            AddBinding(new TriggerBinding(Group, DismissModUpdatedRestart,
                CivicGameLifecycle.GameplayOnly(OnDismissModUpdatedRestart)));
            AddBinding(new TriggerBinding(Group, DismissModUpdatePending,
                CivicGameLifecycle.GameplayOnly(OnDismissModUpdatePending)));
            AddBinding(new TriggerBinding<string, string>(Group, ModUpdateModalAction,
                CivicGameLifecycle.GameplayOnly<string, string>(OnModUpdateModalAction)));
        ...
        /// same one the peak this readout divides by is raised from, so the panel shows a
        /// population against itself.
        ///
        /// <c>false</c> means "cannot answer" (no city entity). An empty city is a real reading
        /// of zero and moves the readout to 0%; a refusal is not a reading at all and is
        /// reported as such on its own binding.
        /// </summary>
        ...
        protected override void OnThrottledUpdate()
        {
            ...
            // A readout, so it is not gated on the measure: the panel shows the share against
            // whatever peak the city has, including one carried in from the superseded counter.
            // That share reads low on such a save until live growth re-establishes the peak —
            // which is a true statement about two numbers the game really recorded, whereas
            // blanking the panel or converting the old peak by a factor would not be.
            //
            // A REFUSAL is different, and this is the place it used to be handled wrongly: the
            // last good percentage was re-published as if it were current, and the comment said
            // so out loud. A display owes a dash, not a stale number wearing a fresh number's
            // clothes — the player cannot tell the two apart, and this readout is the war
            // summary the endgame modals print. The value is still carried (the renderer has
            // something to fall back to) but it now travels next to the flag that says whether
            // anyone measured it.
            bool hasPopulation = TryGetResidentPopulation(out int currentPop);
            int peakPopulation = state.PeakPopulation.Value;
            if (hasPopulation || peakPopulation <= 0)
            {
                m_LastGoodPopulationPercent = peakPopulation > 0
                    ? System.Math.Min((float)currentPop / peakPopulation, 1f)
                    : 1f;
            }
            m_PopulationPercentBinding.Update(m_LastGoodPopulationPercent);
            // No peak to divide by is a full answer (the readout is 100% by construction), so it
            // counts as measured; only the owner's refusal does not.
            m_PopulationMeasuredBinding.Update(hasPopulation || peakPopulation <= 0);
...
#pragma warning disable CIVIC098 // ModalCoordinator.Instance is static readonly = new(), never null
        ...
        // Restart notice from ModInstanceWatchSystem (Core bootstrap): the install folder was
        // deleted under the live process; dismiss lives here with the other modal triggers.
        private void OnDismissModUpdatedRestart()
        {
            ModalCoordinator.Instance.Dismiss("ModUpdatedRestart");
            Log.Info("Mod updated-restart modal dismissed");
        }

        // Pre-publish announce from UpdateAnnounceSystem: an update goes live in N minutes.
        // Dismiss closes only the modal — the countdown badge keeps ticking until publish.
        private void OnDismissModUpdatePending()
        {
            ModalCoordinator.Instance.Dismiss("ModUpdatePending");
            Log.Info("Mod update-pending modal dismissed");
        }

        // Telemetry ack from the ModLoadFailure modal component: "shown" fires on render (the
        // only proof the broken-install notice reached the player's eyes — the coordinator slot
        // alone can't tell, the UI bundle may have failed to load), "continue"/"send" on the
        // buttons. Values are UI input riding a whitelisted contract enum — pass through only
        // known ones so a drifted UI can't inject free-form strings into telemetry.
        private void OnModLoadFailureAction(string action, string cause)
        {
            bool knownAction = action is "shown" or "continue" or "send";
            bool knownCause = cause is "FilesMissingOnDisk" or "AssetsFailedToLoad";
            if (!knownAction || !knownCause)
            {
                Log.Warn($"ModLoadFailure modal action dropped (unknown value): action={action}, cause={cause}");
                return;
            }

            EventBus?.SafePublish(new ModLoadModalActionEvent(action, cause));
            Log.Info($"ModLoadFailure modal action: {action} ({cause})");
        }

        // Telemetry ack from the update-safety modals (ModUpdatePending announce countdown /
        // ModUpdatedRestart deletion notice): "shown" fires on render — the only proof the notice
        // reached the player's eyes (the C#-side show request alone can't tell; the React layer
        // may be dead, the exact blind spot of the 2026-07-21 α sessions) — "dismissed" on the
        // button. Values are UI input riding a whitelisted contract enum — pass through only
        // known ones so a drifted UI can't inject free-form strings into telemetry.
        private void OnModUpdateModalAction(string modal, string action)
        {
            bool knownModal = modal is "pending" or "restart";
            bool knownAction = action is "shown" or "dismissed";
            if (!knownModal || !knownAction)
            {
                Log.Warn($"Update modal action dropped (unknown value): modal={modal}, action={action}");
                return;
            }

            EventBus?.SafePublish(new ModUpdateModalEvent(modal, action));
            Log.Info($"Update modal action: {modal}/{action}");
        }
