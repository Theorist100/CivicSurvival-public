...
namespace CivicSurvival.Domains.Scenario.Systems
{
    ...
    public partial class WaveScheduler : CivicSystemBase, IResettable
    {
        ...
        // Next-wave forecast: the TYPE is rolled once per wave cycle at the first Recovery/Calm
        // entry (the "enemy plans the next raid as soon as the previous one ends") and CONSUMED
        // by ScheduleAlertPhase without a re-roll — so the paid insider preview shown during
        // Calm/Recovery matches the wave that actually launches. The count forecast is the
        // deterministic (noiseMult=1) estimate off the current context, refreshed on each
        // Recovery/Calm (re)entry; the launch still rolls ±20% noise off the live context.
        private WaveType m_NextWaveType;
        private int m_NextWaveThreatsForecast;
        private bool m_HasNextWaveForecast;
...
#pragma warning restore CIVIC066
        ...
        // City population → defence-potential density surcharge (a populous city that COULD crew a
        // strong defence draws denser waves). Read through the single owner of the vanilla city
        // population record instead of a private ComponentLookup + CitySystem pair: same number,
        // same cost, one place that knows how the record is reached.
        ...
        private void StartSchedulerAfterWarStarted()
        {
            ...
            // The intro wave is in flight (siren running or drones flying): WarStarted arrives
            // MID-WAVE by design — the intro cinematic completes on its own timers, seconds after
            // IntroAttackEvent, while the wave itself runs Alert (Scenario.IntroAlertSeconds) and
            // then Attack. Enable scheduling and keep the wave state untouched: the phase loop
            // advances Alert → Attack when the timer lapses, and WaveExecutor finishes
            // Attack → Recovery. Overriding to Calm here kills the Alert timer and the wave with
            // it (live regression 2026-07-23 17:10 — danger staged at the siren, then silence).
            bool introWaveInFlight = m_WaveRole == WaveRole.Intro
                && (m_CurrentPhase == GamePhase.Alert || m_CurrentPhase == GamePhase.Attack);
            if (introWaveInFlight)
            {
                if (m_CurrentPhase == GamePhase.Attack)
                    m_PhaseEndTime = math.max(m_PhaseEndTime, m_TimeInPhase + MIN_ATTACK_PHASE_EXTENSION);
                Log.Info(" WarStartedEvent - scheduling enabled (intro wave in flight, not overriding phase)");
                return;
            }
            ...
            // First calm precedes the first schedulable wave — decide it now so the insider
            // preview describes the actual upcoming wave instead of the singleton defaults.
            EnsureNextWaveForecast();
        ...
        private void OnIntroAttack(IntroAttackEvent evt)
        {
            ...
            // Single config snapshot (CIVIC347 — torn-read guard on hot-reload).
            ...
            float introStrengthMult = cfg.Waves.IntroStrengthMult;
            ...
            // Alert command with a REAL duration (config Scenario.IntroAlertSeconds; historically
            // 0.1 s, which fused siren and spawn into one tick). The gap is what gives the intro
            // wave an evacuation window: target preselection (ThreatSpawnSystem.OnWaveStarting)
            // and the siren-time danger staging happen at Alert, and citizens get the whole gap
            // PLUS the drones' flight time to reach a shelter. The Attack command is NOT published
            // here anymore — the scheduler's own phase loop fires ScheduleAttackPhase() when the
            // Alert timer lapses (case GamePhase.Alert in UpdatePhase), reusing the local wave
            // state set below (including WaveRole.Intro).
            float alertDuration = math.max(0.1f, cfg.Scenario.IntroAlertSeconds);
            ...
            m_TimeInPhase = 0f;
            m_PhaseEndTime = alertDuration;

            Log.Info($" Intro wave {INTRO_WAVE_NUMBER}: {threatCount} threats (Harassment x{introStrengthMult:F2}), {m_Context.CitySizeMW}MW city, alert {alertDuration:F0}s");
        }
        ...
        private void OnPhaseChanged(ThreatNarrativeEvent evt)
        {
            ...
            if (evt.Phase != m_CurrentPhase)
            {
                ...
                // Leaving Calm to any other phase — clear the dawn/dusk recheck transients so a stale
                // wait can't leak into the next Calm. Makes the previously implicit cross-path invariant explicit.
                if (evt.Phase != GamePhase.Calm)
                    ExitCalmTransients();
                ...
                switch (evt.Phase)
                {
                    case GamePhase.Recovery:
                        ...
                        // The wave just ended — decide the NEXT one here so the insider
                        // preview is truthful for the whole Recovery/Calm purchase window.
                        m_Context = GatherContext();
                        EnsureNextWaveForecast();
                        break;
                    case GamePhase.Alert:
                        // Self-scheduled: the scheduler is the ONLY producer of Alert commands
                        // (ScheduleAlertPhase / OnIntroAttack), so this event is always an echo of
                        // a duration set moments ago. Overwriting it with MIN_ATTACK_PHASE_EXTENSION
                        // silently cut EVERY siren to ~40 s regardless of config — the Alert timer
                        // fired 3x early and the evacuation window never actually widened (live
                        // finding 2026-07-23 18:33: "alert 120s" announced, spawn at 41 sim-s).
                        break;
                    case GamePhase.Attack:
                    case GamePhase.PsyAttack:
                        // PsyAttack, like Attack, exits via WaveExecutor (timer→Recovery);
                        // the scheduler just holds a non-immediate timer.
                        m_PhaseEndTime = MIN_ATTACK_PHASE_EXTENSION;
                        break;
                    case GamePhase.Calm:
                        m_Context = GatherContext();
                        m_PhaseEndTime = WaveScalingService.CalculateCalmDuration(m_Context, ref m_Random);
                        // Executor-initiated Calm entry (Recovery self-timeout) — make sure the
                        // forecast exists / refresh its count off the fresh context.
                        EnsureNextWaveForecast();
                        break;
                    default:
        ...
        private static int GetPhaseOrder(GamePhase phase) => phase switch
        {
            ...
            GamePhase.Alert => 1,
            GamePhase.Attack => 2,
            GamePhase.PsyAttack => 3,
            GamePhase.Recovery => 4,
            _ => 0
        };
...
#pragma warning restore CIVIC056
        ...
        private void TryScheduleTransition()
        {
            ...
            switch (m_CurrentPhase)
            {
                ...
                case GamePhase.Attack:
                case GamePhase.PsyAttack:
                    // Attack → PsyAttack → Recovery is handled by WaveExecutor (threats
                    // cleared, then the info-war beat elapses). Scheduler just extends the timer.
        ...
        /// <summary>
        /// Clear the Calm dawn/dusk recheck transients. Single source for both the scheduling path
        /// (window arrived) and the phase-sync path (any transition out of Calm) so a stale wait can't
        /// leak into the next Calm.
        /// </summary>
        private void ExitCalmTransients()
        {
        ...
        /// <summary>
        /// Reset the non-persisted launch-window and readiness-gate runtime transients. Single source
        /// for every reset leg (ResetState / ResetToBootDefaults / Deserialize) so a stale value from
        /// the previous city cannot survive the reused system instance across a load — the "defaults to
        /// 0 after load" assumption only holds if every leg clears these explicitly. Data-only.
        /// </summary>
        private void ClearRuntimeTransients()
        {
            m_InDawnDuskRecheck = false;
            m_DawnDuskWaitElapsed = 0f;
        ...
        private void ScheduleAlertPhase()
        {
            ...
            if (!m_Context.AttacksEnabled)
            {
                // Attacks disabled - stay in Calm. Keep the forecast alive (same wave whenever
                // it eventually launches), refreshing the count estimate off the fresh context.
                m_TimeInPhase = 0f;
                m_PhaseEndTime = WaveScalingService.CalculateCalmDuration(m_Context, ref m_Random);
                EnsureNextWaveForecast();
            ...
            // Consume the forecast decided at Recovery/Calm entry — no re-roll, so the insider
            // preview the player paid for during Calm matches the wave that actually launches.
            // Fallback roll covers saves made before the forecast existed.
            var waveType = m_HasNextWaveForecast
                ? m_NextWaveType
                : WaveScalingService.DetermineWaveType(nextWaveNumber, ref m_Random);
            m_HasNextWaveForecast = false; // Executor clears WaveStateSingleton.NextWaveForecastReady on Alert
        ...
        private void ScheduleCalmPhase()
        {
            ...
            // Type was already decided at Recovery entry (kept); refresh the count estimate
            // off the context gathered for this transition.
            EnsureNextWaveForecast();
        ...
        /// <summary>
        /// Decide (or refresh) the forecast of the NEXT wave. The type is rolled ONCE per wave
        /// cycle — subsequent calls keep it so the insider preview never flips mid-Calm; the
        /// deterministic count estimate is recomputed from the current context on every call so
        /// it tracks city changes during the calm. Published to WaveExecutor (single writer of
        /// WaveStateSingleton.NextWave*) via WaveForecastEvent; consumed by ScheduleAlertPhase.
        /// Requires m_Context to be freshly gathered by the caller.
        /// </summary>
        private void EnsureNextWaveForecast()
        {
            int nextWaveNumber = math.max(m_WaveNumber + 1, FIRST_REGULAR_WAVE_NUMBER);
            if (!m_HasNextWaveForecast)
            {
                m_NextWaveType = WaveScalingService.DetermineWaveType(nextWaveNumber, ref m_Random);
                m_HasNextWaveForecast = true;
            }

            // Deterministic pre-noise estimate (noiseMult=1) — no RNG consumed, safe to recompute.
            m_NextWaveThreatsForecast = WaveScalingService.CalculateThreatCount(
                m_NextWaveType,
                nextWaveNumber,
                m_Context.CitySizeMW,
                m_Context.IntensityModifier,
            ...
            EventBus?.SafePublish(new WaveForecastEvent(
                HasForecast: true,
                NextWaveType: m_NextWaveType,
                NextWaveNumber: nextWaveNumber,
                ThreatsForecast: m_NextWaveThreatsForecast), "WaveScheduler");

            if (Log.IsDebugEnabled)
                Log.Debug($" Next-wave forecast: #{nextWaveNumber} {m_NextWaveType}, ~{m_NextWaveThreatsForecast} threats");
        }
        ...
        private void ApplyInitialState()
        {
            ...
            if (m_HasSavedState && m_SavedState.HasValue)
            {
                ...
                m_NextWaveType = saved.NextWaveType;
                m_NextWaveThreatsForecast = saved.NextWaveThreatsForecast;
                m_HasNextWaveForecast = saved.HasNextWaveForecast;
...
#pragma warning restore CIVIC211
            ...
            // City population → defence-potential density signal. 0 (no city / boot / empty city)
            // → no density surcharge (fail-safe, handled in Gather). No sync point.
            ...
            // Game-hour for the enemy respite window: the respite expiries on
        ...
        /// <summary>
        /// People living in the city right now, from the single owner of the vanilla city
        /// population record. O(1), no sync point (the read waits only on the vanilla Population
        /// writer).
        ///
        /// Returns 0 both when the owner refuses (no city entity) and when the city is genuinely
        /// empty. The two are NOT distinguishable downstream today and that is deliberate for
        /// this change: the number is passed positionally into <c>WaveContextGatherer.Gather</c>,
        /// which itself gates on <c>population &gt; 0</c> and fail-safes the density surcharge off
        /// — so refusal and empty already produced identical waves before the owner existed.
        /// Making the refusal visible means widening the gatherer's signature, which is a separate
        /// change with its own reason to exist; folding it in here would alter wave scaling under
        /// cover of a source swap.
        /// </summary>
        private int GetPopulation()
        {
            // Re-resolve HERE, not only in OnStartRunning: GatherContext is also driven from event
            // handlers (war-started / intro / phase-changed), mirroring the
            // m_DemandPeakBucketLookup.Update inside GatherContext. Idempotent after the first hit.
            m_CityPopulation ??= ServiceRegistry.Instance.Require<ICityPopulationReader>();
            return m_CityPopulation.TryGetResidentCount(out int residents) ? residents : 0;
        }
        ...
        public void ResetState()
        {
            ...
            m_ThreatsExpected = 0;
            m_NextWaveType = WaveType.Harassment;
            m_NextWaveThreatsForecast = 0;
            m_HasNextWaveForecast = false;
            m_LightingMissingWarned = false;
            ClearRuntimeTransients();
        ...
        internal readonly struct SchedulerSaveState
        {
            ...
            public readonly bool HasNextWaveForecast;
            ...
            public SchedulerSaveState(GamePhase phase, float timeInPhase, float phaseEndTime,
                ...
                WaveType nextWaveType, int nextWaveThreatsForecast, bool hasNextWaveForecast,
                uint randomState, bool scenarioStarted, bool warStartedReceived, bool introAttackFired)
            {
                ...
                HasNextWaveForecast = hasNextWaveForecast;
        ...
        internal SchedulerSaveState GetSaveState()
        {
            ...
            return new SchedulerSaveState(
                ...
                m_ThreatsExpected,
                m_NextWaveType,
                m_NextWaveThreatsForecast,
                m_HasNextWaveForecast,
                m_Random.state,
                m_ScenarioStarted,
