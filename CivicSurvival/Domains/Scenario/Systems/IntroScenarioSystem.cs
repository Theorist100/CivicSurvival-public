...
namespace CivicSurvival.Domains.Scenario.Systems
{
    ...
    public partial class IntroScenarioSystem : CivicUISystemBase, IResettable
    {
        ...
        // Beat left after the air-defense quest settles inside the warning window: long enough
        // to read the outcome modal, short enough that the intro does not idle.
        private const float WINDOW_AFTER_QUEST_SECONDS = 8f;
        ...
        // Manpower free when the activists' window opens — the air-defense quest sizes its
        // target off it. Read through the Mobilization read-contract (never its singleton:
        // CIVIC211) and shipped out on the event, so Narrative gets the number without
        // reaching into another domain either. Resolved in OnStartRunning; Require =
        // fail-loud (CIVIC463), the SickLeaveWaveSystem pattern.
        ...
        /// <summary>
        /// Single source for the transient latches (deferred-flag family, siren-hold
        /// accumulator, online-consent beat), called from every reset leg — ResetState,
        /// ResetToBootDefaults and the start of Deserialize — so the legs cannot drift.
        /// A consent deferral or a mid-beat timer armed in the previous session must never
        /// re-fire in the loaded city; Deserialize recomputes the deferred flags it needs
        /// from the loaded state right after this clear.
        /// </summary>
        ...
        protected override void OnCreate()
        {
            ...
            // instead of independently counting citizens, which could return 0 during hot reload
            ...
            // The warning window is a ceiling, not a wait: once the air-defense quest resolves
            // there is nothing left to prepare for, so the sirens come early.
        ...
        protected override void OnStartRunning()
        {
            ...
            // Manpower read-contract (MobilizationSystem registers it in its OnCreate, which
            // precedes any first update). Require = fail-loud, the SickLeaveWaveSystem pattern.
...
#pragma warning restore CIVIC239
        ...
        protected override void OnUpdateImpl()
        {
            ...
            //
            // One SIMULATION TICK per call, not Time.deltaTime: this system runs in
            // GameSimulation, which SimulationSystem.OnUpdate executes in a loop — up to 8
            // times per rendered frame (decompile: SimulationSystem.cs:238 `num = floor(m_Timer
            // * 60f)`, loop at :279-286). Charging a whole frame's delta on every pass burned
            // the phase timers several times over, so a 120s window really lasted 66s (live log
            // 2026-07-24). A fixed 1/60 step is what one tick actually is, so the seconds in the
            // balance contract are the seconds the player gets — and a paused game charges
            // nothing at all, because the phase does not tick (AXIOM 14).
            using (PerformanceProfiler.Measure("IntroScenario.StateMachine"))
            {
                UpdateIntroSequence(1f / Engine.Timing.SIMULATION_FPS);
            }
        ...
        /// <summary>
        /// React to ScenarioTypeDetectedEvent from ScenarioStateMachine. Single source of truth
        /// — no independent citizen count of its own that could return 0 during hot reload. Fires on
        /// every save load, and again if a village grows out of village size mid-session, which
        /// is when its cold open finally plays.
        /// </summary>
        private void OnScenarioTypeDetected(ScenarioTypeDetectedEvent evt)
        {
            ReopenIntroForPreWarVillage(evt.Type);
            ...
            // Village → the cold open is POSTPONED, not finished. Nothing is coming for a
            // settlement this small, so there is no war to open; OminousSignsSystem runs the
            // pre-war atmosphere meanwhile. If the settlement grows past the town boundary,
            // ScenarioStateMachine reclassifies it and re-announces the type, and this handler
            // runs again — that second pass is where the cold open plays.
            //
            // Marking the intro completed here (what this used to do) burned it for good: the
            // early return above swallowed the promotion, and a city of a thousand entered the
            // war with no air raid, no first strike and no air-defense quest. Completed is set
            // when the intro has actually been played or the player turned it off — never
            // because the settlement was small at the time.
            if (evt.Type == ScenarioType.Village)
            {
                Log.Info($" Village detected (pop={evt.Population}) - Cold Open postponed until the settlement outgrows village size");
                m_IntroHudVisibleBinding.Update(true);
                return;
            }
            ...
            if (introSettings.SkipIntro)
            {
                ...
                // The war still has to begin, and IntroCompleteEvent is what asks
                // ScenarioStateMachine to begin it. With the cold open turned off nothing else
                // will — including for a village that just grew into a town, which reaches this
                // branch on its promotion.
        ...
        /// <summary>
        /// Reopen an intro that a save says is finished when the settlement it belongs to has
        /// not been to war at all.
        ///
        /// Saves written before the cold open learned to wait carry IntroCompleted = true on
        /// every village: the old branch marked the intro done the moment it saw a settlement
        /// below town size. Loaded as-is, that flag swallows the promotion event too, and since
        /// nothing else starts the war any more, the settlement would grow into a city and stay
        /// at peace forever — a worse outcome than the bug this all fixes.
        ///
        /// A village still in pre-war is proof the intro was never actually played: the cold
        /// open only runs at the town boundary, and running it is what starts the war. So the
        /// flags are cleared and the intro returns to being postponed. A player who turned the
        /// intro off in the mod settings gets it re-applied on the promotion, from the settings
        /// rather than from a stale save.
        /// </summary>
        private void ReopenIntroForPreWarVillage(ScenarioType detectedType)
        {
            if (detectedType != ScenarioType.Village
                || m_State.IsIntroPlaying
                || (!m_State.IntroCompleted && !m_State.SkipIntro))
            {
                return;
            }

            // The state machine commits the singleton before announcing the type, so this reads
            // the act the announcement belongs to, not the previous one.
            if (!SystemAPI.TryGetSingleton<ScenarioSingleton>(out var scenario) || scenario.IsWarStarted)
                return;

            m_State.IntroCompleted = false;
            m_State.SkipIntro = false;
            m_State.IntroPhase = IntroPhase.None;
            m_IntroPhaseBinding.Update((int)IntroPhase.None);
            Log.Info("Pre-war village save carried a finished intro from the old village path - reopening it, the Cold Open plays when the settlement outgrows village size");
        }
...
#pragma warning restore CIVIC239
        ...
        private void OnAcceptReality()
        {
            ...
            // The activists call before the sirens: hand the HUD over now so the city can put
            // guns up while there is still time. Without this beat the player first touches the
            // game after the opening strike has already landed — which is why the median air
            // defense on wave one was zero.
            TransitionToActivistCall();
        }

        /// <summary>
        /// The air-defense quest settled while the warning window was still open — the city is
        /// as ready as it is going to get, so cut the window to a short beat instead of holding
        /// the player in an empty intro (live log 2026-07-24: target met at 21s, window ran on
        /// for four more minutes with nothing to do). Other quests are none of the intro's
        /// business, and a window already past this point is left alone.
        /// </summary>
        private void OnQuestResolved(QuestResolvedEvent evt)
        {
            if (evt.QuestId != (int)QuestId.AirDefenseReady) return;
            if (!m_State.IsIntroPlaying || m_State.IntroPhase != IntroPhase.ActivistCall) return;
            if (m_State.IntroTimer <= WINDOW_AFTER_QUEST_SECONDS) return;

            m_State.IntroTimer = WINDOW_AFTER_QUEST_SECONDS;
            Log.Info($"Air-defense quest resolved during the window — sirens in {WINDOW_AFTER_QUEST_SECONDS}s");
        }

        /// <summary>
        /// Phase 1-&gt;2: Modal -&gt; ActivistCall. Reveals the HUD early and opens a timed window
        /// in which the player can place air defense before the first strike.
        /// </summary>
        private void TransitionToActivistCall()
        {
            Log.Info("Phase: ACTIVIST CALL");

            // Hand the controls over; the warning itself arrives as the quest call toast that
            // QuestSystem places on the window-opened event, in the same shape as every other
            // character call (no second, parallel notice competing with it).
            m_IntroHudVisibleBinding.Update(true);

            m_State.IntroPhase = IntroPhase.ActivistCall;
            m_State.IntroTimer = BalanceConfig.Current.Scenario.IntroActivistWindow;
            m_IntroPhaseBinding.Update((int)IntroPhase.ActivistCall);

            // Zero free manpower is a legitimate answer here (the quest then declines to arm
            // rather than asking for a gun the city cannot crew) — but an unavailable reader
            // is not, hence the fail-loud resolve in OnStartRunning.
            //
            // The phase transition is NOT deferred on a refusal: it also flips the intro timer
            // and hands over the HUD, and the strike behind it has no waiting limit. So the
            // unknown travels on the event instead — empty, not zero — and the quest side
            // decides what to do with it. Zero here would mean "the city has nobody free",
            // which the quest reads as a legitimate reason to stay silent; that is a different
            // statement from "nobody has counted".
            int? available = m_ManpowerReader.TryGetAvailableManpower(out int free) ? free : null;
            EventBus?.SafePublish(new ActivistWindowOpenedEvent(available), "IntroScenarioSystem");

            Log.Info($"Activist window open for {m_State.IntroTimer:F0}s (available manpower={(available.HasValue ? available.Value.ToString() : "unmeasured")})");
        }

        /// <summary>
        /// Phase 2-&gt;3: ActivistCall -&gt; Silence. The warning window is over, but the quest is
        /// NOT scored here — it runs through the opening strike (QuestSystem.OnWaveEnded), so
        /// guns raised while the drones are overhead still count.
        /// </summary>
        private void TransitionToSilence()
        {
            Log.Info("Phase: SILENCE");
        ...
        private void UpdateIntroSequence(float deltaTime)
        {
            ...
            switch (m_State.IntroPhase)
            {
                ...
                case IntroPhase.ActivistCall:
                    // The window runs on scaled time deliberately: a player who pauses to think
                    // through the placement is not burning their warning.
...
#pragma warning restore CIVIC056
        ...
        private void CompleteIntro()
        {
            ...
            {
                // IntroScenarioSystem is the single publisher of IntroCompleteEvent, and this
                // is the only way into the war for every settlement size.
                // ScenarioStateMachine.StartWar keeps its guard (CurrentAct != PreWar → return)
                // against a second publication, e.g. the post-load re-sync above.
                EventBus?.SafePublish(new IntroCompleteEvent(), "IntroScenarioSystem");
