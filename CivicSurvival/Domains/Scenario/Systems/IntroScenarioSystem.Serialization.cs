...
namespace CivicSurvival.Domains.Scenario.Systems
{
    ...
    public partial class IntroScenarioSystem : IDefaultSerializable, IBootDefaultsReset
    {
        ...
        public void Deserialize<TReader>(TReader reader) where TReader : IReader
        {
            ...
            {
                m_ChirperStorm = null;
                // Clear ALL transient latches before recomputing the deferred flags from the
                // loaded state below — a consent deferral / beat / siren accumulator armed in
                // the previous session must not re-fire in the loaded city.
                ClearTransientState();
                ...
                if (m_State.IsIntroPlaying && m_State.IntroPhase != IntroPhase.Modal)
                {
                    ...
                    // Sequence order, not enum order (IntroPhases): a save taken during the
                    // activists' window is still pre-strike, and a numeric compare would read
                    // ActivistCall = 7 as "past Attack" and drop the opening wave entirely.
                    if (IntroPhases.IsBeforeStrike(m_State.IntroPhase))
                ...
                {
                    m_IntroHudVisibleBinding.Update(IntroPhases.ShowsHud(m_State.IntroPhase));
                }
                else
                {
