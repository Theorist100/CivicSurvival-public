...
#pragma warning restore CIVIC035
        ...
        // Both sidecar and building resolve through the one shared liveness contract
        // (TargetLiveness.IsLiveTarget = exists && !Deleted && !Destroyed). The two named
        // wrappers keep the call-sites self-documenting about which entity is being tested;
        // the predicate itself has a single source so the resolver, the StablePlantId map
        // (PlantWearSimulation.IsLivePlantEntity), and cleanup cannot drift apart.
        private static bool IsLiveWearEntity(ref PlantRepairContext ctx, Entity entity)
            => TargetLiveness.IsLiveTarget(entity, ctx.StorageInfoLookup, ctx.DeletedLookup, ctx.DestroyedLookup);

        private static bool IsLiveBuilding(ref PlantRepairContext ctx, Entity buildingEntity)
            => TargetLiveness.IsLiveTarget(buildingEntity, ctx.StorageInfoLookup, ctx.DeletedLookup, ctx.DestroyedLookup);
    }
}
