...
namespace CivicSurvival.Domains.Engineering.Systems
{
    ...
    public partial class PowerCapacityResolverSystem
    {
        ...
        internal struct ResolveAggregates
        {
            ...
            // Loss-breakdown aggregates (debug-gated; see [CapacityLoss] log line). The four
            // loss buckets + Allowed live in the shared Core decomposition (also used by the
            // PowerGrid UI breakdown, which recomputes from the published per-plant snapshots).
            public PowerLossBreakdown Losses;
        ...
        private struct PlantResolveJob : IJob
        {
            ...
            /// <summary>
            /// Loss buckets via the shared Core decomposition (<see cref="PowerLossBreakdown"/>,
            /// the same chain PowerCapacityMath.ComputeEffectiveFactor applies), plus the
            /// job-only aggregates (foreign boost, delivered flow).
            /// </summary>
            ...
            {
                bool knockedOut = PowerCapacityMath.IsKnockedOut(state);
                agg.Losses.Add(
                    w.OriginalCapacityKW, knockedOut,
                    PowerCapacityMath.GetDamageMultiplier(state),
                    state.SaturationFactor, state.FuelFactor);
                if (!knockedOut)
                {
                    ...
                    agg.HealthyKW += w.OriginalCapacityKW;
                    agg.BoostWeighted += w.OriginalCapacityKW * (double)(EfficiencyLookup.HasBuffer(w.Entity)
