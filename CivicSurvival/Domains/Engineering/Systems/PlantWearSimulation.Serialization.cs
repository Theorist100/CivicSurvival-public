...
namespace CivicSurvival.Domains.Engineering.Systems
{
    ...
    {
        // Deferred boot-reset latch: the boot leg must stay field-only (CIVIC462 /
        // Invariant 5b — no job completion or container disposal mid-load-pass), so
        // ResetToBootDefaults arms this flag and the guaranteed post-load
        // ValidateAfterLoad pass performs the actual CompleteAndClearRuntimeBuffers.
        // Cleared by ResetState so a latch armed by a failed load can never leak a
        // spurious buffer wipe into a subsequent new game.
...
#pragma warning restore CIVIC324
        ...
        private void ResetState()
        {
            ResetManualPersistState();
            InitializeGate();
            // Single-source runtime-buffer reset (shared with the deferred boot-leg pass in
            // ValidateAfterLoad) — completes the in-flight Burst job, clears the native
            // containers (IsCreated guards support early reset/test-world setup) and
            // disposes the output buffer.
            CompleteAndClearRuntimeBuffers();
            // The buffers are clean now — a boot-reset latch armed by a failed load has
        ...
        private void CompleteAndClearRuntimeBuffers()
        {
            ...
            // m_IgniteQueuedThisFrame retired — see comment above.
