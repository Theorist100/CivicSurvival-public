...
#pragma warning restore CIVIC241
        ...
        protected override void OnThrottledUpdate()
        {
            ...
            // into an economy/morale layer; at the deficit boundary the two are identical
...
#pragma warning restore CIVIC070
            {
                balance = singleton.RawBalance;  // physical: Production(flow) − active load. Export excluded.
            ...
            using (Core.Utils.PerformanceProfiler.Measure("GridStress.OnUpdate"))
            {
                ...
                // The achieved scale of the city, not a ratio: the grace threshold is a curve of
                // one number, so it is read for its value regardless of which counter recorded
                // it. A peak carried over from the superseded counter shifts the grace slightly,
                // which is a calibration question (the GridGraceRefPop anchor), not a reason to
                // withhold a threshold and leave the grid ungraced.
                int populationPeak = m_ScenarioQuery.TryGetSingleton<ScenarioSingleton>(out var scenarioSingleton)
                    ? scenarioSingleton.PopulationPeak.Value
                    : 0;
