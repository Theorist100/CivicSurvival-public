...
#pragma warning restore CIVIC487
        ...
        private void ApplyShahed(EntityCommandBuffer ecb, in ThreatSpawnIntent intent)
        {
            ...
            // Outbound axis payload: carries the enemy-axis hit + aimed target to arrival. Default
            // (0 damage, IsOutbound=false) for inbound waves — they never read it (the faction gate
            // routes inbound to city terminalization, not the axis channel). IsOutbound is serialized
            // (the faction tag is not) so a mid-flight save/load can re-assert the stripped tag.
            ecb.SetComponent(entity, new OutboundStrikePayload { Axis = intent.OutboundAxis, Damage = intent.OutboundDamage, IsOutbound = isOutbound, Seed = intent.OutboundSeed, TargetId = intent.OutboundTargetId });
