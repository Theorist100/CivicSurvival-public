...
namespace CivicSurvival.Domains.Waves.Systems
{
    ...
    public partial class WaveExecutor : IDefaultSerializable, IBootDefaultsReset
    {
        ...
        public void Serialize<TWriter>(TWriter writer) where TWriter : IWriter
        {
            ...
            try
            {
                ...
                var payload = new WaveExecutorPersistState(
                    ...
                    state.NextWaveType,
                    state.NextWaveNumber,
                    state.NextWaveThreatsForecast,
                    state.NextWaveForecastReady);
        ...
        public void Deserialize<TReader>(TReader reader) where TReader : IReader
        {
            ...
            // CS2 reuses the system instance across loads, so a transient anchor survives unless
            // cleared here: an Attack wave resumed from a save must report "not measured", not a
            // duration measured against the previous city's sim clock.
            ...
            try
            {
                ...
                SetSaveState(new ExecutorSaveState(
                    ...
                    threatsSpawned,
                    scenarioStarted,
                    payload.NextWaveType,
                    payload.NextWaveNumber,
                    payload.NextWaveThreatsForecast,
                    payload.NextWaveForecastReady));
