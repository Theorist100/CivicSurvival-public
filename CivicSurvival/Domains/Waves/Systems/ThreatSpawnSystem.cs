...
namespace CivicSurvival.Domains.Waves.Systems
{
    ...
    public partial class ThreatSpawnSystem : CivicSystemBase, IPostLoadValidation, IOutboundStrikeService
    {
        ...
        // Scatter around the muzzle so repeated launches from one launcher are not pixel-identical.
        // Metres, deliberately small: the projectile has to leave the launcher it was fired from.
        // (It was 200 m while the outbound origin was the map centre and nothing had to line up.)
        private const float OUTBOUND_LAUNCH_JITTER = 6f;

        // Muzzle geometry, mirroring InterceptorSpawnSystem's LAUNCH_HEIGHT_OFFSET: a projectile
        // spawned at the launcher's own origin sits inside the chassis, and its exhaust flame
        // overlaps the vehicle on the first rendered frames. So it starts clear of the hull and
        // slightly toward the rear, where both the Shahed ramp and the HIMARS pod actually sit
        // (model length runs along local +Z, cab at -Z). Tune by eye.
        private const float DRONE_MUZZLE_HEIGHT = 8f;
        private const float BALLISTIC_MUZZLE_HEIGHT = 10f;
        private const float MUZZLE_REAR_OFFSET = 2f;

        // A ballistic leaves the pod climbing, not flat at the horizon — same reasoning as the
        // interceptor's minimum launch elevation. Only the spawn-frame heading: the lofted profile
        // in BallisticMovementJobEntity drives altitude from there on.
        private const float BALLISTIC_LAUNCH_ELEVATION_DEG = 35f;
        ...
        /// <summary>Simulation frames per second (vanilla danger-producer convention).</summary>
        private const float SIM_FRAMES_PER_SECOND = 60f;
        /// <summary>
        /// Initial preset capacity — sized to hold a full wave without a growth reallocation.
        /// Tracks <c>Waves.MaxThreats</c> (55) with headroom; raise it with that ceiling, or every
        /// maximum-size wave pays a resize on the frame it spawns.
        /// </summary>
        private const int PRESELECT_INITIAL_CAPACITY = 64;

        private static readonly LogContext Log = new("ThreatSpawnSystem");

        // Building target queries live on ThreatTargetCacheSystem;
        // ThreatSpawnSystem reads cached lists via IThreatTargetSource
        ...
        // Live launcher emplacements — the outbound launch origins, one query per munition kind.
        // Read on the main thread in Launch (ModificationEnd) to fire a counter-strike from a
        // built launcher.
        private EntityQuery m_DroneLauncherQuery;
        private EntityQuery m_BallisticLauncherQuery;
        ...
        // Registered singleton query for OnWaveStarting (never SystemAPI there — handler runs in
        // the publisher's system context).
        ...
        /// <summary>
        /// One pre-selected Shahed strike, chosen at the Alert siren — MINUTES before the drones
        /// spawn. Target selection is decoupled from spawn so the evacuation layer can raise
        /// danger at the siren and citizens get the Alert window (120-300 s) PLUS the flight time
        /// to reach a shelter, instead of flight time alone (live finding 2026-07-23: the march
        /// outlives the alarm — thousands arrive after the all-clear). TargetPos already includes
        /// the CEP scatter, so the warned impact point IS the flown-to impact point.
        /// </summary>
        internal struct PreselectedStrike
        {
            public int TargetIndex;      // -1 = ground strike (no building)
        ...
        /// <summary>
        /// Alert-time strike preset for the upcoming wave. Runtime-only handoff (stripped on load:
        /// a save inside the Alert window degrades gracefully — SpawnWave falls back to selecting
        /// targets at spawn, exactly the pre-preset behaviour). Read by
        /// <c>ThreatEvacuationSystem</c> to raise danger at the siren.
        /// </summary>
        [System.NonSerialized] internal NativeList<PreselectedStrike> m_PreselectedStrikes;
        /// <summary>Wave the preset belongs to; -1 = none.</summary>
        [System.NonSerialized] internal int m_PreselectedWaveNumber = -1;
        /// <summary>
        /// Conservative estimate of the wave's LAST impact frame (alert remainder + worst-case
        /// flight), for the evacuation layer's danger end-frames. Generous by design: too long
        /// only delays teardown, too short would release shelters before the real impacts.
        /// </summary>
        [System.NonSerialized] internal uint m_PreselectedImpactFrameEstimate;
...
#pragma warning restore CIVIC150
        ...
        protected override void OnCreate()
        {
            ...
            m_BallisticLauncherQuery = GetEntityQuery(
                ComponentType.ReadOnly<BallisticLauncherInstallation>(),
                ComponentType.Exclude<Deleted>());
            ...
            // Alert-time target preselection (see PreselectedStrike doc): the siren carries the
            // threat count, WaveStateSingleton already carries the wave type and phase duration
            // (WaveExecutor writes them BEFORE publishing, ExecuteAlertTransition).
            SubscribeRequired<WaveStartingEvent>(OnWaveStarting);
            m_PreselectedStrikes = new NativeList<PreselectedStrike>(PRESELECT_INITIAL_CAPACITY, Allocator.Persistent);
            // Registered singleton query for the Alert handler (CIVIC281/292: SystemAPI in an
            // event handler runs in the PUBLISHER's system context — a registered query does not).
        ...
        /// <summary>
        /// Alert-siren hook: pre-select the wave's Shahed targets minutes before the drones spawn,
        /// with EXACTLY the spawn loop's selection logic (category roll → selector with
        /// saturation/focus → CEP scatter), so the buildings warned are the buildings hit.
        /// SpawnWave consumes the preset; a save in the Alert window strips it and SpawnWave
        /// falls back to spawn-time selection.
        /// </summary>
        private void OnWaveStarting(WaveStartingEvent evt)
        {
            if (!m_PreselectedStrikes.IsCreated)
                return;

            m_PreselectedStrikes.Clear();
            m_PreselectedWaveNumber = -1;

            m_CachedConfig = BalanceConfig.Current;
            var wavesCfg = m_CachedConfig.Waves;

            // Wave type + alert duration come from the singleton — WaveExecutor writes both
            // BEFORE publishing this event (ExecuteAlertTransition).
            WaveType waveType = WaveType.Harassment;
            float alertSecondsRemaining = wavesCfg.AlertMax;
            if (m_WaveStateQuery.TryGetSingleton<WaveStateSingleton>(out var waveState))
            {
                waveType = waveState.CurrentWaveType;
                alertSecondsRemaining = math.max(0f, waveState.PhaseEndTime - waveState.TimeInPhase);
            }

            // Drone share estimate: ballistic count is re-derived at spawn (live in-flight
            // headroom can shrink it there) — preselect for the LARGER drone estimate; unused
            // tail slots are simply dropped by SpawnWave.
            int ballisticEst = ThreatMath.CalculateBallisticCount(GetCitySizeMW(), evt.WaveNumber, wavesCfg);
            int droneEst = math.max(0, evt.ThreatsExpected - math.min(ballisticEst, wavesCfg.BallisticMaxConcurrentInFlight));
            if (droneEst == 0)
                return;
            ...
            {
                Log.Warn("Preselect skipped: target cache not ready at Alert — targets will be chosen at spawn");
                return;
            }
            ...
            int focusDrones = (int)math.round(droneEst * focusFraction);

            var targetHitCount = new NativeHashMap<Entity, int>(
                math.max(Engine.Threats.TARGET_MAP_CAPACITY, droneEst), Allocator.Temp);
            try
            {
                for (int i = 0; i < droneEst; i++)
                {
                    float categoryRoll = m_Random.NextFloat(0f, 1f);
                    TargetCategory category = ThreatMath.SelectTargetCategory(waveType, variance, categoryRoll, wavesCfg, intro: evt.WaveRole == WaveRole.Intro);

                    m_TargetSelector.SetRandom(m_Random);
                    ...
                    m_Random = m_TargetSelector.GetRandom();
                    ...
                    if (targetEntity != Entity.Null)
                    {
                        ...
                        m_PreselectedStrikes.Add(new PreselectedStrike
                        {
                            TargetIndex = targetEntity.Index,
                    ...
                    else
                    {
                        m_PreselectedStrikes.Add(new PreselectedStrike
                        {
                            TargetIndex = -1,
                            TargetVersion = 0,
                            TargetPos = GetRandomGroundPoint(),
                            Category = TargetCategory.Civilian,
                            Concentrate = false
                        });
                    }
                }
            ...
            // Worst-case flight: map diagonal (edge spawn to far corner) at Shahed speed.
            float mapDiagonal = math.length(m_MapMax - m_MapMin);
            float shahedSpeed = math.max(1f, m_CachedConfig.Threats.ShahedSpeed);
            float flightWorstSeconds = mapDiagonal / shahedSpeed;

            uint frame = World.GetOrCreateSystemManaged<SimulationSystem>().frameIndex;
            m_PreselectedImpactFrameEstimate = frame
                + (uint)math.max(0f, (alertSecondsRemaining + flightWorstSeconds) * SIM_FRAMES_PER_SECOND);
            m_PreselectedWaveNumber = evt.WaveNumber;

            Log.Info($" Preselected {m_PreselectedStrikes.Length} strike target(s) for wave #{evt.WaveNumber} at Alert "
                     + $"(alertRemaining={alertSecondsRemaining:F0}s, flightWorst={flightWorstSeconds:F0}s)");
        }
        ...
        {
            // Bounds are process-local terrain cache — recalculated from TerrainSystem
            // on the first post-load wave instead of replaying a stale saved snapshot.
            m_MapMin = default;
            m_MapMax = default;
        ...
        /// <summary>
        /// Single source for the [NonSerialized] transients (batch sequence, altitude
        /// diagnostics, per-wave config cache, one-shot log latches), called from every
        /// reset leg — ResetState, ResetToBootDefaults and the post-load path — so the
        /// legs cannot drift and no previous-session latch survives into the next one.
        /// </summary>
        private void ClearTransientState()
        {
            // Wrap-guard re-bases the sequence to 1 on the next increment; the intent
            // buffer is stripped on load, so 0 ("unset") is always safe here.
            m_SpawnBatchSeq = 0;
            ...
            // Alert-time preset does not survive a load: entity identities are recycled and the
            // Alert remainder is meaningless in the new session. SpawnWave falls back to
            // spawn-time selection — the pre-preset behaviour.
            if (m_PreselectedStrikes.IsCreated) m_PreselectedStrikes.Clear();
            m_PreselectedWaveNumber = -1;
            m_PreselectedImpactFrameEstimate = 0;
        }
        ...
        protected override void OnDestroy()
        {
            if (ServiceRegistry.IsInitialized)
            {
                ...
                UnsubscribeSafe<WaveStartingEvent>(OnWaveStarting);
                ServiceRegistry.Instance.Unregister<IOutboundStrikeService>(this);
            }
            if (m_PreselectedStrikes.IsCreated)
                m_PreselectedStrikes.Dispose();
            base.OnDestroy();
        }
...
#pragma warning restore CIVIC231
            ...
            // Alert-time preset for THIS wave: fly to the pre-warned targets (see OnWaveStarting).
            // The evacuation layer raised danger on exactly these buildings at the siren, so the
            // spawn MUST NOT re-roll them — a re-roll would bomb unwarned buildings while the
            // warned ones evacuate for nothing. Preset slots beyond droneCount (spawn-time
            // ballistic headroom shrank the drone share) are dropped; a missing/short preset
            // (save/load inside Alert, cache not ready at Alert) falls back to spawn-time
            // selection per drone.
            bool usePreset = m_PreselectedWaveNumber == waveNumber
                             && m_PreselectedStrikes.IsCreated
                             && m_PreselectedStrikes.Length > 0;
            if (usePreset)
                Log.Info($" Using Alert-time preset: {math.min(m_PreselectedStrikes.Length, droneCount)}/{droneCount} strikes pre-warned");
            ...
            try
            {
                ...
                for (int i = 0; i < droneCount; i++)
                {
                    ...
                    bool concentrate;
                    bool fromPreset = usePreset && i < m_PreselectedStrikes.Length;

                    if (fromPreset)
                    {
                        PreselectedStrike strike = m_PreselectedStrikes[i];
                        targetEntity = strike.TargetIndex >= 0
                            ? new Entity { Index = strike.TargetIndex, Version = strike.TargetVersion }
                            : Entity.Null;
                        targetPos = strike.TargetPos; // CEP already applied at preselect
                        actualCategory = strike.Category;
                    ...
                    else
                    {
                        ...
                        concentrate = i < focusDrones;
                        (targetEntity, targetPos, actualCategory) = m_TargetSelector.FindTargetWithFallback(category, targetHitCount, wavesCfg.MaxThreatsPerTarget, concentrate);
                    ...
                    if (targetEntity != Entity.Null)
                    {
                        ...
                        // Preset slots skip this: their scatter was applied at preselect so the
                        // warned point IS the impact point.
                        if (!fromPreset)
                        {
                            float cep = concentrate
                                ? m_CachedConfig.Threats.FocusStrikeCEP
                    ...
                    {
                        float3 groundTarget = fromPreset ? targetPos : GetRandomGroundPoint();
                ...
                // Preset is single-use: consumed by this wave's spawn.
                if (usePreset)
                {
                    m_PreselectedStrikes.Clear();
                    m_PreselectedWaveNumber = -1;
                ...
                // never records a wave as spawned while its entities do not yet exist.
            }
            finally
            {
                if (targetHitCount.IsCreated) targetHitCount.Dispose();
...
#pragma warning restore CIVIC073
        ...
        private bool SpawnShahed(ref DynamicBuffer<ThreatSpawnIntent> intents, float3 spawnPos, float3 targetPos, TargetCategory category, int targetBuildingIndex, int targetBuildingVersion, Entity droneEntity, bool isFocusStrike, int waveNumber, uint waveBatchKey,
            byte faction = ThreatSpawnIntent.FactionEnemyInbound, AttackCategory outboundAxis = AttackCategory.Kinetic, float outboundDamage = 0f, uint outboundSeed = 0u, ushort outboundTargetId = 0)
        {
        ...
        private bool SpawnBallistic(ref DynamicBuffer<ThreatSpawnIntent> intents, float3 targetPos, int targetBuildingIndex, int targetBuildingVersion, Entity rocketEntity, int waveNumber, uint waveBatchKey,
            byte faction = ThreatSpawnIntent.FactionEnemyInbound, AttackCategory outboundAxis = AttackCategory.Kinetic, float outboundDamage = 0f, uint outboundSeed = 0u, ushort outboundTargetId = 0, float3? spawnPosOverride = null)
        {
            ...
            // An inbound wave round is placed at the perimeter at launch altitude; an outbound
            // counter-strike arrives with its launcher muzzle already resolved (height included),
            // so its Y must survive untouched — overwriting it would put the round in mid-air above
            // the pod instead of leaving it.
            bool outboundFromLauncher = spawnPosOverride.HasValue;
            float3 spawnPos = spawnPosOverride ?? GetRandomSpawnPoint();
            if (!outboundFromLauncher)
            ...
            // Leaving a pod, that first frame must point upward (the target sits past the map edge
            // at ground level, so aiming straight at it would slide the round along the ground);
            // BALLISTIC_LAUNCH_ELEVATION_DEG raises it, mirroring the interceptor's boost.
            float3 direction;
            if (outboundFromLauncher)
            {
                float3 flat = new float3(targetPos.x - spawnPos.x, 0f, targetPos.z - spawnPos.z);
                float horiz = math.length(flat);
                direction = math.normalizesafe(
                    new float3(flat.x, horiz * math.tan(math.radians(BALLISTIC_LAUNCH_ELEVATION_DEG)), flat.z),
                    math.up());
            }
            else
            {
                direction = math.normalizesafe(targetPos - spawnPos);
            }
            quaternion rotation = quaternion.LookRotationSafe(direction, math.up());
        ...
        /// </summary>
        public bool Launch(ArsenalKind kind, AttackCategory axis, float damage, uint seed, ushort targetId)
        {
            Entity droneEntity = m_PrefabInit.AttackDroneEntity;
            if (droneEntity == Entity.Null)
            ...
            // Launch origin is the muzzle of a built launcher of the matching kind (small jitter on
            // top so repeated launches don't stack). No launcher → nothing to launch from; the
            // caller rolls the operation back exactly like a "producer unavailable" failure.
            if (!TryGetLaunchMuzzle(kind, out float3 basePos))
            {
                Log.Warn($"Launch blocked: no {kind} launcher built");
                return false;
            }
            // The muzzle already carries its height (cleared hull, terrain-adjusted) — only X/Z
            // scatter here. Overwriting Y with a cruise altitude, as this did while the origin was
            // the map centre, would spawn the projectile in mid-air above the launcher instead of
            // leaving it: 150 m for a drone, 100 m for a ballistic.
            float3 spawnPos = new float3(
                basePos.x + m_Random.NextFloat(-OUTBOUND_LAUNCH_JITTER, OUTBOUND_LAUNCH_JITTER),
                basePos.y,
                basePos.z + m_Random.NextFloat(-OUTBOUND_LAUNCH_JITTER, OUTBOUND_LAUNCH_JITTER));
            ...
            if (kind == ArsenalKind.Ballistic)
            {
                ...
                recorded = SpawnBallistic(ref intentBuffer, targetPos, -1, 0, rocketEntity, waveNumber: 0, batchKey,
                    faction: ThreatSpawnIntent.FactionPlayerOutbound, outboundAxis: axis, outboundDamage: damage, outboundSeed: seed, outboundTargetId: targetId, spawnPosOverride: spawnPos);
            }
            else
            {
                // Muzzle height stands: ShahedMovementJob climbs to max(terrain + 100 m, spawn.y),
                // so the drone lifts off the launcher to its cruise altitude on its own.
                recorded = SpawnShahed(ref intentBuffer, spawnPos, targetPos, TargetCategory.Civilian, -1, 0, droneEntity, isFocusStrike: false, waveNumber: 0, batchKey,
                    faction: ThreatSpawnIntent.FactionPlayerOutbound, outboundAxis: axis, outboundDamage: damage, outboundSeed: seed, outboundTargetId: targetId);
            }

            if (recorded)
                Log.Info($"[SPAWN:INTENT] Outbound {kind} faction=PlayerOutbound axis={axis} dmg={damage:F1} launcher=({basePos.x:F0},{basePos.z:F0}) from=({spawnPos.x:F0},{spawnPos.z:F0}) to=({targetPos.x:F0},{targetPos.z:F0})");
            return recorded;
        }

#if DEBUG
        /// <summary>
        /// DEBUG ONLY: record one inbound ballistic aimed at <paramref name="building"/> instead of
        /// at whatever the wave target selector would have picked. Everything downstream is the
        /// production path — perimeter spawn, lofted arc, air-defence interception, arrival damage —
        /// so this puts a chosen structure under real fire rather than simulating a hit on it.
        /// Called on the main thread from the DevTools panel; appending to the existing intent host
        /// buffer is not a structural change, so the caller's phase does not matter. The aim point
        /// is resolved here rather than by the caller because building Transform is a render
        /// component and must be read behind the render-write ticket, as TryGetNearestMuzzle does.
        /// Returns false when prefabs are unresolved, the target has no transform, or the intent
        /// host is missing.
        /// </summary>
        public bool DebugStrikeBuilding(Entity building)
        {
            Entity rocketEntity = m_PrefabInit?.RocketEntity ?? Entity.Null;
            if (rocketEntity == Entity.Null)
                rocketEntity = m_PrefabInit?.AttackDroneEntity ?? Entity.Null; // SpawnWave's ballistic→drone fallback
            if (rocketEntity == Entity.Null)
                return false;

            // Same per-call config snapshot discipline as SpawnWave/Launch (H27): SpawnBallistic
            // reads m_CachedConfig, and this entry point has no SpawnWave call ahead of it.
            m_CachedConfig = BalanceConfig.Current;

            if (!m_BoundsCached)
                CacheMapBounds();

            var ticket = ServiceRegistry.Instance.Require<IRenderWriteBarrier>()
                .Consume(typeof(ThreatSpawnSystem), RenderWriteComponentMask.BuildingTransform);
            if (!BuildingDamageHelper.TryGetBuildingTransform(EntityManager, building, ticket, out var transform))
            {
                Log.Warn($"[DEBUG] Strike skipped: building {building.Index}:{building.Version} has no transform");
                return false;
            }
            float3 targetPos = transform.m_Position;

            if (!m_IntentHostQuery.TryGetSingletonBuffer<ThreatSpawnIntent>(out var intentBuffer, isReadOnly: false))
            {
                Log.Error("[DEBUG] Strike on selected building skipped: ThreatSpawnIntent host missing");
                return false;
            }
            ...
            bool recorded = SpawnBallistic(ref intentBuffer, targetPos, building.Index, building.Version,
                rocketEntity, waveNumber: 0, batchKey);

            if (recorded)
                Log.Info($"[SPAWN:INTENT] Debug ballistic → building {building.Index}:{building.Version} at ({targetPos.x:F0},{targetPos.z:F0})");
            return recorded;
        }
#endif
        ...
        /// <summary>
        /// Muzzle position for an outbound strike of <paramref name="kind"/> — the point the
        /// projectile actually leaves from, height included (X/Z jitter is added by the caller).
        /// Each munition launches from its own built emplacement nearest the enemy edge (the map
        /// perimeter the projectile crosses): a drone from a <see cref="DroneLauncherInstallation"/>,
        /// a ballistic from a <see cref="BallisticLauncherInstallation"/>. No launcher of that kind
        /// built → false, and the caller rolls the operation back.
        /// </summary>
        [CompletesDependency("TryGetLaunchMuzzle: main-thread Transform read of built launchers at outbound launch — one search per launch (a player click), mirroring the Center placement snap lookup.")]
        private bool TryGetLaunchMuzzle(ArsenalKind kind, out float3 pos)
        {
            return kind == ArsenalKind.Ballistic
                ? TryGetNearestMuzzle<BallisticLauncherInstallation>(m_BallisticLauncherQuery, BALLISTIC_MUZZLE_HEIGHT, out pos)
                : TryGetNearestMuzzle<DroneLauncherInstallation>(m_DroneLauncherQuery, DRONE_MUZZLE_HEIGHT, out pos);
        }

        /// <summary>
        /// Muzzle of the built <typeparamref name="TInstall"/> emplacement closest to the enemy
        /// edge: its object position pushed <see cref="MUZZLE_REAR_OFFSET"/> toward the rear along
        /// the object's own facing (where both the drone ramp and the ballistic pod sit) and lifted
        /// <paramref name="muzzleHeight"/> clear of the hull. One implementation for every munition
        /// — the launcher kinds differ only in which install component marks them.
        /// </summary>
        private bool TryGetNearestMuzzle<TInstall>(EntityQuery query, float muzzleHeight, out float3 pos)
            where TInstall : unmanaged, IComponentData, IBuildingLinked
        {
            pos = default;
            if (query.IsEmpty)
                return false;

            var installs = query.ToComponentDataArray<TInstall>(Allocator.Temp);
            // Building Transform is a render component: the read goes through the render-write barrier
            // ticket (BuildingTransform has zero out-of-band producers today, so the Consume is free —
            // the ticket is the routing contract, not a wait).
            var ticket = ServiceRegistry.Instance.Require<IRenderWriteBarrier>()
                .Consume(typeof(ThreatSpawnSystem), RenderWriteComponentMask.BuildingTransform);
            bool found = false;
            ...
            {
                // A demolished launcher's installation can survive a tick before ModEntityCleanupSystem
                // deletes it, so trust only a launcher whose building still exists and has a transform.
                Entity building = installs[i].GetBuildingEntity();
                if (building == Entity.Null || !EntityManager.Exists(building))
                    continue;
                if (!BuildingDamageHelper.TryGetBuildingTransform(EntityManager, building, ticket, out var transform))
                    continue;

                float edgeDist = DistanceToNearestEdge(transform.m_Position);
                if (edgeDist < bestEdgeDist)
                {
                    bestEdgeDist = edgeDist;
                    float3 rear = math.mul(transform.m_Rotation, new float3(0f, 0f, MUZZLE_REAR_OFFSET));
                    float3 muzzleXZ = transform.m_Position + rear;
                    pos = new float3(muzzleXZ.x, GetTerrainAdjustedAltitude(muzzleXZ, muzzleHeight), muzzleXZ.z);
                    found = true;
                }
            }
            installs.Dispose();
        ...
        // Metres from a point to whichever map edge is nearest — the same four-edge distance the
        // outbound frontier uses to pick the crossing edge. Smallest value = most forward toward the
        // enemy line, which is how the launch launcher is chosen.
        private float DistanceToNearestEdge(float3 p)
        {
            float distLeft = p.x - m_MapMin.x;
            float distRight = m_MapMax.x - p.x;
            float distBottom = p.z - m_MapMin.z;
            float distTop = m_MapMax.z - p.z;
