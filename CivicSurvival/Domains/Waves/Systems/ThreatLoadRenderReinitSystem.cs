...
namespace CivicSurvival.Domains.Waves.Systems
{
    ...
    /// But a restored drone comes back with empty render buffers (MeshColor/MeshBatch are
    /// IEmptySerializable — decompile-verified — so they deserialize length-0; TransformFrame,
    /// by contrast, is plain ISerializable and IS restored with data, but its OIS history is
    /// re-seeded below anyway for interpolation continuity) and WITHOUT its lifecycle tags /
    /// PreCulling flags (ActiveThreat,
    ...
    public partial class ThreatLoadRenderReinitSystem : CivicSystemBase
    {
        ...
        protected override void OnGameLoaded(Context serializationContext)
        {
            ...
            // DIAGNOSTIC (native-crash attribution): prod war-save loads crash in vanilla
            // lib_burst BEFORE the ModificationEnd reinit runs (18/18 war load-crashes carry no
            // LoadRenderReinit marker). Bracket the pre-reinit window from here — the earliest
            // managed hook that sees the deserialized threats — and hand off to LoadRenderReinit
            // at the reinit. A war load-crash then reads: LoadThreatRestore present + LoadRenderReinit
            // absent → crash in [load callback → first ModificationEnd] (vanilla render of the
            // restored threat); neither → crash before this callback (early deserialize / prefab
            // load / spatial hash). Only marks when restored threats are actually present.
            if (!m_ShahedQuery.IsEmptyIgnoreFilter || !m_BallisticQuery.IsEmptyIgnoreFilter)
                Core.Diagnostics.NativeCrashBreadcrumb.Mark(Core.Diagnostics.NativeCrashMarkers.LoadThreatRestore);
        }
        ...
        protected override void OnUpdateImpl()
        {
            ...
            // Handoff from the pre-reinit bracket: reaching ModificationEnd means the dangerous
            // vanilla-render-of-restored-threat window is behind us, so drop the coarse marker.
            Core.Diagnostics.NativeCrashBreadcrumb.ClearIfCurrent(Core.Diagnostics.NativeCrashMarkers.LoadThreatRestore);
