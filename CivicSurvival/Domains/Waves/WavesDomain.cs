...
namespace CivicSurvival.Domains.Waves
{
    ...
    /// Priority 2520 = Gameplay tier (registered separately via Mod.cs).
    ///
    ...
    public class WavesDomain : IFeatureModule, IDependentFeatureModule
    {
        ...
        public void RegisterSystems(UpdateSystem updateSystem)
        {
            ...
            // CompleteDependencyBeforeRO<Transform> wait is gone.
            ...
            // Shelter evacuation — planner half: reads the same spawn intents and stages danger
            // rows for the buildings inbound drones are aimed at. MUST run in Modification4 BEFORE
            // the spawn consumer: the consumer drains and clears the intent buffer, and
            // Modification4 ticks while paused (the intro wave can spawn into a paused sim) — a
            // GameSimulation reader misses the buffer entirely in that case (the 02:38/02:57
            // silent-wave incident, 2026-07-23). Single ordering edge (one-anchor rule,
            // ECS_SCHEDULING.md). It stages rather than emits: Modification4 sits outside the
            // EndFrameBarrier allow-window (SystemOrder.cs:62 — AllowBarrier opens it in MainLoop
            // AFTER ModificationSystem), and ModificationBarrier4 rows die in the same frame's
            // cleanup sweep before EndangerSystem reads them. See ThreatEvacuationSystem class doc.
            updateSystem.RegisterBefore<ThreatEvacuationSystem, global::CivicSurvival.Domains.Waves.Systems.ThreatSpawnApplySystem>(SystemUpdatePhase.Modification4);

            // Shelter evacuation — applier half: materializes the staged rows through vanilla
            // EndFrameBarrier (the barrier vanilla danger producers use). PostSimulation ticks in
            // pause AND sits inside the EndFrameBarrier allow-window — the only phase satisfying
            // both. Also owns the lapsed danger-owner sweep.
            updateSystem.RegisterAt<ThreatEvacuationApplySystem>(SystemUpdatePhase.PostSimulation);
