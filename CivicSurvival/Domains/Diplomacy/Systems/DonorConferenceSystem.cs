...
namespace CivicSurvival.Domains.Diplomacy.Systems
{
        ...
        ICivicSingletonOwner<DonorSanctionsSingleton>, ICivicSingletonOwner<ExternalPowerSource>
    {
        ...
        // World-attention (WorldShock) level that opens the donor gate on its own,
        // in parallel with the blackout-% crisis path (Phase 15 OR-gate).
        private static float SHOCK_ATTENTION_THRESHOLD => BalanceConfig.Current.Diplomacy.ShockAttentionThreshold;
        // Donor-fatigue reduction applied to the conference cooldown when a use is
        // consumed at GlobalShock (Phase 15B).
        private static float GLOBAL_SHOCK_COOLDOWN_MULT => BalanceConfig.Current.Diplomacy.GlobalShockCooldownMultiplier;
        ...
        // Phase 15A — daily decision log of which path opens the availability gate.
        // NonSerialized so a fresh load logs the current path once (Axiom 1: Grep-able).
        [NonSerialized] private AvailabilityPath m_LastLoggedAvailabilityPath;
        [NonSerialized] private bool m_HasLoggedAvailabilityPath;
...
#pragma warning restore CIVIC070
        ...
        /// <summary>
        /// Single-source transient/latch reset — the one list of NonSerialized state (dialog latch,
        /// act baseline, availability-path decision-log latch). Called from every reset leg so the
        /// boot and load legs can't drift (M33: the availability-path latch was previously reset on
        /// neither, leaking the previous city's logged path into the next).
        /// </summary>
        private void ClearTransientState()
        {
            ...
            m_HasSeenAct = false;
            m_HasLoggedAvailabilityPath = false;
            m_LastLoggedAvailabilityPath = default;
        }
        ...
        /// <summary>
        /// Phase 15A decision log: emit which access path opens the availability gate
        /// whenever it changes (and once on the first daily tick after a load, since the
        /// tracking field is NonSerialized). Runs daily — not per-frame — so it stays
        /// Grep-able for save/load verification without log spam.
        /// </summary>
        private void LogAvailabilityPathIfChanged()
        {
            var path = GetAvailabilityPath();
            if (m_HasLoggedAvailabilityPath && path == m_LastLoggedAvailabilityPath)
                return;

            m_LastLoggedAvailabilityPath = path;
            m_HasLoggedAvailabilityPath = true;
            Log.Info($"[DonorConference] Availability gate openedBy={path} " +
                     $"(shock={GetShockLevel():F0}% / threshold {SHOCK_ATTENTION_THRESHOLD:F0}, " +
                     $"crisis={GetCrisisLevel():F0}% / threshold {CRISIS_THRESHOLD:F0})");
        }
        ...
        public ConferenceStatus GetConferenceStatus()
        {
            ...
            // Phase 15 OR-gate: WorldShock (world attention) is an independent access
            // path alongside the blackout-% crisis. The conference is denied only when
            // BOTH signals are below their thresholds — either one alone opens the gate.
            if (GetAvailabilityPath() == AvailabilityPath.None)
                return ConferenceStatus.InsufficientAttention;
        ...
        /// <summary>
        /// Which access path opens the availability gate right now (Phase 15). The gate
        /// is an OR of world attention (WorldShock ≥ ShockAttentionThreshold) and domestic
        /// humanitarian need (blackout-% crisis ≥ CrisisThreshold). Shared by the status
        /// check, the daily decision log, and the UI "available on world attention" line.
        /// </summary>
        public AvailabilityPath GetAvailabilityPath()
        {
            bool attentionOpen = GetShockLevel() >= SHOCK_ATTENTION_THRESHOLD;
            bool crisisOpen = GetCrisisLevel() >= CRISIS_THRESHOLD;
            return (attentionOpen, crisisOpen) switch
            {
                (true, true) => AvailabilityPath.Both,
                (true, false) => AvailabilityPath.Attention,
                (false, true) => AvailabilityPath.Crisis,
                _ => AvailabilityPath.None
            };
        }
        ...
        public static string GetStatusString()
        {
            ...
            return status switch
            {
                ...
                ConferenceStatus.InsufficientAttention => "insufficient_attention",
        ...
        private void ConsumeConferenceUse()
        {
            ...
            // Phase 15B — world attention lowers donor fatigue. At GlobalShock the
            // cooldown is shortened by GlobalShockCooldownMultiplier (the one "stronger"
            // lever the shock-tier × trust aid matrix does not already cover).
            float cooldown = COOLDOWN_DAYS;
            if (GetShockTier() == AidTier.GlobalShock)
            {
                float mult = Math.Min(Math.Max(GLOBAL_SHOCK_COOLDOWN_MULT, 0f), 1f);
                cooldown *= mult;
                Log.Info($"[DonorConference] GlobalShock cooldown reduction: {COOLDOWN_DAYS:F0} → {cooldown:F0} days (×{mult:F2})");
            }
            m_State.CooldownDaysRemaining = cooldown;
        }
        ...
        private DonorConferenceResult ApplyRefusedResult(TrustLevel trust, DonorAidType aidType)
        {
            // Still consume a use
            m_State.UsesRemaining = Math.Max(0, m_State.UsesRemaining - 1);
            // refused → full cooldown by design (no GlobalShock discount, unlike the accepted path)
