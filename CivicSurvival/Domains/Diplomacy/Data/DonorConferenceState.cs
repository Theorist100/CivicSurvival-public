...
namespace CivicSurvival.Domains.Diplomacy.Data
{
    ...
    public enum ConferenceStatus
    {
        ...
        NoUsesRemaining,
        /// <summary>
        /// Neither access path is open: world attention (shock) is below
        /// ShockAttentionThreshold AND domestic blackout-% crisis is below
        /// CrisisThreshold. Renamed from CrisisTooLow when the availability gate
        /// became an OR of the two independent signals.
        /// </summary>
    ...
    /// <summary>
    /// Which access path (if any) currently opens the donor-conference availability
    /// gate. The gate is an OR of two independent signals: world attention
    /// (WorldShock ≥ ShockAttentionThreshold) and domestic humanitarian need
    /// (blackout-% crisis ≥ CrisisThreshold).
    /// </summary>
    public enum AvailabilityPath
    {
        /// <summary>Both signals below threshold — conference denied.</summary>
        None = 0,
        /// <summary>World attention alone opens the gate (crisis below threshold).</summary>
        Attention,
        /// <summary>Domestic blackout-% crisis alone opens the gate (attention below threshold).</summary>
        Crisis,
        /// <summary>Both signals above threshold.</summary>
        Both
    }
