...
namespace CivicSurvival.Domains.Diplomacy.UI
{
    ...
    public partial class DonorConferenceUISystem : CivicUIPanelSystem
    {
        ...
        protected override void OnPanelUpdate()
        {
            ...
            var conferenceStatus = DonorConferenceSystem.GetStatusString();
            var availabilityPath = DonorConferenceSystem.Instance.GetAvailabilityPath();
            bool availableViaAttention = conferenceStatus == "available"
                && availabilityPath == AvailabilityPath.Attention;

            var dto = new DonorDto
            {
                DonorUsesRemaining = state.UsesRemaining,
                DonorCooldownDays = (int)Math.Ceiling(state.CooldownDaysRemaining),
                DonorStatus = conferenceStatus,
                AvailableViaAttention = availableViaAttention,
                TrustIndex = trust,
        ...
        {
            // Close is a pure managed-flag reset — applied synchronously (route 3) so
            // the dialog can ALWAYS be dismissed, including while paused. Routing it
            // through the ECS request would strand the click until unpause: the
            // consumer ticks in GameSimulation. Open stays on the gated ECS path on
            // purpose — opening the conference while its selection is speed-gated
            // would trap the player in a dead dialog.
            if (action == DonorDialogAction.Close)
            {
                DonorConferenceSystem.Instance?.CloseConferenceDialog();
