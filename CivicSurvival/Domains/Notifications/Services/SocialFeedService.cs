...
namespace CivicSurvival.Domains.Notifications.Services
{
    ...
    public sealed class SocialFeedService : IDisposable
    {
        ...
        /// </summary>
        public void AddPost(string author, string message, SocialMood mood, bool bypassAuthorCooldown = false)
            => TryAddPost(author, message, mood, bypassAuthorCooldown);

        /// <summary>
        /// Add a new post and report whether it was accepted into the feed.
        /// <paramref name="bypassAuthorCooldown"/> skips the per-author window only (rare
        /// lifecycle beats — see SocialPostEvent); the duplicate filter still applies.
        /// </summary>
        public bool TryAddPost(string author, string message, SocialMood mood, bool bypassAuthorCooldown = false)
        {
            ...
            lock (m_PostsLock)
            {
                ...
                // Anti-spam: check cooldown for this author. Lifecycle beats (quest
                // start/resolution) are exempt — losing a resolution to the window reads
                // as a broken story; the duplicate filter below still applies to them.
                if (!bypassAuthorCooldown && m_AuthorCooldowns.TryGetValue(author, out var lastPost))
                {
                ...
                if (!duplicateBlocked)
                {
                    ...
                    var post = new SocialPost(
                        ...
                        // Face resolved from the same registry as the display name — a named
                        // speaker (hero) or an enemy carrier gets an icon, the crowd keeps its
                        // mood glyph. Every publisher goes through here, so nobody can post
                        // without one.
                        avatarId: NewsAuthorRegistry.GetAvatarId(author)
                    );
...
#pragma warning restore CIVIC139
            ...
            lock (m_PostsLock)
            {
                ...
                for (int i = 0; i < m_Posts.Count; i++)
                {
                    ...
                    var dto = new SocialPostDto
                    {
                        ...
                        AvatarId = post.AvatarId ?? string.Empty,
                    };
                    dto.WriteTo(sb);
                }
                sb.Append(']');
        ...
        private void OnSocialPost(SocialPostEvent evt)
        {
            ...
            AddPost(evt.Author ?? "", evt.Message ?? "", evt.Mood, evt.BypassAuthorCooldown);
        }
