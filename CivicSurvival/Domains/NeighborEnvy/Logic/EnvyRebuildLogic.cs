...
namespace CivicSurvival.Domains.NeighborEnvy.Logic
{
    ...
    internal struct PendingRebuildState
    {
        ...
        // Handle of the ONLY job in the chain that reads live ECS chunks
        // (CollectBuildingDataJob; phases 2-5 run on copied Persistent arrays). The
        // owner MUST register it with the ECS dependency manager right after
        // ScheduleRebuild — residentialQuery.AddDependency(CollectJobHandle) + fold
        // into system Dependency — or a structural change to residential chunks
        // during the throttle window frees the chunk under the running worker
        // (UAF → anonymous retail c0000005; crash-hunt 2026-07-21, agent4 finding,
        // NEIGHBORENVY_FENCE_FIX_PLAN.md).
        public JobHandle CollectJobHandle;
    ...
    internal static class EnvyRebuildLogic
    {
            ...
            JobHandle dependency)
        {
            ...
            try
            {
                ...
                // SAFETY-LOCK: scheduled on the CALLER's Dependency (not `default`) — the job
                // reads live residential chunks, and `default` made it read Transform/
                // CurrentDistrict in parallel with same-frame vanilla writers. The handle also
                // rides out in state.CollectJobHandle for query.AddDependency registration at
                // the call site — without it the ECS dependency manager cannot fence structural
                // changes against this worker for the whole throttle window (the chunk-UAF
                // class, NEIGHBORENVY_FENCE_FIX_PLAN.md).
#pragma warning disable CIVIC294 // Reads vanilla Transform/CurrentDistrict only — no prior CivicSurvival main-thread writes to these components
                ...
                var collectHandle = collectJob.ScheduleParallel(residentialQuery, dependency);
                if (Core.Diagnostics.BurstLogBootstrap.MarkersEnabled)
...
#pragma warning restore CIVIC294
                ...
                state.CollectJobHandle = collectHandle;
                if (Core.Diagnostics.BurstLogBootstrap.MarkersEnabled)
