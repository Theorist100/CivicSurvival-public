...
namespace CivicSurvival.Domains.Mobilization.UI
{
    ...
    public partial class MobilizationUISystem : CivicUIPanelSystem
    {
        ...
        protected override void OnPanelUpdate()
        {
            ...
            // Display half: the numbers are shipped as they stand and the flag says whether they
            // stand for anything. The panel used to draw "0 %" through the unmeasured window,
            // which is the most alarming reading the widget has and the one the player is least
            // able to act on. A dash is drawn from IsManpowerMeasured instead — the percentage
            // is still computed so the panel has a last-good number to fall back to once the
            // TypeScript renders both states.
            ...
            {
                IsManpowerMeasured = state.IsPoolMeasured,
                ...
                ManpowerCorruptionScore = (int)Math.Round(state.VisibleCorruptionScore),
                ManpowerMoraleFactor = (int)Math.Round(state.MoraleFactor * 100),
                ManpowerFatigueFactor = (int)Math.Round(state.FatigueFactor * 100),
                ManpowerDodgerFactor = (int)Math.Round(state.DodgerFactor * 100),
                ManpowerDodgerCount = state.DodgerCount,
                ManpowerDeferral = state.DeferralManpower,
                ManpowerDisability = state.DisabilityManpower,
                IsConscriptionActive = state.IsConscriptionActive,
                IsWarFatigued = state.IsWarFatigued,
                // Read the latch, do NOT re-derive from Available/Total: the verdict carries
                // hysteresis (enter 20%, clear 25%) and the raw ratio would drop the badge inside
                // the recovery band while the alert is still armed — badge and toast disagreeing.
                IsManpowerCritical = state.IsManpowerCritical,
                // A VERDICT derived here from two numbers, unlike the latch above — so it is
                // gated on the same numbers being current. Frozen figures produce a verdict
                // about a city as it was, and "you have committed more people than you have" is
                // exactly the claim a player would act on by pulling crew off working guns.
                // The latch is not gated: MobilizationSystem deliberately freezes it too, so it
                // reports the last verdict it actually reached rather than one re-derived here.
                IsManpowerOvercommitted = state.IsPoolMeasured && state.UsedManpower > state.TotalManpower,
        ...
        {
            // No pause reject here, by design — the consumer (MobilizationRequestIntakeSystem)
        ...
        {
            // No pause reject here, by design — see OnToggleConscription above.
