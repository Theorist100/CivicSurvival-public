...
namespace CivicSurvival.Domains.Network.UI
{
    ...
    public partial class GlobalNewsUISystem : CivicUIPanelSystem
    {
        ...
        {
            // The nickname is server-side identity, so Online off makes it unreachable — the
            // gate carries its own reason so the editor can say why instead of going quietly
            // grey. Mirrors the refusal in GlobalNewsSystem.OnSetNickname.
            bool online = m_Settings != null && m_Settings.NetworkConnectionEnabled;
            bool canEditNickname = NewsEligibility.CanEditNickname(online, out var nicknameLockedReasonId);
            ...
            var dto = new NewsDto
            {
                ...
                NetworkConnectionEnabled = online,
                CanEditNickname = canEditNickname,
                NicknameLockedReasonId = nicknameLockedReasonId,
