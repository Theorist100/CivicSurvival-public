...
namespace CivicSurvival.Domains.Network.Systems
{
    ...
    public abstract partial class NewsPollingSystemBase : CivicSystemBase
    {
        ...
        /// <summary>
        /// The live Online state from the source of truth — <see cref="TelemetryConfig.OnlineEnabled"/>
        /// (the settings' NetworkConnectionEnabled, seeded from ConsentStore at boot/load, with a
        /// direct ConsentStore fallback). This is exactly what TelemetryService and ArenaLeaderboardSystem
        /// read, so every online-gated system agrees on one value.
        ///
        /// Call from LOAD boundaries (OnGameLoaded). The Online preference is global and persisted in
        /// ConsentStore, so a player who enabled the Global Grid in a PRIOR session loads with the
        /// setting seeded from ConsentStore but WITHOUT a live OnlineConnectionStateChangedEvent — that
        /// event fires only on an explicit user toggle (GlobalNewsSystem.OnToggleConnection). Without
        /// this re-resolve the boot snapshot stays stale-false, silently killing news / personal
        /// chronicle / nickname while telemetry (which reads Online live) still works — the exact
        /// prod symptom. Reads at a load boundary (single-threaded, no concurrent toggle writer), so it
        /// does NOT reintroduce the dispatch-time settings race that the event-only update avoids.
        /// </summary>
        protected bool ResolveOnlineEnabled()
        {
            LoadSharedConfig();
            return m_TelemetryConfig.OnlineEnabled;
        }
