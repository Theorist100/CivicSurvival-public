...
#pragma warning restore CIVIC230
        ...
        private void OnGameLoaded(GameLoadedEvent evt)
        {
            ...
            // Re-resolve Online from the source of truth, not the boot snapshot. A player who
            // enabled the Global Grid in a PRIOR session loads with the preference seeded from
            // ConsentStore but no live toggle event, so m_Enabled would otherwise stay stale-false
            // and silently kill news + nickname (nickname's !m_Enabled gate then reports
            // NicknameServerUnavailable at a live server). See base ResolveOnlineEnabled.
            m_Enabled = ResolveOnlineEnabled();
