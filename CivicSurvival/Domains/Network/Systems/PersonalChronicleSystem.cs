...
#pragma warning restore CIVIC114
        ...
        private void OnGameLoaded(GameLoadedEvent evt)
        {
            ...
            // Re-resolve Online from the source of truth, not the boot snapshot: a prior-session
            // opt-in seeds the setting from ConsentStore without a live toggle event, so
            // m_OnlineEnabled would otherwise stay stale-false and silently keep the chronicle dark
            // at an online-enabled, live server. See base ResolveOnlineEnabled. Then re-evaluate
            // availability against current registration (may have completed since start).
            m_OnlineEnabled = ResolveOnlineEnabled();
            m_Enabled = IsPersonalChronicleAvailable(m_OnlineEnabled);
