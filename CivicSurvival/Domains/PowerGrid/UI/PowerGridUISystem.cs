...
#pragma warning restore CA2213
        ...
        protected override void OnPanelUpdate()
        {
            ...
            if (m_PowerCapacitySnapshotReader.TryGetSnapshot(out var capacitySnapshot))
            {
                ...
                // Capacity-loss breakdown (FLEET CAPACITY block) — shared decomposition on the
                // snapshot, same numbers as the resolver's [CapacityLoss] debug line and telemetry.
                var losses = capacitySnapshot.ComputeLossBreakdown();
                dto.LossKnockedOutMW = (int)(losses.KnockedOutKW / 1000);
                dto.LossDamageMW = (int)(losses.DamageLossKW / 1000);
                dto.LossSaturationMW = (int)(losses.SaturationLossKW / 1000);
                dto.LossFuelMW = (int)(losses.FuelLossKW / 1000);
                dto.LossAllowedMW = (int)(losses.AllowedKW / 1000);
            }
            else
            {
