...
#pragma warning restore CIVIC278
        ...
        // Fence-gate deferral (option A): consecutive ticks the throttled pass was postponed
        // because a fence was still running on the workers. Stats are a UI read model — a
        // deferred tick only delays the panel refresh, never gameplay. Capped so sustained
        // saturation stretches the cadence instead of freezing the panel.
...
#pragma warning disable CIVIC150
        private int m_DeferredTicks;
#pragma warning restore CIVIC150
        private const int MAX_DEFER_TICKS = 60; // ~0.7s at 85 t/s on top of the 1s throttle

        // PERF-LOCK: combat cadence stretch — while any ActiveThreat is alive the aggregation
        // pass runs once per COMBAT_STATS_INTERVAL_SECONDS of sim time instead of the 1s
        // throttle. Stats feed the Opinion Board panel and the slow-moving coverage posture
        // (CognitiveStatsState → CognitiveCoverageCacheSystem → ICognitiveCoverageReader →
        // PsyOps gap-targeting / Exodus) — the posture tracks infrastructure-timescale inputs,
        // so 4s staleness is invisible; the pass itself (sidecar scan + Complete) is what costs
        // combat frames. Sim-time stamp, not a call counter: the fence gate's ForceNextUpdate
        // retries re-enter OnThrottledUpdate every tick and would devalue a counter. Sim time,
        // not real time: the base 1s throttle counts sim ticks, so only a sim-time stamp keeps
        // the stretch ratio speed-independent.
        // False positives (CIVIC150/168/241): scheduling-cadence transient, reset in
        // ClearTransientState (reached via ResetState/ValidateAfterLoad — the analyzers only scan
        // Deserialize/OnGameLoaded bodies); ElapsedTime rebases on load — the negative-delta
        // guard re-arms, and a lost stamp merely lets one early pass through, never doubles work.
#pragma warning disable CIVIC150, CIVIC168, CIVIC241
        private double m_LastPassSimTime = -1d;
#pragma warning restore CIVIC150, CIVIC168, CIVIC241
        private const float COMBAT_STATS_INTERVAL_SECONDS = 4f;
        ...
        private void ClearTransientState()
        {
            ...
            m_DeferredTicks = 0; // fence-gate deferral is per-city transient state
            m_LastPassSimTime = -1d; // combat-stretch stamp re-arms (ElapsedTime rebases on load)
        }
        ...
        {
            // PERF-LOCK: combat cadence stretch (see m_LastPassSimTime) — placed BEFORE the fence
            // gate: skipping is the common combat case, and the gate's ForceNextUpdate retries
            // would otherwise spin every tick for a pass this skip is not going to run.
            if (!m_ActiveThreatQuery.IsEmpty)
            {
                double nowSim = World.Time.ElapsedTime;
                // FIX #164-B analogue: ElapsedTime rebases to ~0 on load — a stale stamp would
                // sit above `now` and block the pass on a negative delta; re-arm instead.
                if (m_LastPassSimTime > nowSim)
                    m_LastPassSimTime = -1d;
                if (m_LastPassSimTime >= 0d && nowSim - m_LastPassSimTime < COMBAT_STATS_INTERVAL_SECONDS)
                    return;
            }

            // PERF-LOCK: fence gate (option A) — defer the whole pass while either fence is
            // live on the workers: the pending aggregate job (its Complete below would wall the
            // main thread) or this system's input fence (scheduling onto a live resolve chain
            // makes OUR next-tick framework completion pay the chain's remainder — combat A/B
            // 2026-07-22: cogStats ±535ms frame spikes). ForceNextUpdate retries every tick, so
            // the pass lands in the first drained window; the cap bounds a saturated stretch.
            // Stats are a UI read model — deferral has no gameplay effect.
            if ((m_HasPendingJob && !m_PendingJobHandle.IsCompleted) || !Dependency.IsCompleted)
            {
                if (m_DeferredTicks < MAX_DEFER_TICKS)
                {
                    m_DeferredTicks++;
                    ForceNextUpdate();
                    return;
            ...
            m_DeferredTicks = 0;
            // Stamp of the last pass that actually proceeded. Peace passes stamp too, so combat
            // entry measures from the latest pass, not from the tail of the previous wave.
#pragma warning disable CIVIC034, CIVIC040 // Ephemeral cadence stamp, not persisted — reset in ClearTransientState, negative-delta guard re-arms after load
            m_LastPassSimTime = World.Time.ElapsedTime;
#pragma warning restore CIVIC034, CIVIC040
