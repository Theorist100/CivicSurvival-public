...
namespace CivicSurvival.Domains.Cognitive.Core.Jobs
{
    ...
    public partial struct ResolveHouseholdPsyJob : IJobEntity
    {
            ...
            ref HouseholdPsyState psy)
        {
            ...
            // ════════════════════════════════════════════════════════════
            // SLOW-PROFILE REFRESH GATE (~30s): stratum, telecom coverage and resistance are
            // caches of state that drifts over game hours (wealth, education) or lives on a
            // 4096-tick vanilla rebuild (telecom grid). Recomputing them every slot pass is the
            // bulk of this job's random vanilla-table reads — all three refresh together on the
            // existing resistance throttle instead. Unknown stratum forces an immediate pass:
            // post-load (Deserialize resets the stratum while rTime persists) and a brand-new
            // sidecar's first resolve. The homeless reset above stays instant — it early-returns
            // before this gate.
            // ════════════════════════════════════════════════════════════
            bool refreshSlowProfile = ForceResistanceRecalculate
                || psy.CachedStratum == SocialStratum.Unknown
                || ResistanceCalculator.ShouldRecalculate(CurrentTime, psy.Resistance_LastUpdateTime);

            // ════════════════════════════════════════════════════════════
            // TELECOM COVERAGE (30s slow-profile cadence): sample the vanilla 128² signal grid at
            // the building's cell. "In coverage" = the pre-computed signal (radius + terrain shadow
            // + tower overlap, folded by vanilla) is at/above the threshold. Each entity writes its
            // OWN InCoverage → race-free in the parallel resolver; derived → self-heals on load
            // (Unknown stratum forces the refresh branch). Homeless handled above. Outside the
            // refresh the job reads the cached value written on the previous refresh.
            // ════════════════════════════════════════════════════════════
            ...
            // ════════════════════════════════════════════════════════════
            // 0. SOCIAL STRATUM (read-only vanilla wealth classification, 30s slow-profile cadence)
            // Refreshed BEFORE ExposureCalculator and the impact loop so a refresh pass feeds them
            // the just-recomputed value — do NOT move this block below step 3. If thresholds/lookups
            // are not ready the cached value is left untouched (stays Unknown after load, which keeps
            // the refresh gate open) — stratum-keyed effects simply do not match until it resolves.
            // ════════════════════════════════════════════════════════════
            ...
            // ════════════════════════════════════════════════════════════
            // 4. RESISTANCE (30s slow-profile cadence — same gate as stratum/coverage above).
            // The Unknown-stratum force also reopens this branch (early boot before wealth
            // thresholds load, first post-load cycle) — a bounded extra-recalc window.
            // ════════════════════════════════════════════════════════════
