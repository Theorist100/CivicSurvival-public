...
namespace CivicSurvival.Domains.Cognitive.Core.Jobs
{
    ...
    public struct BuildMentalHealthLookupsJob : IJob
    {
        ...
        [ReadOnly] public BufferLookup<BuckwheatAidedDistrictBuffer> BuckwheatLookup;
        public Entity BuckwheatEntity;
        ...
        public NativeParallelHashSet<int> BuckwheatAidedSet;
        ...
        public void Execute()
        {
            ...
            // Build Buckwheat aided district set (Buckwheat per-stratum infection effect)
            if (BuckwheatLookup.HasBuffer(BuckwheatEntity))
            {
                var buffer = BuckwheatLookup[BuckwheatEntity];
                for (int i = 0; i < buffer.Length; i++)
                    BuckwheatAidedSet.Add(buffer[i].DistrictIndex);
            }
        }
    }
}
