...
namespace CivicSurvival.Domains.Cognitive.UI
{
    ...
    public partial class BuckwheatUISystem : CivicUIPanelSystem
    {
        ...
        protected override void OnPanelUpdate()
        {
            ...
            // Payer flag for the daily-cost hint: offshore shadow cash when the
            // ShadowEconomy feature is open, the municipal budget otherwise.
            dto.ShadowFunded = BuckwheatEligibility.ShadowFundingLive;
        ...
        {
            // No pause reject here, by design — the consumer
            // (BuckwheatProcurementLevelRequestSystem) runs in ModificationEnd,
            // which ticks while paused (Axiom 14).
        ...
        {
            // No pause reject here, by design — the consumer runs in ModificationEnd
            // and applies synchronously via BuckwheatSystem (AXIOM 14).
            var ecb = m_EndFrameBarrier.CreateCommandBuffer();
            var entity = ecb.CreateEntity();
        ...
        {
            return BuckwheatEligibility.CanSelectProcurementLevel(targetLevel, currentLevel, out reasonId);
        }
    }
}
