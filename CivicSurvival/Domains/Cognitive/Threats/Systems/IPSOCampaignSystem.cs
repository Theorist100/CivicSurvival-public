...
namespace CivicSurvival.Domains.Cognitive.Threats.Systems
{
    ...
    public partial class IPSOCampaignSystem : ThrottledSystemBase, IActGatedSystem
    {
        ...
        /// <summary>
        /// Network-propaganda threshold to count a district as "affected" (for UI). Applied to
        /// networkTerm (the amplification above the enemy word-of-mouth floor), NOT to the final
        /// exposure — exposure always carries the floor, so gating on it would count every
        /// district and the "N/M districts" readout would collapse to N/N.
        /// </summary>
        private const float AFFECTED_THRESHOLD = 0.1f;

        /// <summary>Enemy word-of-mouth floor fallback (before config loads). Mirrors balance contract default.</summary>
        private const float FALLBACK_ENEMY_EXPOSURE_FLOOR = 0.15f;
...
#pragma warning restore CIVIC269
        ...
        protected override void OnCreate()
        {
            ...
            // Fresh-world load order is Deserialize → OnCreate; EnsureExists above re-created
            // the singleton at Default, so re-apply any spike countdown Deserialize staged
            // (instance-reuse load skips OnCreate → OnStartRunning applies).
            ApplySavedState();
        ...
        protected override void OnStartRunning()
        {
            base.OnStartRunning();
            IPSOState.EnsureExists(EntityManager);
            // OnCreate does not re-run on instance-reuse load, so the staged spike countdown
            // would otherwise never reach the singleton. ApplySavedState is idempotent.
            ApplySavedState();
        }
        ...
        protected override void OnThrottledUpdate()
        {
            ...
            // PostWaveSpikeTimer on the IPSOState singleton is the sole source of truth. It
            // survives save/load: the campaign system's save block carries it (Serialize →
            // IpsoCampaignCodec, restored onto the singleton by ApplySavedState) since the
            // component itself has no engine serializer. It is zeroed on deactivation by
            // ApplyIpsoEventJob.
            ...
            float proximityFactor = wave.CurrentPhase switch
            {
                ...
                GamePhase.PsyAttack => PROXIMITY_ATTACK,
...
#pragma warning restore CIVIC051
            ...
            // Enemy floor: enemyExposure = floor + (1 - floor) * networkTerm. The global switch
            // (BLACKOUT) cuts networkTerm to 0 — the network amplification is gone and the economy pays —
            // but the enemy still seeps at the word-of-mouth floor. Blackout knocks the peak down at the
            // city's expense; it is NOT a free off-switch (reverses the old hard-zero at blackout).
            float enemyFloor = math.saturate(BalanceConfig.Current?.Cognitive?.EnemyExposureFloor ?? FALLBACK_ENEMY_EXPOSURE_FLOOR);

            if (isGlobalBlackout)
            {
                // networkTerm == 0 everywhere → exposure == floor. Skip the per-district network
                // compute. No district is "affected" (see AFFECTED_THRESHOLD): the count tracks
                // active network propaganda above the word-of-mouth floor, which blackout cuts to 0.
            ...
            else
            {
                using (PerformanceProfiler.Measure("IPSO.DistrictLoop"))
                {
                    for (int i = 0; i < districtBuffer.Length; i++)
                    {
                        ...
                        float networkTerm = math.clamp(baseIntensity * internetFactor * proximityFactor, 0f, 1f);
                        float exposure = enemyFloor + (1f - enemyFloor) * networkTerm;
                        ...
                        // Count districts where enemy network propaganda amplifies above the
                        // word-of-mouth floor (networkTerm), not the floor-inflated exposure —
                        // otherwise every district always clears the threshold and the count
                        // degenerates to N/N. See AFFECTED_THRESHOLD.
                        if (networkTerm > AFFECTED_THRESHOLD)
                            affectedCount++;
                    }
                }
            }
            ...
            // The floor is folded into every district value above (blackout included), so the city
            // average is the honest global exposure — no separate blackout zeroing.
            float globalExposure = avgExposure;

            if (Log.IsDebugEnabled && isGlobalBlackout)
                Log.Debug($"[EnemyFloor] BLACKOUT active — enemy exposure held at floor={enemyFloor:F2} across {totalCount} districts (network amplification cut, not zeroed)");
