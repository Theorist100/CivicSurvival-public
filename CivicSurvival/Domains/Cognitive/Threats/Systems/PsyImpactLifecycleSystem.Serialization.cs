...
namespace CivicSurvival.Domains.Cognitive.Threats.Systems
{
    ...
    /// The bulk init must re-run to re-tag them, so m_BulkInitDone must be false.
    ...
    public partial class PsyImpactLifecycleSystem : IDefaultSerializable, IResettable, IBootDefaultsReset
    {
        ...
        private void ResetState()
        {
            ...
            m_SlotsEnsured = false;
            // Enabled=true is required load semantics (Phase 1 re-tag); overrides a dev-only DomainToggleRegistry disable — re-toggle after load if debugging.
            Enabled = true;
