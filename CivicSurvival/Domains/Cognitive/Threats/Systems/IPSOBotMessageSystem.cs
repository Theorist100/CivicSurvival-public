...
namespace CivicSurvival.Domains.Cognitive.Threats.Systems
{
    ...
    public partial class IPSOBotMessageSystem : ThrottledSystemBase
    {
        ...
        {
            new BotPersona("@TruthPatriot_City", SocialMood.Angry, new[]
            {
            ...
            new BotPersona("@FreedomVoice_City", SocialMood.Suffering, new[]
            {
        ...
        protected override void OnCreate()
        {
            ...
            // Fresh-world load order is Deserialize → OnCreate, so apply any RNG state Deserialize
            // staged AFTER the re-seed above (instance-reuse load skips OnCreate → OnStartRunning applies).
            ApplySavedState();
        ...
        {
            base.OnStartRunning();
            // OnCreate does not re-run on instance-reuse load, so the RNG state staged by Deserialize
            // would otherwise never reach m_Random. ApplySavedState is idempotent (clears after use).
            ApplySavedState();
        }
