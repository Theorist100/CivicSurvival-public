    ...
    public partial class SickLeaveWaveSystem : ThrottledSystemBase, IActGatedSystem
    {
            ...
            float baseVulnerability, float expiryHours, ref EntityCommandBuffer ecb, ref bool hasEcb)
        {
            ...
            foreach (var (psyRef, linkRef) in SystemAPI.Query<RefRO<HouseholdPsyState>, RefRO<PsyHouseholdLink>>())
            {
                ...
                // POPULATION-MEASURE-EXEMPT: this walk reaches individual citizens in order to MARK
                // them, not to measure the city. 'marked' counts the male workers of one stratum that
                // this attack actually infected — a result of the mutation, not a population tally,
                // and no reader of ICityPopulationReader could produce it: the owner publishes how
                // many people live here, never which of them are male, employed and unmarked.
