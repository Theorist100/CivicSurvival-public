...
#pragma warning restore CIVIC324
        ...
        private void ProcessLifecycle(float nowHours, ref EntityCommandBuffer ecb, ref bool hasEcb)
        {
            ...
            // Strongest scaled FakeVideo media-trust hit among the contacts landing THIS pass.
            // All contacts of one raid share the same LandTimeHours, so "this pass" = "this raid":
            // three deepfakes in one news cycle are ONE hit on credibility, not three stacked
            // full-magnitude hits (pre-fix a 4×FakeVideo raid drained −0.60 trust in one tick —
            // telemetry showed every long session pinned at MediaTrust 0).
            float maxFakeVideoTrustHit = 0f;
                ...
                switch (atk.Phase)
                {
                    case PsyOpsAttackPhase.InFlight:
                        if (nowHours >= atk.LandTimeHours)
                        {
                            ...
                            // player's own channels — one media-trust hit at land, scaled by the SAME
                            // frozen land verdict the UI shows for this contact: the hero shield cuts
                            // the hit (a "Blunted" strike must not bite at full magnitude) and the
                            // contact intensity keeps the secondary off-type leak below the dominant.
                            // Accumulated (strongest contact wins) and applied once after the loop —
                            // routed through the Telemarathon one-writer (CIVIC175); never written here.
                            if (atk.Type == PsyOpsAttackType.FakeVideo && cwCfg != null)
                            {
                                float trustHit = cwCfg.FakeVideoTrustHit * atk.Intensity * (1f - shield);
                                maxFakeVideoTrustHit = math.max(maxFakeVideoTrustHit, trustHit);
            ...
            // Apply the pass's single FakeVideo media-trust hit (strongest landing contact — see
            // the accumulator's comment for why max, not sum). Zero when nothing landed or the
            // shield fully absorbed the strike.
            if (maxFakeVideoTrustHit > 0f)
            {
                m_Telemarathon ??= World.GetExistingSystemManaged<TelemarathonSystem>();
                m_Telemarathon?.ApplyMediaTrustHit(-maxFakeVideoTrustHit, "fake video");
            }
