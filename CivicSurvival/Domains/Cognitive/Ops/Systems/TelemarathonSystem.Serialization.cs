...
#pragma warning restore CIVIC324
        ...
        {
            // Consume the boot-restore latch up front so an exception in EnsureRuntimeStateExists /
            // RebuildConfig can't leave it armed (which would let the next load be clobbered by boot defaults).
...
#pragma warning restore CIVIC212
        ...
        public void Serialize<TWriter>(TWriter writer) where TWriter : IWriter
        {
            ...
            try
            {
                ...
                var persistState = new TelemarathonPersistState(
                    ...
                    state.FatigueWindowStartHour,
                    state.InactiveSinceHour);
        ...
        public void Deserialize<TReader>(TReader reader) where TReader : IReader
        {
            ...
            try
            {
                ...
                state.FatigueWindowStartHour = persistState.FatigueWindowStartHour;
                state.InactiveSinceHour = persistState.InactiveSinceHour;
                ...
                else if (persistState.ShockHoursRemaining > 0f)
                {
                    ...
                    // Re-anchored shock: the persisted cooldown was anchored to the old clock and can now
                    // sit in the past. Conservatively extend it so the cooldown outlasts the re-anchored
                    // shock (same source as BuildConfig; anti-spam S18-H10 stays intact on the recovery path).
                    float cooldownHours = BalanceConfig.Current?.Cognitive?.ShockCooldownHours ?? FALLBACK_SHOCK_COOLDOWN_HOURS;
                    if (state.ShockCooldownEndHour < state.ShockEndHour)
                        state.ShockCooldownEndHour = state.ShockEndHour + cooldownHours;
                }
