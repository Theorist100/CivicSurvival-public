...
#pragma warning restore CIVIC065
    {
        ...
        protected override void OnCreate()
        {
            ...
            // The write path stays ApplyMediaTrustHit — one writer (CIVIC175).
            SubscribeRequired<AdjustMediaTrustCommand>(OnAdjustMediaTrust);
        ...
        {
            UnsubscribeSafe<AdjustMediaTrustCommand>(OnAdjustMediaTrust);
            base.OnDestroy();
        }

        /// <summary>
        /// External trust adjustment (a narrative quest reward) — routed through the same
        /// single-writer entry point as the deepfake hit.
        /// </summary>
        // Below this magnitude a trust delta is a visual no-op (trust renders at 2 decimals);
        // skipping keeps the trust log free of "0.50 -> 0.50" noise (S1244: no exact float equality).
        private const float TRUST_DELTA_EPSILON = 1e-6f;

        private void OnAdjustMediaTrust(AdjustMediaTrustCommand cmd)
        {
            if (math.abs(cmd.Delta) < TRUST_DELTA_EPSILON)
                return;
            ApplyMediaTrustHit(cmd.Delta, cmd.Reason);
        }
