...
#pragma warning restore CIVIC324
        ...
        protected override void OnCreate()
        {
            ...
            // Quest rewards (Narrative → Cognitive via Core command; CorruptionGainEvent pattern).
            SubscribeRequired<GrantBuckwheatCommand>(OnGrantBuckwheat);
        ...
        protected override void OnDestroy()
        {
            ...
            if (ServiceRegistry.IsInitialized)
            {
                ...
                UnsubscribeSafe<GrantBuckwheatCommand>(OnGrantBuckwheat);
            }
        ...
        /// <summary>
        /// External (non-procurement) reserve grant — a narrative quest reward. Applies on the
        /// singleton owner like every other reserve write; no cost, no LastProcurementHour touch
        /// (this is not a procurement and must not delay the next procurement interval).
        /// </summary>
        private void OnGrantBuckwheat(GrantBuckwheatCommand cmd)
        {
            if (cmd.Tons <= 0f)
                return;
            // Event handlers run outside OnUpdate, so the write goes through the cached
            // EntityQuery (foreign-context-safe, the ApplyMediaTrustHit pattern) — not
            // through a ComponentLookup .Update(this) (Axiom 8: OnUpdate-only).
            if (!m_SingletonQuery.TryGetSingletonRW<BuckwheatSingleton>(out var singletonRef))
            {
                Log.Warn($"BuckwheatSystem: grant '{cmd.Reason}' skipped — reserve singleton not up yet");
                return;
            }

            singletonRef.ValueRW.BuckwheatTons += cmd.Tons;
            ForceNextUpdate();

            Log.Info($"BuckwheatSystem: granted {cmd.Tons:F2} tons ({cmd.Reason}). Reserve: {singletonRef.ValueRO.BuckwheatTons:F2} tons");
        }
