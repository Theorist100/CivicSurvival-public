...
namespace CivicSurvival.Domains.Corruption.UI
{
    ...
    public partial class MaintenanceContractUISystem : CivicUIPanelSystem
    {
...
#if DEBUG
            Triggers.Add(DebugSpawnContractOffer, FeatureIds.Corruption, OnDebugSpawnContractOffer);
#endif
        }

#if DEBUG
        private void OnDebugSpawnContractOffer()
        {
            m_StateSystem?.RequestDebugOffer();
        }
#endif
        ...
        {
            // No pause reject here, by design — ContractResponseSystem consumes the
            // request in ModificationEnd and pays synchronously, both pause-safe
            // (Axiom 14), so the decision modal works at selectedSpeed==0.
