...
#pragma warning restore CIVIC324
        ...
        private void InitializeGate()
        {
            m_Gate = new ActGateController(
                // Counterfeit-battery fallout follows corruption, which runs from PreWar.
                isOpenFor: _ => true,
        ...
        private void OnDayChanged(DayChangedEvent evt)
        {
            ...
            try
            {
                foreach (var districtIndex in GetActiveProgramDistricts(procurementService))
                {
                    ...
                    // Copy the live district version from a counterfeit sidecar in this district (C4)
                    // so the persisted FireRecordRequest key rides the engine remap.
                    int districtVersion = 0;
                    for (int b = 0; b < counterfeitDatas.Length; b++)
                    {
                        if (counterfeitDatas[b].InstallationDistrictId == districtIndex)
                        {
                            districtVersion = counterfeitDatas[b].InstallationDistrictVersion;
                            break;
                        }
                    }
