...
namespace CivicSurvival.Domains.Corruption.Systems
{
        ...
        ICivicSingletonOwner<ContractStatsSingleton>, ICivicSingletonOwner<PendingProcurement>
    {
        ...
        // DEBUG: deferred force-offer request (set by the DebugSpawnContractOffer
        // trigger in UIUpdate, executed in this system's GameSimulation tick where
        // the barrier/toast machinery is legal — same defer pattern as toast clears).
        [System.NonSerialized] private bool m_DebugOfferRequested;
        ...
        protected override void OnThrottledUpdate()
        {
            ...
            // DEBUG: forced offer requested from the devtools panel — bypasses the
            // daily chance and cooldown but respects the single-pending-offer invariant.
            if (m_DebugOfferRequested)
            {
                m_DebugOfferRequested = false;
                TryGenerateDebugOffer();
            }
...
#pragma warning restore CIVIC485
        ...
        private void TryGenerateOffer(float currentGameDay)
        {
            ...
            for (int ci = 0; ci < plantChunks.Length && !stopScanning; ci++)
            {
                ...
                for (int i = 0; i < chunk.Count; i++)
                {
                    ...
                    // Generate offer for this power plant. A failed generation (toast
                    // blocked by the global popup cooldown) must NOT burn the offer
                    // cooldown — the roll retries in the next window instead of the
                    // event being silently lost. Scanning stops either way: the popup
                    // cooldown is global, so every other candidate would fail the same.
                    if (GenerateOfferForEntity(entity, random.NextUInt()))
                        m_LastOfferGameDay = currentGameDay;
                    stopScanning = true;
                    break; // Only one offer roll at a time
                }
            }
        ...
        /// <summary>
        /// DEBUG: queue a forced offer generation for the next tick. Called by the
        /// DebugSpawnContractOffer trigger (UIUpdate) on MaintenanceContractUISystem;
        /// deferred because offer creation needs the GameSimulation-phase barrier and
        /// toast machinery. While paused the offer appears on the first tick after unpause.
        /// </summary>
        internal void RequestDebugOffer() => m_DebugOfferRequested = true;

        /// <summary>
        /// DEBUG: force-generate an offer on the first eligible power plant, reusing
        /// the natural generation path (vendors, prices, toast) — only the daily
        /// chance roll and MinDaysBetweenOffers cooldown are skipped.
        /// </summary>
        private void TryGenerateDebugOffer()
        {
            if (HasPendingOffer())
            {
                Log.Info("Debug offer skipped: an offer is already pending");
                return;
            }

            if (m_PowerPlantQuery.IsEmpty)
            {
                Log.Info("Debug offer skipped: no power plants");
                return;
            }
            ...
            m_EntityTypeHandle.Update(this);
            var plantChunks = m_PowerPlantQuery.ToArchetypeChunkArray(Unity.Collections.Allocator.Temp);
            for (int ci = 0; ci < plantChunks.Length; ci++)
            {
                var chunk = plantChunks[ci];
                var entities = chunk.GetNativeArray(m_EntityTypeHandle);
                ...
                for (int i = 0; i < chunk.Count; i++)
                {
                    ...
                    // Debug spawns are on-demand tooling: the offer must appear even
                    // when the global popup cooldown suppresses the toast.
                    if (!GenerateOfferForEntity(entity, seed, toastRequired: false))
                    {
                        Log.Warn($"Debug offer generation failed for entity {entity.Index}");
                        return;
                    }
                    if (TryGetCurrentGameDay(out var currentGameDay))
                        m_LastOfferGameDay = currentGameDay;
                    Log.Info($"Debug offer forced for entity {entity.Index}");
                    return;
                }
            }

            Log.Info("Debug offer skipped: no eligible plant (all under contract)");
        }

        /// <summary>
        /// Create a pending offer on the plant. Returns false when the offer was NOT
        /// created: the toast was rejected (global popup cooldown / queue full /
        /// service unavailable) while <paramref name="toastRequired"/> is true.
        /// Debug spawns pass toastRequired=false — the modal reads the offer from the
        /// DTO directly and must not depend on popup pacing.
        /// </summary>
        private bool GenerateOfferForEntity(Entity entity, uint seed, bool toastRequired = true)
        {
            var random = new Random(seed);
...
#pragma warning restore CIVIC485
            ...
            EventBus?.SafePublish(new ShadowNarrativeEvent(
                ShadowNarrativeEventType.OfferGenerated,
                BuildingIndex: entity.Index,
                Cost: officialPrice,
                KickbackAmount: kickback,
                ContractType: contractType.ToString()), "MaintenanceContractSystem");
...
#pragma warning restore CA1308
        ...
        private void ClearPendingOfferByToast(int toastId, string reason)
        {
                ...
                .WithEntityAccess())
            {
                ...
                // Offer-funnel telemetry: "rejected" is the player's Not Now on the toast
                // (a decline), anything else here is the toast timing out unanswered.
                EventBus?.SafePublish(new ShadowNarrativeEvent(
                    reason == "rejected"
                        ? ShadowNarrativeEventType.OfferDeclined
                        : ShadowNarrativeEventType.OfferExpired,
                    BuildingIndex: entity.Index), "MaintenanceContractSystem");
        ...
        {
            // Same name resolution as EquipmentUISystem.GetPlantName: vanilla NameSystem
            // (player rename or localized asset name), falling back to the plant-type
            // display name when NameSystem returns an unresolved locale key ("Assets....")
            // or an empty string. Raw prefab.name ("GasPowerPlant01") is never shown.
            string rendered = m_NameSystem != null ? m_NameSystem.GetRenderedLabelName(entity) : "";
            if (!string.IsNullOrEmpty(rendered) && !rendered.StartsWith("Assets.", System.StringComparison.Ordinal))
                return rendered;
...
#pragma warning restore CIVIC458
        ...
        {
            // The offer and its phone-call toast live and die together. The toast-side
            // edge already exists (OnToastExpired/Interacted → deferred offer clear);
            // this is the reverse edge: any offer-side clear (player answered in the
            // modal, expiry, post-load reconcile) retires a still-live toast, so a
            // zombie call can't keep advertising a dead offer. The id is released
            // BEFORE the dismiss: DismissToast synchronously publishes OnToastExpired,
            // and the handler must see the id already gone (no deferred re-clear).
            if (m_LastProcurementToastId > 0 && m_ToastService != null)
            {
                int toastId = m_LastProcurementToastId;
                m_LastProcurementToastId = -1;
                m_ProcurementToastBuildingIndex = -1;
                m_ProcurementToastBuildingVersion = -1;
                m_ToastService.DismissToast(toastId);
            }
