            ...
            IBackupPowerLinkReader backupLinks)
        {
            ...
            try
            {
                for (int i = 0; i < entities.Length && installed < buildingCount; i++)
                {
                    ...
                    int districtVersion = district.m_District.Version;
            ...
            IBackupPowerLinkReader backupLinks)
        {
            ...
            try
            {
                for (int i = 0; i < entities.Length && installed < buildingCount; i++)
                {
                    ...
                    // Capture the live district version from this building's CurrentDistrict so the
                    // persisted key rides the engine remap (all buildings in the district share it).
                    int districtVersion = district.m_District.Version;
                    ...
                    ecb.AddComponent(counterfeitModEntity, new CounterfeitBattery
                    {
                        ...
                        InstallationDistrictVersion = districtVersion
                    });
