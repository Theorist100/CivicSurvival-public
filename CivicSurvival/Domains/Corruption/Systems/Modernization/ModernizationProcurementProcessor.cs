...
#pragma warning disable CIVIC231 // [ActIndependent] — procurement available in all acts (UI gating handles availability)
            ...
            int districtIndex, ContractorType contractor, RequestMeta requestMeta)
        {
            ...
            // Resolve the UI district index to a live (index, version) so the durable intent/receipt
            // ride the engine remap across save/load. A district removed between click and processing
            // no longer resolves → reject.
            if (!DistrictKeyCodec.TryResolveLive(m_Host.World.EntityManager, districtIndex, out var districtRef))
            {
                Log.Warn($"Procurement blocked: district {districtIndex} is no longer live");
                m_Cleanup.ClearScratch();
                return (false, "DistrictGone");
            }
