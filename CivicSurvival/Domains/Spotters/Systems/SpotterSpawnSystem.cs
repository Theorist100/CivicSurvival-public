        ...
        private bool TrySpawnSpotter(string reason, int preferredDistrictIndex, ref NativeList<Entity> residentialEntities)
        {
            ...
            if (preferredDistrictIndex >= 0)
            {
                for (int attempt = 0; attempt < 16; attempt++)
                {
                    ...
                    if (GetDistrictRef(candidate).Index != preferredDistrictIndex)
                        continue;
...
#pragma warning restore CIVIC097
        ...
        private void SpawnSpotterAt(Entity building, string reason)
        {
            ...
            var districtRef = GetDistrictRef(building);
            int districtIndex = districtRef.Index;
            ...
            ecb.AddComponent(spotterEntity, new SpotterData
            {
                ...
                DistrictVersion = districtRef.Version
            });
            IncrementEcbCount();
...
#pragma warning restore CIVIC097
        ...
        private DistrictRef GetDistrictRef(Entity building)
        {
            if (building == Entity.Null)
                return new DistrictRef(-1, 0);

            if (m_CurrentDistrictLookup.TryGetComponent(building, out var district))
            {
                // Entity.Null m_District → unzoned (index 0, version 0 == DistrictRef.Null).
                // Real district → live (index, version) so persistence rides the engine remap.
                return DistrictRef.FromEntity(district.m_District);
            }
