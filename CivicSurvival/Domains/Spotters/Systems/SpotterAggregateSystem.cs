...
#pragma warning restore CIVIC247
        ...
        private void ProcessDailyTick(double currentTime)
        {
            ...
            // While Counter-OSINT is off there is nothing to charge, so keep the anchor sliding.
            // A parked anchor rots: nothing below ever advances it (the confirm path requires an
            // active state), so once a day passed every tick took the overdue branch and spammed
            // the Warn ~8×/s for the rest of the session (prod report 2026-07-21, 0.3.22). The
            // enable path re-anchors too — this keeps the overdue math meaningful the whole time.
            if (!m_CountermeasuresStateQuery.TryGetSingleton<SpotterCountermeasuresState>(out var cmState)
