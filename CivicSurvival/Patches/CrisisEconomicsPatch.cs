...
namespace CivicSurvival.Patches
{
    ...
    public static class CommercePatch
    {
        ...
        // Rumor resource-run: the demand element the run scales UP (Food). Resolved
        // once via the vanilla mapper (the contract) — never hard-coded to 3.
        private static readonly int s_FoodResourceIndex = EconomyUtils.GetResourceIndex(Resource.Food);
            ...
            string surface)
        {
            try
            {
                ...
                // Two composed levers, opposite directions on the same per-resource demand array:
                //  • crisis commerce down-scale (uniform, < 1) — the existing whole-array scale.
                //  • Food run UP-scale (>= 1, element Food only) — the panic-buying tell
                //    (raises the Food commercial-demand/zoning pressure the player sees). Combined
                //    Food demand = vanilla × crisisMult × foodRun; other resources = vanilla × crisisMult.
                float multiplier = CrisisEconomicsAdapter.GetCommerceMultiplier(world);
                float foodRun = ResourceRunAdapter.GetFoodDemandRun(world);
                bool foodRunActive = foodRun > 1f && s_FoodResourceIndex >= 0 && s_FoodResourceIndex < result.Length;

                // Nothing to do when neither lever bites (no crisis down-scale, no active run).
                if (multiplier >= 1f && !foodRunActive)
                    return;
                ...
                var job = new ScaleDemandArrayJob
                {
                    ...
                    // -1 disables the per-index up-scale; a valid index applies foodRun to that element only.
                    TargetIndex = foodRunActive ? s_FoodResourceIndex : -1,
                    TargetMultiplier = foodRunActive ? foodRun : 1f,
                };
        ...
        //
        // Composes the uniform crisis multiplier with an optional per-index run multiplier
        // (Food demand-up). TargetIndex < 0 means no per-index scale.
        private struct ScaleDemandArrayJob : IJob
        {
            ...
            public void Execute()
            {
                for (int i = 0; i < Source.Length; i++)
                {
                    ...
                    if (i == TargetIndex)
                        m *= TargetMultiplier;
                    Destination[i] = (int)math.round(Source[i] * m);
                }
            }
        }
    }
